...
struct grow_capacity_by_fn {
  ...
  ///
  /// \param c The container whose capacity is grown.
  /// \param n The number of additional elements to make room for.
  template <typename C>
...
/// Callable that grows a container's capacity using geometric growth.
...
struct reserve_if_available_fn {
  ...
  ///
  /// \param c The container to reserve capacity in.
  /// \param n The number of elements to reserve space for.
  /// \returns A std::bool_constant that is true when reserve() was called.
  template <typename C>
...
/// Callable that reserves capacity when the container supports it.
