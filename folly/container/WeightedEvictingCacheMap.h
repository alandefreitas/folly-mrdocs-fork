...
class ImplicitlyWeightedEvictingCacheMap {
 ...
 public:
  ...
  /// Constructs a map with the given maximum total weight and weight function.
  ///
  /// \param maxTotalWeight The maximum total weight retained before eviction.
  /// \param weightFn The function computing the weight of each entry.
  /// \param keyHash The hash function used to index keys.
  /// \param keyEqual The equality comparison used to match keys.
  ...
  ///
  /// \param that The map that would be copied.
  ImplicitlyWeightedEvictingCacheMap(
      const ImplicitlyWeightedEvictingCacheMap& that) = delete;
  ...
  ///
  /// \param that The map to move from.
  /// \returns A reference to this map.
  ImplicitlyWeightedEvictingCacheMap& operator=(
  ...
  /// Destroys the map, verifying tracked total weight in debug builds.
  ~ImplicitlyWeightedEvictingCacheMap() {
#ifndef NDEBUG
  ...
  /// Approximate memory used by each cache entry.
  ...
  /// Const iterator over key-value entries.
  using const_iterator = typename ECM::const_iterator;
  /// Const reverse iterator over key-value entries.
   ...
   * the cache.
   * @return the maximum allowed total weight
   */
   ...
   * @param newMaxTotalWeight the new maximum allowed total weight
   */
  ...
  ///
  /// \param key The key to search for.
  /// \returns A const iterator to the element, or end() if it does not exist.
  template <typename K>
  ...
  /// Replace the value associated with an entry, copying the value and
  /// tracking any weight change.
  ///
  /// \param it The const_iterator for the entry to modify.
...
class WeightedEvictingCacheMap {
 public: // types
  /// A stored value together with its tracked weight.
  struct ValueAndWeight {
    /// Constructs from a value and its weight.
    ///
    /// \param _value The value to store.
    /// \param _weight The tracked weight of the value.
 ...
 public:
  /// Callback type invoked on eviction with the key, value, and weight.
  ...
  /// Copy construction is disabled.
  ///
  /// \param other The map that would be copied.
  WeightedEvictingCacheMap(const WeightedEvictingCacheMap& other) = delete;
  /// Copy assignment is disabled.
  ///
  /// \param other The map that would be copied.
  /// \returns A reference to this map.
  WeightedEvictingCacheMap& operator=(const WeightedEvictingCacheMap& other) = delete;
  /// Move-constructs from another map.
  ///
  ...
  /// \param other The map to move from.
  /// \returns A reference to this map.
  WeightedEvictingCacheMap& operator=(WeightedEvictingCacheMap&& other) = default;

  /// Approximate memory used by each cache entry.
  static constexpr std::size_t kApproximateEntryMemUsage =
      IWECM::kApproximateEntryMemUsage;

  /// Mutable iterator that dereferences to a key and ValueAndWeight pair.
  using iterator = typename IWECM::ECM::iterator;
  /// Reverse mutable iterator over key and ValueAndWeight pairs.
  using reverse_iterator = typename IWECM::ECM::reverse_iterator;
  /// Const iterator over key and ValueAndWeight pairs.
  using const_iterator = typename IWECM::const_iterator;
  /// Const reverse iterator over key and ValueAndWeight pairs.
  ...
  /// Same but without LRU promotion, returning a const reference.
  ///
  /// \param key The key to search for.
  /// \returns A const reference to the value if it exists.
  template <typename K>
  const TValue& getWithoutPromotion(const K& key) const {
  ...
  /// Set a key-value pair in the dictionary with a given weight, copying the
  /// value.
  ///
  /// \param key The key to associate with the value.
  /// \param value The value to associate with the key.
  /// \param weight The weight to associate with the key-value entry.
  template <typename K>
  ...
  /// Overwrite the weight associated with a specific entry.
  ///
  /// \param it The iterator for the entry to modify, from this cache map.
  /// \param new_weight The updated weight to assign.
