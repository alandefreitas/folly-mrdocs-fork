...
struct F14TableStats {
 ...
 public:
  ...
  ///
  /// \param m The container to inspect.
  /// \returns The computed statistics, or a default-constructed value if unavailable.
  template <typename T>
...
class F14HashToken final {
 public:
  ...
  ///
  /// \param a The first hash token to compare.
  /// \param b The second hash token to compare.
  /// \returns True if both tokens hold the same processed hash.
  ...
  /// \param a The first hash token to compare.
  /// \param b The second hash token to compare.
  /// \returns True if the tokens hold different processed hashes.
  friend constexpr bool operator!=(
      F14HashToken const& a, F14HashToken const& b) noexcept {
    return !(a == b);
  }
...
class F14HashedKey final {
 ...
 public:
#if FOLLY_F14_VECTOR_INTRINSICS_AVAILABLE
  /// Constructs the key in place and precomputes its hash token.
  ///
  /// \param args Arguments forwarded to the key's constructor.
  ...
  /// Returns the precomputed hash token for the stored key.
  ///
  /// \returns A reference to the precomputed hash token.
  const F14HashToken& getHashToken() const { return hash_; }
  /// Converts implicitly to the stored key so the hashed key behaves as the key itself.
  ///
  ...
  /// Converts explicitly to the precomputed hash token.
  ///
  /// \returns A reference to the precomputed hash token.
  explicit operator const F14HashToken&() const { return hash_; }

  /// Enables range conversion when the key type is transparently convertible to a range.
  template <typename T>
  using IsRangeConvertible =
      std::enable_if_t<detail::TransparentlyConvertibleToRange<T>::value>;

  /// Range view type for a key that is transparently convertible to a range.
  template <typename T>
  using RangeT =
  ...
  ///
  /// \returns A range view over the stored key.
  template <typename K = TKeyType, typename Enable = IsRangeConvertible<K>>
  constexpr explicit operator RangeT<K>() const {
    return key_;
  }

  /// Returns whether two hashed keys compare equal.
  ///
  /// \param a The first hashed key to compare.
  /// \param b The second hashed key to compare.
  /// \returns True if the two hashed keys compare equal.
  ...
  /// \param a The first hashed key to compare.
  /// \param b The second hashed key to compare.
  /// \returns True if the two hashed keys compare unequal.
  friend bool operator!=(const F14HashedKey& a, const F14HashedKey& b) {
    return !(a == b);
  }
  /// Returns whether a hashed key compares equal to a key.
  ///
  /// \param a The hashed key to compare.
  /// \param b The key to compare against.
  /// \returns True if the hashed key compares equal to the key.
  ...
  /// \param a The hashed key to compare.
  /// \param b The key to compare against.
  /// \returns True if the hashed key compares unequal to the key.
  friend bool operator!=(const F14HashedKey& a, const TKeyType& b) {
    return !(a == b);
  }
  /// Returns whether a key compares equal to a hashed key.
  ///
  /// \param a The key to compare.
  /// \param b The hashed key to compare against.
  /// \returns True if the key compares equal to the hashed key.
  ...
  /// \param a The key to compare.
  /// \param b The hashed key to compare against.
  /// \returns True if the key compares unequal to the hashed key.
  friend bool operator!=(const TKeyType& a, const F14HashedKey& b) {
    return !(a == b);
  }
  /// Returns whether a hashed key compares equal to a heterogeneous key.
  ///
  /// \param a The hashed key to compare.
  /// \param b The heterogeneous key to compare against.
  /// \returns True if the hashed key compares equal to the key.
  template <typename K>
  ...
  /// Returns whether a heterogeneous key compares equal to a hashed key.
  ///
  /// \param a The heterogeneous key to compare.
  /// \param b The hashed key to compare against.
  /// \returns True if the key compares equal to the hashed key.
  template <typename K>
  friend EnableHeterogeneousCompare<K, bool> operator==(
