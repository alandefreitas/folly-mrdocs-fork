 ...
 * \returns The value if the key exists, or a value-initialized default.
 */
...
/// Given a map and a key, return the value corresponding to the key in the map,
/// or the given default value if the key doesn't exist in the map.
///
/// \param map The map to search.
/// \param key The key to look up.
/// \param dflt The default value returned when the key doesn't exist.
/// \returns The value if the key exists, or the default value otherwise.
template <
    class Map,
 ...
 * \param map The map to search.
 * \param key The key to look up.
 * \param dflt A functor invoked to produce the default value.
 * \returns The value if the key exists, or the default value otherwise.
 */
template <
    class Map,
...
/// Given a map and a key, return a reference to the corresponding value, or
/// throw an exception of the specified type if the key doesn't exist.
///
/// \param map The map to search.
/// \param key The key to look up.
/// \param exceptionStrPrefix Text prepended to the thrown exception message.
 ...
 * \param map The map to search.
 * \param key The key to look up.
 * \returns The value wrapped in an Optional, or None if absent.
 */
template <
 ...
 * \param map The map to search.
 * \param key The key to look up.
 * \returns The value wrapped in an OptionalValue, or None if absent.
 */
...
/// Given a map and a key, return a reference to the corresponding value, or the
/// given default reference if the key doesn't exist in the map.
///
/// \param map The map to search.
/// \param key The key to look up.
/// \param dflt The default reference returned when the key doesn't exist.
/// \returns A reference to the value, or the default reference otherwise.
template <class Map, typename Key = typename Map::key_type>
const typename Map::mapped_type& get_ref_default(
...
/// Deleted overload that rejects a const temporary default value to prevent a
/// dangling reference.
template <class Map, typename Key = typename Map::key_type>
const typename Map::mapped_type& get_ref_default(
...
/// Given a map and a key, return a reference to the corresponding value, or the
/// reference produced by the default functor if the key doesn't exist.
///
/// \param map The map to search.
/// \param key The key to look up.
/// \param dflt A functor invoked to produce the default reference.
/// \returns A reference to the value, or the functor's result otherwise.
template <
    class Map,
    typename Key = typename Map::key_type,
    typename Func,
...
/// Given a map and a key, return a pointer to the corresponding value, or
/// nullptr if the key doesn't exist in the map.
///
/// \param map The map to search.
/// \param key The key to look up.
/// \returns A pointer to the value, or nullptr if the key doesn't exist.
template <class Map, typename Key = typename Map::key_type>
auto get_ptr(
...
/// Given a map pointer and a key, return a pointer to the corresponding value,
/// or nullptr if the map is null or the key doesn't exist.
///
/// \param map The map to search, or nullptr.
/// \param key The key to look up.
/// \returns A pointer to the value, or nullptr if the map is null or absent.
template <class Map, typename Key = typename Map::key_type>
...
///
/// \param map The map to search.
/// \param key0 The first key to look up.
/// \param key1 The second key to look up.
/// \returns A pair of pointers to the found values, each nullptr if absent.
template <class Map, typename Key = typename Map::key_type>
std::pair<const typename Map::mapped_type*, const typename Map::mapped_type*>
 ...
 *
 * \param map The map of maps to search.
 * \param key1 The first key in the path.
 * \param key2 The second key in the path.
 * \param keys The remaining keys in the path.
 * \returns The nested value if the path exists, or None otherwise.
 */
template <
    template <typename> class Optional = folly::Optional,
    class Map,
...
/// Given a map of maps and a path of keys, return a pointer to the nested
/// value, or nullptr if the key doesn't exist in the map.
///
/// \param map The map of maps to search.
/// \param key1 The first key in the path.
/// \param key2 The second key in the path.
/// \param keys The remaining keys in the path.
/// \returns A pointer to the nested value, or nullptr if the path is absent.
template <class Map, class Key1, class Key2, class... Keys>
auto get_ptr(
    const Map& map [[FOLLY_ATTR_CLANG_LIFETIMEBOUND]],
    const Key1& key1,
...
/// value, or nullptr if the key doesn't exist in the map.
///
/// \param map The map of maps to search, or nullptr.
/// \param key1 The first key in the path.
/// \param key2 The second key in the path.
/// \param keys The remaining keys in the path.
/// \returns A pointer to the nested value, or nullptr if the path is absent.
 ...
 * \param key1 The first key in the path.
 * \param key2 The second key in the path.
 * \param keysDefault The remaining keys followed by the default value.
 * \returns The nested value, or the default value if the path doesn't exist.
 */
template <
    class Map,
    class Key1,
...
/// Given a map of maps and a path of keys, return a reference to the nested
/// value, or the given default reference if the path doesn't exist.
///
/// \param map The map of maps to search.
/// \param key1 The first key in the path.
/// \param key2 The second key in the path.
/// \param keysDefault The remaining keys followed by the default reference.
/// \returns A reference to the nested value, or the default reference.
template <
    class Map,
    class Key1,
    class Key2,
