...
///
/// Similar to Python's enumerate(): each iteration exposes both the running
/// count and a reference to the current element. Usable in constexpr context.
///
/// \param r The range to iterate over.
/// \returns A range adaptor yielding index/element pairs for `r`.
template <class Range>
