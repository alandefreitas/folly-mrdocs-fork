...
/// Aliases of the heap vector containers that use polymorphic allocators.
namespace pmr {
...
class heap_vector_map
 ...
 public:
  ...
  /// Returns a const reference to the value for the given key.
  ///
  /// \param key The key to look up.
  ...
  /// Returns a reference to the value for the given key, inserting a
  /// default-constructed value if the key doesn't exist.
  ///
...
class small_heap_vector_map
 ...
 public:
  ...
  /// Returns a const iterator to the element with the given key, or end().
  ///
  /// \param key The key to search for.
  /// \returns A const iterator to the matching element, or end() if not found.
