...
/// A hash set that stores elements inline in the main table.
...
class F14ValueSet
 ...
 protected:
  ...
  /// Storage policy for this set, using value-based element containers.
 ...
 public:
  ...
  ///
  /// \param rhs The set to swap with.
  void swap(F14ValueSet& rhs) noexcept(Policy::kSwapIsNoexcept) {
    this->table_.swap(rhs.table_);
  }

  /// Invokes the visitor with each contiguous range of stored elements.
  ///
...
/// Deduction guide for F14ValueSet.
template <
    typename InputIt,
...
#if FOLLY_F14_VECTOR_INTRINSICS_AVAILABLE
/// A hash set that stores each element in a separately allocated node.
template <typename Key, typename Hasher, typename KeyEqual, typename Alloc>
class F14NodeSet
    : public f14::detail::F14BasicSet<f14::detail::SetPolicyWithDefaults<
          ...
          Alloc>> {
 protected:
  /// Storage policy for this set, using node-based element containers.
  using Policy = f14::detail::SetPolicyWithDefaults<
      f14::detail::NodeContainerPolicy,
      Key,
      Hasher,
...
/// Deduction guide for F14NodeSet.
template <
    typename InputIt,
    typename Hasher = f14::DefaultHasher<iterator_value_type_t<InputIt>>,
    typename KeyEqual = f14::DefaultKeyEqual<iterator_value_type_t<InputIt>>,
...
/// A hash set that stores elements contiguously for fast iteration.
template <typename Key, typename Hasher, typename KeyEqual, typename Alloc>
class F14VectorSet
    : public f14::detail::
 ...
 public:
  ...
  using typename Super::iterator;
  using typename Super::value_type;
  /// Iterator type that enumerates elements in reverse order.
  using reverse_iterator = typename Super::Policy::ReverseIter;
  /// Const iterator type that enumerates elements in reverse order.
  ...
  /// \param c The set to view.
  /// \returns A range over the set in reverse iteration order.
  friend Range<const_reverse_iterator> tag_invoke(
      order_preserving_reinsertion_view_fn viewFn,
      F14VectorSet const& c) noexcept {
    return {c.rbegin(), c.rend()};
  }
};
#endif // FOLLY_F14_VECTOR_INTRINSICS_AVAILABLE

/// Deduction guide for F14VectorSet.
template <
    typename InputIt,
    typename Hasher = f14::DefaultHasher<iterator_value_type_t<InputIt>>,
    typename KeyEqual = f14::DefaultKeyEqual<iterator_value_type_t<InputIt>>,
...
#if FOLLY_F14_VECTOR_INTRINSICS_AVAILABLE
/// A hash set that picks the fastest storage policy for the key type.
template <typename Key, typename Hasher, typename KeyEqual, typename Alloc>
class F14FastSet
...
///
/// \param lhs The first set to compare.
/// \param rhs The second set to compare.
...
/// \param lhs The first set to compare.
/// \param rhs The second set to compare.
/// \returns True if the sets differ in their elements.
template <typename K, typename H, typename E, typename A>
bool operator!=(
    F14ValueSet<K, H, E, A> const& lhs, F14ValueSet<K, H, E, A> const& rhs) {
  return !(lhs == rhs);
