...
class F14BasicMap {
 ...
 public:
  ...
  /// Prehashes a key with a precomputed hash and returns a reusable token.
  ///
  /// \param key The key to prehash.
  /// \param hash The precomputed hash value for the key.
  /// \returns A hash token that can be reused across F14 containers.
  F14HashToken prehash(key_type const& key, std::size_t hash) const {
    return table_.prehash(key, hash);
  }

  /// Prehashes a heterogeneous key and returns a reusable hash token.
  ///
  /// \param key The key to prehash.
  /// \returns A hash token that can be reused across F14 containers.
  template <typename K>
  ...
  /// Prehashes a heterogeneous key with a precomputed hash and returns a token.
  ///
  /// \param key The key to prehash.
  /// \param hash The precomputed hash value for the key.
  /// \returns A hash token that can be reused across F14 containers.
  ...
  /// metadata, chunk storage, and for F14NodeMap the per-node allocations).
  /// This does NOT include heap memory owned by the stored keys or values
  /// themselves. For example, if keys or values are std::string with
  ...
  /// the map and account for the dynamic allocations of each key/value.
...
/// A hash map that stores elements inline in the main table.
template <
    typename Key,
    typename Mapped,
    typename Hasher,
...
class F14ValueMap
    : public f14::detail::F14BasicMap<f14::detail::MapPolicyWithDefaults<
          ...
          Alloc>> {
 protected:
  /// Storage policy for this map, using value-based element containers.
 ...
 public:
  ...
  ///
  /// \param ilist The elements to assign.
  ...
  /// @methodset Modifiers
  ///
  /// \param rhs The map to swap with.
   ...
   * an API boundary that supports contiguous blocks of items.
   *
   * \param visitor A callable invoked with pointers bounding each contiguous range.
   */
  template <typename V>
  void visitContiguousRanges(V&& visitor) const {
...
/// Deduction guide for F14ValueMap.
template <
    typename InputIt,
...
#if FOLLY_F14_VECTOR_INTRINSICS_AVAILABLE
/// A hash map that stores each element in a separately allocated node.
template <
    typename Key,
    typename Mapped,
    typename Hasher,
...
class F14NodeMap
    : public f14::detail::F14BasicMap<f14::detail::MapPolicyWithDefaults<
          ...
          Alloc>> {
 protected:
  /// Storage policy for this map, using node-based element containers.
  using Policy = f14::detail::MapPolicyWithDefaults<
      f14::detail::NodeContainerPolicy,
      Key,
      Mapped,
...
/// Deduction guide for F14NodeMap.
template <
    typename InputIt,
    typename Hasher = f14::DefaultHasher<iterator_key_type_t<InputIt>>,
    typename KeyEqual = f14::DefaultKeyEqual<iterator_key_type_t<InputIt>>,
...
/// A hash map that stores elements contiguously for fast iteration.
template <
    typename Key,
    typename Mapped,
    typename Hasher,
...
class F14VectorMap
 ...
 public:
  ...
  using typename Super::iterator;
  using typename Super::value_type;
  /// Iterator type that traverses elements in reverse iteration order.
  using reverse_iterator = typename Super::Policy::ReverseIter;
  /// Const iterator type that traverses elements in reverse iteration order.
  ...
  /// Returns a reverse iterator to the first element in reverse order.
  /// @methodset Iterators
  ///
  /// \returns A reverse iterator to the first element in reverse order.
  ...
  /// Returns a const reverse iterator to the first element in reverse order.
  /// @methodset Iterators
  ///
  /// \returns A const reverse iterator to the first element in reverse order.
  ...
  /// Returns a reverse iterator past the last element in reverse order.
  /// @methodset Iterators
  ///
  /// \returns A reverse iterator past the last element in reverse order.
  ...
  /// Returns a const reverse iterator past the last element in reverse order.
  /// @methodset Iterators
  ///
  /// \returns A const reverse iterator past the last element in reverse order.
  ...
  /// @methodset Iterators
  ///
  /// \param riter The reverse iterator to convert.
  /// \returns The equivalent forward iterator.
  iterator iter(reverse_iterator riter) { return this->table_.iter(riter); }
  /// Converts a const reverse iterator to a const forward iterator.
  ///
  /// \param riter The reverse iterator to convert.
  /// \returns The equivalent const forward iterator.
  ...
  ///
  /// \param it The forward iterator to convert.
  /// \returns The equivalent reverse iterator.
  reverse_iterator riter(iterator it) { return this->table_.riter(it); }
  /// Converts a const forward iterator to a const reverse iterator.
  ///
  /// \param it The forward iterator to convert.
  /// \returns The equivalent const reverse iterator.
  ...
  ///
  /// \param viewFn The view customization-point tag.
  /// \param c The map to view.
  /// \returns A range over the map in reverse iteration order.
...
/// Deduction guide for F14VectorMap.
template <
    typename InputIt,
    typename Hasher = f14::DefaultHasher<iterator_key_type_t<InputIt>>,
    typename KeyEqual = f14::DefaultKeyEqual<iterator_key_type_t<InputIt>>,
...
class F14FastMap
 ...
 public:
  ...
  /// Replaces the contents with the elements of an initializer list.
  ///
  /// \param ilist The initializer list of elements to assign.
  /// \returns A reference to this map.
  ...
  ///
  /// \param rhs The map to swap contents with.
  /// @methodset Modifiers
...
/// Deduction guide for F14FastMap.
template <
    typename InputIt,
    typename Hasher = f14::DefaultHasher<iterator_key_type_t<InputIt>>,
    typename KeyEqual = f14::DefaultKeyEqual<iterator_key_type_t<InputIt>>,
...
/// \returns True if both maps contain the same elements.
...
/// \param lhs The first map.
/// \param rhs The second map.
/// \returns True if the maps do not contain the same elements.
template <typename K, typename M, typename H, typename E, typename A>
bool operator!=(
    F14ValueMap<K, M, H, E, A> const& lhs,
    F14ValueMap<K, M, H, E, A> const& rhs) {
...
/// Erases every element of the map for which the predicate returns true.
///
/// \param c The map to erase elements from.
/// \param pred Predicate invoked with each element; erase when it returns true.
