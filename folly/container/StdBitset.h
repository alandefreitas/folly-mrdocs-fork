...
struct std_bit_reference_layout {
  /// Pointer to the word containing the referenced bit.
  size_t* address;
  /// Mask selecting the referenced bit within the word.
  size_t mask;
};

/// Return the content of the 64-bit word, given an index.
///
/// \param bitset The bitset whose underlying storage is returned.
/// \returns A pointer to the underlying word storage of the bitset.
template <size_t N>
...
/// \returns The index of the first set bit at or after `start`, or `N` if none.
template <size_t N>
...
/// Callable that returns the index of the first set bit in a bitset.
struct std_bitset_find_first_fn {
  /// Return the index of the first set bit in a bitset, or bitset.size() if
  /// none.
  ///
  /// \param bitset The bitset to search.
  /// \returns The index of the first set bit, or `bitset.size()` if none.
  template <size_t N>
...
/// Function object that finds the first set bit in a bitset.
inline constexpr std_bitset_find_first_fn std_bitset_find_first{};

/// Callable that returns the index of the first set bit after a given index.
struct std_bitset_find_next_fn {
  ...
  ///
  /// \param bitset The bitset to search.
  /// \param prev The index after which to start searching.
  /// \returns The index of the first set bit after `prev`, or `bitset.size()`
  /// if none.
  template <size_t N>
  FOLLY_ALWAYS_INLINE size_t
...
/// Function object that finds the next set bit in a bitset after a given index.
