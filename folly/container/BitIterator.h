...
class BitIterator;
/// Find the first set bit in a range of bit iterators.
///
/// \param begin Iterator to the beginning of the bit range.
/// \param end Iterator past the end of the bit range.
/// \returns A BitIterator to the first set bit, or end if none are set.
template <class BaseIter>
...
class BitIterator : public bititerator_detail::BitIteratorBase<BaseIter>::type {
 public:
   ...
   *
   * \returns The number of bits per underlying block.
   */
   ...
   * in iter.
   *
   * \param iter The base iterator to wrap.
   * \param bitOff The initial bit offset within the pointed-to block.
   */
  ...
  /// Return the current bit offset within the underlying block.
  ///
  /// \returns The bit offset within the current block.
  size_t bitOffset() const { return bitOffset_; }

  /// Advance to bit offset 0 of the next underlying block.
  void advanceToNextBlock() {
    bitOffset_ = 0;
    ++this->base_reference();
  }

  /// Reset the iterator to a base iterator at bit offset 0.
  ///
  /// \param other The base iterator to point at.
  /// \returns A reference to this BitIterator.
 ...
 private:
  ...
  /// Grants findFirstSet access to the iterator internals.
  ///
  /// \param begin Iterator to the beginning of the bit range.
  /// \param end Iterator past the end of the bit range.
  /// \returns A BitIterator to the first set bit, or end if none are set.
  friend BitIterator findFirstSet<>(BitIterator begin, BitIterator end);
 ...
 *
 * \param iter The base iterator to wrap.
 * \returns A BitIterator positioned at the first bit of iter.
 */
template <class BaseIter>
