 ...
 *  });
 *
 * \param range The range or tuple-like sequence to iterate over.
 * \param func The callable invoked for each element.
 * \returns The `func` callable after it has been applied to every element.
 */
 ...
 * required element.
 *
 * \param sequence The sequence to index into.
 * \param index The position of the element to access.
 * \returns A reference to the element of `sequence` at `index`.
 */
...
/// Forms a local variable name from "FOR_EACH_", `x`, and __LINE__.
///
/// This lets FOR_EACH be nested without creating shadowed declarations.
///
/// \param x Token pasted between the prefix and the current line number.
