...
class GenerationalCacheMap {
 ...
 public:
  /// Constructs a cache that retains approximately `capacity` entries.
  ///
  /// \param capacity The approximate number of entries to retain.
  ...
  /// Deleted copy constructor; the cache is neither copyable nor movable.
  ///
  /// \param other The cache that would be copied.
  GenerationalCacheMap(const GenerationalCacheMap& other) = delete;
  /// Deleted move constructor; the cache is neither copyable nor movable.
  ///
  /// \param other The cache that would be moved from.
  GenerationalCacheMap(GenerationalCacheMap&& other) = delete;
  /// Deleted copy assignment; the cache is neither copyable nor movable.
  ///
  /// \param other The cache that would be copied.
  /// \returns A reference to this cache.
  GenerationalCacheMap& operator=(const GenerationalCacheMap& other) = delete;
  /// Deleted move assignment; the cache is neither copyable nor movable.
  ///
  /// \param other The cache that would be moved from.
  /// \returns A reference to this cache.
  GenerationalCacheMap& operator=(GenerationalCacheMap&& other) = delete;

  /// Destroys the cache and frees the live generations.
  ...
  class Ref {
   public:
    /// Destroys the reference, releasing the generation it pinned.
    ~Ref() = default;
    ...
    /// Deleted copy constructor; a Ref is a unique scoped handle.
    ///
    /// \param other The reference that would be copied.
    Ref(const Ref& other) = delete;
    /// Deleted move assignment; a Ref is not meant to be reused.
    ///
    /// \param other The reference that would be moved from.
    /// \returns A reference to this Ref.
    Ref& operator=(Ref&& other) = delete;
    /// Deleted copy assignment; a Ref is not meant to be reused.
    ///
    /// \param other The reference that would be copied.
    /// \returns A reference to this Ref.
    ...
    /// Returns a reference to the found value.
    ///
    /// \returns A reference to the found value.
    ...
    /// \returns A pointer to the found value.
    const Value* operator->() const { return value_; }
    /// Returns a pointer to the found value, or nullptr if absent.
    ///
    /// \returns A pointer to the found value, or nullptr if absent.
   ...
   * \returns A Ref to the found value, or an empty Ref if the key is absent.
   */
   ...
   *
   * \returns The combined number of entries across both generations.
   */
  size_t size() const {
    auto holder = make_hazard_pointer();
    auto* version = holder.protect(version_);
