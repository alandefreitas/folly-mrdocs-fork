...
/// Function object that static-casts a span's element type to `U`.
template <typename U>
struct static_span_cast_fn {
  /// Casts `in` to a span of `U` over the same memory using static_cast.
  ///
  /// \param in The source span.
  /// \returns A span of `U` referring to the same region.
...
/// Casts a span to `U` by static_cast, preserving the referenced memory.
template <typename U>
inline constexpr static_span_cast_fn<U> static_span_cast;

/// Function object that reinterpret-casts a span's element type to `U`.
template <typename U>
...
/// Casts a span to `U` by reinterpret_cast, preserving the referenced memory.
template <typename U>
inline constexpr reinterpret_span_cast_fn<U> reinterpret_span_cast;

/// Function object that const-casts a span's element type to `U`.
template <typename U>
...
/// Casts a span to `U` by const_cast, preserving the referenced memory.
template <typename U>
