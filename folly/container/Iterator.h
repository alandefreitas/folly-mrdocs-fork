 ...
 *   }
 *
 * \param args The arguments to pack into the emplace_args tuple.
 * \returns An emplace_args holding the (decayed) arguments.
 */
template <typename... Args>
 ...
 *
 * \param args The argument pack to extract from.
 * \returns The Ith argument, forwarded and unwrapped.
 */
...
///
/// \param args The argument pack to extract from.
/// \returns The Ith argument, unwrapped.
template <size_t I, typename... Args>
decltype(auto) get_emplace_arg(emplace_args<Args...>& args) noexcept {
...
/// Getter function for unpacking the Ith element of a generic rvalue tuple.
///
/// \param args The tuple-like object to extract from.
/// \returns The Ith element, moved.
template <size_t I, typename Args>
decltype(auto) get_emplace_arg(Args&& args) noexcept {
  return std::get<I>(std::move(args));
}
/// Getter function for unpacking the Ith element of a generic lvalue tuple.
///
/// \param args The tuple-like object to extract from.
...
/// Getter function for unpacking the Ith element of a generic const tuple.
///
/// \param args The tuple-like object to extract from.
/// \returns The Ith element.
template <size_t I, typename Args>
 ...
 * std::tuple argument.
 *
 * \param c The container to emplace into.
 * \param i The iterator position to emplace at.
 * \returns An emplace_iterator for `c` at position `i`.
 */
 ...
 *
 * \param c The container to emplace into.
 * \param i A hint iterator passed to the container's emplace_hint.
 * \returns A hint_emplace_iterator for `c`.
 */
template <bool implicit_unpack = true, typename Container>
 ...
 *
 * \param c The container to emplace into.
 * \returns A front_emplace_iterator for `c`.
 */
template <bool implicit_unpack = true, typename Container>
 ...
 *
 * \param c The container to emplace into.
 * \returns A back_emplace_iterator for `c`.
 */
template <bool implicit_unpack = true, typename Container>
...
/// Customization point object for accessing a container element by index.
///
...
class index_iterator {
 ...
 public:
  ...
  /// The container type this iterator traverses.
  using container_type = Container;
  /// The unsigned index type used by the container.
  using size_type = detected_or_t<std::size_t, get_size_type_t, Container>;

  // iterator types

  /// The value type of the elements the iterator refers to.
  using value_type = typename std::remove_const_t<container_type>::value_type;
  /// The iterator category, modeling a random access iterator.
  ...
  /// The signed difference type between two iterators.
  ...
  /// Returns a pointer to the container this iterator refers to.
  ///
  /// \returns The associated container pointer.
  container_type* get_container() const { return container_; }
  /// Returns the current index within the container.
  ///
  /// \returns The current index.
  difference_type get_index() const { return index_; }

  /// Constructs a singular iterator not bound to any container.
  constexpr index_iterator() = default;

  /// Constructs an iterator over `container` positioned at `index`.
  ///
  /// \param container The container to iterate over.
  /// \param index The initial index within the container.
  constexpr index_iterator(container_type& container, size_type index)
      : container_(&container), index_(index) {}

  // converting constructors --

  /// Converts a mutable iterator into a const iterator over the same container.
  ///
  ...
  /// Returns the element at the iterator's current position.
  ///
  ...
  /// Returns a pointer-like accessor to the current element.
  ///
  /// \returns A pointer or arrow proxy for the current element.
  ...
  /// \returns A reference to the element at that offset.
  ...
  /// Advances the iterator to the next position.
  ///
  /// \param unused Unused tag distinguishing the postfix operator.
  /// \returns A copy of the iterator before advancing.
  constexpr index_iterator operator++(int unused) {
    auto tmp = *this;
    ++*this;
    return tmp;
  }
  ...
  /// Returns the iterator advanced by `n` positions.
  ///
  /// \param x The iterator to advance.
  /// \param n The number of positions to advance.
  ...
  /// Returns the iterator moved back by `n` positions.
  ///
  /// \param x The iterator to move back.
  /// \param n The number of positions to move back.
  /// \returns The moved iterator.
  constexpr friend index_iterator operator-(
      index_iterator x, difference_type n) {
    return x -= n;
  }
  ...
  ///
  /// \param x The left-hand iterator.
  /// \param y The right-hand iterator.
  /// \returns The signed distance from `y` to `x`.
  ...
  /// Returns true if `x` is ordered before or equal to `y`.
  ///
  /// \param x The left-hand iterator.
  /// \param y The right-hand iterator.
  /// \returns True if `x` does not follow `y`.
  friend constexpr bool operator<=(
      const index_iterator& x, const index_iterator& y) {
    return !(y < x);
  }

  /// Returns true if `x` is ordered after or equal to `y`.
  ///
  /// \param x The left-hand iterator.
  /// \param y The right-hand iterator.
  /// \returns True if `x` does not precede `y`.
  friend constexpr bool operator>=(
      const index_iterator& x, const index_iterator& y) {
    return !(x < y);
  }

  /// Returns true if `x` is ordered after `y`.
  ///
  /// \param x The left-hand iterator.
  /// \param y The right-hand iterator.
  /// \returns True if `x` follows `y`.
  friend constexpr bool operator>(
      const index_iterator& x, const index_iterator& y) {
    return y < x;
  }
