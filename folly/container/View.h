...
///
/// Provides an order such that if entries are inserted into a new instance in
/// that order, iteration order of the new instance matches the original's. This
/// can be useful for containers that have defined but non-FIFO iteration order,
/// such as F14Vector*.
///
...
struct order_preserving_reinsertion_view_fn {
 ...
 public:
  /// Returns the order-preserving reinsertion view for `container`.
  ///
  /// \param container The container to produce a view of.
  /// \returns The tag-invoke result providing the reinsertion view.
...
/// Customization point object yielding an order-preserving reinsertion view.
///
...
struct order_preserving_reinsertion_view_or_default_fn {
 ...
 public:
  /// Returns the reinsertion view for `container` via its tag-invoke extension.
  ///
  /// \param container The container to produce a view of.
  /// \returns The tag-invoke result providing the reinsertion view.
  template <typename Container>
  ...
  /// Returns `container` unchanged when no reinsertion extension is available.
  ///
...
/// Customization point returning a reinsertion view, or the argument by default.
///
/// \implementationdefined
FOLLY_DEFINE_CPO(
