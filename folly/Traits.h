...
///
/// \returns An index_constant whose value is the parsed digits.
...
/// A type which determines whether the type parameter is constexpr
...
template <typename T>
struct remove_cvref {
  /// The type T with all const, volatile, and reference qualifiers removed.
  using type =
...
template <typename Src, typename Dst>
struct like {
  /// The type `Dst` carrying the cvref category derived from `Src`.
...
/// A trait variable that is true when one type is nothrow-convertible to
/// another.
...
/// Detection helpers generated for the IsRelocatable member type.
///
/// \implementationdefined
FOLLY_HAS_TRUE_XXX(IsRelocatable);
/// Detection helpers generated for the IsZeroInitializable member type.
///
/// \implementationdefined
...
struct Ignore {
  /// Default constructor.
  Ignore() = default;
  /// Implicitly constructs from any value, discarding it.
  /// \param value The value to discard.
  template <class T>
  constexpr /* implicit */ Ignore(const T& value) {}
  /// Assigns from any value, discarding it.
  /// \param other The value to discard.
  /// \returns A reference to this instance.
  template <class T>
  const Ignore& operator=(T const& other) const {
    return *this;
  }
};
...
/// \param lhs The left-hand operand, ignored.
/// \param rhs The right-hand operand, ignored.
/// \returns An Ignore value.
Ignore operator==(Ignore lhs, Ignore rhs);
...
template <class... Ts>
struct StrictConjunction : std::bool_constant<(Ts::value && ...)> {};

/// A logical disjunction of traits that evaluates all sub-conditions eagerly.
template <class... Ts>
struct StrictDisjunction : std::bool_constant<(Ts::value || ...)> {};
...
/// \param x The value to test.
/// \returns `true` if `x` is less than or equal to zero.
template <typename T>
...
/// \param x The value to test.
/// \returns `true` if `x` is greater than or equal to zero.
template <typename T>
...
/// \param lhs The runtime left-hand operand.
...
/// Safely compares whether `lhs` is greater than the compile-time constant
/// `rhs`.
/// \param lhs The runtime left-hand operand.
...
/// An alias for the unsigned integer type with `2^lg_bits` bits.
...
/// An alias for the signed integer type with `2^lg_bits` bits.
template <std::size_t lg_bits>
...
/// A specialization that inherits one indexed type per pack element.
template <std::size_t... I, typename... T>
...
struct type_pack_element_test {
  /// Deduces the indexed type for index I from the set pointer.
  /// \param set A pointer to the indexed-type set to probe.
  /// \returns The indexed type at index I.
  template <typename T>
  static type_pack_element_indexed_type<I, T> impl(
      type_pack_element_indexed_type<I, T>* set);
};
...
/// \param list A pointer to the type list to inspect.
/// \returns A std::type_identity wrapping the Ith element type.
...
/// \param list A pointer to the type list to inspect.
/// \returns An index_constant holding the number of elements.
template <template <typename...> class List, typename... T>
index_constant<sizeof...(T)> type_list_size_(List<T...> const* list);
...
/// A specialization exposing the size and elements of a value list.
