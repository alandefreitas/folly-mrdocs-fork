...
class MPMCQueue
 ...
 public:
  ...
  /// Construct a queue that can hold at most `queueCapacity` elements.
  ///
  /// \param queueCapacity The maximum number of elements the queue can hold.
  explicit MPMCQueue(size_t queueCapacity) : Base(queueCapacity) {
    initQueue(queueCapacity);
  }

  /// Construct a queue with a given capacity and slot allocator.
  ///
  /// \param queueCapacity The maximum number of elements the queue can hold.
  /// \param alloc The allocator used to allocate the queue slots.
  MPMCQueue(size_t queueCapacity, const SlotAllocator& alloc)
      : Base(queueCapacity), SlotAllocator(alloc) {
    initQueue(queueCapacity);
  }

  /// Construct an empty queue with no capacity.
  MPMCQueue() noexcept = default;
  /// Move-construct from another queue, leaving it empty.
  ///
  /// \param other The queue to move from.
  MPMCQueue(MPMCQueue&& other) noexcept = default;
  /// Move-assign from another queue.
  ///
  ...
  /// Destroy the queue and release its slots.
  ~MPMCQueue() {
...
class MPMCQueue<T, Atom, true, Allocator>
 ...
 public:
  ...
  /// Move-assign from another dynamic queue.
  ///
  /// IMPORTANT: The move operator is here to make it easier to perform
  /// the initialization phase, it is not safe to use when there are any
  /// concurrent accesses (this is not checked).
