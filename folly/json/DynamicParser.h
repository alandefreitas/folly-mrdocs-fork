...
struct FOLLY_EXPORT DynamicParserParseError : public std::runtime_error {
   ...
   *
   * \param error The structured error report, as produced by releaseErrors().
   */
   ...
   *
   * \returns The structured error report.
   */
...
struct FOLLY_EXPORT DynamicParserLogicError : public std::logic_error {
   ...
   *
   * \param args The values to stringify and join into the error message.
   */
...
/// A small DSL for losslessly parsing a folly::dynamic into another
/// representation while recording any parts that cause errors.
class DynamicParser {
 public:
  /// Controls how the parser reacts to parse errors.
  enum class OnError {
    /// After parsing, releaseErrors() reports all parse errors. Throws
  ...
  /**
   * Constructs a parser over the given input. You MUST NOT destroy `d`
   * before the parser.
   *
   * \param on_error How the parser reports errors.
   * \param d The input dynamic to parse.
   */
   ...
   *
   * \returns A structured description of all parse errors.
   */
   ...
   *
   * \param key The key to look up in the current object.
   * \param fn The callback invoked with the converted key and value.
   */
  template <typename Fn>
  void optional(const folly::dynamic& key, Fn fn);
   ...
   */
  template <typename Fn>
  void required(const folly::dynamic& key, Fn fn);
   ...
   *
   * \param b Whether to report non-string keys in errors.
