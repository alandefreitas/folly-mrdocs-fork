 ...
 * @param d  The dynamic value to convert.
 ...
 * @returns A dynamic representing the given value.
...
struct iterator_class_is_container {
  /// The nested iterator type of T.
  using some_iterator = typename T::iterator;
  /// Container-detection results.
  enum {
    /// True when T has a value_type and is constructible from an iterator range.
...
template <typename T>
struct Dereferencer {
  /// Reject dereferencing an object iterator when a plain value is expected.
  /// \param mem Cache slot for the converted value.
  /// \param it The object item iterator being dereferenced.
  ...
  /// Convert the current array element and store it in the cache.
  /// \param mem Cache slot for the converted value.
  /// \param it The array iterator being dereferenced.
  static inline void derefToCache(
...
struct DynamicConstructor {
  /// Construct a dynamic directly from the given value.
  /// \param x The value to wrap in a dynamic.
  /// \returns A dynamic holding the value.
