...
/// Test-only accessor for ThrottledLifoSem internals.
...
class ThrottledLifoSem {
 public:
  /// Configuration options for the semaphore.
  struct Options {
    /// Maximum delay before a sleeping waiter is awoken.
    std::chrono::nanoseconds wakeUpInterval = {};
  };

  /// Constructs the semaphore with default options and an initial value.
  ///
  /// Setting initialValue is equivalent to calling post(initialValue)
  /// immediately after construction.
  ///
  /// \param initialValue The initial semaphore value.
  explicit ThrottledLifoSem(uint32_t initialValue = 0)
      : ThrottledLifoSem(Options{}, initialValue) {}
  /// Constructs the semaphore with the given options and an initial value.
  ///
  ...
  /// Destroys the semaphore, asserting that no waiters remain.
  ~ThrottledLifoSem() {
  ...
  /// Conditionally posts the given value if enough waiters are present.
  ///
  ...
  ///
  /// \param n The value to post.
  /// \returns `true` if the value was posted.
  bool try_post(uint32_t n = 1) { return postImpl</* kTry */ true>(n); }

  /// Posts the given value to the semaphore.
  ///
  ...
  ///
  /// \param n The value to post.
  /// \returns `true` if there are enough waiters to consume the updated value.
  bool post(uint32_t n = 1) { return postImpl</* kTry */ false>(n); }

  /// Attempts to decrement the semaphore without blocking.
  ///
  /// \returns `true` if a value was consumed.
  ...
  /// Waits until the semaphore can be decremented or the timeout elapses.
  ///
  /// \param timeout The maximum time to wait, measured from now.
  /// \param opt The wait options controlling the spin/block behavior.
  /// \returns `true` if a value was consumed before the timeout.
  ...
  /// Waits until the semaphore can be decremented or the deadline expires.
  ///
  /// \param deadline The time point after which waiting is abandoned.
  /// \param opt The wait options controlling the spin/block behavior.
  /// \returns `true` if a value was consumed before the deadline.
  ...
  /// Returns a best-effort estimate of the current value.
  ///
  /// \returns The approximate semaphore value.
  uint32_t valueGuess() const {
    return static_cast<uint32_t>(state_.load() & kValueMask);
  }

  /// Returns a best-effort estimate of the value not yet claimed by waiters.
  ///
  ...
  ///
  /// \returns The approximate excess value; negative if waiters exceed posts.
