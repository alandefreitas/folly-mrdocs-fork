...
class Baton {
 public:
  ...
  /// Constructs a Baton in the initial, unposted state.
  constexpr Baton() noexcept : state_(INIT) {}

  /// Copy construction is disabled.
  ///
  /// \param other The baton that would be copied.
  Baton(Baton const& other) = delete;
  /// Copy assignment is disabled.
  ///
  /// \param other The baton that would be assigned from.
  /// \returns A reference to this baton.
  Baton& operator=(Baton const& other) = delete;
  ...
  ///
  /// \param timeout The maximum duration to wait for the baton to be posted.
  /// \returns True if the baton was posted before the timeout, false otherwise.
  template <typename Rep, typename Period>
  ...
  ///
  /// \param deadline The time point past which the wait gives up.
