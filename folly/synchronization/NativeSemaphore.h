...
class NativeSemaphore {
 public:
  /// The largest initial count the semaphore accepts.
  ...
  ///
  /// \param value The initial count; must not exceed value_max_v.
  ...
  /// Destroys the semaphore and releases its native handle.
  ~NativeSemaphore() {
    if (!CloseHandle(sem_)) {
  ...
  /// Increments the semaphore count, waking a waiter if any.
  void post() {
  ...
  ///
  /// \returns True if the count was decremented, false otherwise.
  bool try_wait() {
