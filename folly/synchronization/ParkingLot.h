...
/// Implementation details for the parking lot.
...
enum class ParkResult {
  Skip, ///< The pre-park check declined to park.
  Unpark, ///< The waiter was woken by an unpark.
  Timeout, ///< The wait timed out before being unparked.
};
...
///
/// ParkingLot provides an interface similar to Linux's futex system call, built
/// portably on `std::mutex` and `std::condition_variable`. Unlike a futex, its
/// park/unpark operations take lambdas, so nearly anything can be done while
...
class ParkingLot {
 ...
 public:
  /// Constructs a parking lot with a unique lot id.
  ParkingLot() : lotid_(parking_lot_detail::idallocator++) {}

  /// Parks the caller indefinitely until unparked.
  ///
  /// Key is almost always the address of a variable. `toPark` runs while
  /// holding the bucket lock, usually to check whether the caller can sleep.
  /// `preWait` is usually used to implement condition-variable-like handoff.
  ///
  /// \param key The address-like key identifying the waiter.
  /// \param data The data to associate with the wait node.
  /// \param toPark Predicate run under the bucket lock deciding whether to park.
  /// \param preWait Callback run just before waiting.
  /// \returns The result of the park operation.
  ...
  /// Parks the caller until unparked or until the given deadline.
  ///
  /// \param key The address-like key identifying the waiter.
  /// \param data The data to associate with the wait node.
  /// \param toPark Predicate run under the bucket lock deciding whether to park.
  ...
  /// Parks the caller until unparked or until the given timeout elapses.
  ///
  /// \param key The address-like key identifying the waiter.
  /// \param data The data to associate with the wait node.
  /// \param toPark Predicate run under the bucket lock deciding whether to park.
  ...
  /// Visits the waiters for a key, waking or retaining each as directed.
  ///
  /// Key is the same address used in park(). `func` is given each waiter's data
  /// and returns an `UnparkControl`; the Remove* results remove and wake the
  /// waiter, while the Retain* results leave it, and the *Break results stop
  /// iteration.
  ///
  /// \param key The address-like key identifying the waiters.
  /// \param func The callback deciding what to do with each waiter.
