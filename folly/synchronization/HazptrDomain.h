...
class hazptr_domain {
 ...
 public:
  ...
  /// Deleted copy constructor; domains are not copyable.
  hazptr_domain(const hazptr_domain& other) = delete;
  /// Deleted move constructor; domains are not movable.
  hazptr_domain(hazptr_domain&& other) = delete;
  /// Deleted copy assignment; domains are not copyable.
  hazptr_domain& operator=(const hazptr_domain& other) = delete;
  /// Deleted move assignment; domains are not movable.
  hazptr_domain& operator=(hazptr_domain&& other) = delete;

  /// Reports whether this is the default domain.
  ///
  /// \returns true if this is the default domain, otherwise false.
  bool is_default_domain() const noexcept { return is_default_domain_; }

  /// Sets the executor used for asynchronous reclamation.
  ///
  /// \param exfn The executor function to use.
  void set_executor(ExecutorFn exfn) {
    exec_fn_.store(exfn, std::memory_order_release);
  }

  /// Clears the executor, reverting to synchronous reclamation.
  void clear_executor() {
    exec_fn_.store(&no_executor, std::memory_order_release);
  }

  /// Retires an object non-intrusively, allocating a wrapper node.
  ///
  /// \param obj The object to retire.
  /// \param reclaim The deleter used to reclaim the object.
  ...
  /// Reclaims all retired objects tagged with the given cohort.
  ///
  /// \param cohort The cohort whose tagged objects should be reclaimed.
  ...
  /// Partitions a list of objects by whether they carry the given tag.
  ///
  /// \param ftag The full cohort tag to match against.
  /// \param obj The head of the list of objects to partition.
  /// \param match Receives the objects whose tag matches.
  /// \param nomatch Receives the objects whose tag does not match.
  void list_match_tag(
...
///
/// \returns A reference to the process-wide default domain.
template <template <typename> class Atom>
...
/// Pushes a list of retired objects into a domain.
///
/// \param l The list of retired objects to push.
/// \param domain The domain to push the objects into.
template <template <typename> class Atom>
...
/// Retires an object to the default domain.
///
/// \param obj The object to retire.
/// \param reclaim The deleter used to reclaim the object.
...
/// Reclaims all reclaimable objects retired to the domain.
///
