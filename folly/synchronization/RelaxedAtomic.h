...
/// Like `std::atomic`, but without the `std::memory_order` parameters on any
/// member function.
///
/// All operations use `std::memory_order_relaxed` rather than the
/// `std::memory_order_seq_cst` default of `std::atomic`. Useful for values that
/// may be loaded and stored concurrently, such as counters, but which do not
/// guard any associated data.
template <typename T>
...
/// A `relaxed_atomic` specialization for `bool`.
template <>
...
/// An atomic wrapper whose operations default to relaxed memory ordering.
template <typename T>
...
/// A `relaxed_atomic` specialization for pointer types.
template <typename T>
...
/// Deduces the value type of a `relaxed_atomic` from its initializer.
template <typename T>
