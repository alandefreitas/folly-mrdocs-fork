...
struct PicoSpinLock {
  /// The unsigned counterpart of `IntType` used internally.
 ...
 public:
  /// Bit mask selecting the bit used as the lock flag.
  static constexpr UIntType kLockBitMask_ = UIntType(1) << Bit;
  /// The integer holding both the lock bit and the user payload.
  alignas(atomic_ref<UIntType>::required_alignment) mutable UIntType lock_;

  /**
   * Initializes the lock to an unlocked state with the given payload.
   *
   ...
   *
   * \param initialValue The initial payload value; must not set the lock bit.
   */
  ...
  /**
   * Returns the payload stored alongside the lock bit.
   *
   ...
   * as you normally get.)
   *
   * \returns The payload value with the lock bit cleared.
   */
  ...
  /**
   * Sets the payload stored alongside the lock bit.
   *
   ...
   *
   * \param w The new payload value; must not set the lock bit.
   */
