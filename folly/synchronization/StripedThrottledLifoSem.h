...
class StripedThrottledLifoSem {
 public:
  /// Constructs the semaphore with the given number of stripes and payload.
  ///
  /// \param numStripes The number of independent stripes; must be positive.
  /// \param payloadArgs The arguments used to construct each stripe's payload.
  /// \param options The throttling options applied to every stripe.
  ...
  /// Constructs the semaphore with default-constructed stripe payloads.
  ///
  /// \param numStripes The number of independent stripes; must be positive.
  /// \param options The throttling options applied to every stripe.
  ...
  ///
  /// \param stripeIdx The index of the stripe to post to.
  /// \returns `true` if a waiter was available to consume the post.
  ...
  ///
  /// \param preferredStripeIdx The stripe to try first.
  /// \returns The index of the stripe that received the post, or empty if none.
  ...
  ///
  /// \param preferredStripeIdx The stripe to try first.
  /// \param opt The wait options controlling the spin/block behavior.
  /// \returns The index of the stripe that received the post.
  ...
  ///
  /// \param preferredStripeIdx The stripe to try first.
  /// \param timeout The maximum time to wait, measured from now.
  /// \param opt The wait options controlling the spin/block behavior.
  /// \returns The index of the stripe that received the post, or empty on timeout.
  ...
  ///
  /// \param preferredStripeIdx The stripe to try first.
  /// \param deadline The time point after which waiting is abandoned.
  /// \param opt The wait options controlling the spin/block behavior.
  /// \returns The index of the stripe that received the post, or empty on timeout.
  ...
  ///
  /// \returns The approximate sum of values across all stripes.
  ...
  /// \returns A reference to the stripe's payload.
  ...
  ///
  /// \returns The number of posts transferred during this step.
  size_t balanceStep() {
...
class StripedThrottledLifoSemBalancer {
 ...
 public:
  /// Constructs the balancer; the private tag restricts construction.
  ///
  /// \param tag A private tag that gates public construction.
  explicit StripedThrottledLifoSemBalancer(PrivateTag tag) {}

  /// Registers a semaphore to be balanced by the background worker.
  ///
  /// \param sem The semaphore to subscribe.
  ...
  /// Unregisters a previously subscribed semaphore.
  ///
  /// \param sem The semaphore to unsubscribe.
  template <class Payload>
  ...
  /// Returns the total number of posts transferred by the balancer.
  ///
  /// \returns The cumulative transfer count.
