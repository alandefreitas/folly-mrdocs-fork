...
/// Deduces the referenced type of an atomic_ref from its constructor argument.
template <typename T>
atomic_ref(T&) -> atomic_ref<T>;

/// Function object type that builds an atomic_ref from an lvalue reference.
struct make_atomic_ref_t {
  /// Build an atomic_ref referring to the given object.
  ///
  /// \param ref The object to wrap.
  /// \returns An atomic_ref referring to `ref`.
  template <
      typename T,
      typename...,
...
/// The callable that builds an atomic_ref from an lvalue reference.
