...
class SaturatingSemaphore {
 ...
 public:
  /// Returns the default wait options used by the blocking calls.
  ///
  /// \returns A default-constructed set of wait options.
  ...
  /// Checks whether the semaphore has been posted.
  ///
  /// \returns `true` if the semaphore is in the ready state.
  ...
  /// Blocks until the semaphore becomes ready.
  ///
  ...
  /// Checks readiness without blocking.
  ///
  /// \returns `true` if the semaphore is ready.
  FOLLY_ALWAYS_INLINE bool try_wait() noexcept { return ready(); }

  /// Waits until the semaphore is ready or the deadline expires.
  ///
  /// \param deadline The time point after which waiting is abandoned.
  /// \param opt The wait options controlling the spin/block behavior.
  /// \returns `true` if the semaphore became ready before the deadline.
  ...
  /// Waits until the semaphore is ready or the timeout elapses.
  ///
  /// \param duration The maximum time to wait, measured from now.
  /// \param opt The wait options controlling the spin/block behavior.
  /// \returns `true` if the semaphore became ready before the timeout.
