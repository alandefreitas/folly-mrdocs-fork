...
class WaitOptions {
 public:
  /// Default values used by WaitOptions.
  struct Defaults {
    /// spin_max
    ///
    ...
    /// Whether wait logging is enabled by default.
    static constexpr bool logging_enabled = true;
  };

  /// Get the maximum spin duration before blocking.
  ///
  /// \returns The current maximum spin duration.
  constexpr std::chrono::nanoseconds spin_max() const { return spin_max_; }
  /// Set the maximum spin duration before blocking.
  ///
  /// \param dur The new maximum spin duration.
  /// \returns A reference to these options.
  constexpr WaitOptions& spin_max(std::chrono::nanoseconds dur) {
    spin_max_ = dur;
    return *this;
  }
  /// Get whether wait logging is enabled.
  ///
  /// \returns `true` if wait logging is enabled.
  constexpr bool logging_enabled() const { return logging_enabled_; }
  /// Set whether wait logging is enabled.
  ///
  /// \param enable Whether to enable wait logging.
  /// \returns A reference to these options.
