...
enum class annotate_rwlock_level : long {
  rdlock = 0, ///< A shared (reader) lock.
  wrlock = 1, ///< An exclusive (writer) lock.
};
...
class annotate_ignore_thread_sanitizer_guard {
 public:
  /// Begins ignoring sanitizer reads and writes for the current scope.
  ///
  /// \param file The source file name to attribute the annotation to.
  /// \param line The source line number to attribute the annotation to.
  ...
  annotate_ignore_thread_sanitizer_guard(
      const annotate_ignore_thread_sanitizer_guard& other) = delete;
