...
/// Customization-point objects that invoke lock member functions.
namespace access {

//  locks and unlocks

/// Invokes the `lock()` member function on its argument.
/// \implementationdefined
FOLLY_CREATE_MEMBER_INVOKER_SUITE(lock);
/// Invokes the `try_lock()` member function on its argument.
/// \implementationdefined
FOLLY_CREATE_MEMBER_INVOKER_SUITE(try_lock);
/// Invokes the `try_lock_for()` member function on its argument.
/// \implementationdefined
FOLLY_CREATE_MEMBER_INVOKER_SUITE(try_lock_for);
/// Invokes the `try_lock_until()` member function on its argument.
/// \implementationdefined
FOLLY_CREATE_MEMBER_INVOKER_SUITE(try_lock_until);
/// Invokes the `unlock()` member function on its argument.
/// \implementationdefined
FOLLY_CREATE_MEMBER_INVOKER_SUITE(unlock);
/// Invokes the `lock_shared()` member function on its argument.
/// \implementationdefined
FOLLY_CREATE_MEMBER_INVOKER_SUITE(lock_shared);
/// Invokes the `try_lock_shared()` member function on its argument.
/// \implementationdefined
FOLLY_CREATE_MEMBER_INVOKER_SUITE(try_lock_shared);
/// Invokes the `try_lock_shared_for()` member function on its argument.
/// \implementationdefined
FOLLY_CREATE_MEMBER_INVOKER_SUITE(try_lock_shared_for);
/// Invokes the `try_lock_shared_until()` member function on its argument.
/// \implementationdefined
FOLLY_CREATE_MEMBER_INVOKER_SUITE(try_lock_shared_until);
/// Invokes the `unlock_shared()` member function on its argument.
/// \implementationdefined
FOLLY_CREATE_MEMBER_INVOKER_SUITE(unlock_shared);
/// Invokes the `lock_upgrade()` member function on its argument.
/// \implementationdefined
FOLLY_CREATE_MEMBER_INVOKER_SUITE(lock_upgrade);
/// Invokes the `try_lock_upgrade()` member function on its argument.
/// \implementationdefined
FOLLY_CREATE_MEMBER_INVOKER_SUITE(try_lock_upgrade);
/// Invokes the `try_lock_upgrade_for()` member function on its argument.
/// \implementationdefined
FOLLY_CREATE_MEMBER_INVOKER_SUITE(try_lock_upgrade_for);
/// Invokes the `try_lock_upgrade_until()` member function on its argument.
/// \implementationdefined
FOLLY_CREATE_MEMBER_INVOKER_SUITE(try_lock_upgrade_until);
/// Invokes the `unlock_upgrade()` member function on its argument.
/// \implementationdefined
FOLLY_CREATE_MEMBER_INVOKER_SUITE(unlock_upgrade);

//  transitions
/// Invokes the `unlock_and_lock_shared()` member function on its argument.
/// \implementationdefined
FOLLY_CREATE_MEMBER_INVOKER_SUITE(unlock_and_lock_shared);
/// Invokes the `unlock_and_lock_upgrade()` member function on its argument.
/// \implementationdefined
FOLLY_CREATE_MEMBER_INVOKER_SUITE(unlock_and_lock_upgrade);
/// Invokes the `try_unlock_shared_and_lock()` member function on its argument.
/// \implementationdefined
FOLLY_CREATE_MEMBER_INVOKER_SUITE(try_unlock_shared_and_lock);
/// Invokes the `try_unlock_shared_and_lock_for()` member function on its
/// argument.
/// \implementationdefined
FOLLY_CREATE_MEMBER_INVOKER_SUITE(try_unlock_shared_and_lock_for);
/// Invokes the `try_unlock_shared_and_lock_until()` member function on its
/// argument.
/// \implementationdefined
FOLLY_CREATE_MEMBER_INVOKER_SUITE(try_unlock_shared_and_lock_until);
/// Invokes the `try_unlock_shared_and_lock_upgrade()` member function on its
/// argument.
/// \implementationdefined
FOLLY_CREATE_MEMBER_INVOKER_SUITE(try_unlock_shared_and_lock_upgrade);
/// Invokes the `try_unlock_shared_and_lock_upgrade_for()` member function on
/// its argument.
/// \implementationdefined
FOLLY_CREATE_MEMBER_INVOKER_SUITE(try_unlock_shared_and_lock_upgrade_for);
/// Invokes the `try_unlock_shared_and_lock_upgrade_until()` member function on
/// its argument.
/// \implementationdefined
FOLLY_CREATE_MEMBER_INVOKER_SUITE(try_unlock_shared_and_lock_upgrade_until);
/// Invokes the `unlock_upgrade_and_lock()` member function on its argument.
/// \implementationdefined
FOLLY_CREATE_MEMBER_INVOKER_SUITE(unlock_upgrade_and_lock);
/// Invokes the `try_unlock_upgrade_and_lock()` member function on its argument.
/// \implementationdefined
FOLLY_CREATE_MEMBER_INVOKER_SUITE(try_unlock_upgrade_and_lock);
/// Invokes the `try_unlock_upgrade_and_lock_for()` member function on its
/// argument.
/// \implementationdefined
FOLLY_CREATE_MEMBER_INVOKER_SUITE(try_unlock_upgrade_and_lock_for);
/// Invokes the `try_unlock_upgrade_and_lock_until()` member function on its
/// argument.
/// \implementationdefined
FOLLY_CREATE_MEMBER_INVOKER_SUITE(try_unlock_upgrade_and_lock_until);
/// Invokes the `unlock_upgrade_and_lock_shared()` member function on its
/// argument.
/// \implementationdefined
...
class unique_lock_base : public detail::lock_base_unique<Mutex> {
 ...
 public:
  ...
  /// Swaps the held lock state with another lock holder.
  ///
  /// \param that The lock holder to swap with.
  void swap(self& that) noexcept { base::swap(that); }

  /// Swaps the held lock state of two lock holders.
  ///
  /// \param a The first lock holder.
  /// \param b The second lock holder.
...
/// Deduces the mutex type of an `upgrade_lock` from its constructor arguments.
...
/// Deduces the mutex type of a `hybrid_lock` from its constructor arguments.
template <typename Mutex, typename... A>
...
/// Deduces the mutex type of a `hybrid_lock_guard` from its arguments.
template <typename Mutex, typename S>
hybrid_lock_guard(Mutex&, adopt_lock_state_t, S) -> hybrid_lock_guard<Mutex>;

} // namespace folly

/// The C++ standard library namespace, reopened to add deduction guides for
/// lockable-with-state mutex types.
...
/// Atomically transitions from the from-lock to the to-lock, waiting
/// unboundedly for the transition to become available.
///
/// \param lock The lock holder to transition from.
/// \returns A lock holder of the target lock type.
template <
...
/// Attempts an atomic transition from the from-lock to the to-lock, without
/// waiting if the transition is not immediately available.
///
/// \param lock The lock holder to transition from.
/// \returns A lock holder of the target lock type; empty if the attempt failed.
template <
    template <typename> class ToLock,
    typename Mutex,
    template <typename> class FromLock>
...
template <
    ...
    typename Rep,
    typename Period>
/// Attempts an atomic transition from the from-lock to the to-lock, waiting up
/// to the given timeout for the transition to become available.
///
/// \param lock The lock holder to transition from.
...
template <
    ...
    typename Duration>
/// Attempts an atomic transition from the from-lock to the to-lock, waiting up
/// to the given deadline for the transition to become available.
///
/// \param lock The lock holder to transition from.
/// \param deadline The time point past which to stop waiting.
/// \returns A lock holder of the target lock type; empty if the attempt failed.
...
//  to shared lock.
/// Immediately and atomically downgrades an exclusive lock to a shared lock.
///
/// \param lock The exclusive lock holder to transition from.
/// \returns A shared lock holder for the same mutex.
template <typename Mutex>
...
//  shared lock.
/// Immediately and atomically downgrades an upgrade lock to a shared lock.
///
/// \param lock The upgrade lock holder to transition from.
/// \returns A shared lock holder for the same mutex.
template <typename Mutex>
...
///
/// \param lock The exclusive lock holder to transition from.
/// \returns An upgrade lock holder for the same mutex.
template <typename Mutex>
...
///
/// \param lock The upgrade lock holder to transition from.
/// \returns An exclusive lock holder for the same mutex.
template <typename Mutex>
...
//  lock to unique lock.
/// Immediately attempts to atomically upgrade an upgrade lock to an exclusive
/// lock.
///
/// \param lock The upgrade lock holder to transition from.
/// \returns An exclusive lock holder; empty if the attempt failed.
template <typename Mutex>
...
//  lock to unique lock.
/// Attempts, up to a timeout, to atomically upgrade an upgrade lock to an
/// exclusive lock.
///
/// \param lock The upgrade lock holder to transition from.
/// \param timeout The maximum duration to wait for.
...
//  Represents an eventual attempted atomic upgrade transition from upgrade
//  lock to unique lock.
/// Attempts, up to a deadline, to atomically upgrade an upgrade lock to an
/// exclusive lock.
///
/// \param lock The upgrade lock holder to transition from.
/// \param deadline The time point past which to stop waiting.
...
//  lock to unique lock.
/// Immediately attempts to atomically upgrade a shared lock to an exclusive
/// lock.
///
/// \param lock The shared lock holder to transition from.
/// \returns An exclusive lock holder; empty if the attempt failed.
template <typename Mutex>
...
//  lock to unique lock.
/// Attempts, up to a timeout, to atomically upgrade a shared lock to an
/// exclusive lock.
///
/// \param lock The shared lock holder to transition from.
/// \param timeout The maximum duration to wait for.
...
//  Represents an eventual attempted atomic upgrade transition from shared
//  lock to unique lock.
/// Attempts, up to a deadline, to atomically upgrade a shared lock to an
/// exclusive lock.
///
/// \param lock The shared lock holder to transition from.
/// \param deadline The time point past which to stop waiting.
...
///
/// \param lock The shared lock holder to transition from.
/// \returns An upgrade lock holder; empty if the attempt failed.
template <typename Mutex>
...
//  Represents an eventual attempted atomic upgrade transition from shared
//  lock to upgrade lock.
/// Attempts, up to a timeout, to atomically upgrade a shared lock to an upgrade
/// lock.
///
/// \param lock The shared lock holder to transition from.
/// \param timeout The maximum duration to wait for.
