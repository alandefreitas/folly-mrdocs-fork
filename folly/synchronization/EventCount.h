...
class EventCount {
 public:
  /// Constructs an EventCount with a zero epoch and no waiters.
  EventCount() noexcept : val_(0) {}

  /// Opaque token identifying a wait epoch returned by prepareWait().
  class Key {
  ...
  /// Begins a wait, capturing the current epoch.
  ///
  /// \returns A key identifying the captured epoch, to pass to wait().
  Key prepareWait() noexcept;
  /// Cancels a wait started by prepareWait() without blocking.
  void cancelWait() noexcept;
  /// Blocks until notified, if the epoch has not changed since prepareWait().
  ///
  /// \param key The key returned by prepareWait().
   ...
   *
   * \param condition The predicate to wait on; returns true when satisfied.
   */
