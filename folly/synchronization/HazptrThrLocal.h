...
class hazptr_tc_entry {
  ...
  /// Grants the array factory access to the entry internals.
...
class alignas(hardware_constructive_interference_size) hazptr_tc {
 ...
 public:
  /// Constructs a thread cache with the minimum capacity.
  hazptr_tc() : vec_(kCapacity) {}
  /// Evicts all cached records on destruction.
  ~hazptr_tc() { evict(); }

  /// Deleted copy constructor; thread caches are not copyable.
  hazptr_tc(const hazptr_tc& other) = delete;
  /// Deleted move constructor; thread caches are not movable.
  hazptr_tc(hazptr_tc&& other) = delete;
  /// Deleted copy assignment; thread caches are not copyable.
  hazptr_tc& operator=(const hazptr_tc& other) = delete;
  /// Deleted move assignment; thread caches are not movable.
  hazptr_tc& operator=(hazptr_tc&& other) = delete;

  /// Returns the minimum guaranteed capacity of the cache.
  ///
  /// \returns The minimum number of records the cache can hold.
  ...
  /// Returns the allocated capacity of the backing store.
  ///
  /// \returns The number of records the backing store can hold.
 ...
 private:
  ...
  /// Grants the array factory access to the thread cache internals.
  /// \implementationdefined
  template <uint8_t M, template <typename> class A>
  friend hazptr_array<M, A> make_hazard_pointer_array();
  friend void hazptr_tc_evict<Atom>();
  /// Grants the thread cache tester access to the internals.
  /// \implementationdefined
...
/// Tag type identifying the hazard pointer thread cache's thread-local
/// singleton.
struct hazptr_tc_tls_tag {};
/// Returns the calling thread's hazard pointer thread cache.
///
/// \returns A reference to the thread-local hazptr_tc.
template <template <typename> class Atom>
