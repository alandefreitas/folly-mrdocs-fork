...
class HazptrSWMRSet {
 ...
 public:
  ...
  /// Destroy the set and free every remaining node.
  ~HazptrSWMRSet() {
    auto p = head_.load();
    while (p) {
  ...
  /// Insert a value into the set if it is not already present.
  ///
  ...
  /// Test whether a value is in the set. Safe to call from reader threads.
  ///
