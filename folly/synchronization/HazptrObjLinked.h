...
class hazptr_root {
 ...
 public:
  ...
  ///
  /// \param p The initial linked object, or nullptr.
  ...
  /// Returns the underlying link for reading.
  ///
  /// \returns A const reference to the atomic link.
  const Atom<T*>& operator()() const noexcept { return link_; }

  /// Returns the underlying link for modification.
  ///
  /// \returns A mutable reference to the atomic link.
...
class hazptr_obj_base_linked
 ...
 public:
  /// Retires the object, handing reclamation to the hazard pointer library.
  void retire() {
  ...
  /// Retires the object if its last link has been released.
  void unlink() {
  ...
  /// Reclaims the object if the last link is released, without checking hazard
  /// pointers.
  ///
  /// To be called only when the object cannot possibly be protected by any
  /// hazard pointer.
