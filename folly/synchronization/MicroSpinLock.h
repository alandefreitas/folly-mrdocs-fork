...
struct MicroSpinLock {
  /// The two possible states of the lock byte.
  enum {
    FREE = 0, ///< The lock is available.
    LOCKED = 1, ///< The lock is held.
  };
  /// The lock byte; can't be std::atomic<> to preserve POD-ness.
  uint8_t lock_;

  /// Initializes this lock to the free state.
  ///
  ...
  bool try_lock() noexcept {
    bool ret = xchg_acquire(LOCKED) == FREE;
 ...
 private:
  ...
  uint8_t xchg_acquire(uint8_t newVal) noexcept {
...
struct alignas(max_align_v) SpinLockArray {
  ...
  ///
  /// \param i The index of the lock to access.
  /// \returns A reference to the lock at index `i`.
  T& operator[](size_t i) noexcept { return data_[i].lock; }

  /// Returns the lock at the given index.
  ///
  /// \param i The index of the lock to access.
  /// \returns A const reference to the lock at index `i`.
  const T& operator[](size_t i) const noexcept { return data_[i].lock; }

  /// Returns the number of locks in the array.
  ///
