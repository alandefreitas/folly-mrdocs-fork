...
class rcu_domain {
 ...
 public:
  /**
   * Constructs a domain, optionally using the given executor.
   *
   * If an executor is passed, it is used to run calls and delete
   * retired objects.
   *
   * \param executor The executor used to run callbacks; defaults to a
   *   queued-immediate executor when null.
   */
  explicit rcu_domain(Executor* executor = nullptr) noexcept
      : executor_(executor ? executor : &QueuedImmediateExecutor::instance()) {}

  /// Deleted copy constructor; a domain is not copyable.
  rcu_domain(const rcu_domain& other) = delete;
  /// Deleted move constructor; a domain is not movable.
  rcu_domain(rcu_domain&& other) = delete;
  /// Deleted copy assignment; a domain is not copyable.
  rcu_domain& operator=(const rcu_domain& other) = delete;
  /// Deleted move assignment; a domain is not movable.
  rcu_domain& operator=(rcu_domain&& other) = delete;
  ...
  /**
   * Enters a read region on the current thread.
   *
  ...
  /// Exits the read region on the current thread.
  FOLLY_ALWAYS_INLINE void unlock() noexcept { counters_.decrement(); }

  /// Invokes a callback after all pre-existing readers have completed.
  ///
  ...
  ///
  /// \param cbin The callback to invoke once the grace period elapses.
  template <typename T>
  void call(T&& cbin) {
  ...
  /// Schedules a retired node's callback to run after the current grace period.
  ///
  ...
  /// readers and domains.
  ///
  /// \param node The node whose callback runs once readers have completed.
  ...
  /// Waits until all concurrent read-side critical sections have finished.
  ///
...
///
/// \param domain The domain to synchronize; defaults to the default domain.
...
///
/// \param domain The domain to drain; defaults to the default domain.
...
///
/// \param p The pointer to retire.
/// \param d The deleter invoked on `p` once readers have completed.
/// \param domain The domain that owns the retirement; defaults to the default.
...
class rcu_obj_base : detail::ThreadCachedListsBase::Node {
 public:
  /// Retires this object, invoking the deleter after a grace period.
  ///
  /// \param d The deleter invoked on this object once readers have completed.
  /// \param domain The domain that owns the retirement; defaults to the default.
