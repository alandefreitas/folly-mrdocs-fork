...
class MaybeManagedPtr {
 public:
  /// Construct an unmanaged pointer that will not free `t`.
  ///
  /// \param t The raw pointer to wrap without taking ownership.
  /* implicit */ MaybeManagedPtr(T* t)
      : t_(std::shared_ptr<T>(std::shared_ptr<void>(), t)) {}
  /// Construct a managed pointer that shares ownership of `t`.
  ///
  /// \param t The shared pointer to take ownership from.
  /* implicit */ MaybeManagedPtr(std::shared_ptr<T> t) : t_(std::move(t)) {}

  /**
   * Construction from shared_ptr<X>, where X is derived from T.
   *
   * \param q The shared pointer to a derived type to take ownership from.
   */
  template <typename Q>
   ...
   * Boolean equal to operator
   *
   * \param rhs The shared pointer to compare against.
   * @return             Returns true if the underlying shared pointer is equal
   * to rhs.
   */
