...
/// GroupVarint encoding for a group of integers of type `T`.
template <typename T>
class GroupVarint;
...
class GroupVarint<uint32_t> : public detail::GroupVarintBase<uint32_t> {
 public:
   ...
   *
   * \param a The first value to encode.
   * \param b The second value to encode.
   * \param c The third value to encode.
   * \param d The fourth value to encode.
   * \returns The number of bytes used to encode the four values.
   */
   ...
   *
   * \param p Pointer to four consecutive uint32_t values.
   * \returns The number of bytes used to encode the four values.
   */
   ...
   *
   * \param p Pointer to the values to encode.
   * \param count The number of values to encode (at most four).
   * \returns The number of bytes used to encode the first count values.
   */
   ...
   *
   * \param p Pointer to the start of the encoded buffer.
   * \param size The number of bytes available in the buffer.
   * \returns The number of values that can be decoded from the buffer.
   */
   ...
   *
   * \param p Pointer to the beginning of the encoded block.
   * \returns The number of bytes used by the encoding.
   */
   ...
   * \param c The third value to encode.
   * \param d The fourth value to encode.
   * \returns A pointer to one past the last encoded byte.
   */
   ...
   *
   * \param p Pointer to the output buffer.
   * \param src Pointer to four consecutive uint32_t values to encode.
   * \returns A pointer to one past the last encoded byte.
   */
   ...
   *
   * \param p Pointer to the encoded buffer.
   * \param a Receives the first decoded value.
   * \param b Receives the second decoded value.
   * \param c Receives the third decoded value.
   * \param d Receives the fourth decoded value.
   * \returns A pointer to one past the last encoded byte.
   */
   ...
   *
   * \param p Pointer to the encoded buffer.
   * \param dest Pointer to an array that receives the four decoded values.
   * \returns A pointer to one past the last encoded byte.
   */
...
class GroupVarint<uint64_t> : public detail::GroupVarintBase<uint64_t> {
 public:
   ...
   * \param c The third value to encode.
   * \param d The fourth value to encode.
   * \param e The fifth value to encode.
   * \returns The number of bytes used to encode the five values.
   */
  static size_t size(
   ...
   * at consecutive positions in an array.
   *
   * \param p Pointer to five consecutive uint64_t values.
   * \returns The number of bytes used to encode the five values.
   */
   ...
   *
   * \param p Pointer to the output buffer.
   * \param src Pointer to five consecutive uint64_t values to encode.
   * \returns A pointer to one past the last encoded byte.
   */
   ...
   * \param c Receives the third decoded value.
   * \param d Receives the fourth decoded value.
   * \param e Receives the fifth decoded value.
   * \returns A pointer to one past the last encoded byte.
   */
  static const char* decode(
      const char* p,
   ...
   *
   * \param p Pointer to the encoded buffer.
   * \param dest Pointer to an array that receives the five decoded values.
   * \returns A pointer to one past the last encoded byte.
   */
...
class GroupVarintEncoder {
 public:
  /// The underlying GroupVarint codec for values of type `T`.
  using Base = GroupVarint<T>;
  /// The value type being encoded.
  using type = T;

  /// Construct an encoder that writes encoded output through `out`.
  ///
  /// \param out The output function object that receives encoded byte ranges.
  explicit GroupVarintEncoder(Output out) : out_(out), count_(0) {}

  /// Destroy the encoder, flushing any buffered values.
   ...
   *
   * \returns A reference to the output function object.
   */
  Output& output() { return out_; }
  /// Return the appender that was used.
  ///
  /// \returns A const reference to the output function object.
...
class GroupVarintDecoder {
 public:
  ...
  /// Construct an empty decoder with no input.
  GroupVarintDecoder() = default;

  /// Construct a decoder over the given input data.
  ///
  /// \param data The encoded input to decode.
  /// \param maxCount The maximum number of values to decode.
  ...
  /// Reset the decoder to decode a new input.
  ///
  /// \param data The encoded input to decode.
  /// \param maxCount The maximum number of values to decode.
   ...
   *
   * \param val Receives the next decoded value.
   * \returns True if a value was read, false if the input is exhausted.
   */
  bool next(type* val) {
    if (pos_ == count_) {
      // refill
  ...
  /// Return the portion of the input that has not been decoded yet.
  ///
  /// Only valid after next() has returned false.
  ///
  /// \returns The remaining, undecoded portion of the original input.
