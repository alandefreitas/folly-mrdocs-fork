...
class rich_error_base {
 ...
 protected: // Can only construct via `rich_error<>` / `immortal_rich_error<>`
  /// Default-constructs an empty base (constructible only via leaf types).
  constexpr rich_error_base() = default;

  /// Copy constructor.
  ///
  /// \param other The base to copy from.
  rich_error_base(const rich_error_base& other) = default;
  /// Move constructor.
  ///
  /// \param other The base to move from.
  rich_error_base(rich_error_base&& other) = default;
  /// Copy assignment operator.
  ///
  /// \param other The base to copy from.
  /// \returns A reference to `*this`.
  rich_error_base& operator=(const rich_error_base& other) = default;
  /// Move assignment operator.
  ///
  /// \param other The base to move from.
  /// \returns A reference to `*this`.
  rich_error_base& operator=(rich_error_base&& other) = default;
 ...
 public:
  ...
  /// Returns a partial message for this error (prefer `<<`/`fmt` for logging).
  ///
  /// \returns A NUL-terminated partial description of the error.
  virtual const char* partial_message() const noexcept = 0;

  /// Override to support RTTI-free `get_rich_error_code()`.
  ///
  /// See the `rich_error_bases_and_own_codes` doc for how to implement this.
  ///
  ...
  ///
  /// \param query The code query to inspect and populate.
  constexpr virtual void retrieve_code(rich_error_code_query& query) const {}

  /// A `fmt`-formattable view of all codes in this error's hierarchy.
  ///
  /// Renders all the available codes in this error's inheritance hierarchy,
  /// like so: "Specific=1, General=2".
  ///
  ...
  /// Returns a `fmt`-formattable view of all codes in this error's hierarchy.
  ///
  /// \param pre_separator Text emitted before the first code, if any.
  /// \returns A `fmt_all_codes_t` bound to this error.
  ...
  /// Renders this error for `fmt` and `ostream<<`.
  ///
  /// \param out The output appender to write into.
  virtual void format_to(fmt::appender& out) const;

  /// Formats this epitaph stack, starting with its underlying error.
  ///
  /// \param out The output appender to write into.
  void format_with_epitaphs(fmt::appender& out) const;

  /// Formats only the epitaph stack, omitting the first underlying error.
  ///
  ...
  /// Returns the next error in the epitaph/nesting chain, if any.
  ///
  /// \returns Pointer to the next error, or `nullptr` when there is none.
  ...
  /// Returns the error that is actually propagating (the "underlying" error).
  ///
  /// \returns Pointer to the underlying error, or `nullptr` if `this` is it.
  constexpr const rich_exception_ptr* underlying_error() const noexcept {
    return underlying_ptr_;
  }
  /// Passkey granting folly-internal access to the underlying-error pointer.
  ...
  /// Returns mutable access to the underlying error pointer (folly-internal).
  ///
  /// \param priv Folly-internal access tag.
  /// \returns The underlying `rich_exception_ptr` pointer.
  constexpr rich_exception_ptr* mutable_underlying_error(
      underlying_error_private_t priv) noexcept {
    return underlying_ptr_;
  }

 protected:
  /// Sets the underlying error pointer (only used by `epitaph_non_value`).
  ///
  ...
  ///
  /// \param priv Folly-internal access tag.
  /// \param ptr Pointer to the underlying `rich_exception_ptr`.
  void set_underlying_error(
      underlying_error_private_t priv, rich_exception_ptr* ptr) noexcept {
    underlying_ptr_ = ptr;
  }
};
...
/// \param err The rich error to write.
/// \returns The stream `os`.
std::ostream& operator<<(std::ostream& os, const rich_error_base& err);
...
class rich_ptr_to_underlying_error {
 ...
 protected:
  template <typename, typename>
  friend class detail::rich_exception_ptr_impl;
  /// Constructs from a raw error pointer and its top-level rich error.
  ///
  /// \param p Pointer to the underlying error.
  /// \param top Pointer to the top-level rich error (for epitaph formatting).
 ...
 public:
  ...
  explicit constexpr rich_ptr_to_underlying_error(std::nullptr_t null) {}
  ...
  /// Deleted copy constructor: this type is immovable.
  ///
  /// \param other The pointer wrapper to copy from.
  rich_ptr_to_underlying_error(const rich_ptr_to_underlying_error& other) =
      delete;
  /// Deleted copy assignment: this type is immovable.
  ///
  /// \param other The pointer wrapper to copy from.
  rich_ptr_to_underlying_error operator=(
      const rich_ptr_to_underlying_error& other) = delete;
  /// Deleted move constructor: this type is immovable.
  ///
  /// \param other The pointer wrapper to move from.
  rich_ptr_to_underlying_error(rich_ptr_to_underlying_error&& other) = delete;
  /// Deleted move assignment: this type is immovable.
  ///
  /// \param other The pointer wrapper to move from.
  rich_ptr_to_underlying_error operator=(
      rich_ptr_to_underlying_error&& other) = delete;
  /// Destroys the pointer wrapper (does not affect the pointed-to error).
  ~rich_ptr_to_underlying_error() = default;

  /// Dereferences to the pointed-to error.
  ///
  /// \returns A reference to the underlying error.
  constexpr Ex& operator*() const { return *raw_ptr_; }
  /// Accesses members of the pointed-to error.
  ///
  /// \returns A raw pointer to the underlying error.
  constexpr Ex* operator->() const { return raw_ptr_; }
  /// Returns the raw pointer to the pointed-to error.
  ///
  /// \returns A raw pointer to the underlying error, or `nullptr`.
  constexpr Ex* get() const { return raw_ptr_; }

  /// Explicit conversion to a raw `Ex*`, shedding rich-error formatting context.
  ///
  ...
  /// locations, codes, etc.
  ///
  /// \returns The raw pointer to the underlying error, or `nullptr`.
  ...
  /// Tests whether the pointer is non-null, e.g. in `if (auto ex = ...)`.
  ///
  ...
  ///
  /// \param null A `nullptr` literal.
  /// \param p The rich-error pointer.
  /// \returns `true` if `p` is null.
  friend constexpr bool operator==(
      std::nullptr_t null, const rich_ptr_to_underlying_error& p) {
    return p.raw_ptr_ == nullptr;
  }
  /// Compares against a raw `Ex*`.
  ///
  /// \param raw_p The raw pointer to compare.
  /// \param p The rich-error pointer.
  /// \returns `true` if both refer to the same error.
  friend constexpr bool operator==(
      const Ex* raw_p, const rich_ptr_to_underlying_error& p) {
    return raw_p == p.raw_ptr_;
  }
  /// Compares two rich-error pointers.
  ///
...
/// User-opaque handle used by the `get_rich_error_code()` machinery.
class rich_error_code_query {
 private:
  uint64_t uuid_;
 ...
 protected:
  ...
  union {
    /// Type-erased ("mangled") code value; active for non-reserved UUIDs.
    uintptr_t mangled_code_;
    /// Callback that formats all codes; active for `kFormatterUuid`.
    bool (*formatter_fn_)( // Returns `true` if > 0 codes were formatted
        const rich_error_base&, fmt::appender&, const char* pre_separator);
  };

  /// Whether the union holds a valid value.
  ///
  ...
  /// Reserved UUID selecting the `fmt` all-codes formatter callback.
  static constexpr uint64_t kFormatterUuid = 0; // `fmt` formatting
  /// Largest reserved UUID value; user codes use UUIDs above this.
  static constexpr uint64_t kMaxReservedUuid = 100000;

  /// Constructs a query for the given rich-error-code UUID.
  ///
  /// \param uuid The ABI-stable UUID of the desired code type.
  ...
  /// Tests whether this query targets the given UUID.
  ///
  /// \param uuid The UUID to compare against.
  /// \returns `true` if `uuid` matches this query's UUID.
...
struct fmt::formatter<folly::rich_error_base> {
  ...
  /// \returns An iterator past the parsed range.
  ...
  ///
  /// \param e The rich error to format.
  /// \param ctx The format context to write into.
  /// \returns An iterator past the written output.
...
struct fmt::formatter<folly::rich_ptr_to_underlying_error<Ex>> {
 ...
 public:
  ...
  ///
  /// \param p The rich-error pointer to format.
  /// \param ctx The format context to write into.
  /// \returns An iterator past the written output.
  format_context::iterator format(
...
/// Streams a rich-error pointer, including any epitaph stack.
///
/// \param os The output stream.
/// \param ep The rich-error pointer to stream.
/// \returns The stream `os`.
template <typename Ex>
...
struct fmt::formatter<folly::rich_error_base::fmt_all_codes_t> {
  ...
  ///
  /// \param err The rich-error codes accessor to format (mutates `.saw_code_`).
  /// \param ctx The format context to write into.
  /// \returns An iterator past the written output.
  format_context::iterator format(
