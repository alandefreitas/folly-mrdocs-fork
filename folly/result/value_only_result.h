...
[[FOLLY_ATTR_CLANG_CORO_AWAIT_ELIDABLE]] value_only_result final
 ...
 public:
  ...
  value_only_result(value_only_result&& other) = default;
  /// Move-assigns from another result.
  ///
  /// \param other The result to move from.
  /// \returns A reference to `*this`.
  value_only_result& operator=(value_only_result&& other) = default;

  /// Copy from `const value_only_result&`: value types and const-lref types.
  ///
  /// \param other The result to copy from.
  value_only_result(const value_only_result& other)
  ...
  /// \param other The result to copy from.
  /// \returns A reference to `*this`.
  value_only_result& operator=(const value_only_result& other)
    requires(!std::is_rvalue_reference_v<T> && !is_mutable_lref_v &&
             std::is_copy_constructible_v<ref_wrapped_t> &&
  ...
  /// Retrieves the stored lvalue reference as `const`.
  ///
  /// \returns The referenced value as `const`.
  ...
  /// Retrieves the stored lvalue reference.
  ///
  ...
  /// Retrieves the stored rvalue reference.
  ///
  /// \returns The referenced value.
  [[nodiscard]] T value_or_throw() && noexcept
  ...
  /// Pointer to the contained `const` value (never `nullptr`).
  ///
  /// \returns A pointer to the contained value.
  ...
  /// Deleted: taking a pointer into an rvalue would dangle.
  void get_pointer() && = delete;

  /// Converts to a `result<T>`, moving out the contained value.
  ///
  /// \returns A `result<T>` holding the moved value.
  explicit operator result<T>() && noexcept { // T is nothrow-movable
    return result<T>{std::move(*this).value_or_throw()};
  }
};

/// Deduction guide: `std::reference_wrapper<T>` yields `value_only_result<T&>`.
template <typename T>
value_only_result(std::reference_wrapper<T>) -> value_only_result<T&>;

/// Deduction guide: `rvalue_reference_wrapper<T>` yields `value_only_result<T&&>`.
template <typename T>
