...
class [[nodiscard]] error_or_stopped {
 ...
 public:
  ...
  ///
  /// \param s The stopped/cancellation signal.
  explicit error_or_stopped(stopped_result_t s) : rep_{OperationCancelled{}} {}
  /// Assigns the "stopped"/cancellation state.
  ///
  /// \param s The stopped/cancellation signal.
  /// \returns A reference to `*this`.
  error_or_stopped& operator=(stopped_result_t s) {
  ...
  ///
  /// \param err The immortal rich error to reference.
  ...
  ///
  /// \param tag Protocol dispatch tag.
  /// \returns A pointer to the stored `const Ex`, or `nullptr`.
  template <typename Ex>
  rich_ptr_to_underlying_error<const Ex> get_exception(
      get_exception_tag_t tag) const noexcept {
  ...
  /// Implements the mutable `folly::get_mutable_exception<Ex>(res)` protocol.
  ///
  /// \param tag Protocol dispatch tag.
  /// \returns A mutable pointer to the stored `Ex`, or `nullptr`.
  template <typename Ex>
  rich_ptr_to_underlying_error<Ex> get_mutable_exception(
      get_exception_tag_t tag) noexcept {
    static_assert( // Note: `OperationCancelled` is final
        !std::is_same_v<const OperationCancelled, const Ex>,
        "Test results for cancellation via `has_stopped()`");
  ...
  ///
  /// \param eptr The exception pointer to wrap (must be non-empty, non-stopped).
  /// \returns An `error_or_stopped` holding `eptr`.
  ...
  ///
  /// \returns The stored error as a `std::exception_ptr`.
  ...
  ///
  /// \returns An `error_or_stopped` holding the currently handled exception.
  ...
  /// Compares two `error_or_stopped` values for equality.
  ///
  /// \param lhs The left-hand value.
  /// \param rhs The right-hand value.
  /// \returns `true` if both hold the same error-or-stopped representation.
  ...
  /// Wraps a legacy `exception_wrapper` (possibly cancellation) as an `error_or_stopped`.
  ///
  /// \param priv Folly-internal access tag.
  /// \param ew The legacy exception wrapper to ingest.
  /// \returns An `error_or_stopped` holding `ew`.
  static error_or_stopped make_legacy_error_or_cancellation_slow(
      detail::result_private_t priv, exception_wrapper ew) {
    return {std::in_place, std::move(ew).exception_ptr()};
  }
  /// Extracts the stored error as a legacy `exception_wrapper`.
  ///
  /// \param priv Folly-internal access tag.
  /// \returns The stored error as an `exception_wrapper`.
  exception_wrapper get_legacy_error_or_cancellation_slow(
      detail::result_private_t priv) && {
  ...
  /// Moves out the internal `rich_exception_ptr`, leaving `*this` moved-from.
  ///
  /// \returns An rvalue reference to the internal `rich_exception_ptr`.
...
result final : public detail::result_crtp<result<T>, T> {
 private:
  ...
  union {
    /// Stored value or reference wrapper; valid only in the value state.
 ...
 public:
  /// Destroys the stored value when in the value state.
  ~result() {
  ...
  */
  ...
  /// Movable iff `T` is
  ///
  /// \param that The result to move from.
  ...
  /// Move-assigns from another `result`.
  ///
  /// \param that The result to move from.
  /// \returns A reference to `*this`.
  ...
  ///
  /// \param that The result to copy from.
  ...
  ///
  /// \param eos The error-or-stopped state to move from.
  /*implicit*/ result(class error_or_stopped&& eos) : base{std::move(eos)} {}
  /// Explicitly copy-construct an error-state `result` from `error_or_stopped`.
  ///
  /// \param eos The error-or-stopped state to copy.
  ...
  ///
  /// \param t The reference wrapper to store.
  ...
  ///
  /// \param t The value to move into the `result`.
  ...
  /// Move-assign the underlying value type `T` into this `result`.
  ///
  ...
  ///
  /// \param t The value to copy into the `result`.
  ...
  ///
  /// \param that The source result to convert from.
  ...
  ///
  /// \param rt The source result to convert from.
  template <class Arg, typename ResultT = std::remove_cvref_t<Arg>>
    requires(
        !std::is_same_v<ResultT, result> && // Not a move/copy ctor.
  ...
  /// Compares two `result` values for equality.
  ///
  /// \param a The left-hand result.
  /// \param b The right-hand result.
  /// \returns `true` if both hold equal values, or both hold the same error-or-stopped state.
  ...
  /// Retrieve non-reference `T`, or throw the stored error.
  ///
  /// \returns A reference to the stored value.
  ...
  ///
  /// \returns The referenced value as `const`.
  ...
  /// Retrieve the stored lvalue reference, or throw the stored error.
  ///
  ...
  /// Retrieve the stored rvalue reference, or throw the stored error.
  ///
  ...
  /// Returns a pointer to the stored value, or `nullptr` when not in the value state.
  ///
  /// \returns A pointer to the `const` value, or `nullptr`.
  ...
  /// Deleted: taking a pointer into an rvalue `result` would dangle.
  void get_pointer() && = delete;

  /// PRIVATE: See comments in the base class.
  ///
  /// \param s The "has value" sigil.
  explicit result(typename base::has_value_sigil_t s) noexcept : base{s} {}
  /// PRIVATE: Constructs a value-state result and stores its address in `ptr`.
  ///
  /// \param s The "has value" sigil.
  /// \param ptr Set to point at the newly constructed result.
  result(typename base::has_value_sigil_t s, result*& ptr) noexcept : base{s} {
    ptr = this;
  }
};

/// Deduction guide: `std::reference_wrapper<T>` yields `result<T&>`.
template <typename T>
result(std::reference_wrapper<T>) -> result<T&>;

/// Deduction guide: `rvalue_reference_wrapper<T>` yields `result<T&&>`.
template <typename T>
...
class [[nodiscard]] [[FOLLY_ATTR_CLANG_CORO_AWAIT_ELIDABLE]] result<void> final
 ...
 public:
  ...
  ///
  /// \param eos The error-or-stopped state to wrap.
  /*implicit*/ result(class error_or_stopped&& eos) : base{std::move(eos)} {}
  explicit result(const class error_or_stopped& eos) : base{eos} {}
  ...
  /// Compares two `result<void>` values for equality.
  ///
  /// \param a The left-hand result.
  /// \param b The right-hand result.
  /// \returns `true` if both have a value, or both hold the same error-or-stopped state.
  friend bool operator==(const result& a, const result& b) {
    if (a.has_value()) {
      return b.has_value();
    }
  ...
  /// Throws the stored error if this `result<void>` is not in the value state.
...
///
/// \param fn The callable to invoke and wrap.
/// \returns A `result` holding `fn`'s return value, or its thrown exception.
...
/// Streams a textual description of `eos` into `os`.
///
/// \param os The output stream.
/// \param eos The error-or-stopped value to stream.
...
struct fmt::formatter<folly::error_or_stopped> {
  ...
  ///
  /// \param eos The error-or-stopped value to format.
