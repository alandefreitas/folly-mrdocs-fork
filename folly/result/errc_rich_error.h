...
///
/// First get familiar with `docs/rich_error_code.md` and `coded_rich_error.h`.
/// Instantiate via `make_coded_rich_error(std::errc::whatever, "message {}",
/// arg)` or `errc_rich_error::make(...)`.
///
/// Note that this implementation is POSIX-centric, and that the first person
/// wanting to deal with Windows codes may want to add a derived type that
...
struct fmt::formatter<folly::detail::errc_format_as> {
  ...
  /// \returns An iterator past the parsed format specification.
  constexpr auto parse(format_parse_context& ctx) { return ctx.begin(); }
  /// Formats the error code into the output.
  ///
  /// \param ec The error code tag to format.
  /// \param ctx The format context receiving the output.
  /// \returns An output iterator past the formatted text.
  fmt::appender format(
