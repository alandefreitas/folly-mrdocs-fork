...
class [[nodiscard]] rich_error final
 ...
 public:
  ...
  ///
  /// \returns The partial message, or the type name when no message is set.
  ...
  ///
  /// \param t The first argument forwarded to the base constructor.
  /// \param ts The remaining arguments forwarded to the base constructor.
  ...
  ///
  /// \param tag A private tag restricting who may call this constructor.
  /// \param that The base instance to copy from.
  rich_error(detail::immortal_rich_error_private_t tag, const Base& that)
      : Base{that} {}
