...
class Blake2xb {
 public:
   ...
   *
   * \param outputLength Desired hash length in bytes, or kUnknownOutputLength.
   * \param key Optional secret key for a keyed hash.
   * \param salt Optional salt value.
   * \param personalization Optional personalization value.
   */
  explicit Blake2xb(
      size_t outputLength,
      ByteRange key = {},
  ...
  /// Destroys the instance and clears its internal state.
  ~Blake2xb();
   ...
   *
   * \param data The input bytes to append to the running hash.
   */
   ...
   *
   * \param out Destination buffer that receives the final hash.
   */
   ...
   *
   * \param out Destination buffer that receives the hash; its size sets the
   *   output length.
