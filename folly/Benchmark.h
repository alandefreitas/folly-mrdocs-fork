...
/// Command-line flag enabling benchmark execution.
/// \implementationdefined
FOLLY_GFLAGS_DECLARE_bool(benchmark);
/// Command-line flag suppressing non-essential benchmark output.
/// \implementationdefined
FOLLY_GFLAGS_DECLARE_bool(bm_quiet);
/// Command-line flag setting the benchmark result column width in characters.
/// \implementationdefined
FOLLY_GFLAGS_DECLARE_uint32(bm_result_width_chars);
/// Command-line flag setting the minimum number of benchmark iterations.
/// \implementationdefined
FOLLY_GFLAGS_DECLARE_int32(bm_min_iters);
/// Command-line flag setting the maximum number of benchmark iterations.
/// \implementationdefined
 ...
 *
 * \returns True if benchmarks were run (the --benchmark flag was set).
 */
...
class UserMetric {
 public:
  ...
  enum class Type {
    CUSTOM, ///< An arbitrary user-defined value.
    TIME, ///< A duration value.
    METRIC, ///< A general metric value.
  };
  ...
  /// Construct a default metric with a zero custom value.
  UserMetric() = default;
  /// Construct a metric from an integer value.
  ///
  /// \param val The integer metric value.
  /// \param typ The kind of the metric.
  /* implicit */ UserMetric(int64_t val, Type typ = Type::CUSTOM)
      : value(val), type(typ) {}

  /// Construct a metric from a floating-point value.
  ///
  /// Allow users to provide precision values.
  ///
  /// \param precision_val The floating-point metric value.
  /// \param typ The kind of the metric.
  template <typename T>
  ...
  ///
  /// \param x The left-hand metric.
  /// \param y The right-hand metric.
  ...
  /// \param x The left-hand metric.
  /// \param y The right-hand metric.
  /// \returns True if the metrics differ.
  friend bool operator!=(const UserMetric& x, const UserMetric& y) {
    return !(x == y);
  }
 ...
 *
 * \param file The source file the benchmark is defined in.
 * \param name The benchmark's display name.
 * \param lambda The callable implementing the benchmark body.
 */
template <typename Lambda>
...
///
/// \param data The benchmark results to serialize.
/// \param out The dynamic value to populate.
void benchmarkResultsToDynamic(
    const std::vector<detail::BenchmarkResult>& data, dynamic& out);

/// Deserialize benchmark results from a dynamic value.
///
/// \param data The dynamic value to read from.
/// \param out The vector to populate with results.
void benchmarkResultsFromDynamic(
    const dynamic& data, std::vector<detail::BenchmarkResult>& out);

/// Print a comparison between two sets of benchmark results.
///
/// \param base The baseline results.
/// \param test The results to compare against the baseline.
 ...
 * friends below.
 *
 * \param funName The generated benchmark function's identifier.
 * \param stringName The benchmark's display name.
 * \param rv The value the benchmark lambda returns as its iteration count.
 * \param paramType The type of the benchmark's iteration parameter.
 * \param paramName The name of the benchmark's iteration parameter.
 */
 ...
 * }
 *
 * \param name The benchmark's name, also used as its function identifier.
 */
 ...
 *
 * \param name The benchmark's name, also used as its function identifier.
 * \param counters The UserCounters parameter recorded during the benchmark.
 */
 ...
 *
 * \param name The benchmarked function's identifier.
 * \param param The parameter value passed to the benchmarked function.
 */
 ...
 *
 * \param name The benchmarked function's identifier.
 * \param param_name The label used for this parameter set in the output.
 */
 ...
 *
 * \param text The text to print in the benchmark output.
 */
