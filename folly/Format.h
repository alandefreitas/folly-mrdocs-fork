...
class BaseFormatterImpl<
 ...
 public:
   ...
   *
   * @param out The output callback that receives each formatted chunk.
   */
   ...
   * Append to a string.
   *
   * @param str The string to append the formatted output to.
   */
  template <class Str>
   ...
   *
   * @return The formatted output as a `std::string`.
   */
...
class Formatter
 ...
 private:
  ...
  /// \param arg The values to substitute into the format string.
  /// \returns A formatter binding `fmt` to `arg` for deferred output.
  template <class... A>
  friend Formatter<false, A...> format(StringPiece fmt, A&&... arg);
  /// Appends formatted output for `fmt` and `args` to `out`.
  ///
  /// \param out The string to append formatted output to.
  /// \param fmt The format string.
 ...
 *
 * @param out The output stream to write the formatted output to.
 * @param formatter The formatter whose output is written.
 ...
 *
 * @param fmt The format string, using Python str.format-style syntax.
 * @param args The values to substitute into the format string.
 * @return A formatter binding `fmt` to `args` for deferred output.
 */
template <class... Args>
[[deprecated(
...
class FOLLY_EXPORT FormatKeyNotFoundException : public std::out_of_range {
 public:
  /// Constructs the exception for the missing format `key`.
  ///
  /// \param key The format key that was not found.
  explicit FormatKeyNotFoundException(StringPiece key);

  /// Returns the missing format key stored in the message.
  ///
  /// \returns The missing format key.
...
///
/// \param c The container to wrap.
/// \param v The default value to return for missing keys.
/// \returns A wrapper that yields `v` on out-of-range lookups.
template <class Container, class Value>
 ...
 *
 * @param out The string to append the formatted output to.
 * @param fmt The format string, using Python str.format-style syntax.
 * @param args The values to substitute into the format string.
 */
 ...
 * @param arg The format argument describing alignment, padding, and width.
 * @param cb The callback that receives the formatted output.
 */
 ...
 * field width")
 *
 * @param val The number, already rendered as text, to format.
 * @param prefixLen The length of the leading prefix (sign, base marker, etc).
 * @param arg The format argument describing alignment, padding, and width.
 * @param cb The callback that receives the formatted output.
 */
template <class FormatCallback>
 ...
 * string if possible.
 *
 * @param formatter The formatter whose output is formatted.
 * @param arg The format argument describing alignment, padding, and width.
 * @param cb The callback that receives the formatted output.
 */
