...
///
/// A token captures the state and slot chosen when a shared lock is
/// acquired, letting the matching unlock find the record without searching.
struct SharedMutexToken {
  /// Kind of shared lock a token refers to.
  enum class State : uint16_t {
    Invalid = 0, ///< Token does not refer to a held shared lock.
    LockedShared, ///< Held in shared mode, either inline or deferred.
    LockedInlineShared, ///< Held in shared mode recorded inline in the lock.
    LockedDeferredShared, ///< Held in shared mode recorded in a deferred slot.
  };

  /// State of the shared lock this token refers to.
  State state_{};
  /// Index of the deferred reader slot holding the lock, when deferred.
  uint16_t slot_{};

  /// Constructs an invalid token that refers to no held lock.
  constexpr SharedMutexToken() = default;

  /// Returns whether the token refers to a currently held shared lock.
  /// \returns `true` if the token is not in the `Invalid` state.
...
struct SharedMutexPolicyDefault {
  /// Maximum number of CPU cycles to spin before yielding.
  static constexpr uint64_t max_spin_cycles = 4000;
  /// Number of soft yields to perform before blocking on a futex.
  static constexpr uint32_t max_soft_yield_count = 1;
  /// Whether to track the id of the thread holding the lock.
  static constexpr bool track_thread_id = false;
  /// Whether to skip ThreadSanitizer rwlock annotations.
...
struct PolicyTracked : SharedMutexPolicyDefault {
  /// Enables tracking the id of the thread holding the lock.
...
struct PolicySuppressTSAN : SharedMutexPolicyDefault {
  /// Skips ThreadSanitizer rwlock annotations.
...
/// \param ptr Pointer to the SharedMutex whose annotation bits are guarded.
/// \returns A lock guarding annotation access to the given mutex.
...
/// \param cache Cache updated with the computed value.
/// \returns The maximum number of deferred readers.
...
/// \returns The involuntary context switch count, or a negative value if
/// unavailable.
long getCurrentThreadInvoluntaryContextSwitchCount();

/// Returns the current maximum number of deferred readers.
/// \returns The maximum number of deferred reader slots in use.
...
///
/// Used when the policy does not request thread-id tracking; every method
/// is a no-op.
class NopOwnershipTracker {
 public:
  /// Does nothing; ownership is not tracked.
  void beginThreadOwnership() {}

  /// Does nothing; ownership is not tracked.
  /// \param unused Ignored flag indicating whether ownership was taken.
  void maybeBeginThreadOwnership(bool unused) {}
...
///
/// Used when the policy requests thread-id tracking; stores the OS thread
/// id while the lock is held in upgrade or exclusive mode.
class ThreadIdOwnershipTracker {
 public:
  /// Records the current thread as the owner.
  void beginThreadOwnership() {
    assert(ownerTid_ == 0);
    ownerTid_ = tid();
  }

  /// Records the current thread as the owner when ownership is taken.
  /// \param own Whether ownership was actually acquired.
  ...
  /// Clears the recorded owning thread.
  void endThreadOwnership() {
...
///
/// SharedMutex provides exclusive, upgrade, and shared locking modes, with
/// blocking, try, try-for, and try-until variants. Each instance is only a
/// few bytes and records shared acquisitions in a shared set of core-local
/// stripes so that concurrent readers scale linearly.
///
/// \tparam ReaderPriority Whether readers take priority over writers.
/// \tparam Tag_ Tag type used to create an independent instantiation with its
/// own deferred-reader storage.
/// \tparam Atom Atomic template used for the lock state and deferred slots.
/// \tparam Policy Tuning and feature policy; see SharedMutexPolicyDefault.
template <
    bool ReaderPriority,
    typename Tag_ = void,
...
class SharedMutexImpl
 ...
 public:
  /// Whether this instantiation gives priority to readers.
  static constexpr bool kReaderPriority = ReaderPriority;
  /// Tag type used to create an independent instantiation.
  using Tag = Tag_;

  /// Token type used to speed up paired shared lock and unlock calls.
  using Token = SharedMutexToken;

  /// Constructs an unlocked mutex.
  constexpr SharedMutexImpl() noexcept : state_(0) {}

  /// Deleted copy constructor; the mutex is not copyable.
  /// \param other The mutex that would be copied from.
  SharedMutexImpl(const SharedMutexImpl& other) = delete;
  /// Deleted move constructor; the mutex is not movable.
  /// \param other The mutex that would be moved from.
  SharedMutexImpl(SharedMutexImpl&& other) = delete;
  /// Deleted copy assignment; the mutex is not copyable.
  /// \param other The mutex that would be assigned from.
  /// \returns Nothing; this operator is deleted.
  SharedMutexImpl& operator=(const SharedMutexImpl& other) = delete;
  /// Deleted move assignment; the mutex is not movable.
  /// \param other The mutex that would be assigned from.
  /// \returns Nothing; this operator is deleted.
  SharedMutexImpl& operator=(SharedMutexImpl&& other) = delete;

  /// Destroys the mutex.
  ///
  /// It is an error to destroy a SharedMutex that still has any outstanding
  /// locks. This is checked if NDEBUG isn't defined. SharedMutex's exclusive
  /// mode can be safely used to guard the lock's own destruction. If, for
  /// example, you acquire the lock in exclusive mode and then observe that the
  /// object containing the lock is no longer needed, you can unlock() and then
  /// immediately destroy the lock. See
  /// https://sourceware.org/bugzilla/show_bug.cgi?id=13690 for a description
  /// about why this property needs to be explicitly mentioned.
  ~SharedMutexImpl() {
  ...
  /// Checks whether an exclusive lock could succeed so that lock elision can
  /// be enabled.
  ///
  /// Different from the two eligible_for_lock_{upgrade,shared}_elision
  /// functions, this is a conservative check since kMayDefer indicates
  /// "may-existing" deferred readers.
  /// \returns `true` if exclusive locking looks eligible for elision.
  ...
  /// Checks whether an upgrade lock could succeed so that lock elision can be
  /// enabled.
  /// \returns `true` if upgrade locking looks eligible for elision.
  bool eligible_for_lock_upgrade_elision() const {
    auto state = state_.load(std::memory_order_relaxed);
    return (state & (kHasE | kHasU)) == 0;
  }

  /// Checks whether a shared lock could succeed so that lock elision can be
  /// enabled.
  /// \returns `true` if shared locking looks eligible for elision.
  ...
  /// Acquires the lock in exclusive mode, blocking until it is available.
  void lock() {
    WaitForever ctx;
  ...
  /// \param duration Maximum time to wait for the lock.
  /// \returns `true` if the lock was acquired before the duration elapsed.
  ...
  /// Tries to acquire the lock in exclusive mode by a deadline.
  /// \param absDeadline Absolute time by which the lock must be acquired.
  /// \returns `true` if the lock was acquired before the deadline.
  ...
  /// Releases the lock held in exclusive mode.
  void unlock() {
  ...
  /// Acquires the lock in shared mode, recording where it was taken.
  ///
  /// Blocks until the lock is available. Pass the same token to the tokenful
  /// unlock_shared() to speed up release.
  /// \param token Token that records the acquired shared lock.
  ...
  /// Tries to acquire the lock in shared mode without blocking, recording
  /// where it was taken.
  /// \param token Token that records the acquired shared lock.
  /// \returns `true` if the lock was acquired.
  ...
  /// Tries to acquire the lock in shared mode within a duration.
  /// \param duration Maximum time to wait for the lock.
  /// \returns `true` if the lock was acquired before the duration elapsed.
  template <class Rep, class Period>
  ...
  /// Tries to acquire the lock in shared mode within a duration, recording
  /// where it was taken.
  /// \param duration Maximum time to wait for the lock.
  /// \param token Token that records the acquired shared lock.
  /// \returns `true` if the lock was acquired before the duration elapsed.
  ...
  /// Tries to acquire the lock in shared mode by a deadline.
  /// \param absDeadline Absolute time by which the lock must be acquired.
  /// \returns `true` if the lock was acquired before the deadline.
  template <class Clock, class Duration>
  ...
  /// Tries to acquire the lock in shared mode by a deadline, recording where
  /// it was taken.
  /// \param absDeadline Absolute time by which the lock must be acquired.
  /// \param token Token that records the acquired shared lock.
  /// \returns `true` if the lock was acquired before the deadline.
  ...
  /// Releases a lock held in shared mode without a token.
  void unlock_shared() {
  ...
  /// Releases a lock held in shared mode using its token.
  /// \param token Token recorded when the shared lock was acquired.
  ...
  /// Invalidates the given token so the tokenless unlock_shared() can be used.
  ///
  /// Lets a lock obtained from a tokenful lock_shared() be released with the
  /// tokenless unlock_shared(). This does not unlock the mutex at any point.
  ...
  /// Atomically downgrades an exclusive lock to a shared lock.
  ...
  /// Atomically downgrades an exclusive lock to a shared lock, recording it.
  /// \param token Token that records the resulting shared lock.
  void unlock_and_lock_shared(Token& token) {
    unlock_and_lock_shared();
    token.state_ = Token::State::LockedInlineShared;
  }

  /// Acquires the lock in upgrade mode, blocking until it is available.
  void lock_upgrade() {
    WaitForever ctx;
  ...
  /// Tries to acquire the lock in upgrade mode without blocking.
  /// \returns `true` if the lock was acquired.
  ...
  /// Tries to acquire the lock in upgrade mode within a duration.
  /// \param duration Maximum time to wait for the lock.
  /// \returns `true` if the lock was acquired before the duration elapsed.
  template <class Rep, class Period>
  ...
  /// Tries to acquire the lock in upgrade mode by a deadline.
  /// \param absDeadline Absolute time by which the lock must be acquired.
  /// \returns `true` if the lock was acquired before the deadline.
  template <class Clock, class Duration>
  ...
  /// Releases a lock held in upgrade mode.
  void unlock_upgrade() {
    annotateReleased(annotate_rwlock_level::rdlock);
    OwnershipTrackerBase::endThreadOwnership();
  ...
  /// Atomically upgrades an upgrade lock to an exclusive lock.
  ...
  /// Atomically downgrades an upgrade lock to a shared lock.
  ...
  /// Atomically downgrades an upgrade lock to a shared lock, recording it.
  /// \param token Token that records the resulting shared lock.
  ...
  /// Atomically converts an exclusive lock to an upgrade lock.
 ...
 public:
  /// Number of deferred reader slots searched when acquiring or releasing.
  static constexpr uint32_t kDeferredSearchDistance = 2;
  /// Spacing between used deferred reader slots, to avoid cache contention.
 ...
 private:
  ...
  // See kTokenless.
 public:
  /// Storage for one deferred reader slot; holds a tagged mutex pointer.
...
///
/// Uses the tokenful version of unlock_shared for faster unlocking by
/// carrying a token that records where the shared lock was taken.
/// \tparam ReaderPriority Whether the mutex gives priority to readers.
/// \tparam Tag_ Tag type of the mutex instantiation.
/// \tparam Atom Atomic template used by the mutex.
/// \tparam Policy Policy type of the mutex.
template <
    bool ReaderPriority,
    typename Tag_,
    template <typename> class Atom,
...
class shared_lock<
 ...
 public:
  ...
  /// \param mutex Mutex to associate with this lock.
  /// \param unused Tag selecting deferred locking.
  shared_lock(mutex_type& mutex, std::defer_lock_t unused) noexcept
      : mutex_(std::addressof(mutex)) {}

  /// Constructs a lock and tries to acquire the mutex without blocking.
  /// \param mutex Mutex to try to lock.
  /// \param unused Tag selecting try-to-lock.
  [[nodiscard]] shared_lock(mutex_type& mutex, std::try_to_lock_t unused)
      : mutex_(std::addressof(mutex)) {
    try_lock();
  }

  /// Constructs a lock that adopts an already-held shared lock.
  /// \param mutex Mutex whose shared ownership is adopted.
  /// \param unused Tag selecting lock adoption.
  [[nodiscard]] shared_lock(mutex_type& mutex, std::adopt_lock_t unused)
      : mutex_(std::addressof(mutex)) {
  ...
  /// \param mutex Mutex to try to lock.
  /// \param deadline Absolute time by which the lock must be acquired.
  ...
  /// \param other Lock to move from; left owning no mutex.
  /// \returns A reference to this lock.
  ...
  /// \param other Lock to swap state with.
  ...
  /// \returns The previously managed mutex, or `nullptr` if none.
  ...
  /// \returns `true` if a shared lock is held.
  ...
  /// \returns The managed mutex, or `nullptr` if none.
