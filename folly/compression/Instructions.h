...
struct Default {
  ...
  ///
  /// \returns The name "Default".
  static std::string_view name() noexcept { return "Default"; }
  /// Reports whether this backend runs on the given CPU.
  ///
  /// \param cpuId The CPU to check (unused; always supported).
  /// \returns True, since the portable backend is always supported.
  ...
  ///
  /// \param value The nonzero value to inspect.
  ...
  /// \returns The value with its lowest set bit cleared.
  ...
  ///
  /// \param value The source value.
  /// \param index The bit position at and above which bits are cleared.
  /// \returns The value with high bits cleared.
...
/// Detects the best supported instruction-set backend for this CPU.
///
...
///
/// \param type The backend type to dispatch on.
/// \param f The callable to invoke with the selected backend.
/// \returns The result of invoking f with the selected backend.
template <class F>
...
///
/// \param f The callable to invoke with the detected backend.
/// \returns The result of invoking f with the detected backend.
template <class F>
