...
struct EliasFanoCompressedListBase {
  ...
  uint8_t numLowerBits = 0; ///< Number of lower bits stored per element.
  size_t upperSizeBytes = 0; ///< Size in bytes of the upper bits sequence.

  /// The backing bytes of the list.
  ///
  ...
  Pointer lower = nullptr; ///< Pointer to the lower bits section.
  Pointer upper = nullptr; ///< Pointer to the upper bits section.
};
...
struct EliasFanoEncoder {
  ...
  /// Returns the default number of lower bits for the given parameters.
  ///
  /// \param upperBound The largest value that will be stored.
  /// \param size The number of elements that will be stored.
  /// \returns The number of lower bits, floor(log2(upperBound / size)).
  ...
  /// Encodes the sorted range [begin, end) into a new compressed list.
  ///
  /// Requires: input range (begin, end) is sorted (encoding crashes if it's
  /// not). WARNING: encode() mallocates EliasFanoCompressedList::data. As
  /// EliasFanoCompressedList has no ownership of it, you need to call free()
  /// explicitly.
  ///
  ...
  /// Appends the next value to the list. Values must be added in sorted order.
  ///
...
struct EliasFanoEncoder<
  ...
  /// Builds a layout from already-computed section sizes.
  ///
  /// \param numLowerBits Number of lower bits stored per element.
  /// \param upper Size in bytes of the upper bits sequence.
  /// \param size Number of elements in the list.
  /// \returns The computed layout.
  ...
  ///
  /// \returns The combined size of the lower, upper, skip and forward sections.
  size_t bytes() const {
  ...
  // Sizes in bytes.
  size_t lower = 0; ///< Size in bytes of the lower bits sequence.
  size_t upper = 0; ///< Size in bytes of the upper bits sequence.
...
class EliasFanoReader {
 ...
 public:
   ...
   *
   * \returns True if the reader moved to a valid previous element.
   */
  bool previous() {
   ...
   *
   * \param value The value to skip to.
   * \returns True if an element >= value was found.
   */
   ...
   *
   * \param value The value a subsequent skipTo() call will skip to.
   */
  template <bool kCanBeAtValue = true>
   ...
   * \param assumeDistinct Whether all values in the list can be assumed distinct.
   * \returns True if an element >= value was found.
   */
   ...
   *
   * \returns The value immediately preceding the current element.
   */
...
/// Trailing bytes that must be readable past the end of the upper bit sequence,
/// required by loadUnaligned<uint64_t> reads in the upper accessor.
constexpr size_t kUpperTrailingBytes = 7;
/// Trailing bytes that must be readable past the end of the lower bit sequence,
/// required by loadUnaligned<uint64_t> reads in the lower accessor.
constexpr size_t kLowerTrailingBytes = 8;

/// Returns a deep copy of a compressed list, owning a freshly allocated buffer.
///
/// \param src The compressed list to copy.
/// \returns A copy whose data buffer is newly allocated with trailing padding.
