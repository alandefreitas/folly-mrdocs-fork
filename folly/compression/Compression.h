...
class Codec {
 public:
  ...
  /// Sentinel meaning the codec imposes no uncompressed-length limit.
   ...
   *
   * \returns The maximum supported uncompressed length in bytes.
   */
   ...
   *
   * \returns True if the exact uncompressed length is required.
   */
   ...
   *
   * \param data The uncompressed input buffer.
   ...
   * \param uncompressedLength The exact uncompressed length, if known.
   ...
   * version.
   *
   * \param data The compressed input.
   * \param uncompressedLength The exact uncompressed length, if known.
   * \returns The uncompressed data.
   */
   ...
   *
   * \param uncompressedLength The uncompressed length in bytes.
   * \returns An upper bound on the compressed length in bytes.
   */
   ...
   *
   * \param data The compressed input buffer.
   * \param uncompressedLength The expected uncompressed length, if known.
   * \returns The uncompressed length, or the given value if unavailable.
   */
 ...
 protected:
  /// Constructs the codec base with its type, level, and name.
  ///
  /// \param type The codec type.
  /// \param level The compression level, if applicable.
 ...
 public:
   ...
   * default: No prefixes.
   *
   * \returns A superset of the prefixes canUncompress() accepts.
   */
   ...
   * \param data The compressed input buffer.
   * \param uncompressedLength The expected uncompressed length, if known.
   * \returns True if the codec may be able to uncompress the data.
   */
...
class StreamCodec : public Codec {
 public:
  /// Destroys the streaming codec.
  ~StreamCodec() override {}

  /**
   * Does the codec need the data length before compression streaming?
   *
   * \returns True if the uncompressed length is required before streaming.
   */
  ...
  /// Controls how much pending data a streaming operation flushes.
  enum class FlushOp {
    NONE, ///< Flush at the codec's discretion; produce output only as needed.
    FLUSH, ///< Flush all pending data without ending the frame.
    END ///< Flush all pending data and end the frame.
  };
   ...
   *
   * @param input Buffer of input data; advanced past the bytes consumed.
   * @param output Buffer to write compressed data into; advanced past the
   *   bytes written.
   * @param flushOp How much pending data to flush.
   ...
   *
   * @param input Buffer of compressed input; advanced past the bytes consumed.
   * @param output Buffer to write uncompressed data into; advanced past the
   *   bytes written.
   * @param flushOp How much pending data to flush.
 ...
 protected:
  /// Constructs the streaming codec base with its type, level, and name.
  ///
  /// \param type The codec type.
  /// \param level The compression level, if applicable.
  /// \param name The codec name.
  ...
  /// Returns the uncompressed length last passed to resetStream().
  ///
  /// \returns The uncompressed length, or none if resetStream() has not been
  ///   called.
  folly::Optional<uint64_t> uncompressedLength() const {
 ...
 *
 * \param type The codec type to create.
 * \param level The desired compression level.
 * \returns A codec for the given type.
 */
 ...
 * \param type The codec type to create.
 * \param level The desired compression level.
 * \returns A streaming codec for the given type.
 */
 ...
 *
 * \param customCodecs Extra codecs to try before the default set.
 * \param terminalCodec Fallback codec used when no other codec matches.
 * \returns A codec that auto-detects and uncompresses supported formats.
 */
 ...
 *
 * \param type The codec type to check.
 ...
 *
 * \param type The codec type to check.
 * \returns True if the codec is supported and supports streaming.
 */
 ...
 *
 * \param level The abstract or raw compression level to convert.
 * \returns The equivalent raw ZSTD compression level.
 */
