...
class FOLLY_EXPORT ProgramExit : public std::runtime_error {
 public:
  /// Constructs the exception with an exit status and optional message.
  /// \param status The exit code to return from run().
  /// \param msg The message printed on stderr when exiting.
  explicit ProgramExit(int status, const std::string& msg = std::string());
  /// Returns the exit status carried by this exception.
  /// \returns The exit code to return from run().
...
class NestedCommandLineApp {
 public:
  /// Callback run after parsing the command line, before the command runs.
  using InitFunction = std::function<void(
      const std::string& command,
      const boost::program_options::variables_map& options,
      const std::vector<std::string>& args)>;

  /// The function invoked to run a registered command.
  using Command = std::function<void(
      const boost::program_options::variables_map& options,
      const std::vector<std::string>&)>;

  /// Registration information for a single command.
  struct CommandInfo {
    /// Description of the command's arguments shown in help strings.
    std::string argStr;
    ...
    /// The positional options accepted by the command, if any.
  ...
  /// Name of the built-in short-help command.
  static constexpr StringPiece const kShortHelpCommand = "h";
  /// Name of the built-in version command.
   ...
   *
   * \param programName The program name; guessed if empty.
   * \param version The version string printed for the --version flag.
   * \param programHeading The heading shown at the top of the help output.
   * \param programHelpFooter The footer shown at the bottom of the help output.
   * \param initFunction Optional callback run right before the command.
   */
   ...
   *
   * \returns A reference to the global options description.
   */
   ...
   *
   * \returns A map from command name to its registration information.
   */
   ...
   *
   * \returns A map from alias name to its target command name.
   */
   ...
   * \param argStr The description of arguments shown in help strings.
   * \param shortHelp The one-line summary help string.
   * \param fullHelp The full help string.
   * \param command The function to run for this command.
   * \param positionalOptions Optional positional options for this command.
   * \returns A reference to the command's options description.
   */
   ...
   * \param oldName The existing command name the alias points to.
   */
   ...
   * \returns The exit code, 0 on success or non-zero on error.
   */
  int run(int argc, const char* const argv[]);
  /// Runs the command from a list of arguments.
  /// \param args The command-line arguments, excluding the program name.
  /// \returns The exit code, 0 on success or non-zero on error.
   ...
   * \returns True if the name is a built-in command.
   */
   ...
   *
   * \param callback The callback to invoke after parsing.
   */
