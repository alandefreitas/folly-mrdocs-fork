...
struct movemask_fn {
  /// Extracts the bitmask and bits-per-element from a SIMD register.
  ///
  /// \param reg The SIMD register to extract the bitmask from.
  /// \returns A pair of the bitmask and the number of bits per element.
  template <typename Reg>
  auto operator()(Reg reg) const;

  /// Extracts the bitmask from a SIMD register, zeroing out ignored elements.
  ///
  /// \param reg The SIMD register to extract the bitmask from.
  /// \param ignore Tag describing which elements to ignore.
  /// \returns A pair of the bitmask and the number of bits per element.
  template <typename Reg, typename Ignore>
  auto operator()(Reg reg, Ignore ignore) const;
};

/// Callable object that extracts a bitmask from a SIMD register.
template <typename Scalar>
