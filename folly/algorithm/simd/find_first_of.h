...
class composite_finder_first_of : private Vector, Scalar {
 ...
 public:
  /// Constructs the finder from the alphabet to search for.
  ///
  /// \param alphabet The set of characters to match against.
  constexpr explicit composite_finder_first_of(view const alphabet) noexcept
      : Vector{alphabet}, Scalar{alphabet} {}

  /// Returns the index of the first input character that matches the alphabet.
  ///
  /// \param input The input sequence to search.
  /// \param pos The position in the input to start searching from.
  /// \returns The index of the first match, or input.size() if none.
  size_t operator()(view const input, size_t const pos = 0) const noexcept {
