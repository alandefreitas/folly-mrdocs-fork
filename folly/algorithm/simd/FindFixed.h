...
/// Concept for types supported by folly::findFixed: fixed-width integers and
/// enums with such an underlying type.
template <typename T>
...
///
/// \param where The fixed-size range to search.
/// \param x The value to search for.
/// \returns Index of the first matching element, or std::nullopt if not found.
template <
...
#endif
#elif FOLLY_AARCH64
/// Size in bytes of the largest available SIMD register.
constexpr std::size_t kMaxSimdRegister = 16;
#else
...
///
/// \param from Pointer to the 8 bytes to search.
/// \param x The value to search for.
/// \returns Index of the first matching element, or std::nullopt if not found.
template <typename T>
...
///
/// \param from Pointer to the 16 bytes to search.
/// \param x The value to search for.
/// \returns Index of the first matching element, or std::nullopt if not found.
template <typename T>
...
///
/// \param from Pointer to the 32 bytes to search.
/// \param x The value to search for.
/// \returns Index of the first matching element, or std::nullopt if not found.
template <typename T>
...
///
/// \param reg The SIMD comparison-result register.
/// \returns Index of the first matching element, or std::nullopt if none.
