...
struct FloatKey {
  ...
  /// Unsigned integer type used for the sortable key.
  using UInt = uint_bits_t<sizeof(Float) * 8>;
  static_assert(sizeof(Float) == sizeof(UInt));

  /// Maps a floating-point value to a radix-sortable unsigned key.
  ///
  /// \param f The floating-point value to map.
  /// \returns The radix-sortable unsigned key.
...
struct IntegralKey {
  ...
  /// Maps an integer to a radix-sortable unsigned key.
  ///
...
struct IdentityKey {
  ...
  /// \returns The value, negated if sorting in descending order.
  template <typename T>
...
/// Number of counting-sort buckets per pass (2^kRadixBits).
 ...
 *
 * \param key The unsigned key to extract a digit from.
 * \param pass The pass index selecting which digit to extract.
 * \returns The 8-bit radix digit for the given pass.
 */
template <typename Key>
 ...
 * \param last Iterator past the last element to sort.
 * \param proj Projection returning the sort key for each element.
 */
 ...
 *
 * \param data Pointer to the elements to sort in place.
 * \param buffer Scratch buffer of at least n elements.
 * \param n Number of elements to sort.
 * \param proj Projection returning the unsigned sort key for each element.
 */
...
struct DefaultProjection {
  /// Transforms a value into its radix-sortable key.
  ///
  /// \param value The element to project.
  /// \returns The radix-sortable key for the value.
...
struct TransformingProjection {
  ...
  ///
  /// \param value The element to project.
  /// \returns The transformed, radix-sortable key.
  template <typename T>
  auto operator()(const T& value) const noexcept {
