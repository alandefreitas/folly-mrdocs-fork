...
/// Default policy for the token bucket templates, selecting the alignment,
/// atomic type, clock, and concurrency behavior.
struct TokenBucketPolicyDefault {
  /// Alignment used for the atomic state, sized to avoid false sharing.
  using align =
      std::integral_constant<size_t, hardware_destructive_interference_size>;

  /// Atomic wrapper used to store the token bucket state.
  template <typename T>
  using atom = std::atomic<T>;

  /// Steady clock used as the default time source.
  using clock = std::chrono::steady_clock;

  /// Whether the bucket must be safe for concurrent access.
...
class TokenBucketStorage {
 ...
 public:
   ...
   * however.)
   *
   * @param other The token bucket storage to copy from.
   */
   ...
   *
   * @param other The token bucket storage to copy from.
   * @return A reference to this token bucket storage.
   */
   ...
   *
   * @param rate Number of tokens generated per second.
   * @param burstSize Maximum burst size.
   * @param nowInSeconds Current time in seconds.
   * @return The token balance at the given time.
   */
  double balance(
   ...
   * @param burstSize Maximum burst size.
   * @param nowInSeconds Current time in seconds.
   * @param callback Function that receives the available tokens and returns
   *                 the number consumed.
   ...
   *
   * @param rate Number of tokens generated per second.
   * @param target Desired number of available tokens.
   * @return The time in seconds at which the bucket holds `target` tokens.
   */
   ...
   * Thread-safe.
   *
   * @param tokensToReturn Number of tokens to return to the bucket.
   * @param rate Number of tokens generated per second.
   */
...
class BasicDynamicTokenBucket {
 ...
 public:
   ...
   * however.)
   *
   * @param other The token bucket to copy from.
   */
   ...
   * Copy assignment operator.
   *
   * Thread-safe. (Copy assignment of derived classes may not be thread-safe
   * however.)
   *
   * @param other The token bucket to copy from.
   * @return A reference to this token bucket.
   */
   ...
   * @param burstSize Maximum burst size. Must be greater than 0.
   * @param nowInSeconds Current time in seconds.
   * @return An unset optional if the request cannot be satisfied, otherwise
   *         the time in seconds the caller must wait for the reservation to
   *         become valid.
   */
   ...
   * @param burstSize Maximum burst size. Must be greater than 0.
   * @param nowInSeconds Current time in seconds.
   * @return True if the reservation was satisfied, false otherwise.
   */
   ...
   * @param burstSize Maximum burst size.
   * @param nowInSeconds Current time in seconds.
   * @return The number of tokens available at the given time.
   */
  double available(
      double rate,
      double burstSize,
...
class BasicTokenBucket {
 ...
 public:
   ...
   * Thread-safe (but returned value may immediately be outdated).
   *
   * @return The number of tokens generated per second.
   */
...
/// Dynamic token bucket with an adjustable rate and burst size using the
/// default policy.
