...
class ThreadLocalPRNG {
 ...
 public:
  ...
  ///
  /// \returns A pseudorandom value of type result_type.
...
///
/// The members generate pseudorandom and secure random values across integral,
/// boolean, and floating-point types, optionally using a caller-supplied random
/// number generator.
class Random {
 private:
  template <class RNG>
 ...
 public:
  /// The default random number generator type used by this class.
  using DefaultGenerator = detail::DefaultGenerator;
  /// The legacy random number generator type kept for compatibility.
  using LegacyGenerator = detail::LegacyGenerator;

  /**
   * Get secure random bytes. (On Linux and OSX, this means /dev/urandom).
   *
   * \param data Destination buffer to fill with secure random bytes.
   * \param size Number of bytes to write to the buffer.
   */
   ...
   *
   * \returns A secure random value of integral type T.
   */
  template <class T>
   ...
   *
   * \returns A secure random uint32_t value.
   */
   ...
   *
   * \param max Exclusive upper bound for the generated value.
   * \returns A secure random uint32_t in [0, max), or 0 if max == 0.
   */
   ...
   *
   * \param min Inclusive lower bound for the generated value.
   * \param max Exclusive upper bound for the generated value.
   * \returns A secure random uint32_t in [min, max), or min if min == max.
   */
   ...
   *
   * \returns A secure random uint64_t value.
   */
   ...
   *
   * \param max Exclusive upper bound for the generated value.
   * \returns A secure random uint64_t in [0, max), or 0 if max == 0.
   */
   ...
   * \param min Inclusive lower bound for the generated value.
   * \param max Exclusive upper bound for the generated value.
   * \returns A secure random uint64_t in [min, max), or min if min == max.
   */
   ...
   *
   * \param n The reciprocal of the probability of returning true.
   * \returns true with probability 1/n, false otherwise; false if n == 0.
   */
   ...
   *
   * \returns A secure random double in [0, 1).
   */
   ...
   * \param min Inclusive lower bound for the generated value.
   * \param max Exclusive upper bound for the generated value.
   * \returns A secure random double in [min, max), or min if min == max.
   */
   ...
   * in test.
   *
   * \param rng The random number generator to reseed.
   */
   ...
   * in test.
   *
   * \returns A new RNG seeded with a good seed.
   */
  template <class RNG = DefaultGenerator, class /* EnableIf */ = ValidRNG<RNG>>
  static RNG create();
   ...
   *
   * \returns A SecureRNG suitable for security-sensitive randomness.
   */
   ...
   *
   * \param max Exclusive upper bound for the generated value.
   * \returns A random uint32_t in [0, max), or 0 if max == 0.
   */
   ...
   * \param min Inclusive lower bound for the generated value.
   * \param max Exclusive upper bound for the generated value.
   * \returns A random uint32_t in [min, max), or min if min == max.
   */
   ...
   *
   * \param max Exclusive upper bound for the generated value.
   * \returns A random uint64_t in [0, max), or 0 if max == 0.
   */
   ...
   * \param min Inclusive lower bound for the generated value.
   * \param max Exclusive upper bound for the generated value.
   * \returns A random uint64_t in [min, max), or min if min == max.
   */
   ...
   *
   * \param p The probability of returning true, in [0, 1].
   ...
   * \param min Inclusive lower bound for the generated value.
   * \param max Exclusive upper bound for the generated value.
   * \returns A random double in [min, max), or min if min == max.
   */
