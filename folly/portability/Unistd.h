...
/// 64-bit file offset type, aliased to `off_t` on these platforms.
typedef off_t off64_t;

/// Repositions the file offset of an open file using a 64-bit offset.
///
/// \param fh The open file descriptor.
/// \param off The new offset, interpreted according to `orig`.
/// \param orig The reference position (`SEEK_SET`, `SEEK_CUR`, or `SEEK_END`).
/// \returns The resulting offset from the start of the file, or -1 on error.
off64_t lseek64(int fh, off64_t off, int orig);

/// Reads from a file at a given 64-bit offset without changing the offset.
///
/// \param fd The open file descriptor.
/// \param buf The destination buffer.
/// \param count The number of bytes to read.
/// \param offset The absolute file offset to read from.
...
#else
/// Imported global `close`.
using ::close;
/// Imported global `pipe`.
using ::pipe;
/// Imported global `read`.
using ::read;
/// Imported global `write`.
using ::write;
#endif
} // namespace fileops
} // namespace folly
