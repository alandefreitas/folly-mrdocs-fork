...
class MPMCPipeline {
 ...
 public:
  ...
  class Ticket : TicketBase {
   public:
    /// Destroy the ticket, checking that all of its uses were consumed.
    ~Ticket() noexcept {
      CHECK_EQ(remainingUses_, 0) << "All tickets must be completely used!";
    }

    /// Construct an empty ticket with no remaining uses.
    Ticket() noexcept : remainingUses_(0) {}

    /// Move-construct a ticket, leaving `other` empty.
    ///
    /// \param other The ticket to move from.
    Ticket(Ticket&& other) noexcept
        : TicketBase(static_cast<TicketBase&&>(other)),
          remainingUses_(std::exchange(other.remainingUses_, 0)) {}

    /// Move-assign a ticket, leaving `other` empty.
    ///
    /// \param other The ticket to move from.
    /// \returns A reference to this ticket.
   ...
   *
   * @param sizes The capacities of the N+1 queues, one per stage boundary
   */
   ...
   *
   * @param elem Output parameter that receives the element read
   * @return A ticket that must be passed back to blockingWriteStage
   */
  template <size_t Stage>
   ...
   * Non-blocking.
   *
   * @param ticket Output parameter that receives the ticket on success
   * @param elem Output parameter that receives the element read
   * @return true if an element was read, false if the queue was empty
   */
  template <size_t Stage>
  bool readStage(
      Ticket<Stage>& ticket,
   ...
   * Blocking.
   *
   * @param ticket The ticket obtained when reading the input element
   * @param args Arguments used to construct the output element in place
   */
   ...
   *
   * @return Estimated number of values in the pipeline, in last-stage units
   */
