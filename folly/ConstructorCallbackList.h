...
class ConstructorCallbackList {
 public:
  ...
  /// Callback invoked with a pointer to the constructed object.
  using Callback = folly::Function<void(T*)>;
  /// Fixed-size array holding the registered callbacks.
  using CallbackArray = std::array<typename This::Callback, MaxCallbacks>;

  /// Construct the list and fire all registered callbacks for `t`.
  ///
  /// \param t Pointer to the constructed object passed to each callback.
