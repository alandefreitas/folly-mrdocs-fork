...
class Fingerprint {
 public:
  ...
  ///
  /// \param v The byte to feed into the fingerprint.
  /// \returns A reference to this fingerprint, to allow chaining.
  ...
  ///
  /// \param v The 32-bit value to feed into the fingerprint.
  /// \returns A reference to this fingerprint, to allow chaining.
  ...
  ///
  /// Processes the bytes in big-endian order and is faster than calling
  /// update8 in a loop.
  ///
  /// \param v The 64-bit value to feed into the fingerprint.
  /// \returns A reference to this fingerprint, to allow chaining.
  ...
  ///
  /// \param str The string whose bytes are fed into the fingerprint.
  /// \returns A reference to this fingerprint, to allow chaining.
   ...
   *
   * \returns The number of uint64_t words needed to hold the fingerprint.
   */
   ...
   *                                  64 bits in out[1].
   *
   * \param out Pointer to an array of size() uint64_t's to receive the result.
   */
 ...
 * \returns The 64-bit Rabin fingerprint of \p str.
 */
 ...
 *
 * \param str The string to fingerprint.
 * \param msb Receives the 64 most significant bits of the fingerprint.
 * \param lsb Receives the 32 least significant bits of the fingerprint.
 */
 ...
 * \param str The string to fingerprint.
 * \param msb Receives the 64 most significant bits of the fingerprint.
 * \param lsb Receives the 64 least significant bits of the fingerprint.
 */
