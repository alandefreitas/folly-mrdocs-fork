...
class SocketAddress {
 public:
  /// Construct an uninitialized SocketAddress with an unspecified family.
   ...
   * @param host The IP address (or hostname, if allowNameLookup is true)
   * @param port The port (in host byte order)
   * @param allowNameLookup If true, attempt to perform hostname lookup if the
   *        hostname does not appear to be a numeric IP address.
   */
  SocketAddress(
  ...
  /// Copy-construct a SocketAddress from another address.
  ///
  /// \param addr The address to copy from.
  SocketAddress(const SocketAddress& addr) { storage_ = addr.storage_; }

  /// Copy-assign a SocketAddress from another address.
  ///
  /// \param addr The address to copy from.
  /// \returns A reference to this SocketAddress.
  SocketAddress& operator=(const SocketAddress& addr) {
    storage_ = addr.storage_;
    return *this;
  }

  /// Move-construct a SocketAddress from another address.
  ///
  /// \param addr The address to move from.
  SocketAddress(SocketAddress&& addr) noexcept {
    storage_ = std::move(addr.storage_);
  }

  /// Move-assign a SocketAddress from another address.
  ///
  /// \param addr The address to move from.
  /// \returns A reference to this SocketAddress.
  ...
  /// Destroy the SocketAddress, freeing any external storage.
  ~SocketAddress() = default;

  /**
   * Return whether this SocketAddress is initialized.
   *
   * @return true if the address family is specified, false otherwise
   */
   ...
   *
   * @param path Local unix path
   * @param length Length of the path, in bytes
   */
   ...
   * @return A SocketAddress initialized from the given path
   */
   ...
   *
   * @param port The local port number (in host byte order)
   * @return A SocketAddress initialized from the given local port
   */
   ...
   *
   * Raises `folly::InvalidAddressFamilyException` if the family is not IPv4 or
   * IPv6.
   *
   * @return IP address
   */
  ...
  /// Return whether this address equals another address.
  ///
  /// \param other The address to compare against.
  /// \returns true if the two addresses are equal, false otherwise.
  bool operator==(const SocketAddress& other) const;

  /// Return whether this address differs from another address.
  ///
  /// \param other The address to compare against.
  /// \returns true if the two addresses differ, false otherwise.
  bool operator!=(const SocketAddress& other) const {
    return !(*this == other);
  }
   ...
   *
   * @param other The address to compare against
   * @return true if this address orders before `other`, false otherwise
   */
  bool operator<(const SocketAddress& other) const;
...
/// Write a human-readable representation of the address to a stream.
///
