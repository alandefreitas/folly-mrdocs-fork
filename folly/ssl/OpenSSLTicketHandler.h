...
class OpenSSLTicketHandler {
 public:
   ...
   *
   * \param ssl The SSL connection the ticket belongs to.
   * \param keyName The 16-byte ticket key name buffer.
   * \param iv The initialization vector buffer.
   * \param cipherCtx The cipher context to configure.
   * \param hmacCtx The HMAC context to configure.
   * \param encrypt Non-zero to encrypt a new ticket, zero to decrypt.
   * \returns A status code as described above.
   */
