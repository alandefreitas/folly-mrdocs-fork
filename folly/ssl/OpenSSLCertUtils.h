...
class OpenSSLCertUtils {
 public:
  ...
  ///
  /// \param x509 The certificate to read.
  /// \returns The common name, or folly::none.
  ...
  ///
  /// \param x509 The certificate to read.
  /// \returns The issuer common name, or folly::none.
  ...
  ///
  /// \param x509 The certificate to read.
  /// \returns The DNS name entries.
  static std::vector<std::string> getSubjectAltNames(const X509& x509);

  /// Returns URI subjectAltName entries in certificate order.
  ///
  /// \param x509 The certificate to read.
  /// \returns The URI entries.
  static std::vector<std::string> getSubjectAltNameURIs(const X509& x509);

  /// Returns DNS name, URI, and IP address subjectAltName entries in
  /// certificate order. Other GENERAL_NAME types are ignored.
  ///
  /// \param x509 The certificate to read.
  /// \returns The subjectAltName entries.
   ...
   * @param x509    Reference to an X509
   * @return the not-before time as a string
   */
   ...
   * @param x509    Reference to an X509
   * @return the not-after time as a string
   */
   ...
   * @param x509    Reference to an X509
   * @return the summary string, or folly::none
   */
   ...
   *
   * \param range The DER-encoded certificate bytes.
   * \returns The decoded certificate.
   */
  static X509UniquePtr derDecode(ByteRange range);
   ...
   * Throws on error.
   *
   * \param x509 The certificate to encode.
   * \returns The DER-encoded certificate.
   */
  static std::unique_ptr<IOBuf> derEncode(X509& x509);
   ...
   * Throws on error (if a valid certificate can't be decoded).
   *
   * \param range The PEM-encoded certificate bytes.
   * \returns The decoded certificate.
   */
  static X509UniquePtr pemDecode(ByteRange range);
   ...
   */
  static std::string pemEncode(X509& x509);
   ...
   * std::chrono classes.
   *
   * \param asnTime The ASN1 time to convert.
   * \returns The equivalent system clock time point.
   */
