...
struct floating_point_integral_constant {
  /// The floating-point type that holds the integral value.
  using value_type = T;

  /// The stored value, converted to value_type.
  static constexpr value_type value = static_cast<value_type>(Value);

  /// Convert to value_type.
  ///
...
struct constexpr_iterated_squares_desc {
  ...
  /// The unsigned integer type used to count powers.
  using size_type = decltype(Size);

  /// The base type of the scaling values.
  using base_type = T;

  /// A single scaling entry pairing a power with its scale value.
  struct item_type {
    /// The exponent for this scaling entry.
    size_type power;

    /// The scale value, equal to base raised to power.
    base_type scale;
  };

  /// The number of entries in the scaling array.
  static constexpr size_type size = Size;

  /// The base value used to compute the scaling entries.
  base_type base;

  /// The descending array of scaling entries.
 ...
 public:
  /// Construct the scaling array for the given base.
  ///
  /// \param r The base value used to seed the iterated squaring.
  ...
  ///   abs(num) / scale <= max
  ///
  /// \param num The value whose magnitude is to be shrunk.
  /// \param max The upper bound the shrunk magnitude must satisfy.
  /// \returns The scaling params that shrink abs(num) to at most max.
  ...
  ///   abs(num) * scale >= min
  ///
  /// \param num The value whose magnitude is to be grown.
  /// \param min The lower bound the grown magnitude must satisfy.
  /// \returns The scaling params that grow abs(num) to at least min.
...
///
/// Prefer using operator< for ordering. When two values are equivalent, the
/// later argument is returned to make sorting stable. See
/// http://stepanovpapers.com/notes.pdf for details.
///
/// \param a The first value to compare.
/// \param ts The remaining values to compare.
...
///
/// When two values are equivalent, the earlier argument is returned to make
/// sorting stable.
///
/// \param a The first value to compare.
/// \param ts The remaining values to compare.
...
/// Clamp a value to the range [lo, hi] using a comparator.
///
/// \param v The value to clamp.
/// \param lo The lower bound of the range.
/// \param hi The upper bound of the range.
/// \param less The comparator used to order values.
/// \returns A reference to lo if v < lo, hi if hi < v, otherwise v.
...
/// Clamp a value to the range [lo, hi] using std::less.
///
/// \param v The value to clamp.
/// \param lo The lower bound of the range.
/// \param hi The upper bound of the range.
...
/// \returns True if t is not equal to itself, otherwise false.
template <typename T>
...
///
/// \param t The value whose absolute value is computed.
...
///
/// \param t The value whose base-2 logarithm is computed.
/// \returns The floor of the base-2 logarithm of t.
template <typename T>
...
///
/// \param t The value whose base-2 logarithm is computed.
/// \returns The ceiling of the base-2 logarithm of t.
template <typename T>
...
///
/// \param t The value to truncate toward zero.
/// \returns The value of t truncated toward zero.
template <typename T>
...
/// Truncate an integral value toward zero.
///
/// \param t The value to truncate.
/// \returns The value t unchanged, since integers have no fractional part.
template <typename T>
...
///
/// \param t The value to round to the nearest integer.
/// \returns The value of t rounded to the nearest integer.
template <typename T>
...
///
/// \param t The value to round down.
/// \returns The largest integer value not greater than t.
template <typename T>
...
/// mimic: std::ceil (C++23)
///
/// \param t The value to round up.
/// \returns The smallest integer value not less than t.
template <typename T>
...
///
/// \param t The value to round up.
/// \param round The multiple to round up to.
/// \returns The least multiple of round that is at least t.
template <typename T>
...
///
/// \param power The integer exponent to raise e to.
/// \returns An approximation of e raised to power.
...
/// Calculate an approximation of exp(power) as a double.
///
/// \param power The integer exponent to raise e to.
/// \returns An approximation of e raised to power, as a double.
template <typename N>
  requires(std::integral<N> && !std::same_as<N, bool>)
constexpr double constexpr_exp(N const power) {
  return constexpr_exp<double>(power);
}
/// Calculate an approximation of exp(power) for a floating-point exponent.
///
/// \param power The floating-point exponent to raise e to.
/// \returns An approximation of e raised to power.
template <typename T>
  requires std::floating_point<T>
...
///
/// \param num The value whose natural logarithm is computed.
/// \returns An approximation of the natural logarithm of num.
template <typename T>
  requires std::floating_point<T>
...
/// \returns An approximation of base raised to exp.
template <typename T, typename E>
...
/// Calculate an approximation of base raised to a floating-point exponent.
///
/// \param base The base value.
...
/// \returns The 1-based index of the most significant set bit, or 0 if t is 0.
template <typename T>
...
///
/// \param t The value to examine.
/// \returns The 1-based index of the least significant set bit, or 0 if t is 0.
template <typename T>
...
/// \returns The sum of a and b, clamped to the range of T on overflow.
template <typename T>
...
/// \returns The difference a - b, clamped to the range of T on overflow.
template <typename T>
...
/// Convert between integral types, clamping out-of-range values.
///
/// Provide sane numeric conversions between different integral types. It helps
/// to avoid unexpected bugs introduced by bad conversion, and undefined
/// behavior like overflow.
///
/// When doing `constexpr_clamp_cast<Dst>(value)`, if `value` is in the valid
/// range of Dst, it gives the correct result in Dst, equal to `value`.
///
/// If `value` is outside the representable range of Dst, it is clamped to the
/// maximum or minimum in Dst, instead of being undefined behavior.
///
/// Here is some comparison with `static_cast` (with the FB-internal
/// gcc-5-glibc-2.23 toolchain):
///
...
/// \returns The value of src converted to Dst, clamped to the range of Dst.
...
/// Convert a floating-point value to an integral type, clamping out-of-range
/// values.
///
/// Floating-point NaNs are converted to 0 in the integral type.
///
/// \param src The floating-point value to convert.
/// \returns The value of src converted to Dst, clamped to the range of Dst,
///   or 0 if src is NaN.
template <typename Dst, typename Src>
