...
struct AtomicIntrusiveLinkedListHook {
  /// Pointer to the next element in the intrusive list.
  T* next{nullptr};
};

/// A lock-free intrusive single-linked list.
///
/// The list stores elements of type `T` that embed an
/// `AtomicIntrusiveLinkedListHook<T>`, referenced by the `HookMember`
/// pointer-to-member. Insertion is atomic; removal happens in bulk through
...
class AtomicIntrusiveLinkedList {
 public:
  ...
  AtomicIntrusiveLinkedList(const AtomicIntrusiveLinkedList& other) = delete;

  /// Copy assignment is disabled.
  ///
  /// \param other The list that would be assigned from.
  /// \returns A reference to this list.
  AtomicIntrusiveLinkedList& operator=(const AtomicIntrusiveLinkedList& other) =
      delete;

  /// Move-construct from `other`, transferring its elements and leaving it
  /// empty.
  ///
  ...
  ///
  /// Absent because it would be too error-prone to use correctly given the
  /// requirement that lists are empty upon destruction.
  ///
  /// \param other The list that would be moved from.
   ...
   *
   * \returns A new list holding the elements previously in this list.
   */
   ...
   *
   * \param other The list whose elements replace the current ones.
   * \param func Callback invoked on each element of the old list, in
   *        tail-to-head order.
   */
  template <typename F>
   ...
   *
   * \returns A pointer to the current head element, or nullptr if empty.
   */
   ...
   * \returns True if the inserted element is the only one in the list
   *         after the call.
   */
  bool insertHead(T* t) {
   ...
   *
   * \param func Callback invoked on each removed element, tail to head.
   * \returns True if any elements were removed, false if the list was empty.
   */
  template <typename F>
   ...
   *
   * \param func Callback invoked on each removed element, in LIFO order.
   */
  template <typename F>
