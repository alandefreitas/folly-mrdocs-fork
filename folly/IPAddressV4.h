...
class IPAddressV4 {
 public:
   ...
   * @returns true if `ip` can be parsed as an IPv4 address
   */
   ...
   * @returns an IPAddressV4 for `src`
   */
   ...
   *
   * @param bytes the raw address bytes, in network byte order
   * @returns an IPAddressV4 built from `bytes`
   */
   ...
   *
   * @param bytes the raw address bytes, in network byte order
   * @returns the IPAddressV4, or an IPAddressFormatError if the input length is
   * not 4 bytes
   */
   ...
   * @returns the IPAddressV4, or an IPAddressFormatError if `str` is not a valid
   * IP
   */
   ...
   *
   * @returns a ByteRange over the 4 address bytes, in network byte order
   */
   ...
   * representation.
   *
   * @param arpaname the in-addr.arpa name to parse
   * @returns an IPAddressV4 for `arpaname`
   */
   ...
   *
   * @param ip the address string to convert
   * @returns the address as a uint32_t, in network byte order
   */
   ...
   *
   * @param ip the address string to convert
   * @returns the address as a uint32_t, in host byte order
   */
   ...
   *
   * @param src the address bytes, in network byte order
   */
   ...
   * address
   *
   * @param src the `in_addr` value to copy
   */
   ...
   *
   * @returns the IPv6-mapped representation of this address
   */
   ...
   *
   * @returns this address as a 6to4 IPv6 address
   */
   ...
   *
   * @returns a JSON string describing the address
   */
   ...
   * netblock.
   *
   * Throws `IPAddressFormatException` if there is no `/mask`.
   *
   ...
   *
   * @returns true if the address is non-routable
   */
   ...
   *
   * @returns true if the address is the link-local broadcast address
   */
   ...
   *
   * @returns the in-addr.arpa representation of the address
   */
   ...
   *
   * @returns the underlying `in_addr` structure
   */
   ...
   *
   * @returns the address as a `sockaddr_in` struct
   */
   ...
   *
   * @returns a ByteArray4 with the address bytes, in network byte order
   */
   ...
   *
   * Aborts if numBits == 0 or numBits > bitCount().
   *
   ...
   * @returns the longest common (address, mask) prefix of `one` and `two`
   */
   ...
   * @param bitIndex n
   * @returns true if the `bitIndex`-th most significant bit is set
   */
   ...
   * @param byteIndex n
   * @returns the `byteIndex`-th most significant byte
   */
   ...
   * @param bitIndex n
   * @returns true if the `bitIndex`-th least significant bit is set
   */
   ...
   * @param byteIndex n
   * @returns the `byteIndex`-th least significant byte
   */
   ...
   *
   * @returns a pointer to the address bytes, in network byte order
   */
...
/**
 * Append the string representation of an IP address to an fbstring.
 *
 * @param addr the address to format
 * @param result the fbstring to append the representation to
 */
 ...
 * @returns true if `addr1` and `addr2` are equal
 */
 ...
 * @param addr1 the first address
 * @param addr2 the second address
 * @returns true if `addr1` and `addr2` differ
 */
inline bool operator!=(const IPAddressV4& addr1, const IPAddressV4& addr2) {
  return !(addr1 == addr2);
}
