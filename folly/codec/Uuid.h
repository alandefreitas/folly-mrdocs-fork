...
enum class [[nodiscard]] UuidParseCode : unsigned char {
  ...
  WRONG_LENGTH, ///< Input did not have the expected length.
  INVALID_CHAR, ///< Input contained an invalid character.
};
...
///
/// \param out Destination buffer receiving the 16 decoded bytes.
/// \param s Source buffer holding the 36 UUID characters.
/// \returns A code indicating whether parsing succeeded.
FOLLY_ALWAYS_INLINE UuidParseCode
...
///
/// \param out Destination string receiving the 16 decoded bytes.
/// \param s Source view holding the 36 UUID characters.
/// \returns A code indicating whether parsing succeeded.
