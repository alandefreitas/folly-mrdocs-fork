...
struct AtomicUnorderedInsertMap {
  ...
  /// The key-value pair type exposed by iterators.
  ...
  /// Const forward iterator over the key-value pairs in the map.
  struct ConstIterator {
    /// Construct an iterator over the given map positioned at the given slot.
    ///
    /// \param owner The map to iterate over.
    /// \param slot The slot index to position the iterator at.
    ...
    /// Return a reference to the key-value pair at the current position.
    ///
    ...
    /// Return a pointer to the key-value pair at the current position.
    ///
    ...
    /// Advance to the next element (pre-increment).
    ///
    ...
    /// Advance to the next element (post-increment).
    ///
    /// \param dummy Unused disambiguator for post-increment.
    /// \returns A copy of the iterator prior to advancing.
    ...
    /// Return true if both iterators refer to the same slot.
    ///
    /// \param rhs The iterator to compare against.
    /// \returns True if both iterators refer to the same slot.
    bool operator==(const ConstIterator& rhs) const {
      return slot_ == rhs.slot_;
    }
    /// Return true if the iterators refer to different slots.
    ///
    /// \param rhs The iterator to compare against.
    /// \returns True if the iterators refer to different slots.
  ...
  /// Const iterator type over the map's key-value pairs.
  ...
  ///
  /// @param maxSize The maximum number of key-value pairs to support.
  /// @param maxLoadFactor The maximum load factor before insertion fails.
  /// @param alloc The allocator used for the backing storage.
  ...
  /// Destroy the map, freeing the backing storage and any stored keys/values.
  ...
  /// @param func Functor invoked with raw storage to construct the value.
  ...
  ///
  /// @param key The key to search for or insert.
  /// @param value The value to construct if the key is not present.
  /// @return A pair of an iterator to the element and whether it was inserted.
  ...
  /// Return an iterator to the element with the given key, or end() if absent.
  ///
  /// \param key The key to search for.
  /// \returns An iterator to the matching element, or end() if not found.
...
struct MutableAtom {
  ...
  /// Construct the wrapper, initializing the atomic value with init.
  ///
  /// \param init The value to copy into the atomic.
...
struct MutableData {
  ...
  ///
  /// \param init The value to copy into the wrapper.
