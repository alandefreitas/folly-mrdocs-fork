...
struct unique_hash_key_algo_strong_sha256_fn {
  ...
  ///
  /// @param in Span of key items to hash
  /// @return The SHA256-derived digest of the requested size
  std::array<uint8_t, Size> operator()(
      std::span<detail::unique_hash_key_item const> in) const noexcept;
};
/// Callable SHA256 hashing object producing a Size-byte digest, for use with unique_hash_key.
template <size_t Size>
...
class unique_hash_key {
 ...
 public:
  /// Construct a key by cryptographically hashing the given arguments.
  ///
  /// @param algo Hash algorithm object to apply
  /// @param arg Hashable arguments whose bytes form the key input
  template <
      typename Algo,
      typename... Arg,
  ...
  ///
  /// @return A span over the key's bytes reinterpreted as the element type
  template <
      typename T,
      std::size_t E,
  ...
  /// Compare two keys for byte-wise equality.
  ///
  /// @param a First key to compare
  /// @param b Second key to compare
  /// @return True if the keys have identical bytes
  ...
  /// @param a First key to compare
  /// @param b Second key to compare
  /// @return True if the keys differ in any byte
  friend auto operator!=(self const& a, self const& b) noexcept {
    return a.data_ != b.data_;
  }
};
...
class unique_hash_key_with
 ...
 public:
  ...
  ///
  /// @param arg Tuple of hashable values whose bytes form the key input
...
struct hash<::folly::unique_hash_key<Size>> {
  ...
  /// @return The stored hash value of the key
