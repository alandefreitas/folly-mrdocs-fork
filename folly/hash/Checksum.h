 ...
 * @param startingChecksum Initial checksum value to continue from
 * @return The CRC-32C checksum of the buffer
 */
uint32_t crc32c(
 ...
 * @param nbytes Number of bytes in the buffer
 * @param startingChecksum Initial checksum value to continue from
 * @return The CRC-32 checksum of the buffer
 */
uint32_t crc32(
    const uint8_t* data, size_t nbytes, uint32_t startingChecksum = ~0U);
 ...
 * above two examples.
 *
 * @param crc1 CRC-32 checksum of the first buffer
 * @param crc2 CRC-32 checksum of the second buffer
 * @param crc2len Length of the second buffer (in bytes)
 * @return Combined CRC-32 checksum over both buffers
 */
uint32_t crc32_combine(uint32_t crc1, uint32_t crc2, size_t crc2len);

/**
 * Combine two CRC-32C checksums into one, like crc32_combine() but using the
 * CRC-32C polynomial.
 *
 * @param crc1 CRC-32C checksum of the first buffer
 * @param crc2 CRC-32C checksum of the second buffer
 * @param crc2len Length of the second buffer (in bytes)
 * @return Combined CRC-32C checksum over both buffers
 */
