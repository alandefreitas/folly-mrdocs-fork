...
class Function final : private detail::function::FunctionTraits<FunctionType> {
  ...
  friend Traits;
  /// Grants `constCastFunction` access to the private coercion constructor.
 ...
 public:
  ...
  /// Copy construction is deleted; `Function` is move-only.
  ///
  /// \param that The function that would be copied from.
  // not copyable
   ...
   * Move constructor
   *
   * \param that The function to move from.
   */
   ...
   *
   * \param nullPtr A null pointer constant.
   */
  /* implicit */ constexpr Function(std::nullptr_t nullPtr) noexcept {}
   ...
   *
   * \param ptr The pointer to member to wrap.
   */
  template <
      typename Member,
      typename Class,
  ...
  /// Destroy the `Function`, releasing any owned callable.
  ~Function() { exec(Op::NUKE, &data_, nullptr); }

  /// Copy assignment is deleted; `Function` is move-only.
  ///
  /// \param that The function that would be assigned from.
   ...
   *
   * \param fun The callable object to assign.
   * \returns A reference to this function.
   ...
   *
   * \param that The function to move-assign from.
   * \returns A reference to this function.
   */
  template <
      typename Signature,
   ...
   * \returns A reference to this function.
   */
  Function& operator=(std::nullptr_t nullPtr) noexcept {
   ...
   *
   * \param ptr The pointer to member to assign.
   * \returns A reference to this function.
   */
   ...
   * non-empty.
   *
   * \returns True if this function holds a callable.
   */
   ...
   *
   * \returns The number of heap bytes used to store the callable.
   */
   ...
   * share the state.
   *
   * \returns A copyable proxy sharing this function's state.
   */
   ...
   *
   * \returns A `std::function` sharing this function's state.
   */
...
///
/// \param lhs The first function.
/// \param rhs The second function.
template <typename FunctionType>
...
template <typename FunctionType>
bool operator==(const Function<FunctionType>& fn, std::nullptr_t nullPtr) {
  return !fn;
}
...
/// \returns True if `fn` is empty.
template <typename FunctionType>
bool operator==(std::nullptr_t nullPtr, const Function<FunctionType>& fn) {
  return !fn;
}
...
/// \param fn The function to test.
/// \param nullPtr A null pointer constant.
/// \returns True if `fn` holds a callable.
template <typename FunctionType>
bool operator!=(const Function<FunctionType>& fn, std::nullptr_t nullPtr) {
  return !(fn == nullptr);
}
 ...
 * \returns A `Function` with a const signature wrapping `that`.
 */
template <typename ReturnType, typename... Args>
Function<ReturnType(Args...) const> constCastFunction(
...
/// Casts an already-const `folly::Function` to a const signature.
///
/// \param that The function to cast.
/// \returns The function with a const signature.
template <typename ReturnType, typename... Args>
Function<ReturnType(Args...) const> constCastFunction(
...
/// Casts a noexcept `folly::Function` from non-const to a const signature.
///
/// \param that The function to cast.
/// \returns The function with a const noexcept signature.
template <typename ReturnType, typename... Args>
Function<ReturnType(Args...) const noexcept> constCastFunction(
...
/// Casts an already-const noexcept `folly::Function` to a const signature.
///
/// \param that The function to cast.
/// \returns The function with a const noexcept signature.
template <typename ReturnType, typename... Args>
...
/// Deduction guide that infers a `Function`'s signature from a callable `F`.
template <typename F>
...
class FunctionRef<ReturnType(Args...)> final {
 ...
 public:
   ...
   * \param nullPtr A null pointer constant.
   */
  constexpr explicit FunctionRef(std::nullptr_t nullPtr) noexcept {}
   ...
   * empty.
   *
   * \param fun The callable object to reference.
   */
  template <
      typename Fun,
      std::enable_if_t<
   ...
   *
   * \param fun The function pointer to reference.
   */
  template <
      typename Fun,
  ...
  ///
  /// \param args The arguments to forward to the referenced callable.
  /// \returns The result of invoking the referenced callable.
  ReturnType operator()(Args... args) const {
    return call_(static_cast<Args&&>(args)..., object_);
  }

  /// Reports whether this `FunctionRef` references a callable.
  ///
  /// \returns True if a callable is referenced.
  constexpr explicit operator bool() const noexcept { return object_; }

  /// Returns `true` if the `FunctionRef` is empty.
  ///
  /// \param ref The reference to test.
  /// \param nullPtr A null pointer constant.
  /// \returns True if `ref` is empty.
  constexpr friend bool operator==(
      FunctionRef<ReturnType(Args...)> ref, std::nullptr_t nullPtr) noexcept {
  ...
  /// \param ref The reference to test.
  /// \param nullPtr A null pointer constant.
  /// \returns True if `ref` references a callable.
  constexpr friend bool operator!=(
      FunctionRef<ReturnType(Args...)> ref, std::nullptr_t nullPtr) noexcept {
    return ref.object_ != nullptr;
  }
  ...
  /// \returns True if `ref` is empty.
  constexpr friend bool operator==(
      std::nullptr_t nullPtr, FunctionRef<ReturnType(Args...)> ref) noexcept {
    return ref.object_ == nullptr;
  }
  /// Returns `true` if the `FunctionRef` is non-empty.
  ///
