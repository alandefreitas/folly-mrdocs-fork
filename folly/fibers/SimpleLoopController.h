...
class SimpleLoopController : public LoopController {
 public:
  /// Constructs a simple loop controller.
  SimpleLoopController();
  /// Destroys the simple loop controller.
  ~SimpleLoopController();
   ...
   *
   * @param func Function to run each iteration when no ready task is present.
   */
  template <typename F>
  void loop(F&& func) {
  ...
  /// Returns how many times scheduleThreadSafe() has been called.
  ///
  /// @returns The remote schedule call count.
  int remoteScheduleCalled() const { return remoteScheduleCalled_; }

  /// Runs the FiberManager loop until no ready tasks remain.
  ...
  /// Runs the given fiber eagerly.
  ///
  /// @param fiber The fiber to run eagerly.
  void runEagerFiber(Fiber* fiber) override { fm_->runEagerFiberImpl(fiber); }

  /// Schedules the loop function to run at some point in the future.
  void schedule() override { scheduled_ = true; }

  /// Returns the timer used to schedule delayed functions.
  ///
  /// @returns The wheel timer backing this loop controller.
  HHWheelTimer* timer() override { return timer_.get(); }

  /// Reports whether the caller is running on the loop thread.
  ///
  /// @returns True if the current thread is the loop thread.
