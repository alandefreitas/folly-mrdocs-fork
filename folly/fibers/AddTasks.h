 ...
 *
 * @param first Beginning of the range of tasks to be scheduled
 * @param last End of the range of tasks to be scheduled
 *
...
class TaskIterator {
 public:
  /// The type produced by each scheduled task.
  using value_type = T;

  /// Construct an empty iterator bound to the current FiberManager.
  TaskIterator() : fm_(FiberManager::getFiberManager()) {}

  /// Deleted copy constructor; the iterator is not copyable.
  ///
  /// @param other The iterator that would be copied.
  TaskIterator(const TaskIterator& other) = delete;

  /// Deleted copy assignment; the iterator is not copyable.
  ///
  /// @param other The iterator that would be assigned from.
  /// @returns A reference to this iterator.
  TaskIterator& operator=(const TaskIterator& other) = delete;

  /// Move constructor; transfers ownership of the pending tasks.
  ///
  /// @param other The iterator to move from.
  TaskIterator(TaskIterator&& other) noexcept;

  /// Deleted move assignment; the iterator is not move-assignable.
  ///
  /// @param other The iterator that would be moved from.
  /// @returns A reference to this iterator.
  ...
  /**
   * Check whether any completed tasks are ready to be consumed.
   *
  ...
  /**
   * Check whether any tasks are still pending execution.
   *
  ...
  /**
   * Check whether any tasks remain to be consumed.
   *
   ...
   *
   * Rethrows any `exception` thrown by the completed task.
   */
  T awaitNext();
  ...
  /**
   * Retrieve the id of the most recently processed task.
   *
 ...
 private:
  /// Grants addTasks access to construct and populate the iterator.
  ///
  /// @param first Beginning of the range of tasks to be scheduled
  /// @param last End of the range of tasks to be scheduled
  /// @returns movable, non-copyable iterator
  template <class InputIterator>
