...
struct TaskOptions {
  /// Construct task options with default values.
  TaskOptions() {}
  /**
...
class Fiber {
 public:
  ...
  /// Deleted copy constructor; fibers are not copyable.
  ///
  /// @param other The fiber that would be copied.
  Fiber(const Fiber& other) = delete;

  /// Deleted copy assignment; fibers are not copyable.
  ///
  /// @param other The fiber that would be assigned from.
  /// @returns A reference to this fiber.
  Fiber& operator=(const Fiber& other) = delete;

  /// Destroy the fiber and release its resources.
  ~Fiber();
  ...
  /// Retrieve the maximum stack usage observed for this fiber.
  ///
  /// @returns The high watermark of stack usage, in bytes.
  size_t stackHighWatermark() const { return fiberStackHighWatermark_; }

  /// Retrieve how long this fiber's current task has been running.
  ///
  /// @returns The task running time, or an empty optional if not tracked.
  ...
  /// Retrieve the current execution state of this fiber.
  ///
