...
/// True if integer division on this platform yields a remainder, i.e. the
/// hardware division instruction produces both quotient and remainder.
 ...
 * \returns num/denom, rounded toward negative infinity.
 */
template <typename N, typename D>
 ...
 * \param num The dividend.
 * \param denom The divisor.
 * \returns num/denom, rounded toward positive infinity.
 */
template <typename N, typename D>
 ...
 * \param num The dividend.
 * \param denom The divisor.
 * \returns num/denom, rounded toward zero.
 */
template <typename N, typename D>
 ...
 * \param num The dividend.
 * \param denom The divisor.
 * \returns num/denom, rounded away from zero.
 */
template <typename N, typename D>
...
///
/// Rounds toward `__a` when the sum is odd, mimicking std::midpoint (C++20).
///
/// \param __a The first value.
/// \param __b The second value.
/// \returns The value halfway between `__a` and `__b`.
template <class _Tp>
...
///
/// \param __a Pointer to the first element.
/// \param __b Pointer to the second element.
/// \returns A pointer halfway between `__a` and `__b`, rounded toward `__a`.
...
///
/// Mimics std::midpoint (C++20) for floating-point arguments, staying
/// correctly rounded across the range of representable values.
///
/// \param __a The first value.
/// \param __b The second value.
/// \returns The value halfway between `__a` and `__b`.
