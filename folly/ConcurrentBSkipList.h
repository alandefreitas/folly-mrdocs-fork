...
///
/// add()/addOrUpdate() collapse this to a bool via didInsert(); use the enum
/// directly to distinguish Updated from AlreadyPresent.
enum class BSkipInsertOutcome {
  // didInsert() == true:
  Inserted, ///< A new key was inserted.
  Revived, ///< A tombstoned key was brought back to life.
  // didInsert() == false:
  Updated, ///< An existing live key's payload was updated.
  AlreadyPresent, ///< The key was already present and left unchanged.
};

/// Returns whether an outcome represents a fresh insert or a revive.
///
/// \param outcome The outcome to classify.
/// \returns `true` for Inserted or Revived, `false` otherwise.
...
/// Implementation details of ConcurrentBSkipList.
...
enum class InsertSemantics {
  Add, ///< Do not overwrite an existing live key.
  AddOrUpdate, ///< Update an existing live key via the updater.
};
} // namespace bskip_detail
...
/// A concurrent B-tree/skip-list hybrid sorted container.
///
/// Level 0 holds the leaves; higher levels hold routing keys and children.
/// Writes couple locks hand-over-hand; reads escalate from an optimistic leaf
/// scan to an optimistic descent to a locked traversal. Removals tombstone
/// entries and nodes are never reclaimed. add, remove, contains, find,
/// updatePayload, and addOrUpdate are thread-safe; the destructor is not.
///
/// Most users want the named aliases ConcurrentBSkipSet, ConcurrentBSkipMap,
/// and ConcurrentBSkipInlineMap.
///
/// \tparam T The key type; its numeric_limits min/max are reserved sentinels.
/// \tparam PayloadType The mapped payload type, or void for a set.
/// \tparam B The per-node key capacity.
/// \tparam ReadPolicy The policy controlling how keys are read during scans.
/// \tparam StoragePolicy Whether leaf payloads are stored inline or separately.
/// \tparam Policy Supplies Hash, Comp, NodeAlloc, and promotion parameters.
template <
    typename T,
...
class ConcurrentBSkipList {
 ...
 public:
  ...
  using key_type = T;
  /// The mapped payload type, or void for a set.
  using payload_type = PayloadType;
  /// Whether the container stores a payload alongside each key.
  static constexpr bool kHasPayload = Traits::kHasPayload;
  /// Whether payloads are stored inline in the leaf nodes.
  static constexpr bool kInlinePayloadStorage = Traits::kInlinePayload;
  /// The leaf node type holding the stored keys.
  using leaf_node_type = LeafNode;
  /// The policy controlling how keys are read during scans.
  static constexpr KeyReadPolicy kReadPolicy = ReadPolicy;

  /// The result of a find: an optional key, or key/payload pair with payload.
  using FindResult = std::conditional_t<
      kHasPayload,
      std::optional<std::pair<T, PayloadType>>,
      std::optional<T>>;

  /// Constructs an empty container using a default-constructed allocator.
  ...
  /// Destroys the container and frees its nodes.
  ///
  /// Not thread-safe. It must outlive every Skipper instance, since a
  /// Skipper's optimistic path relies on nodes never being reclaimed.
  ~ConcurrentBSkipList() {
  ...
  /// Copy construction is deleted; the container is not copyable.
  ///
  /// \param other The container that would be copied from.
  ConcurrentBSkipList(const ConcurrentBSkipList& other) = delete;
  /// Copy assignment is deleted; the container is not copyable.
  ///
  /// \param other The container that would be assigned from.
  /// \returns A reference to this container.
  ConcurrentBSkipList& operator=(const ConcurrentBSkipList& other) = delete;
  /// Move construction is deleted; the container is not movable.
  ///
  /// \param other The container that would be moved from.
  ConcurrentBSkipList(ConcurrentBSkipList&& other) = delete;
  /// Move assignment is deleted; the container is not movable.
  ///
  /// \param other The container that would be moved from.
  /// \returns A reference to this container.
  ConcurrentBSkipList& operator=(ConcurrentBSkipList&& other) = delete;

  /// Adds a key if it is not already present as a live entry.
  ///
  /// \param key The key to add.
  /// \returns `true` on a fresh insert or tombstone revive, `false` if the
  ///     key was already present.
  bool add(const T& key) {
    return didInsert(insertImpl<bskip_detail::InsertSemantics::Add>(key));
  }

  /// Adds a key with an initial payload if the key is not already live.
  ///
  /// Existing live keys are not overwritten; use addOrUpdate for that.
  ///
  /// \tparam RequestedPayload The payload type being supplied.
  /// \param key The key to add.
  /// \param payload The payload stored when the key is newly inserted.
  /// \returns `true` on a fresh insert or tombstone revive, `false` if the
  ///     key was already present.
  ...
  /// Inserts a key or updates the payload of an existing live key.
  ///
  /// This is a fused upsert. The updater runs under the leaf mutex and the
  /// seqlock write epoch, so it must not allocate, take other locks, or
  /// throw. For a void payload the updater is `void(const T&)`; for a
  /// payload-bearing container it is
  /// `void(const T&, std::optional<PayloadType>& slot)`, where an empty slot
  /// yields a default-constructed payload and a filled slot stores the value.
  ///
  /// \tparam Updater The callable type applied to the slot.
  /// \param key The key to insert or update.
  /// \param updater The callable that sets or updates the payload.
  /// \returns `true` on insert or revive, `false` on update.
  ...
  /// Removes a key by tombstoning its live entry.
  ///
  /// \param key The key to remove; must not be a reserved sentinel.
  /// \returns `true` if a live entry was removed, `false` if absent.
  ...
  /// Removes a key and returns its payload if it was live.
  ///
  /// \tparam RequestedPayload The payload type to return.
  /// \param key The key to remove; must not be a reserved sentinel.
  /// \returns The removed payload, or nullopt if the key was absent.
  template <typename RequestedPayload = PayloadType>
  ...
  /// Updates the payload of a live key in place.
  ///
  /// The updater runs under the leaf mutex and the seqlock write epoch, so it
  /// must do bounded work only. If the key is absent or tombstoned, use add()
  /// plus updatePayload() or addOrUpdate() instead.
  ///
  /// \tparam Updater The callable type applied to the slot.
  /// \param key The key whose payload is updated.
  /// \param updater The callable that updates the payload.
  /// \returns `true` if the payload was updated, `false` if the key is absent
  ///     or tombstoned.
  template <typename Updater>
  ...
  /// Returns whether a live entry for the key exists.
  ///
  /// \param key The key to look up.
  /// \returns `true` if the key is present.
  bool contains(const T& key) const { return find(key).has_value(); }
  /// Counts the live entries for the key.
  ///
  /// \param key The key to count.
  /// \returns 1 if the key is present, 0 otherwise.
  size_t count(const T& key) const { return contains(key) ? 1 : 0; }

  /// Finds the live entry for the key.
  ///
  /// \param key The key to look up.
  /// \returns The key (and payload, if any), or nullopt if absent.
  FindResult find(const T& key) const { return findImpl(key); }

  /// Returns whether a live entry matching a transparent key exists.
  ///
  /// \tparam LookupKey The heterogeneous lookup key type.
  /// \param key The key to look up.
  /// \returns `true` if a matching key is present.
  template <
      typename LookupKey,
      typename C = Comp,
  ...
  /// Counts the live entries matching a transparent key.
  ///
  /// \tparam LookupKey The heterogeneous lookup key type.
  /// \param key The key to count.
  /// \returns 1 if a matching key is present, 0 otherwise.
  template <
      typename LookupKey,
      typename C = Comp,
      typename = typename C::is_transparent>
  ...
  /// Finds the live entry matching a transparent key.
  ///
  /// \tparam LookupKey The heterogeneous lookup key type.
  /// \param key The key to look up.
  /// \returns The key (and payload, if any), or nullopt if absent.
  ...
  ///
  /// The count is eventually consistent with concurrent operations.
  ///
  /// \returns The current live element count.
  size_t size() const { return size_.load(); }

  /// Returns whether the container has no live entries.
  ///
  /// \returns `true` if the container is empty.
  bool empty() const { return size() == 0; }
  /// Removes a key, matching the standard erase signature.
  ///
  /// \param key The key to erase.
  /// \returns The number of elements removed (0 or 1).
  size_t erase(const T& key) { return remove(key) ? 1 : 0; }

  /// Invokes a callable for each live element in the container.
  ///
  /// This is a thread-safe full scan. `func` runs under the leaf's read
  /// guard, so it must do bounded work with no re-entry. Each live key is
  /// emitted exactly once, giving per-slot consistency rather than snapshot
  /// isolation.
  ///
  /// \tparam Func The callable type invoked per element.
  /// \param func The callable applied to each live element.
...
/// A forward-only cursor over a ConcurrentBSkipList.
///
/// A Skipper advances through live elements and can jump forward to the first
/// key not ordered before a target. It is safe against concurrent list
/// mutation, but a single Skipper must be used by one thread at a time.
template <
    typename T,
...
class ConcurrentBSkipList<
 ...
 public:
  /// Constructs a Skipper positioned at the start of the list.
  ///
  /// \param list The list to traverse; it must outlive the Skipper.
  ...
  /// Copy-constructs a Skipper, copying its position and descent cache.
  ///
  /// The copied node pointers are safe because nodes live with the list;
  /// every optimistic use revalidates and falls back to a locked descent if
  /// stale.
  ///
  /// \param other The Skipper to copy from.
  Skipper(const Skipper& other) = default;
  /// Move-constructs a Skipper, taking over its position and descent cache.
  ///
  /// \param other The Skipper to move from.
  Skipper(Skipper&& other) = default;
  /// Destroys the Skipper.
  ~Skipper() = default;
  /// Copy assignment is deleted; a Skipper is not assignable.
  ///
  /// \param other The Skipper that would be assigned from.
  /// \returns A reference to this Skipper.
  Skipper& operator=(const Skipper& other) = delete;
  /// Move assignment is deleted; a Skipper is not assignable.
  ///
  /// \param other The Skipper that would be assigned from.
  /// \returns A reference to this Skipper.
  Skipper& operator=(Skipper&& other) = delete;

  /// Returns whether the Skipper points at a valid element.
  ///
  /// \returns `true` if positioned on a live element.
  FOLLY_ALWAYS_INLINE bool good() const { return currentLeaf_ != nullptr; }

  /// Advances to the first live key not ordered before the target.
  ///
  /// LookupKey allows heterogeneous lookup without materializing a T.
  ///
  /// \tparam LookupKey The heterogeneous target key type.
  /// \param target The key to skip to.
  /// \returns The key reached, or nullopt if the end was passed.
  template <typename LookupKey>
  ...
  /// Advances to the next live key after the current position.
  ///
  /// \returns The next key, or nullopt if the end was reached.
  ...
  ///
  /// \returns The current key, or nullopt if not positioned on an element.
  ...
  /// Returns the payload cached for the current position.
  ///
  /// \returns A const reference to the current element's payload.
  ...
  /// Reads the payload of the current element directly from the leaf.
  ///
  /// \returns A copy of the current element's payload.
  ...
  /// Returns the leaf node holding the current element.
  ///
  /// \returns A pointer to the current leaf node.
  LeafNode* currentLeaf() const { return currentLeaf_; }
  /// Returns the slot index of the current element within its leaf.
  ///
...
///
/// It bundles the hash, comparator, allocator, and the promotion, height, and
/// storage constants that the container and its aliases forward into their
/// per-axis template parameters.
///
/// \tparam T The key type the policy configures.
/// \tparam PromotionHash The hash used for deterministic height assignment.
/// \tparam Compare The comparator used to order keys.
/// \tparam Allocator The allocator used for nodes.
template <
    typename T,
    ...
    typename Allocator>
struct ConcurrentBSkipDefaultPolicy {
  /// The hash used for deterministic height assignment.
  using Hash = PromotionHash;
  /// The comparator used to order keys.
  using Comp = Compare;
  /// The allocator used for nodes.
  using NodeAlloc = Allocator;

  /// The per-leaf key capacity.
  static constexpr uint8_t kLeafSlots = 16;
  /// The inverse promotion probability (a power of two greater than one).
  ...
  /// The policy controlling how leaf payloads are stored.
...
///
/// The alias forwards Policy::k* into the per-axis params; pass a custom
/// Policy to override Hash, Comp, Allocator, kMaxHeight, or
/// kPromotionProbInverse.
...
/// \tparam PayloadType The mapped payload type.
/// \tparam Policy The configuration policy.
template <
    typename T,
    typename PayloadType,
    typename Policy = ConcurrentBSkipDefaultPolicy<T>>
