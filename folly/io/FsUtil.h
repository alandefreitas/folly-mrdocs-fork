...
namespace folly {
/// Filesystem helpers that extend boost::filesystem.
namespace fs {

/// Alias for the standard filesystem namespace.
 ...
 *
 * \param p The path whose parent is canonicalized.
 * \param basePath The base used to resolve relative paths.
 * \returns The path with its parent canonicalized.
 */
...
struct unique_path_fn {
  /// Returns a unique path built from the given model.
  ///
  /// \param model The pattern used to generate the unique path.
  /// \returns A unique path derived from the model.
