...
class ShutdownSocketSet {
 public:
   ...
   * on your system.
   *
   * \param capacity The largest file descriptor value the set can hold.
   */
  explicit ShutdownSocketSet(size_t capacity = 1 << 18);

  /// Deleted copy constructor; the set is non-copyable.
  ///
  /// \param other The set that would be copied.
  ShutdownSocketSet(const ShutdownSocketSet& other) = delete;

  /// Deleted copy assignment; the set is non-copyable.
  ///
  /// \param other The set that would be assigned from.
  /// \returns A reference to this set.
  ShutdownSocketSet& operator=(const ShutdownSocketSet& other) = delete;
   ...
   *
   * \param fd The already open socket to manage.
   */
   ...
   * being shutdown().
   *
   * \param fd The managed socket to remove.
   */
   ...
   *
   * \param fd The managed socket to close.
   * \returns The same return code as `::close()`.
   */
   ...
   *
   * \param fd The managed socket to shut down.
   * \param abortive Whether to perform an abortive shutdown (send RST).
   */
   ...
   *
   * \param abortive Whether to set active connections to not linger.
   */
