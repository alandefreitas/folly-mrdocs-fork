...
class IOBufIovecBuilder {
 ...
 public:
  /// Configuration options for an IOBufIovecBuilder.
  struct Options {
    /// Default allocation block size, in bytes.
    static constexpr size_t kDefaultBlockSize = 16 * 1024;
    size_t blockSize_{kDefaultBlockSize}; ///< Allocation block size, in bytes.

    /// Sets the allocation block size and returns *this.
    ///
    /// \param blockSize The allocation block size, in bytes.
  ...
  IOBufIovecBuilder(const IOBufIovecBuilder& other) = delete;
  /// Deleted move constructor.
  IOBufIovecBuilder(IOBufIovecBuilder&& other) = delete;

  /// Deleted copy assignment operator.
  IOBufIovecBuilder& operator=(const IOBufIovecBuilder& other) = delete;
  /// Deleted move assignment operator.
  IOBufIovecBuilder& operator=(IOBufIovecBuilder&& other) = delete;

  /// Constructs a builder with the given options.
  ///
  /// \param options The configuration options for the builder.
  explicit IOBufIovecBuilder(const Options& options) : options_(options) {}
  /// Releases the builder's references to its allocated buffers.
  ~IOBufIovecBuilder() {
