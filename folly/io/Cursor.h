...
namespace folly {
/// Utilities for iterating over and writing to IOBuf chains.
namespace io {
...
class CursorBase {
 ...
 protected:
   ...
   *
   * \param buf The IOBuf chain to wrap.
   */
  ...
  /**
   * Copy constructor that also imposes a length bound.
   *
   * \param cursor The cursor to copy from.
   * \param len An upper bound on the number of bytes available to this cursor.
   */
 ...
 public:
   ...
   * @methodset Modifiers
   *
   * \param buf The new IOBuf chain to point at.
   */
   ...
   * @methodset Advanced
   *
   * \returns The offset into the current buffer, in bytes.
   */
   ...
   * @methodset Capacity
   *
   * \returns The absolute offset from the head of the chain, in bytes.
   */
   ...
   * @methodset Accessors
   *
   * \returns A pointer to the byte at the current cursor position.
   */
   ...
   * @methodset Advanced
   *
   * \returns The IOBuf the cursor currently points into.
   */
   ...
   *
   * \returns The number of bytes left in the current IOBuf.
   */
   ...
   *
   * \returns The number of bytes left until the end of the chain or boundary.
   */
   ...
   *
   * \param amount The number of bytes to test for.
   * \returns True if the cursor can advance that many bytes.
   */
   ...
   * @methodset Capacity
   *
   * \returns True if no bytes remain in the chain.
   */
  ...
  ///
  /// \param offset The number of bytes to advance.
  /// \returns A new cursor advanced by offset.
  ...
  /// @methodset Modifiers
  ///
  /// \param offset The number of bytes to retreat.
  /// \returns A reference to this cursor.
  Derived& operator-=(size_t offset) {
    Derived* p = static_cast<Derived*>(this);
    p->retreat(offset);
  ...
  ///
  /// \param offset The number of bytes to retreat.
  /// \returns A new cursor retreated by offset.
  Derived operator-(size_t offset) const {
    Derived other(*this);
   ...
   * \returns True if the cursors point to the same location.
   */
  ...
  ///
  /// \param other The cursor to compare against.
  /// \returns True if the cursors point to different locations.
   ...
   *
   * @throws std::out_of_range if there aren't enough bytes left in the cursor.
   * @return The value read from the cursor.
   */
  template <class T>
  T read() {
    if (FOLLY_LIKELY(uintptr_t(crtPos_) + sizeof(T) <= uintptr_t(crtEnd_))) {
   ...
   * vs. using pull().
   *
   * @param termChar The byte that terminates the string.
   ...
   *
   * @param predicate The predicate tested against each byte.
   * @return The matching bytes, as a string.
   */
   ...
   *
   * @param predicate The predicate tested against each byte.
   * @param out The output object that receives each matching chunk.
   */
   ...
   * @methodset Modifiers
   *
   * @param len The maximum number of bytes to advance.
   ...
   * @methodset Modifiers
   *
   * @param len The maximum number of bytes to retreat.
   * @return The number of bytes actually retreated.
   */
   ...
   * @methodset Modifiers
   *
   * @param len The number of bytes to retreat.
   * @throws std::out_of_range if the cursor doesn't have enough bytes to retreat.
   */
   ...
   * @throw std::out_of_range if there aren't enough bytes in the cursor.
   */
   ...
   *
   * @returns The available bytes in the current IOBuf.
   */
  ByteRange peekBytes() {
   ...
   * @methodset Accessors
   *
   * @returns The available bytes in the current IOBuf, as a span.
   */
   ...
   * @throws std::out_of_range if there aren't enough bytes in this cursor.
   */
  void clone(
   ...
   *
   * @param[out] buf The IOBuf into which to place the cloned data.
   * @param len The maximum number of bytes to clone.
   * @param ownership Whether the clone may point at data that IOBuf does not
   *                  own. @see CloneOwnership.
   ...
   *
   * \param other The cursor to measure the distance from.
   * \returns The number of bytes between the two cursors.
   */
   ...
   *
   * \param buf The IOBuf to measure the distance from.
   * \returns The number of bytes between the IOBuf and this cursor.
   */
   ...
   *
   * \returns True if the cursor is bounded.
   */
 ...
 protected:
  ...
  CursorBase(CursorBase const& other) = default;
  /// Move constructor.
  ///
  /// \param other The cursor to move from.
  CursorBase(CursorBase&& other) = default;
  /// Copy assignment operator.
  ///
  /// \param other The cursor to copy from.
  /// \returns A reference to this cursor.
  CursorBase& operator=(CursorBase const& other) = default;
  /// Move assignment operator.
  ///
  /// \param other The cursor to move from.
  /// \returns A reference to this cursor.
  CursorBase& operator=(CursorBase&& other) = default;
  ...
  ///
  /// \returns The head IOBuf of the chain.
  BufType* head() { return buffer_; }

  /// Advance to the next buffer in the chain, if any.
  ///
  /// \returns True if the cursor advanced to a following buffer.
  ...
  ///
  /// \returns True if the cursor retreated to a preceding buffer.
  ...
  ///
  /// \returns A pointer to the borrow flag, or nullptr.
...
class ThinCursor {
 public:
  ...
  ///
  /// \param other The cursor to move from.
  ThinCursor(ThinCursor&& other) = default;
  /// Deleted copy constructor.
  ///
  /// \param other The cursor to copy from.
  ThinCursor(const ThinCursor& other) = delete;
  /// Move assignment operator.
  ///
  /// \param other The cursor to move from.
  /// \returns A reference to this cursor.
  ThinCursor& operator=(ThinCursor&& other) = default;
  /// Deleted copy assignment operator.
  ///
  /// \param other The cursor to copy from.
  /// \returns A reference to this cursor.
  ThinCursor& operator=(const ThinCursor& other) = delete;
  ...
  /// Get the number of readable bytes left in the current buffer.
  ///
  ...
  /// Test whether the cursor can advance the given number of bytes.
  ///
  /// \param amount The number of bytes to test for.
  /// \returns True if the cursor can advance that many bytes.
  bool canAdvance(size_t amount) const { return amount <= length(); }

  /// Test whether the cursor is at the end of the current buffer.
  ///
  /// \returns True if no bytes remain in the current buffer.
  bool isAtEnd() const { return length() == 0; }

  /// Read a value from the cursor, using a fallback for slow paths.
  ///
  /// \param fallback The Cursor used when the read crosses a buffer boundary.
  /// \returns The value read from the cursor.
  template <class T>
  ...
  /// Read a Big-Endian integral from the cursor, using a fallback for slow paths.
  ///
  /// \param fallback The Cursor used when the read crosses a buffer boundary.
  /// \returns The value read from the cursor.
  template <class T>
  ...
  /// Read a Little-Endian integral from the cursor, using a fallback for slow paths.
  ///
  /// \param fallback The Cursor used when the read crosses a buffer boundary.
  /// \returns The value read from the cursor.
  template <class T>
  ...
  /// Advance the cursor by len bytes, using a fallback for slow paths.
  ///
  /// \param fallback The Cursor used when the skip crosses a buffer boundary.
  /// \param len The number of bytes to advance.
  ...
  /// Skip bytes in the current buffer without advancing to the next one.
  ///
  /// \param len The number of bytes to skip.
  void skipNoAdvance(size_t len) {
    DCHECK_LE(len, length());
...
class Cursor : public CursorBase<Cursor, const IOBuf> {
 public:
  /// Construct a cursor wrapping an IOBuf chain.
  ///
  /// \param buf The IOBuf chain to wrap.
  explicit Cursor(const IOBuf* buf) noexcept
      : CursorBase<Cursor, const IOBuf>(buf) {}

  /// Construct a bounded cursor wrapping an IOBuf chain.
  ///
  /// \param buf The IOBuf chain to wrap.
  /// \param len An upper bound on the number of bytes available to this cursor.
  ...
  /// Construct a cursor by copying from another cursor.
  ///
  /// \param cursor The cursor to copy from.
  template <class OtherDerived, class OtherBuf>
  ...
  /// Construct a bounded cursor by copying from another cursor.
  ///
  /// \param cursor The cursor to copy from.
  /// \param len An upper bound on the number of bytes available to this cursor.
  template <class OtherDerived, class OtherBuf>
  ...
  /// Borrow a ThinCursor pointing at the current position.
  ///
  /// \returns A ThinCursor sharing this cursor's current position.
  ThinCursor borrow() {
    FOLLY_IO_CURSOR_BORROW_DCHECK(!std::exchange(*borrowed(), true));
    return {crtPos_, crtEnd_};
  }

  /// Return a borrowed ThinCursor, syncing this cursor to its position.
  ///
  /// \param cursor The ThinCursor previously borrowed from this cursor.
...
class Writable {
 public:
  ...
  /// Write a value to the cursor, with the type given by a tag.
  ///
  /// @methodset Writing
  ///
  /// \param typeTag A tag selecting the value type.
  /// \param value The value to write.
  /// \param n The number of bytes of value to write; defaults to sizeof(T).
  template <class T>
  typename std::enable_if<std::is_arithmetic<T>::value>::type write(
      tag_t<T> typeTag, T value, size_t n = sizeof(T)) {
    assert(n <= sizeof(T));
  ...
  /// Write a value to the cursor in Big-Endian, with the type given by a tag.
  ///
  /// @methodset Writing
  ///
  /// \param typeTag A tag selecting the value type.
  /// \param value The value to write.
  template <class T>
  void writeBE(tag_t<T> typeTag, T value) {
    Derived* d = static_cast<Derived*>(this);
  ...
  /// Write a value to the cursor in Little-Endian, with the type given by a tag.
  ///
  /// @methodset Writing
  ///
  /// \param typeTag A tag selecting the value type.
  /// \param value The value to write.
  template <class T>
  void writeLE(tag_t<T> typeTag, T value) {
    Derived* d = static_cast<Derived*>(this);
   ...
   * @throw std::out_of_range if there isn't enough space in the cursor.
   */
  ...
  /// Write a range of bytes to the cursor.
  ///
  /// @methodset Writing
  ///
  /// \param buf The bytes to write.
  /// \throw std::out_of_range if there isn't enough space in the cursor.
  ...
  /// Copy len bytes from a cursor and write them to this cursor.
  ///
  /// @methodset Writing
  ///
  /// \param cursor The source cursor to read from.
  /// \param len The number of bytes to copy.
  /// \throw std::out_of_range if there isn't enough space in the cursor.
  ...
  /// Copy at most len bytes from a cursor and write them to this cursor.
  ///
  /// @methodset Writing
  ///
  /// \param cursor The source cursor to read from.
...
/// Whether a read-write cursor unshares buffers before writing.
enum class CursorAccess {
  PRIVATE, ///< Assume private access; write in place without unsharing.
  UNSHARE ///< Unshare (copy-on-write) buffers before writing.
};
...
class RWCursor
 ...
 public:
  /// Construct a read-write cursor wrapping an IOBuf chain.
  ///
  /// \param buf The IOBuf chain to wrap.
  explicit RWCursor(IOBuf* buf) noexcept
      : CursorBase<RWCursor<access>, IOBuf>(buf), maybeShared_(true) {}

  /// Construct a read-write cursor over the buffers of an IOBufQueue.
  ///
  /// \param queue The queue whose buffers are wrapped.
   ...
   *
   * \param queue The queue whose buffers are wrapped.
   * \param atEnd Tag selecting the end-of-queue starting position.
   */
  RWCursor(IOBufQueue& queue, AtEnd atEnd) noexcept : RWCursor(queue) {
  ...
  /// Construct a read-write cursor by copying from another cursor.
  ///
  /// \param cursor The cursor to copy from; must be unbounded.
  template <class OtherDerived, class OtherBuf>
   ...
   * @methodset Modifiers
   *
   * @param n The number of contiguous bytes to gather.
   * @throw std::overflow_error if there aren't enough bytes to gather
   */
   ...
   * @methodset Modifiers
   *
   * @param n The maximum number of contiguous bytes to gather.
   */
  ...
  /// Write at most len bytes to the cursor, without extending the chain.
  ///
  /// @methodset Writing
  ///
  /// \param buf The bytes to write.
   ...
   * inserted data.
   *
   * @param buf The IOBuf chain to insert at the cursor position.
   */
   ...
   * @methodset Accessors
   *
   * @returns A pointer to the writable bytes at the cursor position.
   */
...
class Appender : public Writable<Appender> {
 public:
  ...
  ///
  /// \param buf The IOBuf chain to append to.
  /// \param growth The minimum size of each newly allocated buffer.
   ...
   * @methodset Accessors
   *
   * @returns A pointer to the writable tail bytes.
   */
   ...
   * @methodset Capacity
   *
   * @returns The number of writable bytes in the current buffer.
   */
   ...
   * @methodset Appending
   *
   * @param n The number of bytes to mark as appended.
   */
   ...
   * @methodset Appending
   *
   * @param n The number of contiguous writable bytes to ensure.
   */
  ...
  /// Write at most len bytes, growing the chain as needed.
  ///
  /// @methodset Appending
  ///
  /// \param buf The bytes to write.
  ...
  ///
  /// \param fmt The printf-style format string.
  /// \param ap The variadic arguments for the format string.
   ...
   *
   * @param sp The string piece to append.
   */
...
class QueueAppender : public Writable<QueueAppender> {
 public:
   ...
   * @param minGrowth The minimum size of each newly allocated buffer.
   * @param maxGrowth The maximum size of each newly allocated buffer.
   */
  QueueAppender(
   ...
   *
   * @param queue The queue to append to.
   * @param growth The fixed size of each newly allocated buffer.
   */
  ...
  /// Reset this appender to use constant buffer growth, as if constructed anew.
  ///
  /// \param queue The queue to append to.
  /// \param growth The fixed size of each newly allocated buffer.
   ...
   * @methodset Capacity
   *
   * @returns The number of writable bytes in the tail.
   */
  ...
  /// Write an object to the cursor, with the type given by a tag.
  ///
  /// \param typeTag A tag selecting the value type.
  /// \param value The value to write.
  /// \param n The number of bytes of value to write; defaults to sizeof(T).
  ...
  /// Write at most len bytes, growing the queue as needed.
  ///
  /// @methodset Appending
  ///
  /// \param buf The bytes to write.
   ...
   * @methodset Writing
   *
   * @param buf The IOBuf chain to insert.
   * @param pack Whether to pack the data into existing tailroom.
   */
  ...
  /// Inserts a copy of data at the current cursor position.
  ///
  /// @methodset Writing
  ///
  /// \param buf The IOBuf chain to copy and insert.
  /// \param pack Whether to pack the data into existing tailroom.
   ...
   * @methodset Accessors
   *
   * @returns A read-write cursor over the queue.
   */
  template <CursorAccess access>
   ...
   * @methodset Accessors
   *
   * @param n The number of trailing bytes the cursor should cover.
   * @returns A read-write cursor positioned n bytes before the end.
   */
  template <CursorAccess access>
