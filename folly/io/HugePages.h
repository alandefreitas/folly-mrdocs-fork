/*
 * Copyright (c) Meta Platforms, Inc. and affiliates.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

#pragma once

#include <sys/stat.h>
#include <sys/types.h>
#include <cstddef>
#include <string>
#include <utility>
#include <vector>

#include <boost/operators.hpp>

#include <folly/Range.h>
#include <folly/io/FsUtil.h>
#include <folly/portability/Unistd.h>

namespace folly {

/// A supported huge page size and its mount point.
struct HugePageSize : private boost::totally_ordered<HugePageSize> {
  /// Constructs a huge page size of the given number of bytes.
  ///
  /// \param s The huge page size, in bytes.
  explicit HugePageSize(size_t s) : size(s) {}

  /// Returns the mount point joined with the given relative path.
  ///
  /// \param relpath The path relative to the mount point.
  /// \returns The mount point joined with the relative path.
  fs::path filePath(const fs::path& relpath) const {
    return mountPoint / relpath;
  }

  size_t size = 0; ///< The huge page size, in bytes.
  fs::path mountPoint; ///< The hugetlbfs mount point for this size.
  dev_t device = 0; ///< The device id of the mount point.
};

/// Compares two huge page sizes by their size in bytes.
///
/// \param a The left-hand huge page size.
/// \param b The right-hand huge page size.
/// \returns True if `a` is smaller than `b`.
inline bool operator<(const HugePageSize& a, const HugePageSize& b) {
  return a.size < b.size;
}

/// Tests whether two huge page sizes have the same size in bytes.
///
/// \param a The left-hand huge page size.
/// \param b The right-hand huge page size.
/// \returns True if `a` and `b` have the same size.
inline bool operator==(const HugePageSize& a, const HugePageSize& b) {
  return a.size == b.size;
}

/**
 * Vector of (huge_page_size, mount_point), sorted by huge_page_size.
 * mount_point might be empty if no hugetlbfs file system is mounted for
 * that size.
 */
using HugePageSizeVec = std::vector<HugePageSize>;

/**
 * Get list of supported huge page sizes and their mount points, if
 * hugetlbfs file systems are mounted for those sizes.
 *
 * \returns The supported huge page sizes, sorted by size.
 */
const HugePageSizeVec& getHugePageSizes();

/**
 * Return the mount point for the requested huge page size.
 * 0 = use smallest available.
 * Returns nullptr if the requested huge page size is not available.
 *
 * \param size The requested huge page size, or 0 for the smallest available.
 * \returns The matching huge page size, or nullptr if unavailable.
 */
const HugePageSize* getHugePageSize(size_t size = 0);

/**
 * Return the huge page size for a device.
 * returns nullptr if device does not refer to a huge page filesystem.
 *
 * \param device The device to look up.
 * \returns The huge page size for the device, or nullptr if none.
 */
const HugePageSize* getHugePageSizeForDevice(dev_t device);

} // namespace folly
