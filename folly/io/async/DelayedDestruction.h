...
class DelayedDestruction : public DelayedDestructionBase {
 public:
  ...
  class Destructor {
   public:
    /// Requests destruction of the guarded object.
    ///
    /// \param dd The object to destroy.
    void operator()(DelayedDestruction* dd) const { dd->destroy(); }
  };

  /// Returns whether destruction has been requested but deferred.
  ///
  /// \returns `true` if `destroy()` was called while a guard was active.
 ...
 protected:
  ...
  /// Constructs the object with no destruction pending.
...
///
/// \tparam T The DelayedDestruction subtype to create.
/// \tparam A The constructor argument types.
/// \param a The arguments forwarded to the object's constructor.
/// \returns A unique pointer owning the newly created object.
