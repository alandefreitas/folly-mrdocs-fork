...
class EventBaseAtomicNotificationQueue
 ...
 public:
  /// Constructs a queue that dispatches tasks to the given consumer.
  ///
  /// \param consumer The consumer invoked for each task.
  explicit EventBaseAtomicNotificationQueue(Consumer&& consumer);

  /// Constructs a queue with a default-constructed consumer.
  template <
      typename C = Consumer,
  ...
  /// Destroys the queue, detaching it from its EventBase.
  ~EventBaseAtomicNotificationQueue() override;

  /// Returns the maximum number of tasks processed in a single round.
  ///
   ...
   * Can be called from any thread.
   *
   * \param task Arguments forwarded to construct the task.
   */
