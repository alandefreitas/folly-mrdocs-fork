...
class HHWheelTimerBase
 ...
 public:
  ...
  /// Creates a new timer wrapped in a UniquePtr.
  ///
  /// \tparam Args The constructor argument types.
  /// \param args The arguments forwarded to the timer's constructor.
  /// \returns A UniquePtr owning the newly created timer.
  ...
  class Callback
   ...
   public:
    ...
    Callback();
    /// Destroys the callback, canceling it if still scheduled.
    virtual ~Callback();
     ...
     *
     * \returns `true` if this timeout is currently scheduled.
     */
     ...
     *
     * \returns The time remaining until expiration, or 0 if not scheduled.
     */
  ...
  /// Default tick interval, in the timer's duration units.
   ...
   *
   * \param timeoutMananger The manager that drives this timer.
   * \param intervalDuration The duration of a single wheel tick.
   * \param internal Whether the timer is treated as an internal event.
   * \param defaultTimeoutDuration The default timeout, or -1 for none.
   */
   ...
   *
   * \returns The default timeout interval.
   */
   ...
   *
   * \param timeout The new default timeout interval.
   */
   ...
   *
   * \param callback The callback to invoke when the timeout fires.
   * \param timeout The delay before the callback is invoked.
   */
  ...
  ///
  /// \tparam F The callable type to invoke.
  /// \param fn The callable invoked when the timeout fires.
  /// \param timeout The delay before the callable is invoked.
  template <class F>
   ...
   *
   * \returns The number of currently pending timeouts.
   */
  std::size_t count() const { return count_; }

  /// Returns whether the timer can be detached from its event base.
  ///
  /// \returns `true` if no wheel timeout is currently scheduled.
  bool isDetachable() const { return !folly::AsyncTimeout::isScheduled(); }

  /// Attaches the timer to an EventBase.
  using folly::AsyncTimeout::attachEventBase;
  /// Detaches the timer from its EventBase.
  using folly::AsyncTimeout::detachEventBase;
  /// Returns the TimeoutManager driving the timer.
...
/// Millisecond-resolution hashed hierarchical wheel timer.
...
/// Microsecond-resolution hashed hierarchical wheel timer.
