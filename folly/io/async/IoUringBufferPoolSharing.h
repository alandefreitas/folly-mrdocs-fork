...
/// Sets up zero-copy buffer pool sharing for a container of EventBase holders.
///
/// This overload accepts any element type whose `get()` returns a
/// `folly::EventBase*`, extracts the raw pointers, and forwards to the
/// pointer-based overload.
///
/// \tparam T The element type, providing `get()` to a `folly::EventBase*`.
/// \param eventBases The EventBase holders to set up for buffer pool sharing.
/// \param numHwQueues The number of hardware queues allocated for zcrx.
