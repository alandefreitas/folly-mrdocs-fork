...
class NotificationQueue {
 ...
 public:
  ...
  class Consumer : public DelayedDestruction, private EventHandler {
   public:
    /// Owning pointer type honoring delayed destruction.
    using UniquePtr = DelayedDestructionUniquePtr<Consumer>;

    /// Default limits for a consumer.
    enum : uint16_t {
      kDefaultMaxReadAtOnce = 10 ///< Default value for maxReadAtOnce.
    };

    /// Constructs a consumer not attached to any queue.
    Consumer()
        : queue_(nullptr),
          destroyedFlagPtr_(nullptr),
          maxReadAtOnce_(kDefaultMaxReadAtOnce) {}

    /// Creates a consumer in-place, without the need to build a new class.
    ///
    /// \param callback The callable invoked for each message.
    /// \returns An owning pointer to the new consumer.
     ...
     *
     * \param message The message pulled off the queue.
     */
     ...
     * already consuming.
     *
     * \param eventBase The event base to run the consumer on.
     ...
     *
     * \param numConsumed If non-null, set to the number of messages consumed.
     ...
     *
     * \returns The current queue, or nullptr if not consuming.
     */
     ...
     *
     * \param maxAtOnce The maximum messages read per loop iteration.
     */
    void setMaxReadAtOnce(uint32_t maxAtOnce) { maxReadAtOnce_ = maxAtOnce; }
    /// Returns the per-iteration read limit.
    ///
    /// \returns The maximum messages read per loop iteration.
    uint32_t getMaxReadAtOnce() const { return maxReadAtOnce_; }

    /// Returns the event base this consumer runs on.
    ///
    /// \returns The event base pointer.
    EventBase* getEventBase() { return base_; }

    /// Handles readiness of the underlying event handler.
    ///
    /// \param events The bitmask of ready events.
    void handlerReady(uint16_t events) noexcept override;

   protected:
    /// Stops consuming and requests delayed destruction.
  ...
  /// A lightweight consumer that pulls messages without an event loop.
  class SimpleConsumer {
   public:
    /// Registers a simple consumer on the given queue.
    ///
    ...
    /// Unregisters the simple consumer.
    ~SimpleConsumer() { --queue_.numConsumers_; }

    /// Returns the file descriptor used to signal message availability.
    ///
    /// \returns The eventfd, or the pipe read end if eventfd is unavailable.
    int getFd() const {
      return queue_.eventfd_ >= 0 ? queue_.eventfd_ : queue_.pipeFds_[0];
    }

    /// Consumes one available message, invoking the given callable.
    ///
    /// \param f The callable invoked with the dequeued message.
    template <typename F>
    void consume(F&& f);
  ...
  /// The type of file descriptor used to signal message availability.
  enum class FdType {
    PIPE = 1, ///< Signal availability via a pipe.
    EVENTFD, ///< Signal availability via an eventfd.
  };
   ...
   *
   * \param maxSize The advisory maximum queue size, or 0 for unlimited.
   * \param fdType The kind of file descriptor used to signal availability.
   */
  ...
  /// Destroys the queue and releases its file descriptors.
  ~NotificationQueue() {
   ...
   *
   * \param max The advisory maximum queue size.
   */
   ...
   *
   * \param first Iterator to the first message.
   * \param last Iterator past the last message.
   */
   ...
   * unmodified.
   *
   * \param result Set to the message contents when one is available.
   * \returns True if a message was consumed, false otherwise.
   */
