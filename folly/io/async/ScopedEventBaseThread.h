...
template <class Iter>
class Range;
/// A non-owning view over a sequence of characters.
...
class ScopedEventBaseThread : public IOExecutor, public SequencedExecutor {
 public:
   ...
   *
   * @param ebm EventBaseManager to associate the thread's EventBase with.
   */
   ...
   * name.
   *
   * @param eventBaseOptions Options for the thread's EventBase.
   * @param ebm EventBaseManager to associate the thread's EventBase with.
   * @param name Name for the new thread.
   */
  ScopedEventBaseThread(
   ...
   * @methodset Observers
   * @returns The std::thread::id of the thread.
   */
 ...
 protected:
  /// Acquires a keep-alive token on the thread's EventBase.
  ///
  /// \returns True if the keep-alive token was acquired.
  bool keepAliveAcquire() noexcept override {
    return getEventBase()->keepAliveAcquire();
  }

  /// Releases a keep-alive token on the thread's EventBase.
 ...
 private:
  ...
  union {
    mutable EventBase eb_; ///< The EventBase running on the thread.
  };
  std::thread th_;
  folly::Baton<> stop_;
};
