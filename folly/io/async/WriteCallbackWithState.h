...
class WriteCallbackWithState {
 public:
  /// Construct the wrapper around the given write callback.
  ///
  /// \param callback The write callback to wrap.
  explicit WriteCallbackWithState(AsyncWriter::WriteCallback* callback)
      : callback_(callback) {}
  /// Return the wrapped write callback.
  ///
  /// \returns The wrapped write callback.
  AsyncWriter::WriteCallback* getCallback() const { return callback_; }

  /// Notify the callback that a write is starting, once per write.
