...
class AtomicNotificationQueue {
 public:
  ...
  /// Destroys the queue and any remaining tasks.
   ...
   * setMaxReadAtOnce.
   *
   * \returns The maximum number of tasks processed per round.
   */
   ...
   *
   * \param maxAtOnce The maximum number of tasks processed per round.
   */
   ...
   * Can be called from consumer thread only.
   *
   * \returns True if the queue was empty and is now armed.
   */
  bool arm();
   ...
   *
   * \param consumer The callable invoked for each dequeued task.
   ...
   * Can be called from any thread.
   *
   * \param args Arguments forwarded to construct the task.
   * \returns True if the queue was armed, in which case producers are expected
  ...
  /**
   * Same as above but RequestContext is passed from caller.
   *
   * \param rctx The request context to associate with the task.
   * \param args Arguments forwarded to construct the task.
   * \returns True if the queue was armed.
   */
  template <typename... Args>
  ...
  enum class TryPushResult {
    FAILED_LIMIT_REACHED, ///< The queue was full; the task was not added.
    SUCCESS, ///< The task was added; no notification needed.
    SUCCESS_AND_ARMED ///< The task was added and the queue was armed.
  };
  /// Attempts adding a task into the queue, respecting a size limit.
  ///
  /// \param task The task to add.
  /// \param maxSize The maximum queue size to enforce.
...
enum class AtomicNotificationQueueTaskStatus : uint8_t {
  ...
  CONSUMED,
  /// Same as CONSUMED, but drive() stops early even if maxReadAtOnce_ wasn't
  /// reached.
  CONSUMED_STOP,
