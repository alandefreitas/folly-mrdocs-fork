...
class AsyncFdSocket : public AsyncSocket {
 public:
  /// Owning smart pointer type for an `AsyncFdSocket`, using the
  /// `AsyncSocket` `Destructor` deleter.
   ...
   *
   * \param evb The `EventBase` that will drive this socket's I/O.
   */
   ...
   * \param connectTimeout Connection timeout in milliseconds, or 0 for no
   * timeout.
   */
  AsyncFdSocket(
      EventBase* evb,
   ...
   *
   * \param evb The `EventBase` that will drive this socket's I/O.
   * \param fd An already-connected socket file descriptor to adopt.
   * \param peerAddress The address of the connected peer, or `nullptr` if
   * unknown.
   */
  AsyncFdSocket(
      EventBase* evb,
 ...
 protected:
  ...
  /// Constructs from an existing `AsyncSocket` without moving FD state.
  ///
  /// Protected since it's easy to accidentally pass an `AsyncFdSocket` here,
  /// a scenario that's extremely easy to use incorrectly.
  ///
  /// \param tag Disambiguation tag marking this as a non-moving construction.
  /// \param sock The source `AsyncSocket` to construct from.
  AsyncFdSocket(DoesNotMoveFdSocketState tag, AsyncSocket* sock);

 public:
  /// Constructs from an owning `AsyncSocket` pointer without moving FD state.
  ///
  /// \param tag Disambiguation tag marking this as a non-moving construction.
  /// \param sock The source `AsyncSocket` to take ownership of.
   ...
   *
   * \param callback Write callback notified on completion or error.
   * \param buf The `IOBuf` chain to write.
   * \param socketFds The FDs to send as ancillary data with this write.
   * \param flags Write flags to apply to this write.
   */
   ...
   *
   * \returns The next group of received FDs, or an empty `SocketFds` if the
   ...
   *
   * \param fds The FDs to be sent; their socket sequence number is set in
   * place.
   * \returns The assigned sequence number. Non-negative, but may be
   * `SocketFds::kNoSeqNum` on DFATAL errors.
   */
  SocketFds::SeqNum injectSocketSeqNumIntoFdsToSend(SocketFds* fds);

  /// Forbidden override: the "sendmsg params" callback is built-in and not
  /// customizable, so calling this logs a DFATAL error.
  ///
  /// \param cb Ignored; the callback cannot be customized on this socket.
  void setSendMsgParamCB(SendMsgParamsCallback* cb) override {
    LOG(DFATAL) << "AsyncFdSocket::setSendMsgParamCB is forbidden";
  }
  /// Forbidden override: the "read ancillary data" callback is built-in and
  /// not customizable, so calling this logs a DFATAL error.
  ///
  /// \param cb Ignored; the callback cannot be customized on this socket.
  void setReadAncillaryDataCB(ReadAncillaryDataCallback* cb) override {
...
// This class has no ancillary data callbacks on Windows, they wouldn't compile
   ...
   *
   * \param other The socket to swap "read FDs" state with.
   */
  void swapFdReadStateWith(AsyncFdSocket* other);

 protected:
  /// Releases an `IOBuf` after a write, discarding any FDs associated with it.
  ///
  /// \param buf The `IOBuf` being released.
  /// \param callback Callback invoked when the buffer is released.
  void releaseIOBuf(
      std::unique_ptr<folly::IOBuf> buf, ReleaseIOBufCallback* callback) override;
