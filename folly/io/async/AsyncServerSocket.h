...
class AsyncServerSocket : public DelayedDestruction, public AsyncSocketBase {
 public:
  ...
  /// Deleted move constructor; AsyncServerSocket is not movable.
  AsyncServerSocket(AsyncServerSocket&& other) = delete;
  ...
  class ConnectionEventCallback {
   public:
    /// Destroys the connection event callback.
     ...
     * \param addr The address of the accepted peer.
     */
     ...
     * a connection.
     *
     * \param err The errno describing the accept failure.
     */
     ...
     *
     * \param socket The dropped client socket.
     * \param addr The address of the dropped peer.
     * \param errorMsg A message describing why the connection was dropped.
     */
     ...
     *
     * \param socket The enqueued client socket.
     * \param addr The address of the enqueued peer.
     */
     ...
     *
     * \param socket The dequeued client socket.
     * \param addr The address of the dequeued peer.
     */
  ...
  class AcceptCallback {
   public:
    /// Auxiliary information about a newly accepted socket.
    struct AcceptInfo {
      /// Time the socket was recorded before being enqueued.
      std::chrono::steady_clock::time_point timeBeforeEnqueue;
    };

    /// Destroys the accept callback.
     ...
     *
     * @param ew  An exception representing the error.
     */
    ...
    /// Called if an error occurs while accepting a connection.
   ...
   *
   * \returns A shared pointer to the new server socket.
   */
  ...
  /// \param wNewSS The shutdown socket set to register with.
   ...
   *
   * \returns The primary event base driving this socket.
   */
   ...
   *
   * \returns The list of network sockets this server is listening on.
   */
   ...
   *
   * \returns The first network socket, or a default one if none exist.
   */
   ...
   *
   * \param func The function that maps accepted sockets to callbacks.
   */
   ...
   *
   * \param enable Whether to enable zerocopy support.
   * \returns True if zerocopy was enabled successfully.
   */
  ...
  /// Bind to the specified address with socket options.
  /// \param address The local address to bind to.
  /// \param socketOptions PRE_BIND socket options applied before binding.
  virtual void bind(
   ...
   *
   * \param address The local address to bind to.
   * \param ifName The network interface to bind to.
   */
  virtual void bind(const SocketAddress& address, const std::string& ifName);
  /// Bind to the specified address and interface with socket options.
  /// \param address The local address to bind to.
  /// \param ifName The network interface to bind to.
  /// \param socketOptions PRE_BIND socket options applied before binding.
  virtual void bind(
   ...
   * Throws AsyncSocketException on error.
   *
   * \param ipAddresses The local addresses to bind to.
   * \param port The port to bind on.
   */
  virtual void bind(const std::vector<IPAddress>& ipAddresses, uint16_t port);
  /// Bind the given addresses to a port with socket options.
  /// \param ipAddresses The local addresses to bind to.
  /// \param port The port to bind on.
  /// \param socketOptions PRE_BIND socket options applied before binding.
  virtual void bind(
   ...
   * Throws AsyncSocketException on error.
   */
  /// An IP address paired with the network interface name to bind on.
  using IPAddressIfNamePair = std::pair<IPAddress, std::string>;
  /// Bind the given address/interface pairs to a port.
  /// \param addresses The address and interface pairs to bind to.
  /// \param port The port to bind on.
  virtual void bind(
      const std::vector<IPAddressIfNamePair>& addresses, uint16_t port);
  /// Bind the given address/interface pairs to a port with socket options.
  /// \param addresses The address and interface pairs to bind to.
  /// \param port The port to bind on.
  /// \param socketOptions PRE_BIND socket options applied before binding.
  virtual void bind(
  ...
  /// Bind to the specified port with socket options.
  /// \param port The port to bind on.
  /// \param socketOptions PRE_BIND socket options applied before binding.
   ...
   * Throws AsyncSocketException on error.
   *
   * \param addressReturn Receives the local address the socket is bound to.
   */
   ...
   * Throws AsyncSocketException on error.
   *
   * \returns All local addresses the socket is bound to.
   */
   ...
   * Throws AsyncSocketException on error.
   *
   * \param backlog The kernel accept-queue backlog size.
   */
   ...
   *
   * \param shutdownFlags Flags passed to shutdown(2), or -1 to close now.
   * \returns The result of shutdown on the last socket.
   */
   ...
   *
   * \returns The maximum connections accepted per event loop iteration.
   */
   ...
   *
   * \param numConns The maximum connections to accept per event loop iteration.
   */
   ...
   *
   * \returns The queue timeout observer.
   */
   ...
   *
   * \param duration The queue timeout observer to install.
   */
  void setQueueTimeout(
      folly::observer::Observer<std::chrono::nanoseconds> duration) {
    queueTimeout_ = duration;
  }
  /// Set a static queue timeout duration.
  /// \param duration The queue timeout duration to install.
   ...
   * can hold.
   *
   * \returns The maximum number of unprocessed messages in the queue.
   */
   ...
   *
   * \param num The maximum number of unprocessed messages to allow.
   */
   ...
   *
   * \returns The accept-rate adjustment speed.
   */
   ...
   *
   * \param speed The accept-rate adjustment speed.
   */
   ...
   *
   * \param enable Whether to enable TOS reflection.
   */
  void setTosReflect(bool enable);
  /**
   * Get TOS reflection for server socket
   *
   * \returns True if TOS reflection is enabled.
   */
   ...
   *
   * \param tos The default type-of-service value for the listener socket.
   */
   ...
   *
   * \returns The default type-of-service value for the listener socket.
   */
   ...
   * socket's primary EventBase.
   *
   * \returns The current number of unprocessed messages in the queue.
   */
   ...
   *
   * \param enabled Whether to enable SO_KEEPALIVE.
   */
   ...
   *
   * \param enabled Whether to enable SO_REUSEPORT.
   */
   ...
   *
   * \param enable Whether to enable SO_REUSEADDR.
   */
   ...
   *
   * \returns True if SO_REUSEPORT is enabled.
   */
   ...
   *
   * \param enable Whether to enable IP_FREEBIND.
   */
   ...
   *
   * \returns True if IP_FREEBIND is enabled.
   */
   ...
   *
   * \param closeOnExec Whether to enable FD_CLOEXEC.
   */
   ...
   *
   * \returns True if FD_CLOEXEC is enabled.
   */
   ...
   *
   * \param enabled Whether to enable TCP Fast Open.
   * \param maxTFOQueueSize The maximum TFO pending-connection queue size.
   */
   ...
   *
   * \returns True if the socket is accepting new connections.
   */
   ...
   *
   * \param connectionEventCallback The connection event callback to install.
   */
   ...
   *
   * \returns The installed connection event callback, or null.
   */
   ...
   *
   * \returns The local callback index, or -1 if none.
   */
