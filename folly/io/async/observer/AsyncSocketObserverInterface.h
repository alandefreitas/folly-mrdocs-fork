...
/// Exception describing an asynchronous socket error.
class AsyncSocketException;
/// Event loop that drives asynchronous I/O and timeouts.
class EventBase;
/// Abstract base for asynchronous transports.
...
class AsyncSocketObserverInterface {
 public:
  ...
  struct PrewriteState {
    size_t startOffset{0}; ///< Start offset in the raw byte stream.
    size_t endOffset{0}; ///< End offset in the raw byte stream.

    /// Write flags already set on the pending write.
    WriteFlags writeFlags{WriteFlags::NONE};

    /// Timestamp recorded at the AsyncSocket layer for event sequencing.
  ...
  struct PrewriteRequest {
    /// Offset to split the write at; another request may split earlier.
    folly::Optional<size_t> maybeOffsetToSplitWrite;

    /// Write flags to add if the write is split at the requested offset.
    WriteFlags writeFlagsToAddAtOffset{WriteFlags::NONE};

    /// Write flags to add regardless of where the write happens.
  ...
  class PrewriteRequestContainer {
   public:
    /// Constructs the container for the given prewrite state.
    ///
    /// \param prewriteState The state describing the pending write.
     ...
     *
     * \param request The request to merge into the container.
     */
    void addRequest(
     ...
     *
     * \returns The merged request built from all added requests.
     */
   ...
   */
  struct ByteEvent {
    /// Kind of byte event; values start from 0 to enable indexing in arrays.
    enum Type : uint8_t {
      WRITE = 0, ///< The byte was written to the socket.
      SCHED = 1, ///< The kernel scheduled the byte for transmission.
      TX = 2, ///< The byte was transmitted to the network.
      ACK = 3, ///< The byte was acknowledged by the peer.
    };
    /// Kind of this byte event.
    Type type;

    /// Offset of the corresponding byte in the raw byte stream.
    size_t offset{0};

    /// Socket timestamp, as recorded by the AsyncSocket implementation.
    std::chrono::steady_clock::time_point ts = {
        std::chrono::steady_clock::now()};

    /// Kernel software timestamp for non-WRITE events (CLOCK_REALTIME on Linux).
    ...
    /// Hardware timestamp for non-WRITE events.
    // see https://www.kernel.org/doc/Documentation/networking/timestamping.txt
    ...
    /// For WRITE events, the number of raw bytes we tried to write.
    folly::Optional<size_t> maybeRawBytesTriedToWrite;

    /// For WRITE events, the additional WriteFlags passed.
    folly::Optional<WriteFlags> maybeWriteFlags;

    /**
     * For WRITE events, returns if SCHED timestamp requested.
     *
     * \returns `true` if a SCHED timestamp was requested on the write.
     */
     ...
     *
     * \returns `true` if a TX timestamp was requested on the write.
     */
     ...
     *
     * \returns `true` if an ACK timestamp was requested on the write.
     */
  ...
  virtual void connectError(
      AsyncSocket* socket, const AsyncSocketException& ex) noexcept {}
   ...
   */
  virtual void move(AsyncSocket* oldSocket, AsyncSocket* newSocket) noexcept {}
};
