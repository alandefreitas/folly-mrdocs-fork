...
class OpenSSLUtils {
 public:
  ...
  /**
   * Get the TLS Session Master Key from a wrapped SSLSession.
   *
   * @param session ssl session wrapper
   * @param keyOut destination for the master key, the buffer must be at least
   * 48 bytes
   * @return true if the master key is available (>= TLS1) and the output buffer
   * large enough
  ...
  /// Returns the 'initial_ctx' SSL_CTX associated with \p ssl.
  /// \param ssl SSL pointer to read the initial context from.
  /// \returns The initial SSL_CTX pointer.
  static SSL_CTX* getSSLInitialCtx(SSL* ssl);

  /**
   * Get the common name out of a cert.  Return empty if x509 is null.
   *
   * @param x509 Certificate to read the common name from, may be null.
   * @return The certificate's common name, or empty if \p x509 is null.
   */
   ...
   *
   * @return A new socket-backed BIO_METHOD.
   */
  static BioMethodUniquePtr newSocketBioMethod();
  /// Installs a custom read callback on \p bioMeth.
  /// \param bioMeth BIO method to modify.
  /// \param meth Read callback to install.
  /// \returns True if the callback was installed successfully.
  static bool setCustomBioReadMethod(
      BIO_METHOD* bioMeth, int (*meth)(BIO*, char*, int));
  /// Installs a custom write callback on \p bioMeth.
  /// \param bioMeth BIO method to modify.
  /// \param meth Write callback to install.
  /// \returns True if the callback was installed successfully.
  static bool setCustomBioWriteMethod(
      BIO_METHOD* bioMeth, int (*meth)(BIO*, const char*, int));
  /// Returns whether a failed BIO write should be retried, given its result.
  /// \param ret Result of the BIO write operation.
  /// \returns True if the write should be retried.
  static int getBioShouldRetryWrite(int ret);
  /// Sets the application data pointer on BIO \p b.
  /// \param b BIO to modify.
  /// \param ptr Application data pointer to store.
  static void setBioAppData(BIO* b, void* ptr);
  /// Returns the application data pointer stored on BIO \p b.
  /// \param b BIO to read from.
  /// \returns The stored application data pointer.
  static void* getBioAppData(BIO* b);
  /// Returns the socket descriptor backing BIO \p b.
  /// \param b BIO to read from.
  /// \returns The socket descriptor backing the BIO.
  static NetworkSocket getBioFd(BIO* b);
  /// Sets the socket descriptor backing BIO \p b.
  /// \param b BIO to modify.
  /// \param fd Socket descriptor to associate with the BIO.
  /// \param flags BIO close flags controlling descriptor ownership.
  static void setBioFd(BIO* b, NetworkSocket fd, int flags);
  /// Encodes protocol names into the wire ALPN wire format.
  /// \param supported_protocols Protocol names to encode, in preference order.
  /// \returns The ALPN wire-format encoding of the protocol names.
