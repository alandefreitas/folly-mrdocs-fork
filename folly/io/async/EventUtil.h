...
/// Returns a reference wrapper to the libevent event's `arg` field.
/// \implementationdefined
FOLLY_LIBEVENT_DEF_ACCESSORS(arg)
/// Returns a reference wrapper to the libevent event's `flags` field.
/// \implementationdefined
FOLLY_LIBEVENT_DEF_ACCESSORS(flags)
// evcb_callback is inside a union{...} evcb_cb_union
/// Returns a reference wrapper to the libevent event's `callback` field.
/// \implementationdefined
...
class EventUtil {
 public:
  /// Returns whether the given libevent event is currently registered.
  ///
  /// \param ev The libevent event to inspect.
  /// \returns true if the event is currently registered, false otherwise.
