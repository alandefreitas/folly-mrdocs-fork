...
class AsyncDetachFdCallback {
 public:
  ...
  /// Called when the file descriptor has been detached.
  ///
  /// \param ns The detached socket.
  /// \param unread Any buffered data that had been read but not consumed.
  virtual void fdDetached(
      NetworkSocket ns, std::unique_ptr<IOBuf> unread) noexcept = 0;
  /// Called when detaching the file descriptor fails.
  ///
