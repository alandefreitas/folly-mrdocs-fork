...
namespace std::pmr {
/// Abstract interface for polymorphic memory resources.
...
class IOBuf {
 public:
  ...
  /// Tag type selecting the create constructor.
  enum CreateOp {
    CREATE ///< Create a new buffer.
  };
  /// Tag type selecting the wrap-buffer constructor.
  enum WrapBufferOp {
    WRAP_BUFFER ///< Wrap an existing buffer without owning it.
  };
  /// Tag type selecting the take-ownership constructor.
  enum TakeOwnershipOp {
    TAKE_OWNERSHIP ///< Take ownership of an existing buffer.
  };
  /// Tag type selecting the copy-buffer constructor.
  enum CopyBufferOp {
    COPY_BUFFER ///< Copy the contents of an existing buffer.
  };
  /// Tag type indicating a sized-free deleter.
  enum SizedFree {
    SIZED_FREE ///< Free the buffer with a size hint.
  };

  /// Controls whether the IOBuf and its buffer share one allocation.
  enum class CombinedOption {
    DEFAULT, ///< Use the default allocation strategy.
    COMBINED, ///< Allocate the IOBuf and buffer together.
    SEPARATE ///< Allocate the IOBuf and buffer separately.
  };

  /// The element type produced when iterating a chain.
  using value_type = ByteRange;
  /// Iterator over the buffers in a chain.
  using iterator = Iterator;
  /// Const iterator over the buffers in a chain.
  using const_iterator = Iterator;

  /// Custom free function used to release an externally-owned buffer.
  using FreeFunction = void (*)(void* buf, void* userData);

  /**
   * Create an IOBuf with the requested capacity.
   *
   * @param op  Tag selecting this constructor.
   ...
   */
  IOBuf(CreateOp op, std::size_t capacity);
   ...
   *
   * @returns  A unique_ptr to the head of the newly-constructed IOBuf chain
   * @methodset Makers
   */
   ...
   *                     throws an exception
   *
   * Some overloads also accept an `offset` (the position within the buffer at
   * which the data begins; defaults to 0), a `length` (the amount of data
   * already in buf; defaults to capacity), and a `SizedFree` tag (which uses
   ...
   * @copydoc IOBuf(TakeOwnershipOp, void*, std::size_t, FreeFunction, void*,
   *          bool)
   * @param length  The amount of data already in buf.
   * @returns  A unique_ptr to a newly-constructed IOBuf
   * @methodset Makers
   */
  static std::unique_ptr<IOBuf> takeOwnership(
   ...
   * @copydoc IOBuf(TakeOwnershipOp, void*, std::size_t, FreeFunction, void*,
   *          bool)
   * @param sizedFree  Tag selecting the sized-free deleter overload.
   * @param offset  The position within the buffer at which the data begins.
   * @param length  The amount of data already in buf.
   * @returns  A unique_ptr to a newly-constructed IOBuf
   * @methodset Makers
   ...
   *
   * @param op  Tag selecting this constructor.
   * @param br  The buffer to wrap, given as a ByteRange.
   *
   * @post  data() points to br.data()
   * @post  length() == br.size()
   */
  ...
  /**
   * Create a stack-allocated IOBuf that wraps a buffer, without owning it.
   *
   * @param br  The buffer to wrap, given as a ByteRange.
   * @returns  A stack-allocated IOBuf
   * @methodset Makers
   ...
   *
   * @param op  Tag selecting this constructor.
   * @param br  The buffer to copy data from, given as a ByteRange.
   ...
   *
   * @post  data() points to a new buffer whose content is the same as br
   * @post  length() == br.size()
   ...
   *
   * @param buf  The string data to copy.
   * @param headroom  The amount of headroom to add to the destination buffer
   * @param minTailroom  The amount of tailroom to add to the destination buffer
   * @returns  A unique_ptr to a newly-constructed IOBuf
   * @methodset Makers
  ...
  /**
   * Create an IOBuf and copy string data into the buffer.
   *
   * @param op  Tag selecting this constructor.
   * @param buf  The string data to copy.
   * @param headroom  The amount of headroom to add to the destination buffer
  ...
  /**
   * Create an IOBuf and copy string data into the buffer, or null if empty.
   *
   ...
   * @param headroom  The amount of headroom to add to the destination buffer
   * @param minTailroom  The amount of tailroom to add to the destination buffer
   * @returns  A unique_ptr to a newly-constructed IOBuf, or null if buf is empty
   * @methodset Makers
   */
   ...
   *
   * @returns  A writable pointer to the start of the data.
   * @methodset Access
   */
   ...
   * actually safe to write to the buffer.
   *
   * @returns  A writable pointer to the end of the data.
   * @methodset Access
   */
   ...
   *
   * @returns  The length of the data in this IOBuf, in bytes.
   ...
   * actually safe to write to the buffer.
   *
   * @returns  A writable pointer to the start of the buffer.
   * @methodset Access
   */
   ...
   *
   * @returns  A pointer to the next IOBuf in the chain.
   * @methodset Chaining
   */
   ...
   *
   * @returns  A pointer to the previous IOBuf in the chain.
   * @methodset Chaining
   */
   ...
   * or not.
   *
   * @returns  The number of IOBufs in this chain.
   * @methodset Chaining
   */
   ...
   * Beware that this method has to walk the entire chain.
   *
   * @returns  The total length of the data in the chain, in bytes.
   * @methodset Chaining
   */
   ...
   * Beware that this method has to walk the entire chain.
   *
   * @returns  The total capacity of all buffers in the chain, in bytes.
   * @methodset Chaining
   */
   ...
   * (A, B, C, D, E, F).
   *
   * @param iobuf  The chain to append to the end of this chain.
   * @methodset Chaining
   */
   ...
   *
   * @param iobuf  The chain to insert after this chain element.
   * @methodset Chaining
   */
   ...
   *
   * @returns  A unique_ptr owning this IOBuf, now removed from its chain.
   * @methodset Chaining
   */
   ...
   *
   * @returns  A unique_ptr owning the head of the removed subchain.
   * @methodset Chaining
   */
   ...
   *
   * @returns  true if any IOBuf in the chain is shared.
   ...
   *
   * @returns  The approximate reference count of this IOBuf's buffer.
   * @methodset Buffer Management
   */
   ...
   * This only checks the current IOBuf, and not other IOBufs in the chain.
   *
   * @returns  true if this IOBuf's buffer is shared.
   * @methodset Buffer Management
   */
   ...
   *
   * @param contiguousLength  The minimum number of contiguous bytes to ensure.
   * @methodset Chaining
   */
   ...
   * be shared.)
   *
   * @returns  A unique_ptr to the head of the cloned chain.
   * @methodset Makers
   */
   ...
   *
   * @returns  A cloned IOBuf, returned by value.
   */
   ...
   *
   * @returns  A unique_ptr to the cloned IOBuf.
   * @methodset Makers
   */
   ...
   *
   * @returns  A new IOBuf owning the split-off tail, or nullptr on failure.
   */
   ...
   *
   * @tparam Container  The type of container to append into.
   * @param container  The container to append the chain data to.
   ...
   *
   * @returns  A std::string containing the coalesced chain data.
   */
   ...
   *
   * @param str  The string whose contents the IOBuf will take ownership of.
   * @returns  A unique_ptr to a newly-constructed IOBuf.
   */
  static std::unique_ptr<IOBuf> fromString(std::unique_ptr<std::string> str);
  /**
   * Create an IOBuf from a std::string.
   *
   * @param s  The string to copy into the IOBuf.
   * @returns  A unique_ptr to a newly-constructed IOBuf.
   */
   ...
   * chain.
   *
   * @returns  An fbvector of iovec entries describing the chain data.
   * @methodset IOV
   */
  ...
  /// Result of filling an iovec array with IOBuf data.
   ...
   *
   * @returns  A FillIovResult with the number and total length of filled iovecs.
   * @methodset IOV
   */
  ...
  /**
   * Take ownership of a buffer, allocating the IOBuf node with a memory
   * resource.
   *
   * @param mr  The memory resource used to allocate the IOBuf node.
   * @param buf  The pointer to the buffer.
   * @param capacity  The size of the buffer.
   * @param offset  The position within the buffer at which the data begins.
   * @param length  The amount of data already in buf.
  ...
  /**
   * Copy an IOBuf chain, allocating the new nodes with a memory resource.
   *
   * @param mr  The memory resource used to allocate the cloned nodes.
   * @returns  A unique_ptr to the head of the cloned chain.
   */
  ...
  /**
   * Copy an individual IOBuf, allocating the new node with a memory resource.
   *
   * @param mr  The memory resource used to allocate the cloned node.
   * @returns  A unique_ptr to the cloned IOBuf.
   */
  ...
  /**
   * Overridden placement operator new.
   *
   * @param size  The number of bytes to allocate.
   * @param ptr  The address at which to construct the object.
   * @returns  A pointer to the placement storage.
   * @methodset Memory
   */
   ...
   *
   * @param ptr  The storage to free.
   * @methodset Memory
   */
  void operator delete(void* ptr);

  /**
   * Overridden placement operator delete.
   *
   * @param ptr  The storage to free.
   * @param placement  The placement address originally passed to operator new.
   * @methodset Memory
   */
   ...
   * transfer memory.
   *
   * @returns  An fbstring holding the buffer's data.
   * @methodset Conversions
   */
   ...
   *
   * @returns  An iterator to the first IOBuf in the chain.
   * @methodset Iterators
   */
  Iterator cbegin() const;

  /**
   * Get an iterator past the last IOBuf in this chain.
   *
   * @returns  An iterator past the last IOBuf in the chain.
   * @methodset Iterators
   */
  Iterator cend() const;

  /**
   * Get an iterator to the first IOBuf in this chain.
   *
   * @returns  An iterator to the first IOBuf in the chain.
   * @methodset Iterators
   */
   ...
   * node.)
   *
   * @param other  The IOBuf to move from.
   */
   ...
   *
   * @param other  The IOBuf to move from.
   * @returns  A reference to this IOBuf.
   */
   ...
   * Copy constructor.
   *
   * @param other  The IOBuf to copy from.
   * @see cloneAsValue()
   */
 ...
 */
struct IOBufHash {
  /// Compute the hash of an IOBuf chain.
  ///
  /// \param buf The IOBuf chain to hash.
  /// \returns The hash of the chain data.
  size_t operator()(const IOBuf& buf) const noexcept;
  /// Compute the hash of an IOBuf chain.
  ///
  /// \param buf The IOBuf chain to hash.
  /// \returns The hash of the chain data, or 0 if buf is null.
 ...
 */
struct IOBufCompare {
  /// Compare two IOBuf chains.
  ///
  /// \param a The first IOBuf chain.
  /// \param b The second IOBuf chain.
  ...
  /// \param a The first IOBuf chain.
  /// \param b The second IOBuf chain.
  /// \returns The ordering of a relative to b, treating null as less than
  ///          non-null.
  ordering operator()(const IOBuf* a, const IOBuf* b) const {
    // clang-format off
    return
...
/// Forward iterator over the buffers in an IOBuf chain.
class IOBuf::Iterator
 ...
 public:
  ...
  /// Construct an iterator over the range [pos, end).
  ///
  /// \param pos The first IOBuf in the range.
  /// \param end The IOBuf marking the end of the range.
  ...
  /// Access the ByteRange at the current position.
  ///
  /// \returns The ByteRange of the current IOBuf.
  const ByteRange& dereference() const { return val_; }

  /// Test whether two iterators refer to the same position.
  ///
  ...
  /// Advance to the next IOBuf in the chain.
  void increment() {
    pos_ = pos_->next();
    adjustForEnd();
  }
