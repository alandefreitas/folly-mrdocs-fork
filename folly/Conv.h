...
enum class ConversionCode : unsigned char {
  ...
  EMPTY_INPUT_STRING, ///< The input string was empty.
  NO_DIGITS, ///< The input contained no digits.
  BOOL_OVERFLOW, ///< The value is out of range for a boolean.
  BOOL_INVALID_VALUE, ///< The input is not a valid boolean spelling.
  NON_DIGIT_CHAR, ///< An unexpected non-digit character was found.
  INVALID_LEADING_CHAR, ///< The input began with an invalid character.
  POSITIVE_OVERFLOW, ///< The value exceeds the maximum of the target type.
  NEGATIVE_OVERFLOW, ///< The value is below the minimum of the target type.
  STRING_TO_FLOAT_ERROR, ///< The input could not be parsed as a floating-point value.
  NON_WHITESPACE_AFTER_END, ///< Non-whitespace characters followed the parsed value.
  ARITH_POSITIVE_OVERFLOW, ///< An arithmetic conversion overflowed positively.
  ARITH_NEGATIVE_OVERFLOW, ///< An arithmetic conversion overflowed negatively.
  ARITH_LOSS_OF_PRECISION, ///< An arithmetic conversion would lose precision.
  SPLIT_ERROR, ///< A delimited split failed.
  CUSTOM, ///< A custom, user-supplied error code.
  NUM_ERROR_CODES, ///< Sentinel count of error codes; has to be the last entry.
};

/// Base class for exceptions thrown by folly conversion routines.
struct FOLLY_EXPORT ConversionErrorBase : std::range_error {
  /// Inherit the constructors of std::range_error.
...
class FOLLY_EXPORT ConversionError : public ConversionErrorBase {
 public:
  /// Construct from an error message string and a conversion code.
  ///
  /// \param str The human-readable error message.
  /// \param code The conversion error code describing the failure.
  ...
  /// Return the conversion error code describing the failure.
  ///
  /// \returns The stored ConversionCode.
 ...
 * }
 *
 * \param code The custom error code to translate.
 * \param input The input string that failed to convert.
 ...
 * \returns An Expected holding the converted value, or an error code.
 */
 ...
 *
 * \param value The arithmetic value to convert.
 * \returns An Expected holding the resulting boolean.
 */
template <class Tgt, class Src>
typename std::enable_if<
...
/// Unchecked conversion from an arithmetic value to boolean.
///
/// \param value The arithmetic value to convert.
 ...
 * representation.
 *
 * \param value The value whose string length is estimated.
 * \returns The estimated number of characters.
 */
template <class T>
...
///
/// \param value The string literal whose length is estimated.
/// \returns The estimated number of characters.
template <size_t N>
constexpr size_t estimateSpaceNeeded(const char (&value)[N]) {
  return N;
}
...
///
/// \param value The pointer whose pointee string length is estimated.
/// \returns The estimated number of characters.
template <class Src>
typename std::enable_if<
 ...
 *
 * \param value The StringPiece to append.
 * \param result The string to append to.
 */
template <class Tgt>
 ...
 *
 * \param value The fbstring to append.
 * \param result The string to append to.
 */
template <class Tgt>
typename std::enable_if<IsSomeString<Tgt>::value>::type toAppend(
 ...
 *
 * \param value The 128-bit signed integer to append.
 * \param result The string to append to.
 */
...
/// Append a 128-bit unsigned integer to a string.
///
/// \param value The 128-bit unsigned integer to append.
/// \param result The string to append to.
template <class Tgt>
 ...
 *
 * \param value The small integer to append.
 * \param result The string to append to.
 */
template <class Tgt, class Src>
typename std::enable_if<
 ...
 *
 * \param value The enumerated value to append.
 * \param result The string to append to.
 */
template <class Tgt, class Src>
typename std::enable_if<
 ...
 *
 * \param value The floating-point value whose string length is estimated.
 * \returns The estimated number of characters.
 */
template <class Src>
...
constexpr typename std::enable_if<
    ...
    size_t>::type
estimateSpaceNeeded(const Src& value) {
 ...
 * }
 *
 * \param vs The values to append, with the target string pointer last.
 */
template <
    class... Ts,
    std::enable_if_t<
...
/// Single-argument base case of toAppendFit: do nothing.
///
/// \param vs Unused single argument.
template <class Ts>
void toAppendFit(const Ts& vs) {}
 ...
 *
 * \param result The target string pointer.
 */
template <class Tgt>
typename std::enable_if<IsSomeString<Tgt>::value>::type toAppend(
    Tgt* result) {}
 ...
 * Variadic base case: do nothing.
 *
 * \param delim The delimiter placed between appended values.
 * \param result The target string pointer.
 */
 ...
 *
 * \param delim The delimiter (unused for a single element).
...
/// Base case of toAppendDelimFit: do nothing.
///
/// \param de The delimiter (unused).
/// \param ts The single argument (unused).
template <class De, class Ts>
void toAppendDelimFit(const De& de, const Ts& ts) {}

/**
 * to<SomeString>(v1, v2, ...) uses toAppend() (see below) as back-end
 * for all types.
 *
 * \param vs The values to convert and concatenate.
 ...
 *
 * \param delim The delimiter (unused in the identity case).
 * \param value The value to convert.
 * \returns The resulting string.
 */
 ...
 *
 * \param delim The delimiter placed between values.
 * \param vs The values to convert and concatenate.
 * \returns The resulting string.
 */
template <
 ...
 *
 * \param b Pointer to the first character of the input.
 * \param e Pointer past the last character of the input.
 * \returns An Expected holding the parsed value, or an error code.
 */
...
/// Convert a character range to an integral value, throwing on failure.
///
/// \param b Pointer to the first character of the input.
/// \param e Pointer past the last character of the input.
 ...
 * \returns An Expected holding the unconsumed input, or an error code.
 */
template <typename Tgt>
...
/// Checked conversion between arithmetic types, throwing on failure.
///
/// \param value The arithmetic value to convert.
/// \returns The converted value.
template <typename Tgt, typename Src>
...
/// Parse a StringPiece into a StringPiece (identity, consuming all input).
///
/// \param in The input string to parse.
/// \param out Set to the input string.
/// \returns An Expected holding the empty unconsumed tail.
[[nodiscard]]
...
/// Parse a StringPiece into a std::string, consuming all input.
///
/// \param in The input string to parse.
/// \param out Set to the parsed string.
/// \returns An Expected holding the empty unconsumed tail.
[[nodiscard]]
inline Expected<StringPiece, ConversionCode> parseTo(
...
/// Parse a StringPiece into a std::string_view, consuming all input.
///
/// \param in The input string to parse.
/// \param out Set to a view over the input string.
/// \returns An Expected holding the empty unconsumed tail.
[[nodiscard]]
inline Expected<StringPiece, ConversionCode> parseTo(
...
/// Parse a StringPiece into an fbstring, consuming all input.
///
/// \param in The input string to parse.
/// \param out Set to the parsed string.
/// \returns An Expected holding the empty unconsumed tail.
...
/// Parse a StringPiece into any folly string type, consuming all input.
///
/// \param in The input string to parse.
/// \param out Set to the parsed string.
/// \returns An Expected holding the empty unconsumed tail.
 ...
 * \returns An Expected holding the converted value, or an error.
 */
...
/// Convert a folly string to a target type, throwing on failure.
///
/// \param src The input string to convert.
/// \returns The converted value.
template <class Tgt, class Src>
...
/// Convert a StringPiece to a target type, throwing on failure.
///
/// \param src The input string to convert.
/// \returns The converted value.
template <class Tgt>
 ...
 *
 * \param src Pointer to the input string; updated to the unconsumed tail.
 * \returns An Expected holding the converted value, or an error.
 */
template <class Tgt>
...
/// Convert a string by pointer to a target type, updating the pointer.
///
/// \param src Pointer to the input string; updated to the unconsumed tail.
/// \returns The converted value.
template <class Tgt>
...
/// \returns An Expected holding the converted enum value, or an error code.
template <class Tgt, class Src>
typename std::enable_if<
...
/// Convert an enum to another type, throwing on failure.
///
/// \param value The enum value to convert.
/// \returns The converted value.
template <class Tgt, class Src>
...
/// Convert any type to an enum, throwing on failure.
///
/// \param value The value to convert to the enum.
