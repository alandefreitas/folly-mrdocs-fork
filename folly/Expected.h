...
class [[nodiscard]] Unexpected final {
 ...
 public:
  /// Default-constructs an Unexpected with a value-initialized error.
  Unexpected() = default;
  /// Copy constructor.
  /// \param other The Unexpected to copy from.
  Unexpected(const Unexpected& other) = default;
  /// Move constructor.
  /// \param other The Unexpected to move from.
  ...
  /// Constructs an Unexpected holding a copy of an error.
  /// \param err The error value to store.
  [[FOLLY_ATTR_GNU_COLD]] constexpr /* implicit */ Unexpected(const Error& err)
      : error_(err) {}
  /// Constructs an Unexpected by moving an error into it.
  /// \param err The error value to store.
  [[FOLLY_ATTR_GNU_COLD]] constexpr /* implicit */ Unexpected(Error&& err)
      : error_(std::move(err)) {}

  /// Converting constructor from a compatible Unexpected.
  /// \param that The Unexpected whose error is converted and stored.
  ...
  /// Converting assignment from a compatible Unexpected.
  /// \param that The Unexpected whose error is assigned.
  /// \returns A reference to this object.
  template <class Other FOLLY_REQUIRES_TRAILING(
...
/// Compares two Unexpected values for equality.
/// \param lhs The left-hand Unexpected.
/// \param rhs The right-hand Unexpected.
/// \returns `true` if the stored errors compare equal.
template <
...
/// \param lhs The left-hand Unexpected.
/// \param rhs The right-hand Unexpected.
/// \returns `true` if the stored errors are not equal.
template <
    class Error FOLLY_REQUIRES_TRAILING(IsEqualityComparable<Error>::value)>
inline bool operator!=(
    const Unexpected<Error>& lhs, const Unexpected<Error>& rhs) {
 ...
 * \returns An Unexpected holding a decayed copy of `err`.
 */
template <class Error>
...
class FOLLY_EXPORT BadExpectedAccess : public BadExpectedAccess<void> {
 public:
  ...
  /// \param error The error held by the Expected at the time of the bad access.
...
/// True when `T` is both copy constructible and copy assignable.
template <class T>
using IsCopyable = StrictConjunction<
    std::is_copy_constructible<T>,
    std::is_copy_assignable<T>>;

/// True when `T` is both move constructible and move assignable.
template <class T>
...
/// True when `From` can both construct and assign to `To`.
template <class From, class To>
using IsConvertible = StrictConjunction<
    std::is_constructible<To, From>,
    std::is_assignable<To&, From>>;

/// Assigns to `t` in place, preferring assignment over reconstruction.
/// \param rank Overload selector preferring this assignment form.
/// \param t The object to assign to.
/// \param u The value to assign.
/// \returns Nothing.
template <class T, class U>
auto doEmplaceAssign(int rank, T& t, U&& u)
    -> decltype(void(t = static_cast<U&&>(u))) {
  t = static_cast<U&&>(u);
}

/// Assigns to `t` in place by destroying and reconstructing it.
/// \param rank Overload selector for the reconstruction form.
/// \param t The object to reconstruct.
/// \param u The value used to reconstruct `t`.
/// \returns Nothing.
template <class T, class U>
auto doEmplaceAssign(long rank, T& t, U&& u)
    -> decltype(void(T(static_cast<U&&>(u)))) {
...
/// Assigns to `t` in place from multiple arguments, preferring assignment.
/// \param rank Overload selector preferring this assignment form.
/// \param t The object to assign to.
/// \param us The arguments used to construct the assigned value.
/// \returns Nothing.
template <class T, class... Us>
auto doEmplaceAssign(int rank, T& t, Us&&... us)
    -> decltype(void(t = T(static_cast<Us&&>(us)...))) {
  t = T(static_cast<Us&&>(us)...);
}

/// Assigns to `t` in place from multiple arguments by reconstructing it.
/// \param rank Overload selector for the reconstruction form.
/// \param t The object to reconstruct.
/// \param us The arguments used to reconstruct `t`.
/// \returns Nothing.
template <class T, class... Us>
auto doEmplaceAssign(long rank, T& t, Us&&... us)
...
enum class Which : unsigned char {
  eEmpty, ///< Neither a value nor an error (partially-formed).
  eValue, ///< Holds a value.
  eError, ///< Holds an error.
};
/// Storage strategy chosen for an Expected's value and error.
enum class StorageType {
  ePODStruct, ///< Trivial types laid out as a struct.
  ePODUnion, ///< Trivial types laid out as a union.
  eUnion, ///< Non-trivial types laid out as a union.
};

/// Selects the storage strategy for a given value and error type.
/// \returns The StorageType best suited to `Value` and `Error`.
template <class Value, class Error>
...
struct ExpectedStorage {
  ...
  union {
    Value value_; ///< The value alternative.
    Error error_; ///< The error alternative.
    char ch_; ///< Placeholder used while empty.
  };
  Which which_; ///< Discriminator for the active alternative.

  /// Default-constructs the storage in the error state.
  ...
  /// \param tag Tag selecting the empty-state constructor.
  explicit constexpr ExpectedStorage(EmptyTag tag) noexcept
      : ch_{unsafe_default_initialized}, which_(Which::eEmpty) {}
  /// Constructs the storage in the value state.
  /// \param tag Tag selecting the value-state constructor.
  /// \param vs Arguments forwarded to the value constructor.
  template <class... Vs>
  explicit constexpr ExpectedStorage(ValueTag tag, Vs&&... vs) noexcept(
      noexcept(Value(static_cast<Vs&&>(vs)...)))
      : value_(static_cast<Vs&&>(vs)...), which_(Which::eValue) {}
  /// Constructs the storage in the error state.
  /// \param tag Tag selecting the error-state constructor.
  /// \param es Arguments forwarded to the error constructor.
  template <class... Es>
  explicit constexpr ExpectedStorage(ErrorTag tag, Es&&... es) noexcept(
      noexcept(Error(static_cast<Es&&>(es)...)))
      : error_(static_cast<Es&&>(es)...), which_(Which::eError) {}
  /// Releases any held object; a no-op for trivial storage.
  void clear() noexcept {}
  /// Reports whether construction was interrupted by an exception.
  /// \returns Always `false` for trivial storage.
  ...
  /// Assigns a value into the storage.
  /// \param vs Arguments forwarded to construct the value.
  template <class... Vs>
  void assignValue(Vs&&... vs) {
    expected_detail::doEmplaceAssign(0, value_, static_cast<Vs&&>(vs)...);
    which_ = Which::eValue;
  }
  /// Assigns an error into the storage.
  /// \param es Arguments forwarded to construct the error.
  template <class... Es>
  void assignError(Es&&... es) {
    expected_detail::doEmplaceAssign(0, error_, static_cast<Es&&>(es)...);
    which_ = Which::eError;
  }
  /// Copies or moves the active alternative from another storage.
  /// \param that The storage to assign from.
...
struct ExpectedUnion {
  union {
    Value value_; ///< The value alternative.
    Error error_; ///< The error alternative.
    char ch_ = unsafe_default_initialized; ///< Placeholder used while empty.
  };
  Which which_ = Which::eEmpty; ///< Discriminator for the active alternative.

  /// Constructs an empty union.
  /// \param tag Tag selecting the empty-state constructor.
  explicit constexpr ExpectedUnion(EmptyTag tag) noexcept {}
  /// Constructs the union in the value state.
  /// \param tag Tag selecting the value-state constructor.
  /// \param vs Arguments forwarded to the value constructor.
  template <class... Vs>
  explicit constexpr ExpectedUnion(ValueTag tag, Vs&&... vs) noexcept(
      noexcept(Value(static_cast<Vs&&>(vs)...)))
      : value_(static_cast<Vs&&>(vs)...), which_(Which::eValue) {}
  /// Constructs the union in the error state.
  /// \param tag Tag selecting the error-state constructor.
  /// \param es Arguments forwarded to the error constructor.
  template <class... Es>
  explicit constexpr ExpectedUnion(ErrorTag tag, Es&&... es) noexcept(
      noexcept(Error(static_cast<Es&&>(es)...)))
      : error_(static_cast<Es&&>(es)...), which_(Which::eError) {}
  /// Copy constructor; the derived storage performs the actual copy.
  /// \param other The union to copy from.
  ExpectedUnion(const ExpectedUnion& other) {}
  /// Move constructor; the derived storage performs the actual move.
  /// \param other The union to move from.
  ExpectedUnion(ExpectedUnion&& other) noexcept {}
  /// Copy assignment; the derived storage performs the actual assignment.
  /// \param other The union to copy from.
  /// \returns A reference to this object.
  ExpectedUnion& operator=(const ExpectedUnion& other) { return *this; }
  /// Move assignment; the derived storage performs the actual assignment.
  /// \param other The union to move from.
  /// \returns A reference to this object.
  ExpectedUnion& operator=(ExpectedUnion&& other) noexcept { return *this; }
  /// Destructor; the derived storage destroys the active alternative.
  ~ExpectedUnion() {}
  /// Accesses the stored value.
  /// \returns A reference to the stored value.
  Value& value() & { return value_; }
...
struct CopyConstructible {
  ...
  /// Copy constructor that assigns from the derived object.
  ...
  constexpr CopyConstructible(CopyConstructible&& other) = default;
  /// Copy assignment operator.
  /// \param other The object to copy from.
  /// \returns A reference to this object.
  CopyConstructible& operator=(const CopyConstructible& other) = default;
  /// Move assignment operator.
  /// \param other The object to move from.
  /// \returns A reference to this object.
  CopyConstructible& operator=(CopyConstructible&& other) = default;
};
...
struct MoveConstructible {
  ...
  /// Copy constructor.
  /// \param other The object to copy from.
  constexpr MoveConstructible(const MoveConstructible& other) = default;
  /// Move constructor that assigns from the derived object.
  ...
  /// \param other The object to copy from.
  /// \returns A reference to this object.
  MoveConstructible& operator=(const MoveConstructible& other) = default;
  /// Move assignment operator.
  /// \param other The object to move from.
  /// \returns A reference to this object.
  MoveConstructible& operator=(MoveConstructible&& other) = default;
};
...
struct CopyAssignable {
  ...
  /// Copy constructor.
  /// \param other The object to copy from.
  constexpr CopyAssignable(const CopyAssignable& other) = default;
  /// Move constructor.
  /// \param other The object to move from.
  constexpr CopyAssignable(CopyAssignable&& other) = default;
  /// Copy assignment that assigns from the derived object.
  /// \param that The object to copy from.
  /// \returns A reference to this object.
  ...
  /// \param other The object to move from.
  /// \returns A reference to this object.
  CopyAssignable& operator=(CopyAssignable&& other) = default;
};
...
struct MoveAssignable {
  ...
  /// Copy constructor.
  /// \param other The object to copy from.
  constexpr MoveAssignable(const MoveAssignable& other) = default;
  /// Move constructor.
  /// \param other The object to move from.
  constexpr MoveAssignable(MoveAssignable&& other) = default;
  /// Copy assignment operator.
  /// \param other The object to copy from.
  /// \returns A reference to this object.
  MoveAssignable& operator=(const MoveAssignable& other) = default;
  /// Move assignment that assigns from the derived object.
  /// \param that The object to move from.
  /// \returns A reference to this object.
...
/// \param t The value produced by the continuation.
/// \param unit The Unit tag distinguishing this overload.
/// \returns `t` forwarded unchanged.
// Tricky hack so that Expected::then can handle lambdas that return void
template <class T>
inline T&& operator,(T&& t, Unit unit) noexcept {
  return static_cast<T&&>(t);
}

/// Static helpers implementing the Expected continuation operations.
struct ExpectedHelper {
  /// Asserts that an Expected is in the empty state.
  /// \param x The Expected expected to be empty.
  template <typename V, typename E>
  FOLLY_ERASE static void assume_empty(Expected<V, E> const& x) {
    compiler_may_unsafely_assume(x.which_ == Which::eEmpty);
  }

  /// Wraps a plain value in an Expected in the value state.
  /// \param t The value to wrap.
  /// \returns An Expected holding a decayed copy of `t`.
  ...
  /// Rebinds an Expected to a convertible error type.
  /// \param t The Expected to convert.
  /// \returns The Expected with its error type converted to `Error`.
  template <
      class Error,
      class T,
  ...
  /// Terminates a continuation chain by returning the Expected as-is.
  /// \param ex The Expected to return.
  /// \returns `ex` decayed to a value.
  template <class This>
  ...
  /// Applies the first continuation to the value, then chains the rest.
  /// \param ex The Expected whose value feeds the continuation.
  /// \param fn The continuation to apply to the value.
  /// \param fns The remaining continuations in the chain.
  /// \returns The Expected produced by the chain, or the propagated error.
  template <
      class This,
      class Fn,
      class... Fns,
  ...
  /// Applies `yes` to the value, or throws the result of `no` on error.
  /// \param ex The Expected to inspect.
  /// \param yes The continuation applied to the value.
  /// \param no The function producing the exception to throw on error.
  /// \returns The result of applying `yes` to the value.
  template <
      class This,
      class Yes,
      class No,
  ...
  /// Applies `yes` to the value, or invokes `no` and throws on error.
  /// \param ex The Expected to inspect.
  /// \param yes The continuation applied to the value.
  /// \param no The error handler invoked before throwing.
  /// \returns The result of applying `yes` to the value.
  template <
      class This,
      class Yes,
   ...
   *
   * \param ex The Expected to inspect.
   * \param no The error handler invoked on the error state.
   * \param fns The remaining continuations in the chain.
   * \returns The Expected produced by the chain.
   */
  template <
      class This,
      class No,
   ...
   * \param no The error handler invoked on the error state.
   * \param fns The remaining continuations in the chain.
   * \returns The Expected with its value preserved or the error propagated.
   */
  template <
      class This,
      class No,
  ...
  /// Invokes `onError` when the Expected holds an error, then returns it.
  /// \param ex The Expected to inspect.
  /// \param onError The handler invoked on the error state.
  /// \returns `ex` unchanged.
  template <
      class This,
      class OnError,
      class Err =
...
/// \param tag The UnexpectedTag argument (defaulted).
/// \returns An UnexpectedTag value.
inline expected_detail::UnexpectedTag unexpected(
    expected_detail::UnexpectedTag tag = {}) {
  return {};
}
...
class Expected final : expected_detail::ExpectedStorage<Value, Error> {
 ...
 public:
  ...
  using value_type = Value;
  /// The type of the contained error.
  using error_type = Error;

  /// Rebinds this Expected to a new value type while keeping the error type.
  template <class U>
  using rebind = Expected<U, Error>;

  /// The coroutine promise type used when an Expected is a coroutine result.
  ...
  /// \param val The value to convert and store.
  ...
  /// \param tag Tag selecting in-place value construction.
  /// \param ts Arguments forwarded to the value constructor.
  template <class... Ts FOLLY_REQUIRES_TRAILING(
      std::is_constructible<Value, Ts&&...>::value)>
  explicit constexpr Expected(std::in_place_t tag, Ts&&... ts) noexcept(
  ...
  /// \param tag Tag selecting in-place value construction.
  /// \param il The initializer list forwarded to the value constructor.
  /// \param ts Additional arguments forwarded to the value constructor.
  template <
      class U,
  ...
  /// \param err The error operand.
  /* implicit */ Expected(const Error& err) = delete;
  /// Deleted: construct from an error directly; use makeUnexpected instead.
  /// \param err The error operand.
  /* implicit */ Expected(Error&& err) = delete;

  /// Constructs an Expected in the error state from a copied error.
  /// \param tag Tag selecting error-state construction.
  /// \param err The error to store.
  FOLLY_REQUIRES(std::is_copy_constructible<Error>::value)
  constexpr Expected(unexpected_t tag, const Error& err) noexcept(
      noexcept(Error(err)))
  ...
  constexpr Expected(unexpected_t tag, Error&& err) noexcept(
  ...
  /// \param err The Unexpected whose error is copied.
  FOLLY_REQUIRES(std::is_copy_constructible<Error>::value)
  ...
  /// \param err The Unexpected whose error is moved.
  FOLLY_REQUIRES(std::is_move_constructible<Error>::value)
  ...
  /// \param err The Unexpected whose error is converted and copied.
  ...
  /// Constructs an Expected in the error state by moving a convertible
  /// Unexpected.
  /// \param err The Unexpected whose error is converted and moved.
  template <class OtherError FOLLY_REQUIRES_TRAILING(
  ...
  /// Converting assignment from an Expected with compatible types.
  /// \param that The Expected to assign from.
  /// \returns A reference to this object.
  template <
      class V,
      class E FOLLY_REQUIRES_TRAILING(
  ...
  /// Assigns a value into the Expected by copying.
  ...
  /// Assigns a value into the Expected by moving.
  /// \param val The value to assign.
  /// \returns A reference to this object.
  ...
  /// Assigns a value convertible to the value type.
  /// \param val The value to convert and assign.
  /// \returns A reference to this object.
  template <class T FOLLY_REQUIRES_TRAILING(
      std::is_convertible<T, Value>::value &&
      !std::is_convertible<T, Error>::value)>
  ...
  /// Assigns an error into the Expected by copying an Unexpected.
  /// \param err The Unexpected whose error is assigned.
  /// \returns A reference to this object.
  ...
  /// Assigns an error into the Expected by moving an Unexpected.
  /// \param err The Unexpected whose error is assigned.
  /// \returns A reference to this object.
  ...
  /// Constructs a new value in place, replacing any current contents.
  /// \param ts Arguments forwarded to the value constructor.
  template <class... Ts FOLLY_REQUIRES_TRAILING(
      std::is_constructible<Value, Ts&&...>::value)>
  void emplace(Ts&&... ts) {
   ...
   * swap
   *
   * \param that The Expected to swap contents with.
   */
  ...
  // If overload resolution selects one of these deleted functions, that
  // means you need to use makeUnexpected
  /// Deleted: assign from an error directly; use makeUnexpected instead.
  /// \param err The error operand.
  /// \returns Never returns; this overload is deleted.
  /* implicit */ Expected& operator=(const Error& err) = delete;
  /// Deleted: assign from an error directly; use makeUnexpected instead.
  /// \param err The error operand.
  /// \returns Never returns; this overload is deleted.
  /* implicit */ Expected& operator=(Error&& err) = delete;

  /**
   * Relational Operators
   */
  /// Compares two Expected values for equality.
  /// \param lhs The left-hand Expected.
  /// \param rhs The right-hand Expected.
  /// \returns `true` if both hold errors or both hold equal values.
  ...
  /// Accesses the stored value, throwing if none is present.
  /// \returns A const reference to the stored value.
  ...
  /// Accesses the stored error, throwing if none is present.
  /// \returns A const reference to the stored error.
  ...
  /// \param dflt The default returned when no value is present.
  /// \returns A copy of the value, or `dflt`.
  template <class U>
  ...
  /// Returns the stored value, or a default when in the error state.
  /// \param dflt The default returned when no value is present.
  /// \returns The moved value, or `dflt`.
  template <class U>
  ...
  /// Tests whether the Expected holds a value.
  /// \returns `true` if a value is stored.
  explicit constexpr operator bool() const noexcept { return hasValue(); }

  /// Dereferences to the stored value without checking.
  /// \returns A const reference to the stored value.
  ...
  /// Returns a pointer to the value, or null in the error state.
  /// \returns A const pointer to the value, or `nullptr`.
  ...
  /// Deleted: cannot take a pointer into a temporary Expected.
  Value* get_pointer() && = delete;

  /**
   * then
   *
   * \param fns The continuations to apply to the value in order.
   * \returns The Expected produced by the continuation chain.
   */
  ...
  /// Applies the continuations to the value if present.
  /// \param fns The continuations to apply to the value in order.
  /// \returns The Expected produced by the continuation chain.
  template <class... Fns FOLLY_REQUIRES_TRAILING(sizeof...(Fns) >= 1)>
   ...
   * error type
   *
   * \param fns The error handlers and continuations to apply.
   * \returns The Expected produced when handling the error.
   */
  template <class... Fns FOLLY_REQUIRES_TRAILING(sizeof...(Fns) >= 1)>
  auto orElse(Fns&&... fns)
  ...
  /// Returns the value if present, otherwise applies the error handlers.
  /// \param fns The error handlers and continuations to apply.
  /// \returns The Expected produced when handling the error.
  template <class... Fns FOLLY_REQUIRES_TRAILING(sizeof...(Fns) >= 1)>
  auto
   ...
   *
   * \param yes The continuation applied to the value.
   * \param no The function producing the exception thrown on error.
   * \returns The result of applying `yes` to the value.
   */
  ...
  /// Applies `yes` to the value, or throws via `no` on error.
  /// \param yes The continuation applied to the value.
  /// \param no The function producing the exception thrown on error.
  /// \returns The result of applying `yes` to the value.
  template <class Yes, class No = MakeBadExpectedAccess>
   ...
   * onError
   *
   * \param onError The handler invoked when the Expected holds an error.
   * \returns The Expected itself, after invoking the handler on error.
   */
  ...
  /// Invokes a handler when the Expected holds an error.
  /// \param onError The handler invoked when the Expected holds an error.
  /// \returns The Expected itself, after invoking the handler on error.
  template <class OnError>
...
/// \param lhs The left-hand Expected.
/// \param rhs The right-hand Expected.
/// \returns `true` if the two Expected values are not equal.
template <
    class Value,
...
/// \param lhs The left-hand Expected.
/// \param rhs The right-hand Expected.
/// \returns `true` if `lhs` does not order after `rhs`.
template <
    class Value,
...
/// \param lhs The left-hand Expected.
/// \param rhs The right-hand Expected.
/// \returns `true` if `lhs` orders after `rhs`.
template <
    class Value,
    class Error FOLLY_REQUIRES_TRAILING(IsLessThanComparable<Value>::value)>
inline bool operator>(
...
/// \param lhs The left-hand Expected.
/// \param rhs The right-hand Expected.
/// \returns `true` if `lhs` does not order before `rhs`.
template <
    class Value,
    class Error FOLLY_REQUIRES_TRAILING(IsLessThanComparable<Value>::value)>
inline bool operator>=(
 ...
 *
 * \param lhs The first Expected to swap.
 * \param rhs The second Expected to swap.
 */
template <class Value, class Error>
...
/// Returns a pointer to the value of an Expected, or null on error.
/// \param ex The Expected to inspect.
/// \returns A const pointer to the value, or `nullptr`.
template <class Value, class Error>
 ...
 * \returns An Expected in the value state holding a decayed copy of `val`.
 */
template <class Error, class Value>
[[nodiscard]] constexpr Expected<typename std::decay<Value>::type, Error>
...
/// \param exp The Expected operand.
/// \param other The value operand.
/// \returns Never returns; this overload is deleted.
template <class Value, class Error>
bool operator==(const Expected<Value, Error>& exp, const Value& other) = delete;
/// Deleted: comparing an Expected with a bare value is not allowed.
/// \param exp The Expected operand.
/// \param other The value operand.
/// \returns Never returns; this overload is deleted.
...
/// \returns Never returns; this overload is deleted.
template <class Value, class Error>
bool operator==(const Value& other, const Expected<Value, Error>& exp) = delete;
/// Deleted: comparing a bare value with an Expected is not allowed.
/// \param other The value operand.
/// \param exp The Expected operand.
/// \returns Never returns; this overload is deleted.
...
template <typename Value, typename Error>
struct PromiseReturn {
  Expected<Value, Error> storage_{EmptyTag{}}; ///< Storage for the result.
  Expected<Value, Error>*& pointer_; ///< Reference to the promise's pointer.

  /// Constructs the return object and wires it into the promise.
  /// \param p The coroutine promise owning this return object.
  /* implicit */ PromiseReturn(PromiseBase<Value, Error>& p) noexcept
      : pointer_{p.value_} {
    pointer_ = &storage_;
  }
  /// Deleted copy constructor.
  /// \param other The return object to copy from.
  PromiseReturn(PromiseReturn const& other) = delete;
  // letting dtor be trivial makes the coroutine crash
  // TODO: fix clang/llvm codegen
  /// Destructor.
  ~PromiseReturn() {}
  /// Converts the return object into the resulting Expected.
  /// \returns The Expected holding the coroutine result.
...
template <typename Value, typename Error>
struct PromiseBase {
  Expected<Value, Error>* value_ = nullptr; ///< Points at the result storage.

  /// Default constructor.
  PromiseBase() = default;
  /// Deleted copy constructor.
  /// \param other The promise to copy from.
  PromiseBase(PromiseBase const& other) = delete;
  /// Deleted copy assignment operator.
  /// \param other The promise to copy from.
  void operator=(PromiseBase const& other) = delete;

  /// Reports the initial suspension behavior.
  /// \returns An awaitable that never suspends.
  coro::suspend_never initial_suspend() const noexcept { return {}; }
  /// Reports the final suspension behavior.
  /// \returns An awaitable that never suspends.
  coro::suspend_never final_suspend() const noexcept { return {}; }
  /// Handles an exception escaping the coroutine body by rethrowing.
  ...
  /// Creates the coroutine return object.
  /// \returns The PromiseReturn wired to this promise.
...
struct PromiseReturnsValue : public PromiseBase<Value, Error> {
  /// Stores the value produced by a `co_return` expression.
  /// \param u The value to store as the coroutine result.
...
struct PromiseReturnsVoid : public PromiseBase<Value, Error> {
  ...
  /// Stores a default-constructed value on `co_return;`.
...
struct UnexpectedAwaitable {
  Unexpected<Error> o_; ///< The error that aborts the coroutine.

  /// Wraps an Unexpected to be awaited.
  /// \param o The Unexpected error to propagate.
  explicit UnexpectedAwaitable(Unexpected<Error> o) : o_(std::move(o)) {}

  /// Reports whether the awaited operation is ready.
  /// \returns A false constant, so the coroutine always suspends.
  constexpr std::false_type await_ready() const noexcept { return {}; }
  /// Resumes the coroutine; never actually called for this awaitable.
  void await_resume() { compiler_may_unsafely_assume_unreachable(); }

  /// Stores the error into the promise and aborts the coroutine.
  /// \param h The handle of the suspending coroutine.
  template <typename U>
...
struct ExpectedAwaitable {
  Expected<Value, Error> o_; ///< The awaited Expected.

  /// Wraps an Expected to be awaited.
  /// \param o The Expected to unwrap.
  explicit ExpectedAwaitable(Expected<Value, Error> o) : o_(std::move(o)) {}

  /// Reports whether the Expected already holds a value.
  /// \returns `true` if a value is present and no suspension is needed.
  bool await_ready() const noexcept { return o_.hasValue(); }
  /// Yields the contained value to the awaiting expression.
  /// \returns The value held by the Expected.
  Value await_resume() { return std::move(o_.value()); }

  // Explicitly only allow suspension into a Promise
  /// Propagates the error into the promise and aborts the coroutine.
  /// \param h The handle of the suspending coroutine.
  template <typename U>
  FOLLY_ALWAYS_INLINE void
  await_suspend(coro::coroutine_handle<Promise<U, Error>> h) noexcept(
...
/// Enables `co_await` on an Unexpected to short-circuit with an error.
/// \param o The Unexpected error to await.
/// \returns An awaitable that propagates the error.
template <typename Error>
...
/// Enables `co_await` on an Expected to unwrap its value or propagate its error.
/// \param o The Expected to await.
/// \returns An awaitable that yields the value or propagates the error.
template <typename Value, typename Error>
