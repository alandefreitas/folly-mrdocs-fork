...
class FunctionScheduler {
 public:
  /// Constructs a FunctionScheduler with no scheduled functions.
  FunctionScheduler();
   ...
   *
   * \param steady Whether the scheduler should attempt to catch up on lag.
   */
  ...
  struct LatencyDistribution {
    /// Whether to draw intervals from a Poisson distribution.
    bool isPoisson;
    /// The mean interval used when isPoisson is true.
    std::chrono::microseconds poissonMean;

    /**
     * Constructs a latency distribution.
     *
     * \param poisson Whether to draw intervals from a Poisson distribution.
     * \param mean The mean interval used when poisson is true.
     */
   ...
   * \param interval The interval between scheduled runs.
   * \param nameID The unique name for the function.
   * \param startDelay The delay before the function is first run.
   */
  void addFunction(
      Function<void()>&& cb,
   ...
   * \param cb The function to run.
   * \param interval The interval between scheduled runs.
   * \param latencyDistr The latency distribution controlling the interval.
   * \param nameID The unique name for the function.
   * \param startDelay The delay before the function is first run.
   */
  void addFunction(
   ...
   * \param cb The function to run.
   * \param nameID The unique name for the function.
   * \param startDelay The delay before the function is run.
   */
   ...
   *
   * \param cb The function to run.
   * \param minInterval The minimum interval between scheduled runs.
   * \param maxInterval The maximum interval between scheduled runs.
   * \param nameID The unique name for the function.
   * \param startDelay The delay before the function is first run.
   */
   ...
   *
   * \param cb The function to run.
   * \param intervalFunc The functor that determines the interval before each
   * scheduled run.
   * \param nameID The unique name for the function.
   * \param intervalDescr A description of the scheduling interval.
   * \param startDelay The delay before the function is first run.
   */
   ...
   *
   * \param cb The function to run.
   * \param fn The functor that returns the next run time.
   * \param nameID The unique name for the function.
   * \param intervalDescr A description of the scheduling interval.
   * \param startDelay The delay before the function is first run.
   */
   ...
   *
   * \param nameID The name of the function to cancel.
   *
   * \returns False if no function exists with the specified name, true
   * otherwise.
   */
  bool cancelFunction(StringPiece nameID);

  /**
   * Cancels the function with the specified name, waiting for it to complete
  ...
  /**
   * Cancels all registered functions, waiting for any currently running
   ...
   * Returns false if no function exists with the specified name.
   *
   * \param nameID The name of the function whose timer should be reset.
   *
   * \returns False if no function exists with the specified name, true
   * otherwise.
   */
   ...
   *
   * \returns False if the scheduler was already running, true otherwise.
   */
  bool start();
   ...
   * Thread-safe.
   *
   * \returns False if the scheduler was not running, true otherwise.
   */
  bool shutdown();
   ...
   *
   * \param threadName The name to assign to the worker thread.
   */
