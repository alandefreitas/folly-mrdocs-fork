...
class ManualExecutor
 ...
 public:
  ...
  /// Enqueue a function to be run when the executor is driven.
  ///
  ...
  ///
  /// \returns Number of functions that were executed.
  size_t run();
  ...
  ///
  /// \returns 1 if a function was executed, 0 otherwise.
  size_t step();
  ...
  ///
  /// \param f Future to wait on until it is ready.
  template <class F>
  ...
  /// Schedule a function to run once the clock reaches the given time.
  ///
  /// \param f Function to schedule.
  /// \param t Absolute time at which the function becomes runnable.
  ...
  ///
  /// \param dur Amount of time to advance the clock by.
  void advance(Duration const& dur) { advanceTo(now_ + dur); }

  /// Advance the clock to this absolute time. If t is <= now(),
  /// this is a noop.
  ///
  /// \param t Absolute time to advance the clock to.
  void advanceTo(TimePoint const& t);

  /// Return the current time of the manual clock.
  ///
  /// \returns The current manual clock time.
  ...
  ///
  /// \returns Number of functions removed from the queue.
  std::size_t clear() {
