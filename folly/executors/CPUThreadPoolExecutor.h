...
/// Declares the gflag controlling dynamic CPUThreadPoolExecutor behavior.
///
...
class CPUThreadPoolExecutor
 ...
 public:
  ...
  /// Configuration options for the CPUThreadPoolExecutor.
  struct Options {
    /// Controls whether tasks may block on this thread pool.
    enum class Blocking {
      prohibit, ///< Blocking on this thread pool is not allowed.
      allow, ///< Blocking on this thread pool is allowed.
    };

    /// Constructs options with blocking allowed by default.
    constexpr Options() noexcept : blocking{Blocking::allow} {}

    /// Sets the blocking behavior and returns the options for chaining.
    ///
    /// \param b The blocking behavior to set.
  ...
  /// Returns an unbounded blocking queue using the default semaphore.
  ///
  /// \returns An unbounded blocking queue.
  static std::unique_ptr<BlockingQueue<CPUTask>> makeDefaultQueue();

  /// Returns an unbounded blocking priority queue using the default semaphore.
  ///
  /// \param numPriorities The number of priority levels.
  /// \returns An unbounded blocking priority queue.
  ...
  /// Returns an unbounded blocking priority queue using LifoSem.
  ///
  /// \param numPriorities The number of priority levels.
  /// \returns An unbounded blocking priority queue.
  ...
  /// Returns an unbounded blocking queue using ThrottledLifoSem.
  ///
  /// \param wakeUpInterval The interval used to throttle thread wake-ups.
  /// \returns An unbounded blocking queue.
  static std::unique_ptr<BlockingQueue<CPUTask>> makeThrottledLifoSemQueue(
      std::chrono::nanoseconds wakeUpInterval = {});

  /// Returns an unbounded blocking priority queue using ThrottledLifoSem.
  ///
  /// \param numPriorities The number of priority levels.
  /// \param wakeUpInterval The interval used to throttle thread wake-ups.
  /// \returns An unbounded blocking priority queue.
  ...
  /// Constructs an executor with the given task queue.
  ///
  /// \param numThreads The number of worker threads.
  /// \param taskQueue The queue that holds pending tasks.
  /// \param threadFactory The factory used to create worker threads.
  /// \param opt Additional executor options.
  ...
  /// Constructs an executor with the given task queue and thread bounds.
  ///
  ...
  /// Constructs an executor with a default queue and thread factory.
  ///
  /// \param numThreads The number of worker threads.
  /// \param opt Additional executor options.
  explicit CPUThreadPoolExecutor(size_t numThreads, Options opt = {});

  /// Constructs an executor with a default queue and the given thread factory.
  ///
  /// \param numThreads The number of worker threads.
  /// \param threadFactory The factory used to create worker threads.
  /// \param opt Additional executor options.
  ...
  /// Constructs an executor with a priority queue.
  ///
  /// \param numThreads The number of worker threads.
  /// \param numPriorities The number of priority levels.
  /// \param threadFactory The factory used to create worker threads.
  ...
  /// Constructs an executor with a priority queue of a bounded size.
  ///
  /// \param numThreads The number of worker threads.
  /// \param numPriorities The number of priority levels.
  /// \param maxQueueSize The maximum number of pending tasks.
  /// \param threadFactory The factory used to create worker threads.
  /// \param opt Additional executor options.
  CPUThreadPoolExecutor(
      size_t numThreads,
  ...
  /// Joins all worker threads and destroys the executor.
  ...
  /// Schedules a function to run on the thread pool with an expiration.
  ///
  /// \param func The function to run.
  /// \param expiration The duration after which the task expires.
  /// \param expireCallback The callback invoked if the task expires.
  void add(
      Func func,
      std::chrono::milliseconds expiration,
      Func expireCallback = nullptr) override;

  /// Schedules a function to run on the thread pool at the given priority.
  ///
  /// \param func The function to run.
  /// \param priority The priority of the task.
  void addWithPriority(Func func, int8_t priority) override;

  /// Schedules a function to run at the given priority with an expiration.
  ///
  /// \param func The function to run.
  /// \param priority The priority of the task.
  /// \param expiration The duration after which the task expires.
  ...
  /// Returns the number of tasks currently pending in the task queue.
  ///
  /// \returns The number of pending tasks.
  size_t getTaskQueueSize() const;

  /// Returns the number of priority levels supported by the executor.
  ///
  ...
  ///
  /// \returns The worker provider that collects worker thread ids.
  ...
  struct CPUTask : public ThreadPoolExecutor::Task {
    /// Constructs an empty poison task used to signal worker shutdown.
    CPUTask(); // Poison.

    /// Constructs a task wrapping the given function and metadata.
    ///
    /// \param f The function to run.
    /// \param context The request context associated with the task.
    /// \param expiration The duration after which the task expires.
    /// \param expireCallback The callback invoked if the task expires.
  ...
  /// The default maximum size of the task queue.
  static const size_t kDefaultMaxQueueSize;

 protected:
  /// Returns the executor's underlying task queue.
  ///
  /// \returns A pointer to the underlying task queue.
  BlockingQueue<CPUTask>* FOLLY_NONNULL getTaskQueue();

  /// Enqueues a task using the given enqueue operation, adding threads if
  /// needed.
  ///
  /// \tparam EnqueueTask The callable type used to enqueue the task.
  /// \param enqueueTask The operation that enqueues the task.
  /// \param task The task to enqueue.
  template <typename EnqueueTask>
  void addImpl(EnqueueTask&& enqueueTask, CPUTask&& task);

  /// Collects the thread ids of the executor's worker threads.
