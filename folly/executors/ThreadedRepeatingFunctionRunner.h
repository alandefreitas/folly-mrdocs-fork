...
class ThreadedRepeatingFunctionRunner final {
 public:
  /// A repeating function returning how long to wait before the next call.
  ///
  /// The returned duration is how long the loop sleeps before the next
  /// repetition. The function must not throw.
  using RepeatingFn = folly::Function<std::chrono::milliseconds() noexcept>;

  /// Constructs a runner with no threads yet.
  ThreadedRepeatingFunctionRunner();

  /// Stops all threads and waits for running functions to return.
  ~ThreadedRepeatingFunctionRunner();
   ...
   * your threads.
   *
   * \param name Thread name, truncated to the first 15 characters.
   * \param f The noexcept function to run in a loop.
   * \param initialSleep How long to wait before the first call to `f`.
   */
  void add(
      std::string name,
      RepeatingFn f,
