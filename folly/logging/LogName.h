...
class LogName {
 public:
   ...
   *
   * \param input The log name to canonicalize.
   * \returns The canonicalized log name.
   */
   ...
   *
   * \param name The log name to hash.
   * \returns The hash value for the log name.
   */
   ...
   *
   * \param nameA The first log name to compare.
   * \param nameB The second log name to compare.
   * \returns 0 if the names are equal, -1 if nameA < nameB, 1 otherwise.
   */
   ...
   *
   * \param name The log name whose parent should be returned.
   * \returns The parent log name, or an empty StringPiece for the root.
   */
  ...
  struct Hash {
    /// Hash a log name.
    ///
    /// \param key The log name to hash.
    /// \returns The hash value for the log name.
   ...
   */
  struct Equals {
    /// Compare two log names for equality.
    ///
    /// \param a The first log name.
    /// \param b The second log name.
    /// \returns True if both names refer to the same log category.
