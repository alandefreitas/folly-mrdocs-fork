...
/// A named category that log messages are associated with.
class LogCategory;
/// Configuration describing log categories and handlers.
class LogConfig;
/// Consumes and processes log messages.
class LogHandler;
/// Creates LogHandler objects from configuration.
...
class LoggerDB {
 ...
 public:
   ...
   *
   * \returns A reference to the main LoggerDB singleton.
   */
  static LoggerDB& get();

  /// Destroy the LoggerDB.
  ~LoggerDB();
   ...
   * already.
   *
   * \param name The name of the log category to look up.
   * \returns The LogCategory for the specified name.
   */
   ...
   *
   * \param name The name of the log category to look up.
   * \returns The LogCategory for the name, or nullptr if it does not exist.
   */
   ...
   *
   * \param name The name of the log category to set the level on.
   * \param level The log level to set.
   * \param inherit Whether the effective level inherits from parent categories.
   */
  void setLevel(folly::StringPiece name, LogLevel level, bool inherit = true);
  /// Set the log level for the specified category.
  ///
  /// \param category The log category to set the level on.
  /// \param level The log level to set.
  /// \param inherit Whether the effective level inherits from parent categories.
   ...
   *
   * \returns A LogConfig describing the current state of the LoggerDB.
   */
   ...
   *
   * \returns A LogConfig fully describing the state of the LoggerDB.
   */
   ...
   *
   * \param config The configuration settings to apply.
   */
   ...
   *
   * \param config The configuration to reset the LoggerDB to.
   */
   ...
   * existing factory.
   *
   * \param factory The LogHandlerFactory to register.
   * \param replaceExisting Whether to replace an existing factory of the same
   * type.
   */
   ...
   *
   * \param type The handler type name of the factory to remove.
   */
   ...
   *
   * \param categoryName The name of the log category to initialize.
   * \param xlogCategoryLevel Pointer to the atomic cached level to initialize.
   * \param xlogCategory Pointer to the LogCategory pointer to initialize.
   * \returns The current effective LogLevel of the category.
   */
  LogLevel xlogInit(
  ...
  /// Initialize the LogCategory pointer used by an XLOG() statement.
  ///
  /// \param categoryName The name of the log category to initialize.
  /// \param xlogCategory Pointer to the LogCategory pointer to initialize.
  /// \param isInitialized Pointer to the atomic flag tracking initialization.
  /// \returns The LogCategory for the given name.
  LogCategory* xlogInitCategory(
      folly::StringPiece categoryName,
      LogCategory** xlogCategory,
      std::atomic<bool>* isInitialized);

  /// Tag type used to select the testing constructor.
  enum TestConstructorArg {
    TESTING ///< Tag value selecting the testing constructor.
  };
   ...
   * unintentionally.
   *
   * \param arg A tag argument that guards against accidental construction.
   */
  explicit LoggerDB(TestConstructorArg arg);
   ...
   *
   * \param callback The context string callback to add.
   */
  void addContextCallback(ContextCallback callback);
   ...
   * LoggerDB objects.
   *
   * \returns The context string built from the registered callbacks.
   */
   ...
   *
   * \param file The source file where the warning originated.
   * \param lineNumber The source line where the warning originated.
   * \param args The message components to concatenate into the warning.
   */
  ...
  /// Function type for handling internal logging library warnings.
   ...
   *
   * \param handler The handler to call on internal warnings, or null to use
   * the default built-in handler.
   */
 ...
 * that used by GLOG.
 *
 * \param db The LoggerDB singleton to configure.
 */
