...
class GlogStyleFormatter : public LogFormatter {
 public:
  /// Construct a GlogStyleFormatter.
  ///
  /// \param log_thread_name Whether to include the thread name in output.
  explicit GlogStyleFormatter(bool log_thread_name = false)
      : log_thread_name_(log_thread_name) {}

  /// Format a log message in the glog-style format.
  ///
  /// \param message The log message to format.
  /// \param handlerCategory The category of the handler formatting the message.
