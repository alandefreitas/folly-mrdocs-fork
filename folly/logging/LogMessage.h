...
class LogMessage {
 public:
  /// Constructs a log message, taking ownership of the message text.
  ///
  /// \param category The category the message was logged to.
  ...
  LogMessage(
      ...
      std::string&& msg);
  /// Constructs a log message from a `StringPiece` message.
  ///
  /// \param category The category the message was logged to.
  /// \param level The level the message was logged at.
  /// \param filename The source filename where the message was logged.
   ...
   * @param category The category the message was logged to.
   * @param level The level the message was logged at.
   * @param timestamp The explicit timestamp to record.
   * @param filename The source filename where the message was logged.
   * @param lineNumber The source line number where the message was logged.
   * @param functionName The function name where the message was logged.
   * @param msg The message text.
  ...
  /// Returns the category this message was logged to.
  ///
  /// \returns The log category.
  const LogCategory* getCategory() const { return category_; }

  /// Returns the level this message was logged at.
  ///
  /// \returns The log level.
  LogLevel getLevel() const { return level_; }

  /// Returns the source filename where the message was logged.
  ///
  /// \returns The full filename.
  folly::StringPiece getFileName() const { return filename_; }
  /// Returns the base name of the source filename.
  ///
  /// \returns The filename with directory components removed.
  folly::StringPiece getFileBaseName() const;

  /// Returns the source line number where the message was logged.
  ///
  /// \returns The line number.
  unsigned int getLineNumber() const { return lineNumber_; }

  /// Returns the function name where the message was logged.
  ///
  ...
  /// Returns the ID of the thread that logged the message.
  ///
  /// \returns The thread ID.
  uint64_t getThreadID() const { return threadID_; }

  /// Returns the message text, sanitized if necessary.
  ///
  ...
  /// Returns the original, unsanitized message text.
  ///
  /// \returns The raw message string.
  const std::string& getRawMessage() const { return rawMessage_; }

  /// Tests whether the message contains embedded newlines.
  ///
  /// \returns `true` if the message spans multiple lines.
  bool containsNewlines() const { return numNewlines_ > 0; }

  /// Returns the number of embedded newlines in the message.
  ///
  /// \returns The newline count.
  size_t getNumNewlines() const { return numNewlines_; }

  /// Returns the additional context string associated with the message.
  ///
