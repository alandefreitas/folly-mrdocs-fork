...
class IntervalRateLimiter {
 public:
  ...
  /// Construct an IntervalRateLimiter.
  ///
  /// \param maxPerInterval The maximum number of events allowed per interval.
  /// \param interval The length of each rate-limiting interval.
  ...
  /// Check whether an event is allowed under the current rate limit.
  ///
  /// \returns `true` if the event is permitted, `false` if it is rate-limited.
  bool check() {
