...
class LogStreamProcessor {
 public:
  /// Tag type selecting the append constructor overloads.
  enum AppendType {
    APPEND ///< Append streamed arguments to the message.
  };
  /// Tag type selecting the format-string constructor overloads.
  enum FormatType {
    FORMAT ///< Format the arguments using a format string.
  };
   ...
   * preprocessor macro.)
   *
   * \param category The log category the message belongs to.
   * \param level The log level of the message.
   * \param filename The source filename where the message was logged.
   * \param lineNumber The source line number where the message was logged.
   * \param functionName The function name where the message was logged.
   * \param appendType A tag selecting append mode.
   */
  LogStreamProcessor(
      ...
      AppendType appendType) noexcept;
   ...
   * \param lineNumber The source line number where the message was logged.
   * \param functionName The function name where the message was logged.
   * \param formatType A tag selecting format mode.
   ...
   * size.
   */
  /// Constructs a processor for an XLOG() statement with no extra arguments.
  ///
  /// \param categoryInfo Per-statement XLOG category information.
  /// \param level The log level of the message.
  /// \param categoryName The log category name.
  /// \param isCategoryNameOverridden Whether the category name was overridden.
  /// \param filename The source filename where the message was logged.
  /// \param lineNumber The source line number where the message was logged.
  /// \param functionName The function name where the message was logged.
  /// \param appendType A tag selecting append mode.
  LogStreamProcessor(
      ...
      folly::StringPiece functionName,
      AppendType appendType) noexcept;
  /// Constructs a processor for an XLOG() statement with appended arguments.
  ///
  /// \param categoryInfo Per-statement XLOG category information.
  /// \param level The log level of the message.
  /// \param categoryName The log category name.
  ...
  LogStreamProcessor(
      ...
      : LogStreamProcessor(
            ...
            INTERNAL,
            createLogString(std::forward<Args>(args)...)) {}
  /// Constructs a processor for an XLOG() statement with a format string.
  ///
  /// \param categoryInfo Per-statement XLOG category information.
  /// \param level The log level of the message.
  /// \param categoryName The log category name.
   ...
   */
  /// Constructs a processor for a file-scoped XLOG() statement.
  ///
  /// \param fileScopeInfo File-scope XLOG category information.
  /// \param level The log level of the message.
  /// \param filename The source filename where the message was logged.
  /// \param lineNumber The source line number where the message was logged.
  /// \param functionName The function name where the message was logged.
  ...
  LogStreamProcessor(
      ...
      folly::StringPiece functionName,
      AppendType appendType) noexcept;
  /// Constructs a file-scoped processor, ignoring the category name arguments.
  ///
  /// \param fileScopeInfo File-scope XLOG category information.
  /// \param level The log level of the message.
  /// \param categoryName The log category name (unused in this overload).
  /// \param isCategoryNameOverridden Whether the category name was overridden (unused).
  /// \param filename The source filename where the message was logged.
  /// \param lineNumber The source line number where the message was logged.
  /// \param functionName The function name where the message was logged.
  /// \param appendType A tag selecting append mode.
  ...
  /// Constructs a file-scoped processor with appended arguments.
  ///
  /// \param fileScopeInfo File-scope XLOG category information.
  /// \param level The log level of the message.
  /// \param categoryName The log category name (unused in this overload).
  ...
  LogStreamProcessor(
      ...
      : LogStreamProcessor(
            ...
            INTERNAL,
            createLogString(std::forward<Args>(args)...)) {}
  /// Constructs a file-scoped processor with a format string.
  ///
  /// \param fileScopeInfo File-scope XLOG category information.
  /// \param level The log level of the message.
  /// \param categoryName The log category name (unused in this overload).
  ...
  /// Logs the accumulated message and destroys the processor.
   ...
   *
   * \param stream The stream holding the accumulated log message.
   */
   ...
   *
   * \param stream The log stream holding the accumulated log message.
   */
  void operator&(LogStream&& stream) noexcept;

  /// Returns the underlying output stream for the log message.
  ///
  /// \returns The output stream to write log arguments to.
  std::ostream& stream() noexcept { return stream_; }

  /// Immediately logs the accumulated message.
...
class LogStreamVoidify {
 public:
   ...
   *
   * \param stream The stream holding the log arguments (unused).
   */
  void operator&(std::ostream& stream) noexcept {}
};
...
class LogStreamVoidify<true> {
 public:
   ...
   *
   * \param stream The stream holding the log arguments.
   */
  [[noreturn]] void operator&(std::ostream& stream);
};
 ...
 *
 * \param isFatal A tag whose type indicates whether the log level is fatal.
 */
// clang-format off
inline void logDisabledHelper(std::false_type isFatal) noexcept {}
/// Overload of logDisabledHelper() for fatal log levels; never returns.
///
/// \param isFatal A true_type tag indicating a fatal log level.
[[noreturn]] void logDisabledHelper(std::true_type isFatal) noexcept;
// clang-format on
} // namespace folly
