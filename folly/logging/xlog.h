 ...
 * passes.
 *
 * \param level The `folly::LogLevel` value at which to log.
 * \param cond A predicate; the message is logged only when it is true.
 */
 ...
 *
 * \param level The `folly::LogLevel` value at which to log.
 * \param fmt A `folly::format`-style format string for the message.
 */
 ...
 *
 * \param level The `folly::LogLevel` value at which to log.
 * \param cond A predicate; the message is logged when it is true.
 */
 ...
 *
 * \param level The `folly::LogLevel` value at which to log.
 * \param n The number of initial invocations to log.
 */
 ...
 * \param level The `folly::LogLevel` value at which to log.
 * \param cond A predicate; the message is logged only when it is true.
 * \param always_fatal Whether the statement always aborts the program.
 * \param type The `LogStreamProcessor` append or format mode.
 */
 ...
 *
 * \param level An unqualified `folly::LogLevel` enumerator.
 */
 ...
 *
 * \param level A fully qualified `folly::LogLevel` value.
 */
 ...
 *
 * \param cond The condition expression that must evaluate to true.
 */
#define XCHECK(cond, ...)         \
  XLOG_IF(                        \
      FATAL,                      \
 ...
 *
 * \param arg1 The left-hand value to compare.
 * \param arg2 The right-hand value to compare.
 */
...
class XlogFileScopeInfo {
 public:
  /// The effective log level for this file's default category.
  std::atomic<::folly::LogLevel> level{folly::LogLevel::UNINITIALIZED};
  /// The resolved log category for this file.
...
class XlogLevelInfo {
 public:
  ...
  ///
  /// \param levelToCheck The level of the message being considered.
  /// \param categoryName The category name to resolve on first use.
  /// \param isOverridden Whether the category name was explicitly overridden.
  /// \param fileScopeInfo The file-scope info object (unused in this overload).
  /// \returns `true` if a message at `levelToCheck` should be logged.
  bool check(
...
class XlogCategoryInfo {
 public:
  ...
  ///
  /// \returns `true` once the category pointer has been resolved.
  bool isInitialized() const {
    return isInitialized_.load(std::memory_order_acquire);
  }

  /// Resolves and stores the log category for this statement.
  ///
  /// \param categoryName The category name to look up.
  /// \param isOverridden Whether the category name was explicitly overridden.
  /// \returns The resolved log category.
  LogCategory* init(folly::StringPiece categoryName, bool isOverridden);

  /// Returns the resolved log category.
  ///
  /// \param fileScopeInfo The file-scope info object (unused in this overload).
  /// \returns The stored log category pointer.
   ...
   */
  XlogCategoryInfo<IsInHeaderFile>* getInfo(XlogFileScopeInfo* fileScopeInfo) {
    return this;
  }
...
class XlogCategoryInfo<false> {
 public:
   ...
   * so that it is able to look up the LogCategory information.
   *
   * \param fileScopeInfo The file-scope info object for this file.
   * \returns The same `fileScopeInfo` pointer.
   */
 ...
 *
 * \param filename The source filename to derive a category name from.
 * \returns The default XLOG() category name for that file.
 */
...
/// \returns `true` if `c` separates path components on the target platform.
 ...
 *
 * \param filename The filename to process, normally `__FILE__`.
 * \param prefixes A colon-separated list of directory prefixes to strip.
 * \returns A pointer into `filename` past the first matching prefix.
 */
