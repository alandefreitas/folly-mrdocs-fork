 ...
 * \returns A generator that yields buffered reads from the file.
 */
 ...
 *
 * \param file The file to read from.
 * \param buffer The buffer used to hold each read value.
 * \returns A generator that yields buffered reads from the file.
 */
template <class S = detail::FileReader>
 ...
 * \param bufferSize The size of the write buffer in bytes; 0 means unbuffered.
 * \returns A sink that writes values to the file.
 */
 ...
 *
 * \param file The file to write to.
 * \param buffer The buffer used to batch writes, or nullptr for unbuffered.
 * \returns A sink that writes values to the file.
 */
template <class S = detail::FileWriter>
