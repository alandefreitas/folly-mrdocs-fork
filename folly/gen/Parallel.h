 ...
 *
 * \param container The container whose elements are sliced.
 * \param chunkSize The maximum number of values per slice.
 * \returns A generator yielding generators over slices of the container. */
template <
    class Container,
...
/// Produces values from a mutable container in slices.
///
/// \param container The container whose elements are sliced.
/// \param chunkSize The maximum number of values per slice.
/// \returns A generator yielding generators over slices of the container.
 ...
 *
 * \param ops The pipeline of operations to run in parallel.
 * \param threads The number of worker threads to use; 0 selects a default.
 * \returns An operator that applies `ops` across threads.
 */
 ...
 *
 * \param sink The sink function applied to the input sequence.
 * \returns An operator yielding the single sink result as a generator.
 */
