 ...
 *      });
 *
 * @returns An output receiver yielding the transformed values.
 */
template <
 ...
 * @param inputReceiver: The input receiver.
 *
 * @param transformer: The transformer object described above.
 *
 * @returns An output receiver yielding the transformed values.
 */
template <
 ...
 * std::shared_ptr<RateLimiter> getRateLimiter(); // Can return nullptr
 *
 * @param initializeArg: The initial argument passed to the transformer's
...
struct ReinitializeException : public std::exception {
  /// Constructs the exception carrying the argument for re-initialization.
  /// \param _initializeArg The argument to re-initialize the transform with.
  explicit ReinitializeException(InitializeArg _initializeArg)
      : initializeArg(std::move(_initializeArg)) {}

  /// Returns the exception message.
  /// \returns A message indicating the transform should be re-initialized.
  const char* what() const noexcept override {
    return "This resumable transform should be re-initialized.";
  }

  /// The argument to re-initialize the resumable transform with.
  InitializeArg initializeArg;
};
} // namespace channels
} // namespace folly
