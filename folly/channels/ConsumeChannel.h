 ...
 * received.
 *
 * \param receiver The receiver whose values are consumed.
 * \param executor The executor on which each callback invocation runs.
 * \param onNext The callback invoked for each received value.
 * \returns A handle that cancels the consumption operation when destroyed.
 */
template <
    typename TReceiver,
    typename OnNextFunc,
