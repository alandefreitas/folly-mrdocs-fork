...
class hash_counter_engine : private Hash {
 ...
 public:
  ...
  /// Constructs the engine with the given initial counter state.
  ///
  /// \param state The initial counter value.
  explicit hash_counter_engine(state_type state) noexcept : state_{state} {}
  /// Constructs the engine with the given hash and initial counter state.
  ///
