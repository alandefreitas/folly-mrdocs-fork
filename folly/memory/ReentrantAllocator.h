...
class reentrant_allocator_options {
 public:
  ...
  ///
  /// The block size is the size of the blocks from which small allocations
  /// are returned.
  ///
  /// \returns The log2 of the block size.
  ...
  ///
  /// \param value The new log2 of the block size.
  ...
  /// Returns the log2 of the large size.
  ///
  /// The large size is the size starting at which allocations are considered
  /// large and are returned directly from mmap.
  ///
  /// \returns The log2 of the large size.
  std::size_t large_size_lg() const noexcept { return large_size_lg_; }
  /// Sets the log2 of the large size.
  ///
  /// \param value The new log2 of the large size.
  /// \returns A reference to these options.
...
class reentrant_allocator : private detail::reentrant_allocator_base {
 ...
 public:
  ...
  /// Signed integer type used for pointer differences.
  using difference_type = std::ptrdiff_t;

  /// Rebinds the allocator to another element type.
  template <typename U>
  struct rebind {
  ...
  /// Constructs an allocator sharing state with an allocator for another type.
  ///
  /// \param that The allocator to share state with.
  ...
  /// Returns the largest number of objects that could be allocated.
  ///
  ...
  /// \returns `true` if the allocators share state.
  ...
  /// \param a The first allocator.
  /// \param b The second allocator.
  /// \returns `true` if the allocators do not share state.
  template <typename A, typename B>
  friend bool operator!=(
      reentrant_allocator<A> const& a,
      reentrant_allocator<B> const& b) noexcept;
