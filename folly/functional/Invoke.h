...
struct ok_one_ {
  /// True if `T` is void, a reference, a function, or an unbounded array.
  template <typename T>
  ...
  /// Preferred overload selected when `T` is a complete type.
  ///
  /// \param rank overload-rank disambiguator preferring this overload
  template <typename T, std::size_t = sizeof(T)>
  static void test(int rank);
  ...
  /// Fallback overload that static-asserts the requirement on `T`.
  ///
  /// \returns Nothing; exists only to force instantiation and the static assert.
  template <typename T>
  static auto test(...) {
  ...
  /// Substitutes as void when `T` satisfies the requirement.
  template <typename T>
...
/// Yields `T` when every `A...` is complete, cv-void, or unbounded-array.
template <typename T, typename... A>
using ok_ = type_t<T, ok_one_::apply<A>...>;

/// Invocation traits for a callable type `F`.
template <typename F>
struct traits {
  ...
  using nothrow_t = std::bool_constant<noexcept( //
      FOLLY_DECLVAL(F&&)(FOLLY_DECLVAL(A&&)...))>;
#else
  /// True if invoking `F` with `A...` is noexcept.
  template <typename... A>
...
/// True if `FR` converts to `R`, honoring the noexcept requirement `NX`.
template <bool NX, typename R, typename FR>
...
/// Holds the result type of invoking `F` with `A...`, or is empty if ill-formed.
template <typename Void, typename F, typename... A>
struct invoke_result {};

/// Specialization exposing the result type when the invocation is well-formed.
template <typename F, typename... A>
...
/// True if a nothrow invocation of `F` yields a result convertible to `R`.
template <typename Void, typename R, typename F, typename... A>
...
struct invoke_traits : detail::invoke_traits_base<I> {
 public:
  /// The invocable type these traits describe.
  using invoke_type = I;
  ...
  /// Holds the result type of invoking `invoke_type` with `A...`.
  template <typename... A>
  using invoke_result = invoke_detail::invoke_result<void, I, A...>;
  /// The result type of invoking `invoke_type` with `A...`.
  template <typename... A>
  using invoke_result_t = invoke_detail::invoke_result_t<I, A...>;
  /// True if `invoke_type` is invocable with `A...`.
  template <typename... A>
  inline static constexpr bool is_invocable_v =
      invoke_detail::is_invocable_v<void, I, A...>;
  /// Trait testing whether `invoke_type` is invocable with `A...`.
  template <typename... A>
  struct is_invocable //
      : std::bool_constant<invoke_detail::is_invocable_v<void, I, A...>> {};
  /// True if invoking `invoke_type` with `A...` yields a result convertible to `R`.
  template <typename R, typename... A>
  inline static constexpr bool is_invocable_r_v =
      invoke_detail::is_invocable_r_v<void, R, I, A...>;
  /// Trait testing whether the result of invoking `invoke_type` is convertible to `R`.
  template <typename R, typename... A>
  struct is_invocable_r //
      : std::bool_constant< //
            invoke_detail::is_invocable_r_v<void, R, I, A...>> {};
  /// True if `invoke_type` is nothrow-invocable with `A...`.
  template <typename... A>
  inline static constexpr bool is_nothrow_invocable_v =
      invoke_detail::is_nothrow_invocable_v<void, I, A...>;
  /// Trait testing whether `invoke_type` is nothrow-invocable with `A...`.
  template <typename... A>
  struct is_nothrow_invocable //
      : std::bool_constant<
            invoke_detail::is_nothrow_invocable_v<void, I, A...>> {};
  /// True if a nothrow invocation of `invoke_type` yields a result convertible to `R`.
  template <typename R, typename... A>
  inline static constexpr bool is_nothrow_invocable_r_v =
      invoke_detail::is_nothrow_invocable_r_v<void, R, I, A...>;
  /// Trait testing the nothrow invocation of `invoke_type` with result convertible to `R`.
  template <typename R, typename... A>
  struct is_nothrow_invocable_r //
      : std::bool_constant<
            invoke_detail::is_nothrow_invocable_r_v<void, R, I, A...>> {};
...
struct invoke_first_match : private Invoker... {
 ...
 public:
  /// Invokes the first invoker that matches the given arguments.
  ///
  /// \param a the arguments forwarded to the matching invoker
  /// \returns the result of the first matching invoker
 ...
 *
 *  \param classname the name of the generated invoker type
 *  \param funcname the free-invocable name to bind
 */
 ...
 *
 *  \param classname the name of the generated invoker type
 *  \param funcpath the qualified free-invocable name to bind
 */
 ...
 *
 *  \param name the base name for the generated invoker type and variable
 *  \param funcpath the qualified free-invocable name to bind
 */
 ...
 *
 *  \param funcpath the qualified free-invocable name to invoke
 */
 ...
 *
 *  \param classname the name of the generated invoker type
 *  \param membername the member-invocable name to bind
 */
...
struct invoke_member_wrapper_fn : private F {
  /// Constructs the wrapper from an underlying callable.
  ///
...
/// Deduction guide for `invoke_member_wrapper_fn`.
template <typename F>
 ...
 *
 *  \param membername the member name to invoke on the object
 */
 ...
 *
 *  \param classname the name of the generated invoker type template
 *  \param membername the static-member-invocable name to bind
 */
 ...
 *
 *  \param classname the name of the generated accessor type
 *  \param membername the data-member name to access
 */
...
/// Poison-pill declaration ensuring ADL lookup of `tag_invoke`.
void tag_invoke();

/// Function object dispatching to an ADL `tag_invoke` call.
struct tag_invoke_fn {
  /// Invokes `tag_invoke(tag, args...)` via argument-dependent lookup.
  ///
  /// \param tag the customization-point tag selecting the overload
  /// \param args the arguments forwarded to the `tag_invoke` call
  /// \returns the result of the ADL `tag_invoke` call
...
/// The result type of `tag_invoke(Tag, Args...)`.
template <typename Tag, typename... Args>
using tag_invoke_result_t = decltype(tag_invoke(
    static_cast<Tag && (*)() noexcept>(nullptr)(),
    static_cast<Args && (*)() noexcept>(nullptr)()...));

/// Detects a valid `tag_invoke(Tag, Args...)` call; preferred overload.
///
/// \param rank overload-rank disambiguator preferring this overload
/// \returns std::true_type if the `tag_invoke` call is well-formed
template <typename Tag, typename... Args>
auto try_tag_invoke(int rank) noexcept(
...
/// Fallback detector when `tag_invoke(Tag, Args...)` is not well-formed.
///
/// \returns std::false_type
template <typename Tag, typename... Args>
std::false_type try_tag_invoke(...) noexcept(false);

/// Defers instantiation of `T<Args...>` to its member type alias.
template <template <typename...> class T, typename... Args>
struct defer {
  /// The deferred instantiation `T<Args...>`.
  using type = T<Args...>;
};
...
/// Defines the `tag_invoke` customization-point object.
///
