...
struct ApplyInvoke {
 ...
 public:
  ...
  /// \param t the tuple whose elements are passed to `f`
  /// \returns the result of invoking `f` with the elements of `t`
 ...
 * Returns a std::tuple<int&, int&>
 *
 * \param tuple the tuple whose elements are forwarded
 * \returns a tuple of references to the elements of `tuple`
 */
template <typename Tuple>
 ...
 * \returns a callable that unpacks a tuple and forwards its elements to `f`
 */
template <class F>
auto uncurry(F&& f)
