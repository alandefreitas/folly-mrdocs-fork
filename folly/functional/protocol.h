...
/// Function object evaluating the empty-function protocol for a value.
struct match_empty_function_protocol_fn {
 ...
 public:
  ...
  /// \returns true if `t` compares equal to nullptr
  template <typename T, std::enable_if_t<cx_matches_v<T>, int> = 0>
  constexpr bool operator()(T const& t) const noexcept {
    return static_cast<bool>(t == nullptr);
  }
};
/// Callable object that evaluates the empty-function protocol for a value.
