 ...
 */
struct TcpInfo {
  /// Identifiers for known TCP congestion control algorithms.
  enum class CongestionControlName {
    UNKNOWN = 0, ///< Unknown or unrecognized algorithm.
    CUBIC = 1, ///< CUBIC.
    BIC = 2, ///< BIC.
    DCTCP = 3, ///< Data Center TCP.
    DCTCP_RENO = 4, ///< DCTCP with Reno fallback.
    BBR = 5, ///< BBR.
    RENO = 6, ///< Reno.
    DCTCP_CUBIC = 7, ///< DCTCP with CUBIC fallback.
    VEGAS = 8, ///< Vegas.
    NumCcTypes, ///< Number of congestion control algorithm identifiers.
  };
  ...
  struct LookupOptions {
    ...
    /// Default-constructs lookup options with all fields disabled.
    LookupOptions() {}
  };
  ...
  class IoctlDispatcher {
   public:
    /// Returns the default IoctlDispatcher instance.
    ///
    /// \returns The process-wide default IoctlDispatcher.
    static IoctlDispatcher* getDefaultInstance();
    /// Performs an ioctl call on a file descriptor.
    ///
    /// \param fd The file descriptor.
    /// \param request The ioctl request code.
    /// \param argp The request argument.
    /// \returns The ioctl result, or -1 on error.
    virtual int ioctl(int fd, unsigned long request, void* argp);

   protected:
    /// Default-constructs an IoctlDispatcher.
    IoctlDispatcher() = default;
    /// Destroys the IoctlDispatcher.
   ...
   *                          facilitates mocking during unit tests.
   * @return TcpInfo on success, or an error code on failure.
   */
   ...
   *
   * @return The minimum round-trip time, or folly::none if unavailable.
   */

  Optional<std::chrono::microseconds> minrtt() const;
  /// Returns the smoothed round-trip time.
  ///
  /// \returns The smoothed RTT, or folly::none if unavailable.
  Optional<std::chrono::microseconds> srtt() const;

  /// Returns the total number of bytes sent.
  ///
  /// \returns The bytes sent, or folly::none if unavailable.
  Optional<uint64_t> bytesSent() const;
  /// Returns the total number of bytes received.
  ///
  /// \returns The bytes received, or folly::none if unavailable.
  Optional<uint64_t> bytesReceived() const;
  /// Returns the total number of bytes retransmitted.
  ///
  /// \returns The bytes retransmitted, or folly::none if unavailable.
  Optional<uint64_t> bytesRetransmitted() const;
  /// Returns the number of bytes queued but not yet sent.
  ///
  /// \returns The bytes not sent, or folly::none if unavailable.
  Optional<uint64_t> bytesNotSent() const;
  /// Returns the total number of bytes acknowledged.
  ///
  /// \returns The bytes acknowledged, or folly::none if unavailable.
  Optional<uint64_t> bytesAcked() const;

  /// Returns the total number of packets sent.
  ///
  /// \returns The packets sent, or folly::none if unavailable.
  Optional<uint64_t> packetsSent() const;
  /// Returns the number of sent packets that carried data.
  ///
  /// \returns The data packets sent, or folly::none if unavailable.
  Optional<uint64_t> packetsWithDataSent() const;
  /// Returns the total number of packets received.
  ///
  /// \returns The packets received, or folly::none if unavailable.
  Optional<uint64_t> packetsReceived() const;
  /// Returns the number of received packets that carried data.
  ///
  /// \returns The data packets received, or folly::none if unavailable.
  Optional<uint64_t> packetsWithDataReceived() const;
  /// Returns the total number of packets retransmitted.
  ///
  /// \returns The packets retransmitted, or folly::none if unavailable.
  Optional<uint64_t> packetsRetransmitted() const;
  /// Returns the number of packets currently in flight.
  ///
  /// \returns The packets in flight, or folly::none if unavailable.
  Optional<uint64_t> packetsInFlight() const;
  /// Returns the total number of packets delivered.
  ///
  /// \returns The packets delivered, or folly::none if unavailable.
  Optional<uint64_t> packetsDelivered() const;
  /// Returns the number of delivered packets carrying CE (congestion) marks.
  ///
  /// \returns The CE-marked packets delivered, or folly::none if unavailable.
  Optional<uint64_t> packetsDeliveredWithCEMarks() const;

  /// Returns the congestion window size in packets.
  ///
  /// \returns The congestion window in packets, or folly::none if unavailable.
  Optional<uint64_t> cwndInPackets() const;
  /// Returns the congestion window size in bytes.
  ///
  /// \returns The congestion window in bytes, or folly::none if unavailable.
  Optional<uint64_t> cwndInBytes() const;
  /// Returns the slow-start threshold.
  ///
  /// \returns The slow-start threshold, or folly::none if unavailable.
  Optional<uint64_t> ssthresh() const;
  /// Returns the maximum segment size.
  ///
  /// \returns The maximum segment size, or folly::none if unavailable.
  Optional<uint64_t> mss() const;

  /// Returns the delivery rate in bits per second.
  ///
  /// \returns The delivery rate, or folly::none if unavailable.
  Optional<uint64_t> deliveryRateBitsPerSecond() const;
  /// Returns the delivery rate in bytes per second.
  ///
  /// \returns The delivery rate, or folly::none if unavailable.
  Optional<uint64_t> deliveryRateBytesPerSecond() const;
  /// Returns whether the delivery rate is application-limited.
  ///
  /// \returns True if application-limited, or folly::none if unavailable.
   ...
   * folly::none if the underlying field is unavailable.
   *
   * @return The raw congestion control algorithm name, or folly::none.
   */
  Optional<std::string> ccNameRaw() const;
  /// Returns the congestion control algorithm as an enum value.
  ///
  /// \returns The algorithm enum, or folly::none if unavailable.
  Optional<CongestionControlName> ccNameEnum() const;
  /// Returns the congestion control algorithm enum as a string.
  ///
  /// \returns The algorithm name string, or folly::none if unavailable.
  Optional<folly::StringPiece> ccNameEnumAsStr() const;

  /// Returns the BBR estimated bandwidth in bits per second.
  ///
  /// \returns The bandwidth, or folly::none if unavailable.
  Optional<uint64_t> bbrBwBitsPerSecond() const;
  /// Returns the BBR estimated bandwidth in bytes per second.
  ///
  /// \returns The bandwidth, or folly::none if unavailable.
  Optional<uint64_t> bbrBwBytesPerSecond() const;
  /// Returns the BBR minimum round-trip time.
  ///
  /// \returns The minimum RTT, or folly::none if unavailable.
  Optional<std::chrono::microseconds> bbrMinrtt() const;
  /// Returns the BBR pacing gain.
  ///
  /// \returns The pacing gain, or folly::none if unavailable.
  Optional<uint64_t> bbrPacingGain() const;
  /// Returns the BBR congestion window gain.
  ///
  /// \returns The congestion window gain, or folly::none if unavailable.
   ...
   * folly::none if the underlying field is unavailable.
   *
   * @return Send buffer bytes in use, or folly::none if unavailable.
   */

  Optional<size_t> sendBufInUseBytes() const;
  /// Returns the receive buffer bytes currently in use.
  ///
  /// \returns Bytes in use, or folly::none if unavailable.
  Optional<size_t> recvBufInUseBytes() const;

  /// Sets the send buffer bytes currently in use.
  ///
  /// \param numBytes The number of bytes in use.
  void setSendBufInUseBytes(int numBytes) { maybeSendBufInUseBytes = numBytes; }
  /// Sets the receive buffer bytes currently in use.
  ///
  /// \param numBytes The number of bytes in use.
 ...
 public:
  /// Platform-specific tcp_info struct type.
  using tcp_info = folly::detail::tcp_info;

  /// Default-constructs an empty TcpInfo.
  TcpInfo() = default;
  /// Constructs a TcpInfo wrapping a platform tcp_info struct.
  ///
  /// \param tInfo The tcp_info struct to wrap.
   ...
   *
   * @param field Pointer-to-member of the requested tcp_info field.
   * @return Pointer to the field if available, else nullptr.
   */
   ...
   *
   * @param field Pointer-to-member of the requested tcp_info field.
   * @return The field value if available, else folly::none.
   */
  template <typename T1>
