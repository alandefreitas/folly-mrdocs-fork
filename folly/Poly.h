...
/// A list of member pointers describing the members of a `Poly` interface.
template <auto... Ps>
struct PolyMembers {};
...
struct PolyExtends : virtual I... {
  ...
  template <class Base>
  struct Interface : Base {
    /// Default-construct the interface.
    Interface() = default;
    using Base::Base;
  };

  /// The (empty) member list contributed by this extension.
  template <class...>
 ...
 *     };
 *
 * \param _this The interface object to dispatch the call on.
 * \param as The arguments forwarded to the member.
 * \returns The result of invoking the N-th member.
 * \sa PolySelf
 */
 ...
 * Throws `BadPolyAccess` if `that` is empty.
 * Throws `BadPolyCast` if `that` does not store or refer to an object of type
 *        `T`.
 */
 ...
 * `typeid(void)`.
 *
 * \param that The `Poly` object to inspect.
 * \returns The `std::type_info` of the stored object, or `typeid(void)`.
 */
template <class I>
 ...
 * `std::move`.
 *
 * \param that The `Poly` object to move from.
 * \returns An rvalue-reference `Poly` referring to `that`.
 */
template <class I>
