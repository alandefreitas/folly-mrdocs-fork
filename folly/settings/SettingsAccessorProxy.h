...
class ISettingsAccessorProxy {
 public:
  ...
  /// Sets a setting to a new value from its string representation.
  ///
  /// \param settingName Name of the setting to update.
  /// \param newValue New value to assign, as a string.
  /// \param reason Reason recorded for the update.
  ...
  ///
  /// \param flag Name of the setting to reset.
...
class SettingsAccessorProxy : public ISettingsAccessorProxy {
 public:
  /// Maps an alias name to the fully qualified setting name it refers to.
  using SettingAliases = std::unordered_map<std::string, std::string>;
  /// Maps a fully qualified setting name to its static metadata.
  using SettingMetaMap = std::unordered_map<std::string, SettingMetadata>;

  /**
   * Constructs a proxy that applies setting changes to the given snapshot.
   *
   ...
   *
   * \returns A const reference to the map from setting name to metadata.
   */
  ...
  /// Returns the settings metadata map by moving it out of this proxy.
  ///
  /// \returns The settings metadata map.
  SettingMetaMap getSettingsMetadata() && { return std::move(settingsMeta_); }

  /// Returns the metadata for a single flag, if it is a registered setting.
  ///
  /// \param flag The command line flag name to look up.
  /// \returns The setting metadata, or an empty optional if not found.
  ...
  /// Returns the snapshot associated with this proxy.
  ///
  /// \returns A reference to the associated settings snapshot.
  Snapshot& snapshot() noexcept { return snapshot_; }

 protected:
  /// Looks up the metadata entry for a fully qualified setting name.
  ///
  /// \param flag The fully qualified setting name to look up.
  /// \returns An iterator into the metadata map, or the end iterator.
  virtual SettingMetaMap::const_iterator findSettingMeta(
      std::string_view flag) const;

  /// Default project name used to expand unqualified flag names.
  std::string project_;
  /// Reverse map of alternative names for settings.
  SettingAliases aliases_;
  /// Map from fully qualified setting name to its metadata.
