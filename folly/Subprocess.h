...
class ProcessReturnCode {
 public:
  /// Lifecycle state of the process wrapped by a ProcessReturnCode.
  enum State {
    /// Process hasn't been started; a default-initialized or moved-out
    /// ProcessReturnCode.
    NOT_STARTED,
  ...
  /// Creates a ProcessReturnCode in the NOT_STARTED state.
  /// \returns A return code whose state() is NOT_STARTED.
  static ProcessReturnCode makeNotStarted() {
    return ProcessReturnCode(RV_NOT_STARTED);
  }

  /// Creates a ProcessReturnCode in the RUNNING state.
  /// \returns A return code whose state() is RUNNING.
  static ProcessReturnCode makeRunning() {
    return ProcessReturnCode(RV_RUNNING);
  }

  /// Creates a ProcessReturnCode from a raw wait() status.
  /// \param status The raw status as returned by wait().
  /// \returns A return code reflecting the given status.
  static ProcessReturnCode make(int status);

  /// Default-constructs a return code in the NOT_STARTED state.
  ///
  /// Subprocess::returnCode() will never produce this value.
  ProcessReturnCode() : rawStatus_(RV_NOT_STARTED) {}

  /// Copy-constructs a return code. ProcessReturnCode is trivially copyable.
  /// \param p The return code to copy.
  ProcessReturnCode(const ProcessReturnCode& p) = default;
  /// Copy-assigns a return code. ProcessReturnCode is trivially copyable.
  /// \param p The return code to copy.
  /// \returns A reference to this return code.
  ProcessReturnCode& operator=(const ProcessReturnCode& p) = default;
  /// Move-constructs a return code, leaving the source in the NOT_STARTED
  /// state.
  ///
  /// The moved-out state must not be RUNNING, or ~Subprocess() will abort.
  /// \param p The return code to move from.
  ProcessReturnCode(ProcessReturnCode&& p) noexcept;
  /// Move-assigns a return code, leaving the source in the NOT_STARTED state.
  /// \param p The return code to move from.
  /// \returns A reference to this return code.
  ...
  /// Whether the process has not been started.
  /// \returns True if state() is NOT_STARTED.
   ...
   *
   * \returns True if the process exited with a zero exit status.
   */
   ...
   *
   * \param expected The state this return code is required to be in.
   */
...
class FOLLY_EXPORT CalledProcessError : public SubprocessError {
 public:
  ...
  /// \param rc The return code of the process that failed.
...
class FOLLY_EXPORT SubprocessSpawnError : public SubprocessError {
 public:
  ...
  /// \param executable The executable that failed to spawn.
  /// \param errCode An internal code identifying the failed step.
  /// \param errnoValue The errno value captured at the point of failure.
...
class Subprocess {
 public:
  /// Duration type used for the subprocess wait and terminate timeouts.
  using TimeoutDuration = std::chrono::milliseconds;

  // removed CLOSE = -1
  /// fd() action: open a pipe, using PIPE_IN for stdin and PIPE_OUT for
  /// stdout/stderr; an error for other file descriptors.
  static const int PIPE = -2;
  /// fd() action: open an input pipe to the child (used for stdin).
  static const int PIPE_IN = -3;
  /// fd() action: open an output pipe from the child (used for stdout/stderr).
  static const int PIPE_OUT = -4;
  /// fd() action: redirect the file descriptor to /dev/null.
  static const int DEV_NULL = -5;
  /// fd() action: inherit the file descriptor without setting close-on-exec.
  ...
  class Options {
   ...
   public:
    /// Minimum buffer size that can hold the decimal representation of a
    /// pid_t, including the terminating null.
    ...
    /// Constructs an Options object with default settings.
     ...
     *
     * \param fd The child file descriptor whose action is being set.
     * \param action Another file descriptor to dup2() onto fd, or one of
     * CLOSE, PIPE_IN, PIPE_OUT, PIPE, DEV_NULL, NO_CLOEXEC.
     * \returns A reference to this Options object.
     */
     ...
     *
     * \param action The action to apply to standard input.
     * \returns A reference to this Options object.
     */
     ...
     *
     * \param action The action to apply to standard output.
     * \returns A reference to this Options object.
     */
     ...
     *
     * \param action The action to apply to standard error.
     * \returns A reference to this Options object.
     */
    Options& stderrFd(int action) { return fd(STDERR_FILENO, action); }

    /// Opens a pipe to the child's standard input.
    /// \returns A reference to this Options object.
    Options& pipeStdin() { return fd(STDIN_FILENO, PIPE_IN); }
    /// Opens a pipe from the child's standard output.
    /// \returns A reference to this Options object.
    Options& pipeStdout() { return fd(STDOUT_FILENO, PIPE_OUT); }
    /// Opens a pipe from the child's standard error.
    /// \returns A reference to this Options object.
     ...
     *
     * \param dir The directory to change into before exec.
     * \returns A reference to this Options object.
     */
     ...
     *
     * \param timeout How long to wait after SIGTERM before sending SIGKILL.
     * \returns A reference to this Options object.
     */
     ...
     *   similar mechanisms.
     *
     * \param val True to allow destruction while the child is running.
     * \returns A reference to this Options object.
     */
    ...
    /// Sets the real user ID of the child process before exec.
    /// \param uid The user ID to set.
    /// \param errout Optional location to report failure; if null, failure
    /// is fatal.
    /// \returns A reference to this Options object.
    ...
    /// Sets the real group ID of the child process before exec.
    ...
    /// Sets the effective user ID of the child process before exec.
    /// \param uid The effective user ID to set.
    /// \param errout Optional location to report failure; if null, failure
    /// is fatal.
    /// \returns A reference to this Options object.
    ...
    /// Sets the effective group ID of the child process before exec.
    /// \param gid The effective group ID to set.
    /// \param errout Optional location to report failure; if null, failure
    /// is fatal.
    /// \returns A reference to this Options object.
    ...
    /// Sets the signal mask to install in the child before exec.
    /// \param sigmask The signal mask to apply.
    /// \returns A reference to this Options object.
    ...
    /// \param buf The buffer to receive the child's PID; must outlive the
    /// spawn.
    /// \returns A reference to this Options object.
    Options& addPrintPidToBuffer(std::span<char> buf);

    /// Sets a resource limit (setrlimit) for the child process.
    /// \param resource The resource to limit (an RLIMIT_* constant).
    /// \param limit The soft and hard limit values to apply.
    /// \param errout Optional location to report failure; if null, failure
    /// is fatal.
    /// \returns A reference to this Options object.
    Options& addRLimit(
  ...
  /// Copy construction is deleted; Subprocess is not copyable.
  /// \param other The Subprocess to copy from.
  Subprocess(const Subprocess& other) = delete;
  /// Copy assignment is deleted; Subprocess is not copyable.
  /// \param other The Subprocess to copy from.
  /// \returns A reference to this Subprocess.
  Subprocess& operator=(const Subprocess& other) = delete;
  /// Move-constructs a Subprocess, transferring ownership of the child.
  /// \param other The Subprocess to move from.
  Subprocess(Subprocess&& other) = default;
  /// Move-assigns a Subprocess, transferring ownership of the child.
  /// \param other The Subprocess to move from.
  /// \returns A reference to this Subprocess.
   ...
   *
   * \param argv The argument vector; argv[0] must be listed.
   * \param options Options controlling file descriptors and spawn behavior.
   * \param executable The actual executable to run, or null to use argv[0].
   * \param env name=value strings for the child's environment, or null to
   * inherit the parent's; must be null if options.usePath is set.
   */
  explicit Subprocess(
  ...
  ///
  /// The behavior when the child is still running is controlled by the
  /// Options destruction settings.
  ~Subprocess();
   ...
   *
   * \param pid The process ID of an immediate child process.
   * \returns A Subprocess wrapping the given child process.
   */
   ...
   *
   * \returns The child's process ID, or -1 if unavailable.
   */
  pid_t pid() const;
   ...
   * Subprocess instance.
   *
   * \param ru Optional location to receive the child's resource usage.
   * \returns The child's exit status, or RUNNING if still running.
   */
   ...
   * std::logic_error.
   *
   * \returns True if the process exited with status 0; false if still
   * running.
   */
  bool pollChecked();
   ...
   *
   * \param ru Location to receive the child's resource usage.
   * \returns The child's exit status.
   */
   ...
   *
   * \param timeout The maximum time to wait for the process to terminate.
   * \returns The child's exit status, or running() if waiting timed out.
   */
   ...
   * signals are below.
   *
   * \param signal The signal number to send to the child.
   */
  void sendSignal(int signal);
  /// Send SIGTERM to the child, requesting graceful termination.
   ...
   *
   * \param waitTimeout The maximum time to wait before falling back to
   * terminateOrKill.
   * \param sigtermTimeout The time to wait after SIGTERM before sending SIGKILL.
   * \returns The child's exit status.
   */
   ...
   *
   * \param input The data to write to the child's stdin pipe.
   * \returns A pair of buffers holding the data read from stdout and stderr.
   */
  std::pair<IOBufQueue, IOBufQueue> communicateIOBuf(
      IOBufQueue input = IOBufQueue());

  /// Communicate with the child, buffering input and output as strings.
  ///
  /// This is the string-based counterpart to communicateIOBuf(); it buffers
  /// the entire input and output in memory.
  ///
  /// \param input The data to write to the child's stdin pipe.
  /// \returns A pair of strings holding the data read from stdout and stderr.
  ...
  /// Communicate with the child using custom read and write callbacks.
  ///
  /// The callbacks are invoked as data becomes available on pipes from the
  /// child and as pipes to the child become writable, until all pipes are
  /// closed. See the semantics described above for the requirements callbacks
  /// must satisfy.
  ///
  /// \param readCallback Called when data is available on a pipe from the
  /// child; return true to close that pipe.
  /// \param writeCallback Called when a pipe to the child is writable; return
  /// true to close that pipe.
  ...
  class ReadLinesCallback {
   ...
   public:
    /// Construct a ReadLinesCallback wrapping a per-line callback.
    ///
    /// \param fdLineCb Callback invoked with (fd, line) for each delimited
    /// piece read from the child.
    /// \param maxLineLength Maximum length of a line before it is split into
    /// chunks; 0 means no limit.
    /// \param delimiter The character used to split the stream into lines.
    /// \param bufSize The size of the read buffer, in bytes.
    ...
    /// Read available data from a child pipe and dispatch complete lines.
    ///
    /// \param pfd The parent file descriptor to read from.
    /// \param cfd The child file descriptor identifying the stream.
    /// \returns True if the pipe should be closed (end-of-file or the callback
    /// asked to stop); false to keep reading.
  ...
  /// Construct a ReadLinesCallback, deducing the callback type.
  ///
  /// \param fdLineCb Callback invoked with (fd, line) for each delimited piece
  /// read from the child.
  /// \param maxLineLength Maximum length of a line before it is split into
  /// chunks; 0 means no limit.
  /// \param delimiter The character used to split the stream into lines.
  /// \param bufSize The size of the read buffer, in bytes.
  /// \returns A ReadLinesCallback wrapping the given callback.
  template <class Callback>
   ...
   *
   * \param childFd The child file descriptor identifying the pipe.
   * \param enabled Whether to enable or disable notifications for that pipe.
   */
   ...
   *
   * \param childFd The child file descriptor identifying the pipe.
   * \returns True if notifications for that pipe are enabled.
   */
   ...
   *
   * \param childFd The child file descriptor identifying the pipe to close.
   */
   ...
   *
   * \param childFd The child file descriptor identifying the pipe.
   * \returns The corresponding parent file descriptor.
   */
  int parentFd(int childFd) const {
    return pipes_[findByChildFd(childFd)].pipe.fd();
  }
  /// Get the parent file descriptor for the child's standard input.
  /// \returns The parent file descriptor for the child's standard input.
  int stdinFd() const { return parentFd(0); }
  /// Get the parent file descriptor for the child's standard output.
  /// \returns The parent file descriptor for the child's standard output.
  int stdoutFd() const { return parentFd(1); }
  /// Get the parent file descriptor for the child's standard error.
  /// \returns The parent file descriptor for the child's standard error.
   ...
   */
  struct ChildPipe {
    /// Construct a ChildPipe from a child file descriptor and its parent pipe.
    /// \param fd The child file descriptor identifying the stream.
    /// \param ppe The parent-side pipe that owns the parent file descriptor.
    ChildPipe(int fd, folly::File&& ppe) : childFd(fd), pipe(std::move(ppe)) {}
    /// The child file descriptor identifying the stream.
    int childFd;
    /// The parent-side pipe; owns the parent file descriptor.
    folly::File pipe;
  };
  /// Take ownership of the child's pipes, detaching them from this Subprocess.
  ///
  /// After this call the Subprocess no longer knows about these pipes and the
  /// caller is responsible for managing their lifetimes.
  ///
  /// \returns The child's pipes, each paired with its child file descriptor.
