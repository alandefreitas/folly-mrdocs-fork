...
class SingletonVault {
 public:
  /// Whether singletons may be created before `registrationComplete()`.
  enum class Type {
  ...
  struct ScopedExpunger {
    /// The vault whose singletons are cleared on construction and destruction.
    SingletonVault* vault;
    /// Construct the expunger and clear the vault.
    /// @param v The vault to clear.
    explicit ScopedExpunger(SingletonVault* v) : vault(v) { expunge(); }
    /// Destroy the expunger and clear the vault again.
    ~ScopedExpunger() { expunge(); }
    /// Destroy then re-enable all singletons in the vault.
    void expunge() {
  ...
  /// Return the default vault type used when none is specified.
  /// @return The default `Type` for a newly constructed vault.
  static Type defaultVaultType();

  /// Construct a vault with the given type.
  /// @param type Whether the vault is `Strict` or `Relaxed`.
  explicit SingletonVault(Type type = defaultVaultType()) noexcept;

  /// Destructor is only called by unit tests to check destroyInstances.
  ~SingletonVault();

  /// Teardown function used to destroy a singleton instance.
  using TeardownFunc = std::function<void(void*)>;
  /// Create function used to build a singleton instance.
  using CreateFunc = std::function<void*(void)>;

  /// Register a singleton with the vault.
  ///
  ...
  /// functions.
  ///
  /// @param entry The singleton holder to register.
   ...
   * for more info.
   *
   * @param entry The singleton holder to eagerly initialize.
   */
  void addEagerInitSingleton(detail::SingletonHolderBase* entry);

  /// Mark a singleton for eager initialization when singletons reenable.
  /// @param entry The singleton holder to eagerly initialize on reenable.
   ...
   *
   * @param exe The executor used to schedule the initializations.
   * @param done Optional baton posted once all initialization completes.
   */
  ...
  /// @return The number of registered singletons.
   ...
   *
   * @return `true` if eager initialization ran and completed.
   */
  bool eagerInitComplete() const;

  /// For testing; how many living singletons we have.
  /// @return The number of singletons with a live instance.
  ...
  /// called.
  ///
  /// @return A token triggered when singleton destruction begins.
  ...
  /// Set the timeout used when shutting down singletons.
  ...
  /// Append a message to the shutdown log.
  /// @param message The message to record.
  void addToShutdownLog(std::string message);

  /// Fire the shutdown timer, aborting the process.
  [[noreturn]] void fireShutdownTimer();

  /// Set whether using a singleton after fork should fail.
  /// @param failOnUseAfterFork Whether use after fork should fail.
  void setFailOnUseAfterFork(bool failOnUseAfterFork) {
    failOnUseAfterFork_ = failOnUseAfterFork;
  }

  /// Return whether singleton creation is currently disabled.
  /// @return `true` if the vault is disabled.
...
class Singleton {
 public:
  ...
  using CreateFunc = std::function<T*(void)>;
  /// Teardown function used to destroy the singleton instance.
  using TeardownFunc = std::function<void(T*)>;
   ...
   * Deprecated in favor of try_get().
   *
   * @return A weak_ptr to the singleton instance.
   */
   ...
   *
   * @return A ReadMostlySharedPtr to the singleton instance.
   */
   ...
   *
   * @param f The callback to invoke with the singleton instance pointer.
   ...
   * type.
   *
   * @param _  Unused null placeholder selecting default construction.
  ...
  /**
   * Create a singleton using the given create and teardown functions.
   *
   ...
   * regular singletons.
   *
   * @param c  Unused null placeholder selecting default construction.
   * @param t  The teardown function to use (in lieu of delete).
   */
  ...
  /// Inject a mock singleton built with the given create and teardown
...
class LeakySingleton {
 public:
  ...
  /// @param createFunc The create function to use (in lieu of new).
  ...
  /// Inject a mock singleton using the default constructor of `T`, for testing.
  ///
  /// \param c Unused null pointer selecting the default-constructed mock.
  static void make_mock(std::nullptr_t c = nullptr) {
    (void)c;
    make_mock([]() { return new T; });
  }

  /// Inject a mock singleton built with the given create function, for testing.
  /// @param createFunc The create function to use (in lieu of new).
