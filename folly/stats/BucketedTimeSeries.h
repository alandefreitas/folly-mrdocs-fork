...
class BucketedTimeSeries {
 public:
  ...
  /// The bucket type used to aggregate values.
   ...
   *
   * \param numBuckets The number of buckets to divide the duration into.
   * \param duration The amount of time to track data for.
   */
   ...
   *
   * \param theFirstTime The time of the first data point.
   * \param theLatestTime The time of the latest data point.
   * \param maxDuration The maximum duration of data to track.
   * \param bucketsList The previously saved buckets to reconstruct from.
   */
  BucketedTimeSeries(
   ...
   * \returns true on success, or false if now was older than the tracked time window.
   */
   ...
   *
   * \param now The time to update the container to.
   * \returns The current bucket index after the update.
   */
   ...
   * See latestTime()
   *
   * \returns The latest time observed since the last clear or construction.
   */
   ...
   * duration()).
   *
   * \returns The time of the earliest data point stored in this timeseries.
   */
   ...
   * BucketedTimeSeries.
   *
   * \returns The maximum duration of data that can be tracked.
   */
   ...
   *
   * \returns true if this timeseries stores data for all-time.
   */
   ...
   * clear().
   *
   * \returns true if no data points have been observed since the last clear.
   */
  bool empty() const {
   ...
   *
   * \returns The oldest time observed since the last clear or construction.
   */
   ...
   *
   * \returns The amount of time tracked by this timeseries.
   */
   ...
   * \returns The amount of time tracked between start and end.
   */
   ...
   * make sure you are not reading stale data.
   *
   * \returns The sum of all data points currently tracked.
   */
   ...
   * make sure you are not reading stale data.
   *
   * \returns The number of data points currently tracked.
   */
   ...
   * make sure you are not reading stale data.
   *
   * \returns The average value (sum divided by count).
   */
   ...
   * make sure you are not reading stale data.
   *
   * \returns The sum divided by the elapsed time.
   */
   ...
   *
   * \returns The count divided by the elapsed time.
   */
  template <typename ReturnType = double, typename Interval = Duration>
   ...
   * \param start The inclusive start of the time period.
   * \param end The exclusive end of the time period.
   * \returns The estimated sum per interval during the period.
   */
  template <typename ReturnType = double, typename Interval = Duration>
   ...
   * \param start The inclusive start of the time period.
   * \param end The exclusive end of the time period.
   * \returns The estimated count of data points per interval during the period.
   */
  template <typename ReturnType = double, typename Interval = Duration>
   ...
   *
   * \param now The latest time to include in the total.
   * \returns The sum and count of the tracked buckets up to now.
   */
   ...
   *
   * \param fn The function to invoke for each bucket.
   */
   ...
   * \returns The index of the bucket containing the specified time.
   */
   ...
   * This method may not be called for all-time data.
   *
   * \param idx The index of the bucket to return.
   ...
   *
   * \param time The time to look up.
   * \param bucketIdx Output parameter for the bucket index.
   * \param bucketStart Output parameter for the bucket start time.
   * \param nextBucketStart Output parameter for the next bucket's start time.
   */
  void getBucketInfo(
      TimePoint time,
      size_t* bucketIdx,
  ...
  /// Add the value the given number of times at the given time.
  ///
  /// \param now The time, relative to the clock epoch, to add the value at.
  /// \param val The value to add.
  /// \param times The number of times to add the value.
  ...
  /// Add an aggregated total representing the given number of samples.
  ///
  ...
  /// Update the container to the specified time.
  ///
   ...
   *
   * \param latestTime The latest time to compute the earliest trackable time from.
   * \returns The earliest time that could be tracked given latestTime.
   */
  TimePoint getEarliestTrackableTimeBy(TimePoint latestTime) const;

  /// Compute a rate from a numerator and an elapsed time.
  ///
  /// \param numerator The value to divide by the elapsed time.
  /// \param elapsedTime The amount of time over which the rate is computed.
  /// \returns The numerator divided by the elapsed time.
  template <typename ReturnType = double, typename Interval = Duration>
