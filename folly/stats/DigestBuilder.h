...
class DigestBuilder {
 public:
  /// Constructs a builder with the given buffer and digest sizes.
  ///
  /// \param bufferSize The number of values to buffer before merging.
  /// \param digestSize The size passed to each digest that is built.
   ...
   *
   * \returns A digest built from the buffered values.
   */
  DigestT build();
