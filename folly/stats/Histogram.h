...
class Histogram {
 public:
  ...
  /// \param min The minimum value at which bucketing begins.
  /// \param max The maximum value at which bucketing ends.
  ...
  ///
  /// \param value The data point to add.
  /// \param nSamples The number of occurrences to add.
  ...
  ///
  /// \param value The data point to remove.
  /// \param nSamples The number of occurrences to remove.
  ...
  ///
  /// \param hist The histogram to subtract. Its bucket layout must match.
  ...
  ///
  /// \param hist The histogram to merge in. Its bucket layout must match.
  ...
  ///
  /// \param hist The histogram to copy from. Its bucket layout must match.
  ...
  /// Returns the min value at which bucketing begins.
  ///
  /// \returns The minimum bucketing value.
  ValueType getMin() const { return buckets_.getMin(); }
  /// Returns the max value at which bucketing ends.
  ///
  /// \returns The maximum bucketing value.
   ...
   *
   * \param idx The bucket index.
   * \returns The minimum threshold for the bucket.
   */
  ValueType getBucketMin(size_t idx) const {
   ...
   *
   * \param idx The bucket index.
   * \returns The maximum threshold for the bucket.
   */
  ValueType getBucketMax(size_t idx) const {
   ...
   *
   * \param pct The desired percentile, as a value from 0.0 to 1.0.
   * \param lowPct Optional out-parameter for the lowest percentile in the
   *        selected bucket.
   * \param highPct Optional out-parameter for the highest percentile in the
   *        selected bucket.
   * \returns The index of the bucket containing the given percentile.
   */
  size_t getPercentileBucketIdx(
   ...
   *
   * \returns A string describing the histogram contents.
   */
   ...
   * \param skipEmptyBuckets Whether to omit buckets with no values.
   */
  ...
  struct CountFromBucket {
    /// Returns the number of values in the given bucket.
    ///
    /// \param bucket The bucket to inspect.
    /// \returns The count of values in the bucket.
  ...
  struct AvgFromBucket {
    /// Returns the average of the values in the given bucket.
    ///
    /// \param bucket The bucket to inspect.
    /// \returns The average value in the bucket, or zero when empty.
