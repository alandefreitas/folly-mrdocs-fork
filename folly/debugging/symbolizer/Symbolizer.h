...
///
/// \param fa The frame array to fill.
...
/// Capture the current stack trace into fa, async-signal-safely.
///
/// \param fa The frame array to fill.
/// \returns true on success, false on failure.
template <size_t N>
...
/// Capture the current stack trace into fa, allocating its context on the heap.
///
/// \param fa The frame array to fill.
/// \returns true on success, false on failure.
template <size_t N>
...
class SafeStackTracePrinter {
 public:
  /// Construct a printer that writes to the given file descriptor.
  ///
   ...
   * makes it easy to skip printing it.
   *
   * \param symbolize Whether to symbolize the frames before printing.
   */
  FOLLY_NOINLINE void printStackTrace(bool symbolize);

  /// Print a string to the underlying printer.
  ///
 ...
 protected:
  /// Symbolize the captured frames and print them.
  virtual void printSymbolizedStackTrace();
  /// Print the captured frames without symbolizing them.
...
/// Stack trace as a string; empty when symbolization is unavailable.
///
/// \param showFullInfo If true, include file names and line numbers.
/// \param skip Skip the top `skip` frames.
/// \returns An empty string on platforms without symbolizer support.
...
///
/// \returns An empty vector on platforms without symbolizer support.
 ...
 *
 * \param fa The frame array to render.
 * \param showFullInfo If true, include file names and line numbers.
 * \param skip Skip the top `skip` frames.
