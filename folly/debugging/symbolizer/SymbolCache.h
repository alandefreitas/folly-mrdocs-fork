...
class SymbolCacheBase {
 public:
   ...
   * \param out Filled with the cached frames on a hit.
   * \returns true if address was cached, false otherwise.
   */
   ...
   *
   * \param address The instruction address to cache.
   * \param frames The symbolized frames to store for address.
   */
 ...
 * \returns A generational symbol cache.
 */
 ...
 *
 * \returns A cache that discards every insertion and never hits.
 */
 ...
 *
 * \param mode The location-info mode the cache is keyed for.
 * \returns The process-wide cache for mode, or a null cache when disabled.
 */
