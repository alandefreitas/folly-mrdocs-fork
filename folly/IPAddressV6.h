...
/// An IPv6 address, with helpers for parsing, formatting, and classification.
class IPAddressV6 {
 public:
  /**
   ...
   */
  enum Type {
    TEREDO, ///< A Teredo tunneling address.
    T6TO4, ///< A 6to4 transition address.
    NORMAL, ///< A normal IPv6 address.
  };
   ...
   */
  enum LinkLocalTag {
    LINK_LOCAL, ///< Tag requesting a link-local address.
  };
   ...
   * byte order.
   *
   * \param bytes The 16-byte binary address, in network byte order.
   ...
   *
   * \param bytes The binary address bytes.
   * \returns The parsed address, or an error if the input is invalid.
   */
   ...
   * \returns The parsed address, or an error if the string is invalid.
   */
   ...
   *
   * \param arpaname The ip6.arpa representation to parse.
   * \returns The parsed IPv6 address.
   ...
   *
   * \returns A ByteRange over the 16 bytes of the address.
   */
   ...
   * Throws `IPAddressFormatException` if the string is not a valid IPv6
   * address.
   */
   ...
   *
   * \param src The 16 bytes of the address, in network byte order.
   */
   ...
   * address
   *
   * \param src The `in6_addr` representation of the address.
   */
   ...
   * address
   *
   * \param src The `sockaddr_in6` representation of the address.
   */
   ...
   *
   * \param tag Tag selecting the link-local constructor.
   * \param mac The MAC address to derive the link-local address from.
   */
   ...
   *
   * \returns The mapped IPv4 address.
   ...
   *
   * \returns True if this is a 6to4 address.
   */
   ...
   *
   * \returns True if this is a Teredo address.
   */
   ...
   *
   * \returns The type classification of this address.
   * @see Type
   */
  Type type() const;
   ...
   *
   * \returns A JSON string representation of the address.
   */
   ...
   * RFC 6890.
   *
   * \returns True if this address is not routable.
   */
   ...
   *
   * \returns True if this address is routable.
   */
   ...
   *
   * \returns The multicast flags of the address.
   */
   ...
   *
   * \returns True if this is a link-local broadcast address.
   */
   ...
   *
   * \returns The underlying `in6_addr` value.
   */
   ...
   *
   * \returns The link-local scope id.
   */
   ...
   * This should always be 0 for IP addresses that are *not* link-local.
   *
   * \param scope The link-local scope id to set.
   */
   ...
   *
   * \returns The address as a `sockaddr_in6` value.
   */
   ...
   *
   * \returns A ByteArray16 holding the address bytes.
   */
   ...
   *
   * \param out The string to append the representation to.
   */
   ...
   *
   * \returns The ip6.arpa representation of the address.
   */
   ...
   * mask) pair
   *
   * \param one The first (address, mask) pair.
   * \param two The second (address, mask) pair.
   * \returns The longest common (address, mask) pair.
   */
   ...
   * @param bitIndex n
   * \returns The value of the nth most significant bit.
   */
   ...
   * @param byteIndex n
   * \returns The value of the nth most significant byte.
   */
   ...
   * @param bitIndex n
   * \returns The value of the nth least significant bit.
   */
   ...
   * @param byteIndex n
   * \returns The value of the nth least significant byte.
   */
   ...
   *
   * \returns A pointer to the 16 address bytes.
   */
 ...
 protected:
   ...
   * by addr.
   *
   * \param addr The two-byte subnet prefix to test against.
   * \param numBits The number of prefix bits to compare.
   * \returns True if the address is in the specified binary subnet.
   */
 ...
 public:
   ...
   * \returns The ordering between the two addresses.
   */
 ...
 * \param result The string to append the representation to.
 */
void toAppend(IPAddressV6 addr, std::string* result);
/// Appends the address representation to an `fbstring`.
///
/// \param addr The address to append.
/// \param result The string to append the representation to.
