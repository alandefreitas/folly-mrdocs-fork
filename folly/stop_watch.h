...
struct custom_stop_watch : private detail::inheritable<Clock> {
 ...
 public:
  /// The clock type used as the time source.
  using clock_type = Clock;
  /// The duration type used when reporting elapsed time.
  using duration = Duration;
  /// The time point type produced by the clock at the reporting duration.
   ...
   *  std::cout << "time elapsed: " << watch.elapsed() << std::endl;
   *
   * @param checkpoint The time to use as the initial checkpoint.
   */
   ...
   * Initializes the stop watch with the current time as its checkpoint.
   *
   * @param clock The clock object to use as the time source.
   */
   ...
   *
   * @param amount The duration to test against the elapsed time.
   * @return True if at least `amount` has elapsed since the last checkpoint.
   */
   ...
   *  };
   *
   * @return The elapsed time since the previous checkpoint.
   */
  duration lap() {
   ...
   * @param amount The duration to test against the elapsed time.
   * @return True if at least `amount` has elapsed since the last checkpoint,
   *         in which case the checkpoint is updated.
   */
  template <typename UDuration>
   ...
   *
   * @return The current checkpoint time point.
   */
