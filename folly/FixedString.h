...
class BasicFixedString : private detail::fixedstring::FixedStringBase {
 ...
 public:
  ...
  using difference_type = std::ptrdiff_t;
  /// A reference to a character element.
  using reference = Char&;
  /// A reference to a constant character element.
  using const_reference = const Char&;
  /// A pointer to a character element.
  using pointer = Char*;
  /// A pointer to a constant character element.
  using const_pointer = const Char*;
  /// A random-access iterator over the characters.
  using iterator = Char*;
  /// A random-access iterator over the constant characters.
  using const_iterator = const Char*;
  /// A reverse iterator over the characters.
  using reverse_iterator = detail::fixedstring::ReverseIterator<Char>;
  /// A reverse iterator over the constant characters.
  ...
  /** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** **
   * Copy construct
   * \param that The source string to copy from.
   * \post `size() == that.size()`
  ...
  /// Deleted constructor from a string and a position.
  ///
  /// This overload is deleted to avoid confusion with the constructor that
  /// takes a `const Char*` and a count.
  ///
   ...
   * \param that The source string literal.
   * \pre `M-1 <= N`
   * \pre `that[M-1] == Char(0)`
   ...
   * \param that A pointer to the source characters.
   ...
   * \param ch The character value used to fill the string.
   * \pre `count <= N`
   * \post `size() == count`
   ...
   *   characters.
   * \param il The initializer list of characters to copy.
   * \pre `il.size() <= N`
   * \post `size() == count`
  ...
  /// \param that The source string to copy from.
  /// \returns `*this`.
  constexpr BasicFixedString& operator=(const BasicFixedString& that) noexcept =
      default;

  /** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** **
   * Assign from a `BasicFixedString<Char, M>`.
   * \param that The source string to assign from.
   * \pre `that.size() <= N`
   * \post `size() == that.size()`
   * \post `0 == strncmp(data(), that.begin(), size())`
   * \post `at(size()) == Char(0)`
   ...
   * \param that The source null-terminated array to assign from.
   * \pre `M < N`
   ...
   * \param il The initializer list of characters to assign from.
   * \pre `il.size() <= N`
   * \post `size() == il.size()`
   * \post `0 == strncmp(data(), il.begin(), size())`
   * \post `at(size()) == Char(0)`
  ...
  /// Converts this string to a `std::basic_string<Char>`.
  ///
  /// \returns `std::basic_string<Char>{begin(), end()}`.
   ...
   * \param ch The character value to assign.
   * \pre `count <= N`
   * \post `size() == count`
   * \post `npos == find_first_not_of(ch)`
   * \post `at(size()) == Char(0)`
  ...
  /// Deleted assignment from a string and a position.
  ///
  /// This overload is deleted so users aren't confused by the difference
  /// between `str.assign("foo", N)` and `str.assign("foo"_fs, N)`. In the
  /// former, `N` is a count of characters. In the latter, it would be a
  /// position, which totally changes the meaning of the code.
  ///
  /// \param that The source string.
  /// \param pos The starting position in the source string.
  template <std::size_t M>
  ...
  /** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** **
   * Assigns characters from an `BasicFixedString` to this object.
   * \param that The null-terminated array of characters to assign from.
   ...
   * \param that The string to swap contents with.
   */
   ...
   * \returns A reverse iterator to the beginning of the reversed string.
   */
  ...
  /**
   * Returns a const reverse iterator to the beginning of the reversed string.
   ...
   * \returns A reverse iterator to the end of the reversed string.
   */
  ...
  /**
   * Returns a const reverse iterator to the end of the reversed string.
  ...
  /**
   * Accesses the character at a given index with bounds checking.
  ...
  /**
   * Returns a new string with a single character appended to the end.
   * \param ch The character to append.
   * \note Equivalent to `cappend(1u, ch)`.
   * \returns A new string one character longer than this one.
   */
   ...
   * \param that The source string to append.
  ...
  /// Deleted append from a string and a position.
  ///
  /// This overload is deleted so as not to get confused with
  ...
  /**
   * Appends the characters of a null-terminated string to this one.
   * \param that A pointer to the null-terminated character array to append.
   ...
   * \param that A pointer to the character array to append from.
   * \param count The number of characters to append.
   ...
   * \returns A new string with `ch` appended.
   */
   ...
   * \param that The source string to append.
   * \note Equivalent to `*this + ch`
   * \returns A new string with `that` appended.
   */
  template <std::size_t M>
  ...
  /// Deleted `cappend` from a string and a position.
  ///
  /// Deleted to avoid confusion with `append("char*", N)`, where `N` is a
  /// count instead of a position.
  ///
  /// \param that The source string.
  /// \param pos The starting position in the source string.
  template <std::size_t M>
   ...
   * \param that The source string to append from.
   * \param pos The starting position in the source string.
   * \param count The number of characters to append.
   * \note Equivalent to `*this + that.substr(pos, count)`
   * \returns A new string with the selected characters appended.
   */
  template <std::size_t M>
  constexpr BasicFixedString<Char, N + M> cappend(
      const BasicFixedString<Char, M>& that,
   ...
   *   which is left unmodified.
   * \param that The string literal to append.
  ...
  /// Deleted `cappend` from a string literal and a position.
  ///
  /// Deleted to avoid confusion with `append("char*", N)`, where `N` is a
  /// count instead of a position.
  ///
  /// \param that The string literal.
  /// \param pos The starting position in the source string.
  template <std::size_t M>
  constexpr BasicFixedString<Char, N + M - 1u> cappend(
   ...
   * Creates a new string by appending characters from one string to another,
   *   which is left unmodified.
   * \param that The string literal to append from.
   * \param pos The starting position in the source string.
   * \param count The number of characters to append.
  ...
  /**
   * Erases the character pointed to by `first`.
   * \param first A pointer to the character to erase.
  ...
  /**
   * Erases the characters in the half-open range [`first`, `last`).
   * \param first A pointer to the first character to erase.
   * \param last A pointer past the last character to erase.
   ...
   * \returns A new string with the selected characters erased.
   */
   ...
   * \returns A negative, zero, or positive value if this string orders before,
   *   equal to, or after `that`.
   */
  template <std::size_t M>
  ...
  /**
   * Compare two strings for lexicographical ordering.
   * \param this_pos The starting offset within `*this` of the comparison range.
   * \param this_count The number of characters from `*this` to compare.
   * \param that The string to compare against.
   * \note Equivalent to
   * `compare(this_pos, this_count, that.data(), that.size())`
   * \returns A negative, zero, or positive value if this range orders before,
   *   equal to, or after `that`.
   */
  template <std::size_t M>
   ...
   * \param this_count The number of characters from `*this` to compare.
   * \param that The string to compare against.
   * \param that_pos The starting offset within `that` of the comparison range.
   * \param that_count The number of characters from `that` to compare.
   * \note Equivalent to
   * `compare(this_pos, this_count, that.data() + that_pos, that_count)`
   * \returns A negative, zero, or positive value if this range orders before,
   *   equal to, or after the selected range of `that`.
   */
  template <std::size_t M>
  constexpr int compare(
      std::size_t this_pos,
  ...
  /**
   * Compare two strings for lexicographical ordering.
   * \param that A pointer to the null-terminated string to compare against.
   ...
   * \param this_pos The starting offset within `*this` of the comparison range.
   * \param this_count The number of characters from `*this` to compare.
   * \param that A pointer to the character sequence to compare against.
   * \param that_count The number of characters from `that` to compare.
   * \return
   ...
   * \param this_pos The starting offset within `*this` of the comparison range.
   * \param this_count The number of characters from `*this` to compare.
   * \param that The range of characters to compare against.
   * \param that_count The number of characters from `that` to compare.
   * \returns A negative, zero, or positive value if this range orders before,
   *   equal to, or after the selected range of `that`.
   */
   ...
   * \returns A new string containing the selected characters.
   */
   ...
   * \param first A pointer to the first character to replace.
   * \param last A pointer past the last character to replace.
   * \param that The replacement string.
   ...
   * \param this_pos The starting offset within `*this` of the range to replace.
   * \param this_count The number of characters in `*this` to replace.
   * \param that The replacement string.
   * \param that_pos The starting offset within `that` of the replacement range.
   * \pre `that_pos <= that.size()`
   * \note Equivalent to
   *   <code>replace(this_pos, this_count, that.data() + that_pos,
   *   that.size() - that_pos)</code>
   * \returns `*this`.
   */
  template <std::size_t M>
  constexpr BasicFixedString& replace(
   ...
   * \param that The replacement string.
   * \param that_pos The starting offset within `that` of the replacement range.
   * \param that_count The number of characters from `that` to use.
   ...
   * \param this_pos The starting offset within `*this` of the range to replace.
   * \param this_count The number of characters in `*this` to replace.
   * \param that A pointer to the null-terminated replacement string.
   * \note Equivalent to
   ...
   * \param this_pos The starting offset within `*this` of the range to replace.
   * \param this_count The number of characters in `*this` to replace.
   * \param that_count The number of replacement characters.
  ...
  /**
   * Replace the characters denoted by the half-open range [`first`,`last`) with
   *   the characters from the initializer list `il`.
   * \param first A pointer to the first character to replace.
   * \param last A pointer past the last character to replace.
   * \param il The initializer list of replacement characters.
   * \pre `first` and `last` point to characters within this string (including
   *   the terminating null).
   * \note Equivalent to
   ...
   * \pre `that_pos <= that.size()`
   * \note Equivalent to
   *   <code>creplace(this_pos, this_count, that, that_pos,
   *   that.size() - that_pos)</code>
   * \returns A new string with the replacement applied.
   */
  template <std::size_t M>
   ...
   * \note Equivalent to <code>BasicFixedString<Char, N + M>{substr(0, this_pos) +
   *    that.substr(that_pos, that_count) + substr(this_pos + this_count)}</code>
   * \throw std::out_of_range on any of the following conditions:
   *   - `this_pos > size()`
   *   - `this_count > size() - this_pos`
   *   - `that_pos > that.size()`
   ...
   * \pre `that_pos <= that.size()`
   * \note Equivalent to
   *   <code>creplace(first - data(), last - first, that, that_pos,
   *   that.size() - that_pos)</code>
   * \returns A new string with the replacement applied.
   */
  template <std::size_t M>
   ...
   * \param this_pos The starting offset within `*this` of the range to replace.
   * \param this_count The number of characters in `*this` to replace.
   * \param that The character array containing the replacement string.
   * \pre `strlen(that) == M-1`
   * \note Equivalent to
   *   <code>creplace(this_pos, this_count, that, 0, M - 1)</code>
   * \returns A new string with the replacement applied.
   */
  template <std::size_t M>
   ...
   * \post The size of the returned string is `size() - this_count + that_count`
   * \note Equivalent to <code>BasicFixedString<Char, N + M - 1>{
   *    substr(0, this_pos) +
   *    makeFixedString(that).substr(that_pos, that_count) +
   *    substr(this_pos + this_count)}</code>
   * \throw std::out_of_range on any of the following conditions:
   *   - `this_pos > size()`
   *   - `this_count > size() - this_pos`
   *   - `that_pos >= M`
   ...
   * \pre `strlen(that) == M-1`
   * \note Equivalent to
   *   <code>creplace(first - data(), last - first, that, 0, M-1)</code>
   * \returns A new string with the replacement applied.
   */
  template <std::size_t M>
  constexpr BasicFixedString<Char, N + M - 1u> creplace(
   ...
   * \param dest A pointer to the destination buffer.
   * \param count The maximum number of characters to copy.
   * \param pos The starting offset within this string.
   * \pre `pos <= size()`
   * \return The number of characters copied.
   ...
   * \param count The new size of the string.
   * \param ch The character used to fill any newly added positions.
   */
   ...
   * \param that The character sequence to search for.
   * \note Equivalent to `find(that.data(), 0, that.size())`
   * \returns The offset of the first match, or `npos` if there is no match.
   */
  template <std::size_t M>
   ...
   * \param that The character sequence to search for.
   * \param pos The offset at which to start the search.
   * \pre `pos <= size()`
  ...
  /**
   * Finds the first occurrence of the character sequence `that` in this string.
   * \param that A pointer to the null-terminated character sequence to search
   *   for.
   ...
   * \param that A pointer to the character buffer to search for.
   * \param pos The offset at which to start the search.
   * \param count The number of characters from `that` to search for.
   * \pre `pos <= size()`
   ...
   * \returns The offset of the last match, or `npos` if there is no match.
   */
  template <std::size_t M>
   ...
   * \param that The set of characters to search for.
   * \note Equivalent to `find_first_of(that.data(), 0, that.size())`
   * \returns The offset of the first matching character, or `npos` if there is
   *   none.
   */
  template <std::size_t M>
   ...
   * \param that A pointer to the null-terminated set of characters to search
   *   for.
   ...
   *  `pos`.
   * \param that A pointer to the buffer of characters to search for.
   * \param pos The offset at which to start the search.
   * \param count The number of characters from `that` to search for.
   * \pre `pos <= size()`
   * \pre `that` points to a buffer containing at least `count` contiguous
   ...
   * \param that The set of characters to exclude from the search.
   * \note Equivalent to `find_first_not_of(that.data(), 0, that.size())`
   * \returns The offset of the first character not in `that`, or `npos` if
   *   there is none.
   */
  template <std::size_t M>
   ...
   * \param that The set of characters to exclude from the search.
   * \param pos The offset at which to start the search.
   * \note Equivalent to `find_first_not_of(that.data(), pos, that.size())`
   * \returns The offset of the first character not in `that`, or `npos` if
   *   there is none.
   */
  template <std::size_t M>
   ...
   *   character sequence pointed to by `that` in this string.
   * \param that A pointer to the null-terminated set of characters to exclude.
   ...
   *   offset `pos`.
   * \param that A pointer to the buffer of characters to exclude.
   * \param pos The offset at which to start the search.
   * \param count The number of characters from `that` to exclude.
   * \pre `pos <= size()`
   * \pre `that` points to a buffer containing at least `count` contiguous
   *   characters.
   * \return The smallest offset `i` such that `i >= pos` and
   ...
   * \param ch The character to exclude from the search.
   * \note Equivalent to `find_first_not_of(&ch, 0, 1)`
   * \returns The offset of the first character other than `ch`, or `npos` if
   *   there is none.
   */
   ...
   * \returns The offset of the last matching character, or `npos` if there is
   *   none.
   */
  template <std::size_t M>
   ...
   * \returns The offset of the last character not in `that`, or `npos` if there
   *   is none.
   */
  template <std::size_t M>
   ...
   * \returns The offset of the last character other than `ch`, or `npos` if
   *   there is none.
   */
   ...
   *
   * Compares a `BasicFixedString` against a null-terminated string or a
   * character range for equality.
   * \param a The left-hand operand.
   * \param b The right-hand operand.
   * \returns True if the two operands hold the same character sequence.
   */
  friend constexpr bool operator==(
  ...
  /**
   * Compares a `BasicFixedString` against a null-terminated string or a
   * character range for inequality.
   * \param a The left-hand operand.
   * \param b The right-hand operand.
   * \returns True if the two operands hold different character sequences.
   */
  friend constexpr bool operator!=(
      const Char* a, const BasicFixedString& b) noexcept {
    return !(a == b);
  ...
  /**
   * Orders a `BasicFixedString` against a null-terminated string or a
   * character range.
   * \param a The left-hand operand.
   * \param b The right-hand operand.
   * \returns The three-way ordering of `a` relative to `b`.
   */
   ...
   *
   * Concatenates a `BasicFixedString` with a string literal or a single
   * character.
   * \param a The left-hand operand.
   * \param b The right-hand operand.
   * \returns A new string holding the concatenation of `a` and `b`.
   */
  template <std::size_t M>
...
/// Writes a `BasicFixedString` to an output stream.
///
 ...
 *
 * Compares two `BasicFixedString` objects for equality.
 * \param a The left-hand operand.
 * \param b The right-hand operand.
 * \returns True if both strings hold the same character sequence.
 */
...
/// \param a The left-hand operand.
/// \param b The right-hand operand.
/// \returns True if the strings hold different character sequences.
template <class Char, std::size_t A, std::size_t B>
constexpr bool operator!=(
 ...
 *
 * Concatenates two `BasicFixedString` objects.
 * \param a The left-hand operand.
 * \param b The right-hand operand.
 * \returns A new string holding the concatenation of `a` and `b`.
 */
 ...
 * \param a The null-terminated array of characters to build the string from.
 * \pre `a` contains no embedded null characters.
 * \pre `a[N-1] == Char(0)`
 * \post For a returned string `s`, `s[i]==a[i]` for every `i` in [`0`,`N-1`].
 * \returns A `BasicFixedString` holding the characters of `a`.
 */
template <class Char, std::size_t N>
 ...
 * Swap function
 *
 * Swaps the contents of two `BasicFixedString` objects.
...
/// The sentinel value returned by search functions when no match is found.
///
/// The type is `const std::size_t&` so that `folly::npos` has the same address
/// in every translation unit. This avoids potential violations of the ODR.
 ...
 * \returns A `BasicFixedString` holding the characters of the literal.
 */
...
/// \param that A pointer to the literal's characters.
/// \param count The number of characters in the literal.
/// \returns A `FixedString<4>` holding the literal's characters.
FOLLY_DEFINE_FIXED_STRING_UDL(4)

/// Creates a `FixedString<8>` from a string literal.
/// \param that A pointer to the literal's characters.
/// \param count The number of characters in the literal.
/// \returns A `FixedString<8>` holding the literal's characters.
FOLLY_DEFINE_FIXED_STRING_UDL(8)

/// Creates a `FixedString<16>` from a string literal.
/// \param that A pointer to the literal's characters.
/// \param count The number of characters in the literal.
/// \returns A `FixedString<16>` holding the literal's characters.
FOLLY_DEFINE_FIXED_STRING_UDL(16)

/// Creates a `FixedString<32>` from a string literal.
/// \param that A pointer to the literal's characters.
/// \param count The number of characters in the literal.
/// \returns A `FixedString<32>` holding the literal's characters.
FOLLY_DEFINE_FIXED_STRING_UDL(32)

/// Creates a `FixedString<64>` from a string literal.
/// \param that A pointer to the literal's characters.
/// \param count The number of characters in the literal.
/// \returns A `FixedString<64>` holding the literal's characters.
FOLLY_DEFINE_FIXED_STRING_UDL(64)

/// Creates a `FixedString<128>` from a string literal.
/// \param that A pointer to the literal's characters.
/// \param count The number of characters in the literal.
/// \returns A `FixedString<128>` holding the literal's characters.
