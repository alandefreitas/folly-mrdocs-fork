...
///
/// It is optimized for increment/decrement performance under contention. Each
/// unique pair of integral type and tag type names a distinct counter. The
/// value is computed by summing a global counter and all per-thread counters,
/// so it is a relaxed approximation rather than a point-in-time snapshot; it
/// is exact once the system quiesces. For unsigned types, decrements are
/// assumed not to exceed increments and any observed excess is ignored.
///
/// \tparam Int The integral counter type.
/// \tparam Tag A tag type selecting which counter instance to use.
...
class SingletonRelaxedCounter
 ...
 public:
  ...
  ///
  /// \returns The relaxed sum of the global and per-thread counters.
...
///
/// The running count is maintained with a relaxed SingletonRelaxedCounter. To
/// avoid adding names to the counted type, the count is read through the
/// separate type SingletonRelaxedCountableAccess.
///
/// \tparam Counted The derived type being counted.
template <typename Counted>
class SingletonRelaxedCountable {
 public:
  /// Constructs the base and increments the instance count.
  ...
  /// Copy-constructs the base and counts the new instance.
  ///
  /// \param other The instance being copied from; its count is unaffected.
  SingletonRelaxedCountable(const SingletonRelaxedCountable& other) noexcept
      : SingletonRelaxedCountable() {}

  /// Move-constructs the base and counts the new instance.
  ///
  /// \param other The instance being moved from; its count is unaffected.
  SingletonRelaxedCountable(SingletonRelaxedCountable&& other) noexcept
      : SingletonRelaxedCountable() {}

  /// Copy-assigns without changing the instance count.
  ///
  /// \param other The instance being assigned from.
  /// \returns A reference to this instance.
  SingletonRelaxedCountable& operator=(const SingletonRelaxedCountable& other) =
      default;

  /// Move-assigns without changing the instance count.
  ///
  /// \param other The instance being assigned from.
  /// \returns A reference to this instance.
  SingletonRelaxedCountable& operator=(SingletonRelaxedCountable&& other) =
      default;
...
///
/// \tparam Counted The type whose instances are counted.
template <typename Counted>
class SingletonRelaxedCountableAccess {
 public:
  /// Reports the current number of live instances of the counted type.
  ///
  /// \returns The relaxed instance count.
  static size_t count() {
