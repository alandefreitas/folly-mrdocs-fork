...
class UnboundedQueue {
 ...
 public:
  ...
  using value_type = T;
  /// The unsigned integer type used for sizes.
  ...
  /// Enqueues an element by copying it into the queue.
  ///
  /// \param arg The element to copy into the queue.
  FOLLY_ALWAYS_INLINE void enqueue(const T& arg) { enqueueImpl(arg); }

  /// Enqueues an element by moving it into the queue.
  ///
  /// \param arg The element to move into the queue.
  FOLLY_ALWAYS_INLINE void enqueue(T&& arg) { enqueueImpl(std::move(arg)); }

  /// Dequeues the next element, blocking until one is available.
  ///
  /// \param item Receives the dequeued element.
  ...
  /// Attempts to dequeue an element without blocking.
  ///
  /// \returns The dequeued element, or an empty optional if the queue was empty.
  FOLLY_ALWAYS_INLINE folly::Optional<T> try_dequeue() noexcept {
    return tryDequeueUntil(std::chrono::steady_clock::time_point::min());
  }

  /// Attempts to dequeue an element, waiting until the given deadline.
  ///
  /// \param item Receives the dequeued element on success.
  /// \param deadline The time point after which to give up.
  ...
  ///
  /// \param deadline The time point after which to give up.
  /// \returns The dequeued element, or an empty optional on timeout.
  template <typename Clock, typename Duration>
  ...
  /// Attempts to dequeue an element, waiting for at most the given duration.
  ///
  /// \param item Receives the dequeued element on success.
  ...
  ///
  /// \returns The number of enqueued elements not yet dequeued.
  ...
  ///
  /// \returns True if the queue has no pending elements.
