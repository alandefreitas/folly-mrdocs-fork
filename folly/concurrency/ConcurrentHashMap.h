...
class ConcurrentHashMap {
 ...
 public:
  ...
  /// Type of the mapped values stored in the map.
  using mapped_type = ValueType;
  /// Type of the key/value pairs stored in the map.
  using value_type = std::pair<const KeyType, ValueType>;
  /// Unsigned integer type used for sizes.
  using size_type = std::size_t;
  /// Function object type used to hash keys.
  using hasher = HashFn;
  /// Function object type used to compare keys for equality.
  using key_equal = KeyEqual;
  /// Const iterator type over the map's elements.
 ...
 public:
  /** Construct a ConcurrentHashMap with 1 << ShardBits shards.
   *
   * Both size and max_size are rounded up to the next power of two, if
   * they are not already a power of two, so that we can index in to
   * shards efficiently.
   *
   * Insertion functions will throw bad_alloc if max_size is exceeded.
   *
   * \param size Initial number of buckets to reserve.
   * \param max_size Maximum number of elements, or 0 for unbounded.
   */
  ...
  /// Move constructor that transfers ownership of all segments from another map.
  ///
  /// \param o The map to move from, which is left empty.
  ...
  /// Move assignment that releases current segments and takes ownership of another map's.
  ///
  /// \param o The map to move from, which is left empty.
  ...
  /// Destroy the map, running a final garbage collection pass so no elements remain.
  ~ConcurrentHashMap() {
  ...
  /// Check whether the map currently contains no elements.
  ///
  ...
  /// Find the element matching a heterogeneous key.
  ///
  /// \tparam K The heterogeneous key type eligible for heterogeneous find.
  /// \param k The key to search for.
  /// \returns A const iterator to the element, or end() if not found.
  ...
  /** Deleted to guard against common misuse in concurrent contexts.
   *
   ...
   *
   * \param k The key that would be checked for existence.
  ...
  ///
  /// \returns A const iterator representing the end of the map.
  ...
  ///
  /// \param foo The key/value pair to insert.
  /// \returns A pair of an iterator to the element and a bool that is
  ...
  /// Insert a pair with a heterogeneous key if the key is not already present.
  ///
  /// \tparam Key The heterogeneous key type eligible for heterogeneous insert.
  /// \param foo The key/value pair to insert.
  /// \returns A pair of an iterator to the element and a bool that is
  /// true if insertion took place.
  ...
  /// Insert a value constructed from a key and value if the key is not present.
  ///
  /// \tparam Key The forwarded key type.
  /// \tparam Value The forwarded value type.
  ...
  /// Insert an element constructed in place, only if the key is not present.
  ///
  /// \tparam Key The forwarded key type.
  /// \tparam Args The forwarded argument types for constructing the value.
  /// \param k The key to insert.
  ...
  /// Construct an element in place and insert it if the key is not present.
  ///
  /// \tparam Args The forwarded argument types for constructing the element.
  /// \param args Arguments used to construct the key/value pair.
  /// \returns A pair of an iterator to the element and a bool that is
  /// true if insertion took place.
  ...
  /** Insert the value if the key is absent, otherwise assign to the existing value.
   *
  ...
  /** Insert desired if the key is absent, or assign it when a predicate holds.
   *
   ...
   * \tparam Key The forwarded key type.
   * \tparam Value The forwarded value type.
   * \tparam Predicate The predicate type evaluated on the current value.
   * \param k The key to insert or update.
   * \param desired The value to store.
   * \param predicate Predicate evaluated on the current value to decide assignment.
   * \returns A pair of an iterator to the element and a bool that is
   * true if the map was updated.
   */
  ...
  /// Assign a new value to an existing key, if the key is present.
  ///
  /// \tparam Key The forwarded key type.
  /// \tparam Value The forwarded value type.
  /// \param k The key to update.
  /// \param v The new value to store.
  /// \returns An iterator to the updated element, or none if the key was absent.
  template <typename Key, typename Value>
  ...
  /// Assign to desired only if the predicate returns true for the current value.
  ///
  /// \tparam Key The forwarded key type.
  /// \tparam Value The forwarded value type.
  /// \tparam Predicate The predicate type evaluated on the current value.
  ...
  /// \param predicate Predicate evaluated on the current value to decide assignment.
  /// \returns An iterator to the updated element, or none if not updated.
  template <typename Key, typename Value, typename Predicate>
  ...
  /// Assign to desired only if the current value equals expected.
  ///
  /// \tparam Key The forwarded key type.
  /// \tparam Value The forwarded value type.
  /// \param k The key to update.
  /// \param expected The value the current mapped value must equal.
  /// \param desired The new value to store.
  /// \returns An iterator to the updated element, or none if not updated.
  template <typename Key, typename Value>
  ...
  /// Return a copy of the value for key, inserting a default value if absent.
  ///
  /// A copying wrapper around insert and find, only available for copyable
  /// types. The returned value is const, since changes do not affect the map.
  ///
  /// \param key The key to look up or insert.
  /// \returns A const copy of the mapped value.
  const ValueType operator[](const KeyType& key) {
    auto item = insert(key, ValueType());
    return item.first->second;
  }

  /// Return a copy of the value for a heterogeneous key, inserting a default if absent.
  ///
  /// \tparam Key The heterogeneous key type eligible for heterogeneous insert.
  /// \param key The key to look up or insert.
  /// \returns A const copy of the mapped value.
  ...
  /// Return a copy of the value for key, throwing if the key is absent.
  ///
  /// \param key The key to look up.
  /// \returns A const copy of the mapped value.
  const ValueType at(const KeyType& key) const { return atImpl(key); }

  /// Return a copy of the value for a heterogeneous key, throwing if absent.
  ///
  /// \tparam K The heterogeneous key type eligible for heterogeneous find.
  /// \param key The key to look up.
  /// \returns A const copy of the mapped value.
  ...
  /// Erase the element matching a heterogeneous key.
  ///
  /// \tparam K The heterogeneous key type eligible for heterogeneous erase.
  /// \param k The key to erase.
  /// \returns The number of elements removed (0 or 1).
  ...
  /// Erase the element associated with the key that pos refers to.
  ///
  /// Calls the hash function, and therefore may throw. This function doesn't
  /// necessarily delete the item that pos points to. It simply tries erasing
  /// the item associated with the same key. While this behavior can be
  /// confusing, erase(iterator) is often found in std data structures so we
  /// follow a similar pattern here.
  ///
  /// \param pos Iterator whose key identifies the element to erase.
  ...
  /// Erase the element only if its value equals expected.
  ///
  /// \param k The key to erase.
  /// \param expected The value the current mapped value must equal.
  /// \returns The number of elements removed (0 or 1).
  ...
  /// Erase the element for a heterogeneous key only if its value equals expected.
  ///
  /// \tparam K The heterogeneous key type eligible for heterogeneous erase.
  /// \param k The key to erase.
  /// \param expected The value the current mapped value must equal.
  ...
  /// Erase the element only if the predicate evaluates to true on its value.
  ///
  /// \tparam Predicate The predicate type evaluated on the existing value.
  /// \param k The key to erase.
  /// \param predicate Predicate evaluated on the existing value.
  /// \returns The number of elements removed (0 or 1).
  ...
  /// Erase the element for a heterogeneous key if the predicate holds on its value.
  ///
  /// \tparam K The heterogeneous key type eligible for heterogeneous erase.
  /// \tparam Predicate The predicate type evaluated on the existing value.
  /// \param k The key to erase.
  ...
  /// Remove all elements, initializing new shard segments; not noexcept.
  void clear() {
    uint64_t begin = beginSeg_.load(std::memory_order_acquire);
    uint64_t end = endSeg_.load(std::memory_order_acquire);
    for (uint64_t i = begin; i < end; ++i) {
  ...
  /// Reserve capacity across shards for at least count elements.
  ///
  /// \param count The total number of elements to reserve space for.
  ...
  ///
  /// \returns An approximate count of stored elements.
  ...
  /// Set the maximum load factor on all existing segments.
  ///
  /// \param factor The new maximum load factor.
  ...
  /// Const forward iterator that holds a hazard pointer to the current element.
  class ConstIterator {
   public:
    friend class ConcurrentHashMap;

    /// Access the key/value pair the iterator refers to.
    ///
    /// \returns A const reference to the current element.
    const value_type& operator*() const { return *it_; }

    /// Access members of the key/value pair the iterator refers to.
    ///
    ...
    /// Copy assignment is deleted since iterators hold a hazard pointer.
    ///
    /// \param o The iterator that would be copied from.
    /// \returns Never returns; this operator is deleted.
    ConstIterator& operator=(const ConstIterator& o) = delete;

    /// Move assignment that transfers the hazard pointer from another iterator.
    ///
    /// \param o The iterator to move from, left at the end position.
    ...
    /// Copy construction is deleted since iterators hold a hazard pointer.
    ///
    /// \param o The iterator that would be copied from.
    ConstIterator(const ConstIterator& o) = delete;

    /// Move construct by transferring the hazard pointer from another iterator.
    ///
    /// \param o The iterator to move from, left at the end position.
    ...
    /// Construct an iterator positioned at a given segment of a map.
    ///
    /// \param parent The owning map.
    /// \param segment The index of the segment to point into.
...
///
/// Uses vectorized key lookup where the required intrinsics are available,
/// falling back to the regular bucket table otherwise.
template <
    typename KeyType,
    typename ValueType,
    typename HashFn = std::hash<KeyType>,
