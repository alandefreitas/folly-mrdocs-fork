...
///
/// This fallback implementation forwards to the standard library's free
/// function atomic operations on shared_ptr.
template <typename T>
class atomic_shared_ptr {
 private:
 ...
 public:
  /// The underlying shared_ptr type stored by this wrapper.
  using value_type = std::shared_ptr<T>;

  /// Constructs an empty atomic_shared_ptr holding no object.
  atomic_shared_ptr() = default;

  /// Constructs an empty atomic_shared_ptr holding no object.
  ///
  /// \param unused A nullptr tag selecting the empty constructor.
  atomic_shared_ptr(std::nullptr_t unused) noexcept {}

  /// Constructs an atomic_shared_ptr holding the given shared_ptr.
  ///
  /// \param desired The initial shared_ptr value.
  atomic_shared_ptr(std::shared_ptr<T> desired) noexcept
      : rep_{std::move(desired)} {}

  /// Deleted copy constructor; atomic_shared_ptr is not copyable.
  ///
  /// \param other The atomic_shared_ptr that would be copied.
  atomic_shared_ptr(atomic_shared_ptr const& other) = delete;

  /// Deleted move constructor; atomic_shared_ptr is not movable.
  ///
  /// \param other The atomic_shared_ptr that would be moved.
  atomic_shared_ptr(atomic_shared_ptr&& other) = delete;

  /// Atomically stores a null pointer.
  ///
  /// \param unused A nullptr tag.
  void operator=(std::nullptr_t unused) noexcept { store(nullptr); }

  /// Atomically stores the given shared_ptr.
  ///
  /// \param desired The shared_ptr value to store.
  void operator=(std::shared_ptr<T> desired) noexcept {
    store(std::move(desired));
  }

  /// Deleted copy assignment; atomic_shared_ptr is not copyable.
  ///
  /// \param other The atomic_shared_ptr that would be assigned.
  void operator=(atomic_shared_ptr const& other) = delete;

  /// Deleted move assignment; atomic_shared_ptr is not movable.
  ///
  /// \param other The atomic_shared_ptr that would be assigned.
  void operator=(atomic_shared_ptr&& other) = delete;

  /// Atomically loads the stored shared_ptr.
  ///
  /// \returns The current shared_ptr value.
  /* implicit */ operator std::shared_ptr<T>() const noexcept { return load(); }

  /// Reports whether operations on this object are lock-free.
  ///
  /// \returns True if the operations are lock-free.
  ...
  /// Atomically stores a new shared_ptr value.
  ///
  /// \param desired The shared_ptr value to store.
  ...
  ///
  /// \param desired The new shared_ptr value to store.
  /// \param order The memory order for the exchange.
  /// \returns The previously stored shared_ptr value.
  ...
  /// Atomically compares and, on success, stores a new value, possibly failing
  /// spuriously.
  ///
  /// \param expected The expected value; updated with the actual value on
  /// failure.
  /// \param desired The value to store on success.
  /// \param success The memory order used on a successful exchange.
  /// \param failure The memory order used on a failed exchange.
  ...
  /// failure.
  /// \param desired The value to store on success.
  /// \param order The memory order used for the operation.
  /// \returns True if the exchange succeeded.
  bool compare_exchange_weak(
      std::shared_ptr<T>& expected,
      std::shared_ptr<T> desired,
  ...
  /// Atomically compares and, on success, stores a new value without spurious
  /// failures.
  ///
  /// \param expected The expected value; updated with the actual value on
  /// failure.
