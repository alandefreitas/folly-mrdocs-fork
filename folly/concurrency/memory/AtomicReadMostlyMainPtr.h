...
///
/// What `atomic_shared_ptr` is to `shared_ptr`, this is to
/// `ReadMostlyMainPtr`: it allows racy conflicting accesses to one, giving true
/// shared_ptr-like semantics, including reclamation at the point where the last
/// pointer to an object goes away. It runs at about the same speed as
/// `ReadMostlyMainPtr` and avoids reader-reader contention and atomic RMWs in
...
class AtomicReadMostlyMainPtr {
 public:
  ...
  /// Constructs a pointer that initially owns the given object.
  ///
  /// \param ptr The shared pointer whose ownership is taken.
  explicit AtomicReadMostlyMainPtr(std::shared_ptr<T> ptr)
      : curMainPtrIndex_(0) {
    mainPtrs_[0] = ReadMostlyMainPtr<T>{std::move(ptr)};
  }

  /// Atomically replaces the held pointer.
  ///
  /// \param desired The new pointer to store.
  void operator=(std::shared_ptr<T> desired) { store(std::move(desired)); }

  /// Reports whether operations on this object are lock-free.
  ///
  /// \returns Always false; the implementation uses a mutex on the write path.
  bool is_lock_free() const { return false; }

  /// Atomically reads the held pointer.
  ///
  /// \param order The memory order for the load.
  /// \returns A read-mostly shared pointer to the current object.
  ...
  /// Atomically replaces the held pointer.
  ///
  /// Any object left without owners is destroyed after the internal lock is
  /// released.
  ///
  ...
  /// Atomically replaces the held pointer and returns the previous one.
  ///
  /// \param ptr The new pointer to store.
  /// \param order The memory order for the exchange.
  /// \returns The pointer that was held before the exchange.
  ...
  /// Atomically replaces the held pointer if it equals the expected one.
  ///
  /// This may fail spuriously. On failure, expected is updated to the current
  /// value.
  ///
  /// \param expected The pointer expected to be held; updated on failure.
  /// \param desired The pointer to store on success.
  /// \param successOrder The memory order applied when the exchange succeeds.
  /// \param failureOrder The memory order applied when the exchange fails.
