...
///
/// The count is maintained in thread-local sub-counters while in the local
/// phase, then folded into a single global atomic counter once useGlobal() is
/// called. This makes increments and decrements cheap in the common case.
class TLRefCount {
 public:
  /// The signed integer type used for the count.
  using Int = int64_t;

  /// Constructs the counter in the local phase with an initial count of one.
  TLRefCount()
      : localCount_([&]() { return LocalRefCount(*this); }),
        collectGuard_(this, [](void*) {}) {}

  /// Destroys the counter; requires the global count to have reached zero.
  ~TLRefCount() noexcept {
  ...
  ///
  /// This cannot bring the count up from zero.
  ///
  /// \returns The new count, or zero if the count had already reached zero.
  ...
  ///
  /// The exact count is only meaningful once the counter is in the global
  /// phase; otherwise a nonzero placeholder is returned.
  ///
  /// \returns The global count, or a nonzero placeholder while still local.
  Int operator*() const {
  ...
  /// Switches this counter to the global phase.
  ///
  /// After this call, increments and decrements operate on the global atomic
  /// counter instead of the per-thread counters.
  void useGlobal() noexcept {
    std::array<TLRefCount*, 1> ptrs{{this}};
    useGlobal(ptrs);
  }

  /// Switches several counters to the global phase together.
  ///
  /// \tparam Container A range type holding pointers to TLRefCount.
  /// \param refCountPtrs The counters to transition to the global phase.
