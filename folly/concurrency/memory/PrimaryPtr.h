...
class EnablePrimaryFromThis {
 ...
 public:
  /// Gets a non-owning reference to the pointer.
  ///
  /// PrimaryPtr::join() and the PrimaryPtr::cleanup() work do *NOT* wait for
  /// outstanding PrimaryPtrRef objects to be released.
  ///
  /// \returns A non-owning reference to the managed pointer.
  PrimaryPtrRef<T> masterRefFromThis() {
    return PrimaryPtrRef<T>(outerPtrWeak_);
  }

  /// Gets a non-owning const reference to the pointer.
  ///
  /// PrimaryPtr::join() and the PrimaryPtr::cleanup() work do *NOT* wait for
  /// outstanding PrimaryPtrRef objects to be released.
  ///
  /// \returns A non-owning const reference to the managed pointer.
  PrimaryPtrRef<const T> masterRefFromThis() const {
    return PrimaryPtrRef<const T>(outerPtrWeak_);
  }

  /// Attempts to lock a pointer.
  ///
  /// Returns null if pointer is not set or if PrimaryPtr::join() was called or
  /// the PrimaryPtr::cleanup() task was started (even if the call to
  /// PrimaryPtr::join() hasn't returned yet and the PrimaryPtr::cleanup() task
  /// has not completed yet).
  ///
  /// \returns A shared pointer to the object, or null if unavailable.
  ...
  /// Attempts to lock a pointer for const access.
  ///
  /// Returns null if pointer is not set or if PrimaryPtr::join() was called or
  /// the PrimaryPtr::cleanup() task was started (even if the call to
  /// PrimaryPtr::join() hasn't returned yet and the PrimaryPtr::cleanup() task
  /// has not completed yet).
  ///
  /// \returns A shared pointer to the const object, or null if unavailable.
...
class PrimaryPtr {
 ...
 public:
  /// Deleted default constructor; a PrimaryPtr must manage a pointer.
  PrimaryPtr() = delete;

  /// Constructs a PrimaryPtr that takes ownership of a unique_ptr.
  ///
  /// \param ptr The unique_ptr whose object will be managed.
  template <class T2, class Deleter>
  PrimaryPtr(std::unique_ptr<T2, Deleter> ptr) {
    set(std::move(ptr));
  }

  /// Constructs an empty PrimaryPtr that manages no object.
  ///
  /// \param unused A nullptr tag selecting the empty constructor.
  explicit PrimaryPtr(std::nullptr_t unused) {}

  /// Destroys the PrimaryPtr. Aborts if it was not joined first.
  ~PrimaryPtr() {
    if (*this) {
  ...
  /// Move-constructs a PrimaryPtr, taking ownership from another.
  ///
  /// \param other The PrimaryPtr to move from.
  PrimaryPtr(PrimaryPtr<T>&& other) = default;

  /// Swaps the managed state with another PrimaryPtr.
  ///
  /// \param other The PrimaryPtr to swap with.
  ...
  /// Replaces the managed state with a new value and returns the old one.
  ///
  /// \param newVal The new PrimaryPtr to take ownership of.
  /// \returns The previous PrimaryPtr state.
  ...
  /// Reports whether the PrimaryPtr currently manages an object.
  ///
  /// \returns True if an object is managed.
  explicit operator bool() const { return !!innerPtr_; }

  /// Attempts to lock a pointer.
  ///
  /// Returns null if pointer is not set or if join() was called or the
  /// cleanup() task was started (even if the call to join() hasn't returned yet
  /// and the cleanup() task has not completed yet).
  ///
  /// \returns A shared pointer to the object, or null if unavailable.
  ...
  /// Waits until all references obtained via lock() are released, then destroys
  /// the object in the current thread.
  ///
  ...
  /// Returns a SemiFuture that waits until all references obtained via lock()
  /// are released, then destroys the object on the Executor provided to the
  /// SemiFuture.
  ///
  /// The returned SemiFuture must run to completion before calling set().
  ///
  /// \returns A SemiFuture that performs the deferred cleanup.
  ...
  ///
  /// Can not be called concurrently with lock() or join() or ref() or while the
  /// SemiFuture returned from cleanup() is running.
  ///
  /// \param ptr The unique_ptr whose object will be managed.
  template <class T2, class Deleter>
  ...
  /// Gets a non-owning reference to the pointer.
  ///
  /// join() and the cleanup() work do *NOT* wait for outstanding PrimaryPtrRef
...
///
/// \param x The first PrimaryPtr.
/// \param y The second PrimaryPtr.
template <typename T>
...
///
/// \param x The PrimaryPtr to update.
/// \param newVal The new PrimaryPtr to take ownership of.
/// \returns The previous state of x.
template <typename T>
...
class PrimaryPtrRef {
 public:
  /// Constructs an empty PrimaryPtrRef that references no pointer.
  PrimaryPtrRef() = default;

  /// Attempts to lock a pointer.
  ///
  /// Returns null if pointer is not set or if join() was called or cleanup()
  /// work was started (even if the call to join() hasn't returned yet or the
  /// cleanup() work has not completed yet).
  ///
  /// \returns A shared pointer to the object, or null if unavailable.
  std::shared_ptr<T> lock() const {
    if (auto outerPtr = outerPtrWeak_.lock()) {
