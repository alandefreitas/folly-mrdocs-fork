...
class FlatCombiningPriorityQueue
 ...
 public:
  /// Constructs a flat combining priority queue.
  ///
  /// \param maxSize The bound on the number of items, or 0 for unbounded.
  /// \param dedicated Whether to use a dedicated combiner thread.
  /// \param numRecs The number of flat combining records, or 0 for the default.
  /// \param maxOps The maximum operations per combining pass, or 0 for the
  /// default.
  /// \param args Arguments forwarded to the underlying PriorityQueue
  /// constructor.
  template <
      typename... PQArgs,
  ...
  ///
  /// \returns True if the priority queue holds no items.
  bool empty() const {
    bool res;
  ...
  ///
  /// \param val Set to the highest priority item on success.
  ...
  ///
  /// \param val Set to the highest priority item on success.
  /// \returns True if an item was peeked.
  bool try_peek(T& val) {
  ...
  /// it is nonempty.
  ///
  /// \param val Set to the highest priority item that was removed.
  void pop(T& val) {
    try_pop_impl(
        val, std::chrono::time_point<std::chrono::steady_clock>::max());
  }
  ...
  ///
  /// \param val Set to the highest priority item.
  void peek(T& val) {
    try_peek_impl(
        val, std::chrono::time_point<std::chrono::steady_clock>::max());
  }

  /// Non-blocking pop returning the item as an Optional.
  ///
  /// \returns The highest priority item, or folly::none if the queue is empty.
  ...
  /// Non-blocking peek returning the item as an Optional.
  ///
  /// \returns The highest priority item, or folly::none if the queue is empty.
  ...
  /// Timed pop that waits up to a relative timeout.
  ///
  /// \param timeout The maximum duration to wait for an item.
  /// \returns The highest priority item, or folly::none on timeout.
  ...
  /// Timed push that waits up to a relative timeout for space.
  ///
  /// \param val The item to insert.
  /// \param timeout The maximum duration to wait for space.
  /// \returns True if the item was inserted before the timeout.
  template <typename Rep, typename Period>
  bool try_push_for(
  ...
  /// Timed peek that waits up to a relative timeout.
  ///
  /// \param timeout The maximum duration to wait for an item.
  /// \returns The highest priority item, or folly::none on timeout.
  template <typename Rep, typename Period>
  ...
  /// Timed pop that waits until an absolute deadline.
  ///
  /// \param deadline The time point past which to stop waiting.
  /// \returns The highest priority item, or folly::none if the deadline passes.
  ...
  /// Timed push that waits until an absolute deadline for space.
  ///
  /// \param val The item to insert.
  /// \param deadline The time point past which to stop waiting.
  /// \returns True if the item was inserted before the deadline.
  template <typename Clock, typename Duration>
  bool try_push_until(
  ...
  /// Timed peek that waits until an absolute deadline.
  ///
  /// \param deadline The time point past which to stop waiting.
  /// \returns The highest priority item, or folly::none if the deadline passes.
  template <typename Clock, typename Duration>
