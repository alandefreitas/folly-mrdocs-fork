...
struct atomic_grow_array_policy_default {
  /// The atomic wrapper template used for the array's atomic members.
  template <typename V>
  using atom = Atom<V>;

  /// Compute the new capacity when the array grows to reach an index.
  ///
  /// \param curr The current capacity of the array.
  /// \param index The index that access must accommodate.
  /// \returns The new capacity, the next power of two greater than `index`.
  std::size_t grow(
      std::size_t curr, std::size_t const index) const noexcept {
    return nextPowTwo(index + 1);
  }

  /// Construct a new element value.
  ///
  /// \returns A default-constructed `Item`.
...
class atomic_grow_array : private Policy {
 public:
  /// The unsigned integer type used to represent sizes and indices.
  using size_type = std::size_t;

  /// The type of elements stored in the array.
  using value_type = Item;

  /// A span of pointers to mutable elements.
  using pointer_span = std::span<value_type* const>;

  /// A span of pointers to const elements.
 ...
 private:
  ...
  class basic_iterator {
   ...
   public:
    ...
    /// The signed integer type for distances between iterators.
    ...
    /// \returns The signed number of elements between the two positions.
 ...
 public:
  /// Construct an empty array with a default-constructed policy.
  atomic_grow_array() = default;

  /// Construct an empty array with the given policy.
  ///
  /// \param policy_ The policy that controls growth and element construction.
  explicit atomic_grow_array(Policy const& policy_) //
      noexcept(noexcept(Policy{policy_}))
      : Policy{policy_} {}

  /// Destroy the array and all of its elements.
  ~atomic_grow_array() { reset(); }

  /// Access the policy governing this array.
  ///
  /// \returns A reference to the array's policy.
  ...
  ///   }
  ///
  /// \returns A recent lower-bound value of the true size.
  ...
  ///
  /// \returns `true` if the array is observed to be empty.
  ...
  class const_iterator : private basic_iterator<true, const_iterator> {
   ...
   public:
    ...
    /// Construct a const_iterator from a mutable iterator.
    ///
  ...
  class const_view : private basic_view<true> {
   ...
   public:
    ...
    /// Construct a const_view from a mutable view.
    ///
    /// \param that The view to convert.
  ...
  ///
  /// \returns A view over the elements available at the time of the call.
  view as_view() noexcept { return view{array_.load(mo_acquire)}; }

  /// Get a const view over the elements available at the time of the call.
  ///
  /// \returns A const view over the elements available at the time of the call.
  ...
  ///
  /// \returns A span of pointers to the elements available at the call.
  pointer_span as_ptr_span() noexcept { return as_view().as_ptr_span(); }

  /// Get a span of const pointers to the elements available at the call.
  ///
  /// \returns A span of const pointers to the available elements.
  const_pointer_span as_ptr_span() const noexcept {
    return as_view().as_ptr_span();
  }

  /// Get a span of pointers to at most `sz` elements available at the call.
  ///
  /// \param sz The maximum number of element pointers to include.
  /// \returns A span of pointers to at most `sz` available elements.
  pointer_span as_ptr_span(size_type const sz) noexcept {
    return as_view().as_ptr_span(sz);
  }

  /// Get a span of const pointers to at most `sz` elements available at the
  /// call.
  ///
  /// \param sz The maximum number of element pointers to include.
  /// \returns A span of const pointers to at most `sz` available elements.
