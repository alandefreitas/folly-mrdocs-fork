...
///
/// The queue is built on the Mound data structure and trades strict priority
/// ordering for throughput under contention. It supports arbitrary priorities,
/// unbounded size, and optional blocking on pop.
///
/// \tparam T The element type; larger values have higher priority.
/// \tparam MayBlock Whether pop can block waiting for elements.
/// \tparam SupportsSize Whether size() is tracked and reported.
/// \tparam PopBatch Number of nodes kept in the shared buffer for fast pops; 0 pops one node at a time and keeps strict highest-priority order.
/// \tparam ListTargetSize The preferred length for each per-element list.
/// \tparam Mutex The mutex type guarding each tree element.
/// \tparam Atom The atomic template used for internal state.
template <
    typename T,
    bool MayBlock = false,
...
class RelaxedConcurrentPriorityQueue {
 ...
 public:
  ...
  ///
  /// When blocking is enabled, this waits until an element is available.
  ///
  /// \param val Set to the popped value.
  void pop(T& val) {
    moundPop(val);
    if (SupportsSize) {
