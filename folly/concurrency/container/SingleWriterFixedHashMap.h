...
class SingleWriterFixedHashMap {
 ...
 public:
  ...
  /// Constructs an empty map with the given capacity.
  ///
  /// \param capacity Requested capacity, rounded up to the next power of two.
  explicit SingleWriterFixedHashMap(size_t capacity)
      : capacity_(folly::nextPowTwo(capacity)) {}

  /// Constructs a map by copying another map, with optional capacity expansion.
  ///
  /// \param capacity Requested capacity, rounded up to the next power of two.
  /// \param o Source map to copy from; must not be mutated concurrently.
  ...
  ///
  /// \returns An iterator to the first element, or end() if the map is empty.
  ...
  /// Returns the total number of slots in the map.
  ///
  /// \returns The capacity, a power of two.
  size_t capacity() const { return capacity_; }

  /// Returns the number of slots occupied by valid entries or tombstones.
  ///
  /// Not data-race-free; call only from the single writer.
  ///
  /// \returns The number of used slots.
  size_t used() const { return used_; }

  /// Returns the number of slots not yet occupied.
  ///
  /// Not data-race-free; call only from the single writer.
  ///
  ...
  /// Returns the number of valid elements in the map.
  ///
  /// Data-race-free; may be called by readers.
  ///
  ...
  /// Checks whether the map holds no valid elements.
  ///
  ...
  /// Erases the element referenced by an iterator.
  ///
  /// \param it Iterator to the element to erase; must not be end().
  ...
  /// Erases the element with the given key, if present.
  ///
  /// \param key Key to erase.
  /// \returns True if an element was erased, false if the key was not found.
  bool erase(Key key) {
  ...
  /// \returns An iterator to the element, or end() if the key is not present.
  FOLLY_ALWAYS_INLINE Iterator find(Key key) const {
    size_t index = reader_find(key);
    return Iterator(*this, index);
  }

  /// Checks whether an element with the given key is present.
  ///
  /// \param key Key to search for.
 ...
 public:
  ...
  class Iterator {
   ...
   public:
    ...
    /// Advances the iterator to the next valid element.
    ///
