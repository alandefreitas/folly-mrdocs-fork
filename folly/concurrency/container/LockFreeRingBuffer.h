...
class LockFreeRingBuffer {
 ...
 public:
  ...
  struct Cursor : boost::totally_ordered<Cursor> {
    ...
    ///
    /// \param initialTicket The ticket the cursor initially points to.
    explicit Cursor(uint64_t initialTicket) noexcept : ticket(initialTicket) {}

    /// Advances the cursor toward more recent writes.
    ///
    /// Returns true if this cursor now points to a different
    /// write, false otherwise.
    ///
    /// \param steps The number of writes to move forward by.
    /// \returns True if the cursor moved to a different write.
    ...
    /// Moves the cursor toward older writes.
    ///
    /// Returns true if this cursor now points to a previous
    /// write, false otherwise.
    ///
    /// \param steps The number of writes to move backward by.
    /// \returns True if the cursor moved to a different write.
    ...
    ///
    /// \param that The cursor to compare against.
    /// \returns True if both cursors point to the same write.
    bool operator==(const Cursor& that) const noexcept {
      return ticket == that.ticket;
    }

    /// Orders two cursors by their position in the write stream.
    ///
    /// \param that The cursor to compare against.
    /// \returns True if this cursor points to an earlier write.
    bool operator<(const Cursor& that) const noexcept {
      return ticket < that.ticket;
    }

   protected: // for test visibility reasons
    /// The write ticket this cursor points to.
    uint64_t ticket;
  ...
  ///
  /// \param capacity The number of slots retained in the buffer.
  ...
  /// Returns the number of writes the buffer retains.
  ///
  ...
  ///
  /// \param value The value to write into the buffer.
  /// \returns A cursor pointing to the just-written value.
  template <typename V>
  ...
  ///
  /// \param dest Receives the value read from the buffer.
  /// \param cursor The cursor identifying the write to read.
  ...
  ///
  /// \returns A pair of the buffer's base address and its size in bytes.
