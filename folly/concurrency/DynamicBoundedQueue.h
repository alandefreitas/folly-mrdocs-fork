...
struct DefaultWeightFn {
  /// Compute the weight of an element.
  ///
  /// \param arg The element whose weight is measured.
  /// \returns The element weight, which is always 1.
  template <typename Arg>
  uint64_t operator()(Arg&& arg) const noexcept {
    return 1;
  }
};

/// A bounded, FIFO, multi-producer multi-consumer queue with adjustable
/// capacity and dynamic memory usage.
template <
    typename T,
    bool SingleProducer,
    bool SingleConsumer,
...
class DynamicBoundedQueue {
 ...
 public:
  ...
  /// The unsigned integer type used to represent sizes.
  using size_type = size_t;

  /// Construct a queue with the given capacity.
  ///
  /// \param capacity The initial weight capacity of the queue.
  ...
  /// Add an element to the end of the queue, waiting for capacity if needed.
  ///
  /// \param v The element to enqueue.
  ...
  /// Try to add an element to the end of the queue if capacity allows.
  ///
  /// \param v The element to enqueue.
  ...
  /// Try to add an element to the end of the queue, waiting until a deadline.
  ///
  /// \param v The element to enqueue.
  /// \param deadline The time point past which the attempt gives up.
  /// \returns `true` if the element was enqueued, `false` otherwise.
  ...
  /// Try to add an element to the end of the queue, waiting for a duration.
  ///
  /// \param v The element to enqueue.
  /// \param duration The maximum time to wait for capacity.
  /// \returns `true` if the element was enqueued, `false` otherwise.
  ...
  /// Extract an element from the front of the queue, waiting if needed.
  ///
  /// \param elem Set to the extracted element.
  FOLLY_ALWAYS_INLINE void dequeue(T& elem) {
    q_.dequeue(elem);
    addCredit(WeightFn()(elem));
  }

  /// Try to extract an element from the front of the queue if one is available.
  ///
  /// \param elem Set to the extracted element on success.
  /// \returns `true` if an element was extracted, `false` otherwise.
  ...
  /// Try to extract an element from the front of the queue if one is available.
  ///
  /// \returns The extracted element, or an empty optional if none was
  ///   available.
  ...
  /// Try to extract an element from the front of the queue, waiting until a
  /// deadline.
  ///
  /// \param elem Set to the extracted element on success.
  /// \param deadline The time point past which the attempt gives up.
  ...
  /// Try to extract an element from the front of the queue, waiting for a
  /// duration.
  ///
  /// \param elem Set to the extracted element on success.
  /// \param duration The maximum time to wait for an element.
  /// \returns `true` if an element was extracted, `false` otherwise.
  template <typename Rep, typename Period>
  ...
  /// Change the capacity of the queue without affecting its current contents.
  ///
  /// \param capacity The new weight capacity, which must be at least 1000.
  ...
  /// Estimate the total weight of the elements in the queue.
  ///
  /// \returns An estimate of the total element weight.
  ...
  /// Estimate the total number of elements in the queue.
  ///
  /// \returns An estimate of the element count.
  size_t size() const noexcept { return q_.size(); }

  /// Check whether the queue was empty during the call.
  ///
  /// \returns `true` if the queue was observed to be empty.
