...
class ObserverContainerStoreBase {
 public:
  /// Type of observer stored in this container.
  ...
  /// Shared pointer to an observer that may or may not own the observer.
...
struct ObserverContainerStorePolicyDefault {
  /// Underlying container type used to store observers.
  template <typename Observer>
  using container = std::conditional_t<
      !kIsMobile,
      folly::small_vector<Observer, ReserveElements>,
      std::vector<Observer>>;
  /// Number of elements reserved by the container.
...
struct ObserverContainerBasePolicyDefault {
  /// Return the configured bitset size.
  ///
  /// \returns The configured bitset size.
...
class ObserverContainerBase {
 public:
  ...
  class ObserverEventSet {
   public:
    /// Construct an event set with no events enabled.
     ...
     *
     * @param eventEnums   Events to enable.
    ...
    class Builder {
     public:
      /// Construct a builder with no events enabled.
      explicit Builder() = default;

      /**
       * Enables all events.
       *
       * @return This builder with all events enabled.
       */
       ...
       *
       * @param eventEnums   Events to enable.
       * @return This builder with the given events enabled.
       */
      template <typename... EventEnums>
  ...
  class ObserverBase : public ObserverInterface, public DestructorCheck {
   public:
    ...
    /// Set of events an observer can subscribe to.
    using EventSet = ObserverEventSet;
    /// Builder for constructing an EventSet.
     ...
     *
     * @param eventSet     Events the observer subscribes to.
     */
     ...
     *
     * @return The observer's EventSet.
     */
  ...
  class ManagedObserver : public Observer {
   public:
    ...
    /// Destroy the observer, detaching it from its container if attached.
 ...
 protected:
  ...
  ///
  /// \returns A reference to the underlying observer store.
  virtual ObserverContainerStoreBase<Observer>& getStore() = 0;

  /// Return the underlying observer store, const overload.
  ///
  /// \returns A const reference to the underlying observer store.
  ...
  ///
  /// \param observed The observed object passed to each observer.
  /// \param fn The interface method to invoke for each observer.
  /// \param maybeEvent The optional event that triggered the invocation.
...
class ObserverContainer
 ...
 public:
  ...
  /// Construct a container for the given observed object.
  ///
  /// \param obj The observed object this container belongs to.
  explicit ObserverContainer(Observed* obj)
      : obj_(CHECK_NOTNULL(obj)), constructorCallbackList_(this) {}

  /// Construct a container adopting observers from another container.
  ///
  /// \param obj The observed object this container belongs to.
  /// \param observerContainer The container whose observers are adopted.
   ...
   *
   * @param cb           Callback to invoke on each construction.
  ...
  ObserverContainer(const ObserverContainer& other) = delete;
  /// Deleted move constructor.
  ///
  /// \param other The container to move from.
  ObserverContainer(ObserverContainer&& other) = delete;
  /// Deleted copy assignment operator.
  ///
  /// \param other The container to copy from.
  /// \returns A reference to this container.
  ObserverContainer& operator=(const ObserverContainer& other) = delete;
