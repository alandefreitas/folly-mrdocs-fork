...
///
/// \param b Iterator to the first element to copy.
/// \param n The number of elements to copy.
/// \param d Output iterator to the start of the destination range.
/// \returns A pair holding the input and output iterators advanced past the copied range.
...
///
/// \param b Pointer to the first element to fill.
/// \param e Pointer past the last element to fill.
/// \param c The value written to every element in the range.
 ...
 * adaptation outside).
 *
 * \param b Pointer to the first element to copy.
 * \param e Pointer past the last element to copy.
 * \param d Pointer to the start of the destination range.
 */
template <class Pod>
 ...
 * some asserts.
 *
 * \param b Pointer to the first element to move.
 * \param e Pointer past the last element to move.
 * \param d Pointer to the start of the destination range.
 */
template <class Pod>
...
class fbstring_core {
 public:
  /// Constructs an empty string core.
  fbstring_core() noexcept { reset(); }

  /// Constructs a copy of another string core.
  ///
  /// \param rhs The string core to copy from.
  ...
  ///
  /// \param rhs The string core that would be assigned from.
  /// \returns A reference to this string core.
  fbstring_core& operator=(const fbstring_core& rhs) = delete;

  /// Constructs a string core by taking ownership of another's storage.
  ///
  /// \param goner The string core to move from; left empty afterwards.
  ...
  /// Constructs a string core from a character buffer.
  ///
  /// \param data Pointer to the characters to copy.
  /// \param size The number of characters to copy.
  /// \param disableSSO Whether to disable the small-string optimization.
  fbstring_core(
  ...
  /// Destroys the string core, releasing any allocated storage.
  ...
  /// Constructs a string core by taking custody of a malloc-allocated buffer.
  ///
  ...
  ///
  /// \param data Pointer to the malloc-allocated, null-terminated buffer.
  /// \param size The number of characters in the string.
  /// \param allocatedSize The size of the allocated block.
  /// \param tag Tag selecting the acquire-mallocated-string overload.
  fbstring_core(
      Char* const data,
      const size_t size,
      const size_t allocatedSize,
      AcquireMallocatedString tag) {
    if (size > 0) {
  ...
  /// Swaps the contents of this string core with another.
  ///
  ...
  /// worth.
  ///
  /// \param rhs The string core to swap with.
  ...
  ///
  /// \returns A const pointer to the character buffer.
  ...
  /// Returns a writable pointer to the string's buffer, forking shared data.
  ///
  /// \returns A pointer to the mutable character buffer.
  Char* mutableData() {
    switch (category()) {
      case Category::isSmall:
        return small_;
  ...
  /// Returns a pointer to a null-terminated version of the string's buffer.
  ///
  /// \returns A const pointer to the null-terminated character buffer.
  const Char* c_str() const {
  ...
  ///
  /// \param delta The number of characters to remove from the end.
  ...
  ///
  /// \param minCapacity The minimum capacity to guarantee.
  /// \param disableSSO Whether to disable the small-string optimization.
  FOLLY_NOINLINE
  ...
  ///
  /// \param delta The number of characters to add.
  /// \param expGrowth Whether to guarantee exponential growth.
  /// \param disableSSO Whether to disable the small-string optimization.
  /// \returns A pointer to the start of the uninitialized expanded region.
  Char* expandNoinit(
      const size_t delta,
  ...
  /// Returns the maximum size the string can hold without reallocation.
  ///
  ...
  /// Returns whether the underlying data is shared across multiple strings.
  ///
  /// \returns `true` if the data is reference-counted and shared.
  bool isShared() const {
  ...
  ///
  /// \returns `true` if the string uses Large (reference-counted) storage.
 ...
 private:
  ...
  union {
    /// Raw byte view of the storage, used to access the last (category) byte.
    uint8_t bytes_[sizeof(MediumLarge)]; // For accessing the last byte.
    /// In-situ buffer holding a small string.
    Char small_[sizeof(MediumLarge) / sizeof(Char)];
    /// Pointer/size/capacity view used for medium and large strings.
    MediumLarge ml_;
  };
...
class dummy_fbstring_core {
 public:
  /// Constructs an empty dummy core.
  dummy_fbstring_core() {}
  /// Constructs a copy of another dummy core.
  ///
  /// \param another The dummy core to copy from.
  dummy_fbstring_core(const dummy_fbstring_core& another)
      : backend_(another.backend_) {}
  /// Constructs a dummy core from a character buffer.
  ///
  /// \param s Pointer to the characters to copy.
  /// \param n The number of characters to copy.
  dummy_fbstring_core(const Char* s, size_t n) : backend_(s, n) {}
  /// Swaps the contents with another dummy core.
  ///
  /// \param rhs The dummy core to swap with.
  ...
  ///
  /// \param delta The number of characters to add.
  /// \returns A pointer to the start of the expanded region.
  ...
  ///
  /// \returns Always `false` for the dummy core.
  bool isShared() const { return false; }
  /// Returns the reference count of the underlying storage.
  ///
  /// \returns Always 1 for the dummy core.
...
class basic_fbstring {
 ...
 private:
  ...
  /// Constructs a string from a string view (private delegating constructor).
  ///
  /// \param view The string view whose characters are copied.
  /// \param a The allocator (ignored; fbstring ignores custom allocators).
  FOLLY_NOINLINE basic_fbstring(
      string_view_type view, const A& a, string_view_ctor)
 ...
 public:
  ...
  /// Construction from `nullptr` is deleted.
  /// \param null The null pointer that would be converted from.
  /* implicit */ basic_fbstring(std::nullptr_t null) = delete;

  /// Constructs an empty string with the given allocator.
  ///
  /// \param a The allocator (ignored; fbstring ignores custom allocators).
  ...
  /// Constructs a string by moving from another string.
  ///
  /// \param goner The string to move from; left empty afterwards.
  basic_fbstring(basic_fbstring&& goner) noexcept
      : store_(std::move(goner.store_)) {}

  /// Constructs a string from a `std::basic_string`.
  ///
  /// This is defined for compatibility with std::string.
  ///
  /// \param str The standard string to copy from.
  template <typename A2>
  /* implicit */ basic_fbstring(const std::basic_string<E, T, A2>& str)
      : store_(str.data(), str.size()) {}

  /// Constructs a string from a substring of another string.
  ///
  /// \param str The source string.
  /// \param pos The starting position of the substring.
  /// \param n The number of characters to copy.
  /// \param a The allocator (ignored).
  basic_fbstring(
  ...
  ///
  /// \param s Pointer to the null-terminated characters to copy.
  /// \param a The allocator (ignored).
  FOLLY_NOINLINE
  /* implicit */ basic_fbstring(const value_type* s, const A& a = A())
      : store_(s, traitsLength(s)) {}

  /// Constructs a string from the first `n` characters of an array.
  ///
  /// \param s Pointer to the characters to copy.
  /// \param n The number of characters to copy.
  /// \param a The allocator (ignored).
  ...
  /// Constructs a string of `n` copies of a character.
  ///
  ...
  /// \param end Iterator past the last character.
  /// \param a The allocator (ignored; fbstring ignores custom allocators).
  template <class InIt>
  FOLLY_NOINLINE basic_fbstring(
      InIt begin,
  ...
  /// Constructs a string from a pair of character pointers.
  ///
  /// \param b Pointer to the first character.
  /// \param e Pointer past the last character.
  /// \param a The allocator (ignored).
  FOLLY_NOINLINE
  basic_fbstring(const value_type* b, const value_type* e, const A& a = A())
      : store_(b, size_type(e - b)) {}

  /// Constructs a string by taking custody of a malloc-allocated buffer.
  ///
  /// \param s Pointer to the malloc-allocated buffer.
  /// \param n The number of characters in the string.
  /// \param c The capacity of the allocated buffer.
  /// \param a Tag selecting the acquire-mallocated-string overload.
  basic_fbstring(
      value_type* s, size_type n, size_type c, AcquireMallocatedString a)
      : store_(s, n, c, a) {}

  /// Constructs a string from an initializer list of characters.
  ///
  /// \param il The initializer list of characters.
  FOLLY_NOINLINE
  basic_fbstring(std::initializer_list<value_type> il) {
    assign(il.begin(), il.end());
  }

  /// Constructs a string from a string-view-like object.
  ///
  /// \param view The string-view-like object to copy from.
  /// \param a The allocator (ignored).
  template <
  ...
  /// Constructs a string from a substring of a string-view-like object.
  ///
  /// \param view The string-view-like object to copy from.
  /// \param pos The starting position of the substring.
  /// \param n The number of characters to copy.
  ...
  /// Copy-assigns from another string.
  ///
  /// \param lhs The string to copy from.
  ...
  /// Assigns from a `std::basic_string`.
  ///
  /// Compatibility with std::string.
  ///
  /// \param rhs The standard string to copy from.
  /// \returns A reference to this string.
  template <typename A2>
  ...
  /// Compatibility with std::string.
  ///
  /// \returns A `std::basic_string` copy of this string's contents.
  std::basic_string<E, T, A> toStdString() const {
    return std::basic_string<E, T, A>(data(), size());
  }

  /// Assignment from `nullptr` is deleted.
  ///
  /// \param null The null pointer that would be assigned from.
  /// \returns A reference to this string.
  basic_fbstring& operator=(std::nullptr_t null) = delete;

  /// Assigns from a null-terminated character array.
  ///
  /// \param s Pointer to the null-terminated characters to copy.
  /// \returns A reference to this string.
  basic_fbstring& operator=(const value_type* s) { return assign(s); }

  /// Assigns a single character to the string.
  ///
  /// \param c The character to assign.
  /// \returns A reference to this string.
  basic_fbstring& operator=(value_type c);

  /// Assignment from a type convertible to `value_type` is deleted.
  ///
  ...
  ///
  /// \param c The value that would be assigned.
  /// \returns A reference to this string.
  ...
  /// Assigns from an initializer list of characters.
  ///
  /// \param il The initializer list of characters.
  /// \returns A reference to this string.
  ...
  ///
  /// \returns A string view referring to this string's contents.
  ...
  /// Returns a reverse iterator past the first character.
  ///
  ...
  ///
  /// \param res_arg The minimum capacity to guarantee.
  ...
  ///
  /// \param pos The index of the character.
  /// \returns A reference to the character at `pos`.
  ...
  /// \returns A const reference to the character at `n`.
  ...
  ///
  /// \param n The index of the character.
  /// \returns A reference to the character at `n`.
  ...
  ///
  /// \param s Pointer to the null-terminated characters to append.
  /// \returns A reference to this string.
  ...
  /// Appends an initializer list of characters.
  ///
  /// \param il The initializer list of characters to append.
  /// \returns A reference to this string.
  basic_fbstring& operator+=(std::initializer_list<value_type> il) {
    append(il);
    return *this;
  ...
  /// Appends a string-view-like object.
  ///
  /// \param like The string-view-like object to append.
  /// \returns A reference to this string.
  template <
      typename StringViewLike,
      if_is_string_view_like_t<StringViewLike, int> = 0>
  ...
  /// Appends a substring of another string.
  ///
  /// \param str The source string.
  /// \param pos The starting position of the substring.
  ...
  /// Appends the first `n` characters of an array.
  ///
  /// \param s Pointer to the characters to append.
  /// \param n The number of characters to append.
  /// \returns A reference to this string.
  ...
  /// Appends `n` copies of a character.
  ///
  /// \param n The number of characters to append.
  /// \param c The character to repeat.
  /// \returns A reference to this string.
  ...
  /// Appends the characters in an iterator range.
  ///
  ...
  /// Assigns the contents of another string.
  ///
  ...
  /// Assigns by moving from another string.
  ///
  ...
  /// Assigns a substring of another string.
  ///
  /// \param str The source string.
  /// \param pos The starting position of the substring.
  ...
  /// Assigns the first `n` characters of an array.
  ///
  ...
  /// Assigns a null-terminated character array.
  ///
  /// \param s Pointer to the null-terminated characters to assign.
  /// \returns A reference to this string.
  basic_fbstring& assign(const value_type* s) {
    return assign(s, traitsLength(s));
  }

  /// Assigns an initializer list of characters.
  ///
  /// \param il The initializer list of characters to assign.
  /// \returns A reference to this string.
  basic_fbstring& assign(std::initializer_list<value_type> il) {
    return assign(il.begin(), il.end());
  }

  /// Assigns from a count and character, or from an iterator range.
  ///
  /// \param first_or_n Either the first iterator or the number of characters.
  /// \param last_or_c Either the last iterator or the character to repeat.
  /// \returns A reference to this string.
  ...
  /// Inserts another string at a given position.
  ///
  /// \param pos1 The position at which to insert.
  ...
  /// Inserts a substring of another string at a given position.
  ///
  /// \param pos1 The position at which to insert.
  /// \param str The source string.
  /// \param pos2 The starting position within `str`.
  ...
  /// Inserts the first `n` characters of an array at a given position.
  ///
  /// \param pos The position at which to insert.
  /// \param s Pointer to the characters to insert.
  /// \param n The number of characters to insert.
  /// \returns A reference to this string.
  ...
  /// Inserts a null-terminated character array at a given position.
  ///
  /// \param pos The position at which to insert.
  /// \param s Pointer to the null-terminated characters to insert.
  /// \returns A reference to this string.
  basic_fbstring& insert(size_type pos, const value_type* s) {
    return insert(pos, s, traitsLength(s));
  }

  /// Inserts `n` copies of a character at a given position.
  ///
  /// \param pos The position at which to insert.
  /// \param n The number of characters to insert.
  /// \param c The character to repeat.
  ...
  /// Inserts a single character before an iterator position.
  ///
  /// \param p Iterator to the position before which to insert.
  /// \param c The character to insert.
  /// \returns An iterator to the inserted character.
 ...
 public:
  ...
  /// \param delim The delimiter character that stops reading.
 ...
 public:
  /// Inserts a count and character, or an iterator range, at a position.
  ///
  /// \param p Iterator to the position before which to insert.
  /// \param first_or_n Either the first iterator or the number of characters.
  /// \param last_or_c Either the last iterator or the character to repeat.
  /// \returns An iterator to the first inserted character.
  template <class ItOrLength, class ItOrChar>
  ...
  /// Inserts an initializer list of characters at a position.
  ///
  /// \param p Iterator to the position before which to insert.
  /// \param il The initializer list of characters to insert.
  /// \returns An iterator to the first inserted character.
  iterator insert(const_iterator p, std::initializer_list<value_type> il) {
    return insert(p, il.begin(), il.end());
  }

  /// Erases up to `n` characters starting at a position.
  ///
  /// \param pos The starting position of the range to erase.
  ...
  /// Erases the character at an iterator position.
  ///
  /// \param position Iterator to the character to erase.
  /// \returns An iterator to the character after the erased one.
  ...
  /// Erases the characters in an iterator range.
  ///
  /// \param first Iterator to the first character to erase.
  /// \param last Iterator past the last character to erase.
  /// \returns An iterator to the character after the erased range.
  ...
  /// content of str.
  ///
  /// \param pos1 The starting position of the range to replace.
  /// \param n1 The maximum number of characters to replace.
  ...
  /// \param str The replacement string.
  /// \param pos2 The starting position within `str`.
  /// \param n2 The maximum number of characters from `str` to use.
  /// \returns A reference to this string.
  basic_fbstring& replace(
      size_type pos1,
      size_type n1,
  ...
  /// \param n1 The maximum number of characters to replace.
  /// \param s Pointer to the null-terminated replacement characters.
  /// \returns A reference to this string.
  ...
  /// \param pos The starting position of the range to replace.
  /// \param n1 The maximum number of characters to replace.
  /// \param s_or_n2 Either the replacement buffer or a repeat count.
  /// \param n_or_c Either the buffer length or the character to repeat.
  /// \returns A reference to this string.
  ...
  ///
  /// \param i1 Iterator to the first character of the range to replace.
  /// \param i2 Iterator past the last character of the range to replace.
  /// \param str The replacement string.
  /// \returns A reference to this string.
 ...
 public:
  ...
  /// \param i1 Iterator to the first character of the range to replace.
  /// \param i2 Iterator past the last character of the range to replace.
  /// \param first_or_n_or_s The replacement buffer, count, or first iterator.
  /// \param last_or_c_or_n The buffer length, character, or last iterator.
  /// \returns A reference to this string.
  ...
  /// \param pos The starting position to copy from.
  ...
  ///
  /// \returns A default-constructed allocator.
  ...
  /// Finds the first occurrence of the first `nsize` characters of a buffer.
  ///
  /// \param needle Pointer to the characters to search for.
  /// \param pos The position to start searching from.
  /// \param nsize The number of characters in the needle.
  /// \returns The position of the first match, or `npos` if not found.
  size_type find(
      const value_type* needle, size_type pos, size_type nsize) const;

  /// Finds the first occurrence of a null-terminated array.
  ///
  /// \param s Pointer to the null-terminated characters to search for.
  /// \param pos The position to start searching from.
  /// \returns The position of the first match, or `npos` if not found.
  ...
  ///
  /// \param str The string to search for.
  /// \param pos The position at which to begin searching backwards.
  /// \returns The position of the last match, or `npos` if not found.
  size_type rfind(const basic_fbstring& str, size_type pos = npos) const {
    return rfind(str.data(), pos, str.length());
  }

  /// Finds the last occurrence of the first `n` characters of a buffer.
  ///
  /// \param s Pointer to the characters to search for.
  /// \param pos The position at which to begin searching backwards.
  /// \param n The number of characters in the needle.
  /// \returns The position of the last match, or `npos` if not found.
  size_type rfind(const value_type* s, size_type pos, size_type n) const;

  /// Finds the last occurrence of a null-terminated array.
  ///
  /// \param s Pointer to the null-terminated characters to search for.
  /// \param pos The position at which to begin searching backwards.
  /// \returns The position of the last match, or `npos` if not found.
  ...
  /// Finds the first character equal to any character in another string.
  ///
  /// \param str The set of characters to search for.
  /// \param pos The position to start searching from.
  /// \returns The position of the first match, or `npos` if not found.
  ...
  /// Finds the first character equal to any of the first `n` characters.
  ///
  /// \param s Pointer to the set of characters to search for.
  /// \param pos The position to start searching from.
  /// \param n The number of characters in the set.
  /// \returns The position of the first match, or `npos` if not found.
  size_type find_first_of(
      const value_type* s, size_type pos, size_type n) const;

  /// Finds the first character equal to any character in a null-terminated set.
  ///
  /// \param s Pointer to the null-terminated set of characters.
  /// \param pos The position to start searching from.
  /// \returns The position of the first match, or `npos` if not found.
  ...
  /// Finds the last character equal to any character in another string.
  ///
  /// \param str The set of characters to search for.
  /// \param pos The position at which to begin searching backwards.
  /// \returns The position of the last match, or `npos` if not found.
  ...
  /// Finds the last character equal to any of the first `n` characters.
  ///
  /// \param s Pointer to the set of characters to search for.
  /// \param pos The position at which to begin searching backwards.
  /// \param n The number of characters in the set.
  ...
  /// Finds the last character equal to any character in a null-terminated set.
  ///
  /// \param s Pointer to the null-terminated set of characters.
  /// \param pos The position at which to begin searching backwards.
  /// \returns The position of the last match, or `npos` if not found.
  ...
  ///
  /// \param str The set of characters to exclude.
  /// \param pos The position to start searching from.
  /// \returns The position of the first non-match, or `npos` if none.
  ...
  ///
  /// \param s Pointer to the set of characters to exclude.
  /// \param pos The position to start searching from.
  /// \param n The number of characters in the set.
  /// \returns The position of the first non-match, or `npos` if none.
  size_type find_first_not_of(
  ...
  ///
  /// \param s Pointer to the null-terminated set of characters to exclude.
  /// \param pos The position to start searching from.
  /// \returns The position of the first non-match, or `npos` if none.
  ...
  /// Finds the last character not equal to any character in another string.
  ///
  /// \param str The set of characters to exclude.
  /// \param pos The position at which to begin searching backwards.
  /// \returns The position of the last non-match, or `npos` if none.
  size_type find_last_not_of(
      const basic_fbstring& str, size_type pos = npos) const {
    return find_last_not_of(str.data(), pos, str.length());
  }

  /// Finds the last character not equal to any of the first `n` characters.
  ///
  /// \param s Pointer to the set of characters to exclude.
  /// \param pos The position at which to begin searching backwards.
  /// \param n The number of characters in the set.
  ...
  /// Finds the last character not in a null-terminated set.
  ///
  /// \param s Pointer to the null-terminated set of characters to exclude.
  /// \param pos The position at which to begin searching backwards.
  /// \returns The position of the last non-match, or `npos` if none.
  ...
  /// Finds the last character not equal to a given character.
  ///
  /// \param c The character to exclude.
  /// \param pos The position at which to begin searching backwards.
  /// \returns The position of the last non-match, or `npos` if none.
  ...
  /// Returns a substring as a new string (copy overload).
  ///
  /// \param pos The starting position of the substring.
  /// \param n The number of characters in the substring.
  /// \returns A new string holding the requested substring.
  basic_fbstring substr(size_type pos = 0, size_type n = npos) const& {
    enforce<std::out_of_range>(pos <= size(), "");
    return basic_fbstring(data() + pos, std::min(n, size() - pos));
  }

  /// Returns a substring, reusing this string's storage (rvalue overload).
  ///
  /// \param pos The starting position of the substring.
  /// \param n The number of characters in the substring.
  /// \returns A string holding the requested substring.
  ...
  /// \returns A negative, zero, or positive value for less, equal, or greater.
  int compare(const basic_fbstring& str) const {
    // FIX due to Goncalo N M de Carvalho July 18, 2005
    return compare(0, size(), str);
  }

  /// Compares a substring of this string with another string.
  ///
  /// \param pos1 The starting position of this string's substring.
  /// \param n1 The length of this string's substring.
  /// \param str The string to compare against.
  /// \returns A negative, zero, or positive value for less, equal, or greater.
  ...
  /// Compares a substring of this string with a null-terminated array.
  ///
  /// \param pos1 The starting position of this string's substring.
  /// \param n1 The length of this string's substring.
  /// \param s Pointer to the null-terminated characters to compare against.
  /// \returns A negative, zero, or positive value for less, equal, or greater.
  int compare(size_type pos1, size_type n1, const value_type* s) const {
    return compare(pos1, n1, s, traitsLength(s));
  }

  /// Compares a substring of this string with the first `n2` characters.
  ///
  /// \param pos1 The starting position of this string's substring.
  /// \param n1 The length of this string's substring.
  /// \param s Pointer to the characters to compare against.
  /// \param n2 The number of characters to compare against.
  /// \returns A negative, zero, or positive value for less, equal, or greater.
  int compare(
  ...
  /// \param n1 The length of this string's substring.
  /// \param str The string to compare against.
  /// \param pos2 The starting position of the other string's substring.
  /// \param n2 The length of the other string's substring.
  /// \returns A negative, zero, or positive value for less, equal, or greater.
  int compare(
      size_type pos1,
      size_type n1,
  ...
  /// Compares this string with a null-terminated array.
  ///
  /// \param s Pointer to the null-terminated characters to compare against.
  /// \returns A negative, zero, or positive value for less, equal, or greater.
  ...
  ///
  /// \param lhs The left-hand string.
  /// \param rhs Pointer to the null-terminated right-hand characters.
  /// \returns The ordering of `lhs` relative to `rhs`.
  ...
  ///
  /// \param lhs The left-hand string.
  /// \param rhs The right-hand standard string.
  /// \returns The ordering of `lhs` relative to `rhs`.
  template <typename A2>
  friend auto operator<=>(
...
/// \param lhs The left-hand string.
/// \param rhs The right-hand string.
/// \returns A new string holding the concatenation.
template <typename E, class T, class A, class S>
...
///
/// \param lhs The left-hand string, moved from.
/// \param rhs The right-hand string.
/// \returns A new string holding the concatenation.
template <typename E, class T, class A, class S>
inline basic_fbstring<E, T, A, S> operator+(
...
///
/// \param lhs The left-hand string.
/// \param rhs The right-hand string, moved from.
/// \returns A new string holding the concatenation.
template <typename E, class T, class A, class S>
inline basic_fbstring<E, T, A, S> operator+(
...
///
/// \param lhs Pointer to the null-terminated left-hand characters.
/// \param rhs The right-hand string.
/// \returns A new string holding the concatenation.
template <typename E, class T, class A, class S>
inline basic_fbstring<E, T, A, S> operator+(
...
///
/// \param lhs The left-hand character.
/// \param rhs The right-hand string.
/// \returns A new string holding the concatenation.
template <typename E, class T, class A, class S>
inline basic_fbstring<E, T, A, S> operator+(
...
///
/// \param lhs The left-hand string.
/// \param rhs The right-hand character.
/// \returns A new string holding the concatenation.
template <typename E, class T, class A, class S>
inline basic_fbstring<E, T, A, S> operator+(
...
/// \param lhs The left-hand string.
/// \param rhs The right-hand string.
/// \returns `true` if the strings have equal contents.
template <typename E, class T, class A, class S>
...
///
/// \param lhs The null pointer left-hand operand.
/// \param rhs The right-hand string.
/// \returns Never returns; this overload is deleted.
template <typename E, class T, class A, class S>
inline bool operator==(
    std::nullptr_t lhs, const basic_fbstring<E, T, A, S>& rhs) = delete;

/// Compares a null-terminated array and a string for equality.
///
/// \param lhs Pointer to the null-terminated left-hand characters.
/// \param rhs The right-hand string.
...
///
/// \param lhs The left-hand string.
/// \param rhs The null pointer right-hand operand.
/// \returns Never returns; this overload is deleted.
template <typename E, class T, class A, class S>
inline bool operator==(
    const basic_fbstring<E, T, A, S>& lhs, std::nullptr_t rhs) = delete;
...
/// \param lhs The left-hand string.
/// \param rhs The right-hand string.
/// \returns `true` if the strings differ.
template <typename E, class T, class A, class S>
inline bool operator!=(
    const basic_fbstring<E, T, A, S>& lhs,
    const basic_fbstring<E, T, A, S>& rhs) {
...
/// \param lhs Pointer to the null-terminated left-hand characters.
/// \param rhs The right-hand string.
/// \returns `true` if the contents differ.
template <typename E, class T, class A, class S>
inline bool operator!=(
    const typename basic_fbstring<E, T, A, S>::value_type* lhs,
    const basic_fbstring<E, T, A, S>& rhs) {
...
/// Extracts a whitespace-delimited word from a stream into a string.
///
/// \param is The input stream to read from.
/// \param str The string that receives the extracted word.
/// \returns A reference to the input stream.
...
///
/// \param lhs The left-hand fbstring.
/// \param rhs The right-hand standard string.
/// \returns `true` if the contents are equal.
...
/// Compares a `std::basic_string` and an fbstring for equality.
///
/// \param lhs The left-hand standard string.
/// \param rhs The right-hand fbstring.
/// \returns `true` if the contents are equal.
template <typename E, class T, class A, class S, class A2>
inline bool operator==(
...
/// Orders a `std::basic_string` after an fbstring.
///
/// \param lhs The left-hand standard string.
/// \param rhs The right-hand fbstring.
/// \returns `true` if `lhs` is lexicographically greater than `rhs`.
...
/// Orders a `std::basic_string` at or before an fbstring.
///
/// \param lhs The left-hand standard string.
/// \param rhs The right-hand fbstring.
/// \returns `true` if `lhs` is not greater than `rhs`.
...
/// Orders a `std::basic_string` at or after an fbstring.
///
/// \param lhs The left-hand standard string.
/// \param rhs The right-hand fbstring.
/// \returns `true` if `lhs` is not less than `rhs`.
...
/// Marks `basic_fbstring` as trivially relocatable.
...
///
/// \param s The fbstring to convert.
/// \returns A `std::string` copy of `s`.
...
///
/// \param s The string to move from.
/// \returns An rvalue reference to `s`.
...
#define FOLLY_FBSTRING_HASH1(T)                                               \
  /** Hash support for `fbstring`. \implementationdefined */                  \
  template <>                                                                 \
  struct hash<::folly::basic_fbstring<T>> {                                   \
    /** Hashes the string contents. \implementationdefined */                 \
...
/// Specialization marking fbstring as a string-like type.
template <>
...
struct fmt::formatter<folly::fbstring> : private formatter<fmt::string_view> {
  ...
  ///
  /// \param s The fbstring to format.
  /// \param ctx The formatting context to write to.
  /// \returns An iterator past the last written character.
