...
/// A Traits type that controls the object lifecycle strategy of an
/// IndexedMemPool.
template <
    typename T,
...
struct IndexedMemPoolTraits {
  /// Determines whether elements are eagerly constructed and destroyed.
  /// \returns `true` when the eager-recycle strategy is used for T.
  ...
  /// first time.
  /// \param ptr A pointer to the element being initialized.
  ...
  /// destruction time.
  /// \param ptr A pointer to the element being cleaned up.
  ...
  /// \param ptr A pointer to the element being allocated.
  ...
  /// \param ptr A pointer to the element being recycled.
...
struct IndexedMemPool {
  /// The type of element managed by the pool.
  using value_type = T;

  /// A unique-pointer type that recycles its element back into the pool.
  using UniquePtr =
      std::unique_ptr<T, detail::IndexedMemPoolRecycler<IndexedMemPool>>;

  /// Deleted copy constructor.
  /// \param other The instance that would be copied.
  IndexedMemPool(const IndexedMemPool& other) = delete;
  /// Deleted copy assignment.
  /// \param other The instance that would be assigned from.
  /// \returns A reference to this pool.
  IndexedMemPool& operator=(const IndexedMemPool& other) = delete;

  static_assert(LocalListLimit_ <= 255, "LocalListLimit must fit in 8 bits");
  /// Compile-time constants describing the pool's local-list configuration.
  enum {
    NumLocalLists = NumLocalLists_, ///< The number of per-thread local lists.
    LocalListLimit = LocalListLimit_, ///< The maximum size of a local list.
  };
  ...
  /// Computes the largest index that may be produced for a given capacity.
  /// \param capacity The requested minimum capacity.
  /// \returns The maximum index that the pool may hand out.
  ...
  /// Computes the capacity that corresponds to a given maximum index.
  /// \param maxIndex The maximum index handed out by the pool.
  /// \returns The corresponding lower-bound capacity.
  ...
  /// \param capacity The minimum number of elements the pool can allocate.
  ...
  /// successfully
  /// \returns A lower bound on the number of simultaneously allocatable
  /// elements.
  ...
  /// \returns The maximum index ever allocated.
  ...
  /// allocated.
  /// \param args The arguments forwarded to Traits::onAllocate.
  /// \returns The index of the allocated element, or 0 if none are available.
  template <typename... Args>
  ...
  /// \param args The arguments forwarded to Traits::onAllocate.
  /// \returns A unique-pointer to the element, or a null pointer if none are
  /// available.
  template <typename... Args>
  ...
  /// \param idx The index of the element to recycle.
  ...
  /// \returns A reference to the pooled element.
  T& operator[](uint32_t idx) { return *(slot(idx).elemPtr()); }

  /// Provides access to the pooled element referenced by idx
  /// \param idx The index of the element to access.
  /// \returns A const reference to the pooled element.
  ...
  /// \param elem A pointer to a pooled element, or nullptr.
  /// \returns The index of the element, or 0 when elem is nullptr.
  ...
  /// \returns `true` when the index is currently allocated.
 ...
 public:
  /// The size in bytes of a single internal slot.
