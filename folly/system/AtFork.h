...
class AtForkList {
 public:
  ...
  ///
  ///  \param handle Key identifying the set of handlers, or nullptr.
  ///  \param prepare Handler run before fork in the parent.
  ///  \param parent Handler run after fork in the parent.
  ///  \param child Handler run after fork in the child.
  void append( //
      void const* handle,
  ...
  ///  null.
  ///
  ///  \param handle Key identifying the set of handlers to remove.
  void remove( //
      void const* handle);
  ...
  ///
  ///  \param handle Key identifying the set of handlers to find.
  ///  \returns True if a set of handlers with the given handle is found.
  bool contains( //
      void const* handle);
...
class AtFork {
 public:
  /// Initializes the AtFork singleton, installing the pthread_atfork handlers.
  static void init();

  /// Registers a set of fork handlers indexed by handle.
  ///
  /// \param handle Key identifying the set of handlers.
  /// \param prepare Handler run before fork in the parent.
  /// \param parent Handler run after fork in the parent.
  /// \param child Handler run after fork in the child.
  ...
  /// Unregisters the set of fork handlers identified by handle.
  ///
  /// \param handle Key identifying the set of handlers to remove.
  static void unregisterHandler(void const* handle);

  /// Signature of a fork function.
  using fork_t = pid_t();

  /// Performs a fork with the registered handlers invoked around it.
  ///
  /// \param forkFn The fork function to invoke.
  /// \returns The value returned by the fork function.
