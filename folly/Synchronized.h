...
class SynchronizedBase<Subclass, detail::SynchronizedMutexLevel::Shared> {
 ...
 public:
   ...
   *
   * \returns A `LockedPtr` holding the datum under an exclusive lock.
   *
   ...
   * validity.)
   *
   * \returns A `LockedPtr` holding the exclusive lock, or null if it could
   *          not be acquired.
   *
   * @methodset Exclusive lock
   */
   ...
   *
   * \returns A `LockedPtr` holding the datum under a shared lock.
   *
   ...
   * validity.)
   *
   * \returns A `LockedPtr` holding the shared lock, or null if it could not
   *          be acquired.
   *
   * @methodset Shared lock
   */
   ...
   * \returns A `LockedPtr` holding the exclusive lock, or null if the timeout
   *          elapsed.
   *
   * @methodset Exclusive lock
   */
   ...
   *
   * \param timeout The maximum time to wait for the lock.
   * \returns A `LockedPtr` holding the shared lock, or null if the timeout
   *          elapsed.
   *
   * @methodset Shared lock
   */
   ...
   *   });
   *
   * \param function The callable invoked with a reference to the datum.
   ...
   * desired.
   *
   * \param function The callable invoked with a `LockedPtr` to the datum.
   * \returns The value returned by `function`.
   *
   * @methodset Exclusive lock
   */
   ...
   * only argument.
   *
   * \param function The callable invoked with a const reference to the datum.
   * \returns The value returned by `function`.
   *
   * @methodset Shared lock
   */
...
class SynchronizedBase<Subclass, detail::SynchronizedMutexLevel::Upgrade>
 ...
 public:
   ...
   *
   * \returns A `LockedPtr` holding the datum under an upgrade lock.
   *
   ...
   * validity.)
   *
   * \returns A `LockedPtr` holding the upgrade lock, or null if it could not
   *          be acquired.
   *
   * @methodset Upgrade lock
   */
   ...
   *
   * \param timeout The maximum time to wait for the lock.
   * \returns A `LockedPtr` holding the upgrade lock, or null if the timeout
   *          elapsed.
   *
   * @methodset Upgrade lock
   */
...
class SynchronizedBase<Subclass, detail::SynchronizedMutexLevel::Unique> {
 ...
 public:
   ...
   * access the datum.
   *
   * \returns A `ConstLockedPtr` holding the datum under an exclusive lock.
   *
   * @methodset Non-shareable lock
   */
   ...
   * validity.)
   *
   * \returns A `LockedPtr` holding the lock, or null if it could not be
   *          acquired.
   *
   * @methodset Non-shareable lock
   */
   ...
   *
   * \param timeout The maximum time to wait for the lock.
   * \returns A `LockedPtr` holding the lock, or null if the timeout elapsed.
   *
   * @methodset Non-shareable lock
   */
  template <class Rep, class Period>
   ...
   *
   * \param timeout The maximum time to wait for the lock.
   * \returns A `ConstLockedPtr` holding the lock, or null if the timeout
   *          elapsed.
   *
   * @methodset Non-shareable lock
   */
...
struct Synchronized
 ...
 public:
   ...
   * deprecated
   *
   * \param rhs The source object to move from.
   */
   ...
   *
   * \param rhs The datum to copy into the new object.
   */
   ...
   *
   * \param rhs The datum to move into the new object.
   */
   ...
   *
   * \param ip A tag selecting in-place construction of the datum.
   * \param args Arguments forwarded to the datum's constructor.
   */
  template <typename... Args>
  explicit constexpr Synchronized(std::in_place_t ip, Args&&... args)
   ...
   *
   * \param pc A tag selecting piecewise construction.
   * \param datumArgs Arguments forwarded to the datum's constructor.
   * \param mutexArgs Arguments forwarded to the mutex's constructor.
   */
   ...
   * deprecated
   *
   * \param rhs The source object to copy-assign from.
   ...
   * deprecated
   *
   * \param rhs The source object to move-assign from.
   * \returns A reference to this object.
   */
   ...
   *
   * \param rhs The datum to copy-assign.
   * \returns A reference to this object.
   */
   ...
   *
   * \param rhs The datum to move-assign.
   * \returns A reference to this object.
   */
   ...
   *
   * \returns A `LockedPtr` holding the datum under an appropriate lock.
   */
  LockedPtr contextualLock() { return LockedPtr(this); }
  /// Acquire a lock on the datum, in const context.
  ///
  /// \returns A `ConstLockedPtr` holding the datum under an appropriate lock.
  ConstLockedPtr contextualLock() const { return ConstLockedPtr(this); }
  /// Acquire a lock on the datum, or fail if the timeout elapses.
  ///
  /// \param timeout The maximum time to wait for the lock.
  /// \returns A `LockedPtr` holding the datum, or null if the timeout elapsed.
  template <class Rep, class Period>
  LockedPtr contextualLock(const std::chrono::duration<Rep, Period>& timeout) {
    return LockedPtr(this, timeout);
  }
  /// Acquire a lock on the datum in const context, or fail if the timeout
  /// elapses.
  ///
  /// \param timeout The maximum time to wait for the lock.
  /// \returns A `ConstLockedPtr` holding the datum, or null if the timeout
  ///          elapsed.
  template <class Rep, class Period>
   ...
   *
   * \returns A `ConstLockedPtr` holding the datum under a read lock.
   */
  ConstLockedPtr contextualRLock() const { return ConstLockedPtr(this); }
  /// Acquire a read lock on the datum, or fail if the timeout elapses.
  ///
  /// \param timeout The maximum time to wait for the lock.
  /// \returns A `ConstLockedPtr` holding the datum, or null if the timeout
  ///          elapsed.
   ...
   * held only briefly.
   *
   * \param rhs The datum to swap with.
   */
  void swap(T& rhs) {
   ...
   *
   * \param rhs The new datum to assign.
   * \returns The previous value of the datum.
   */
  T exchange(T&& rhs) {
    swap(rhs);
    return std::move(rhs);
   ...
   *
   * \param target The destination assigned a copy of the datum.
   */
  ...
  /// Access the datum without locking, in const context.
  ///
  /// \returns A const reference to the datum.
...
ImplicitSynchronized : Synchronized<T, Mutex> {
 ...
 public:
   ...
   *
   * \returns A `LockedPtr` that locks the object and forwards member access.
   */
   ...
   * Prefer using lock(), wlock(), or rlock() instead.
   *
   * \returns A `ConstLockedPtr` that locks the object and forwards member
   *          access.
   */
  [[deprecated(
...
class ScopedUnlocker {
 public:
  ...
  ///
  /// \param p The locked pointer to temporarily unlock.
  ...
  ///
  /// \param other The unlocker that would be copied from.
  ScopedUnlocker(const ScopedUnlocker& other) = delete;
  /// Copy assignment is disabled.
  ///
  /// \param other The unlocker that would be assigned from.
  /// \returns A reference to this unlocker.
  ScopedUnlocker& operator=(const ScopedUnlocker& other) = delete;
  /// Move-construct, taking over the unlock state from `other`.
  ///
  /// \param other The unlocker to move from.
  ScopedUnlocker(ScopedUnlocker&& other) noexcept
      : ptr_(std::exchange(other.ptr_, nullptr)),
        parent_(std::exchange(other.parent_, nullptr)) {}
  /// Move assignment is disabled.
  ///
  /// \param other The unlocker that would be moved from.
  /// \returns A reference to this unlocker.
  ScopedUnlocker& operator=(ScopedUnlocker&& other) = delete;

  /// Re-lock the associated pointer, unless this unlocker was moved from.
...
class LockedPtr {
 private:
  ...
  static constexpr bool kSameLevelAs =
      LockPolicy::level == LockPolicyOther::level;
 ...
 public:
   ...
   *
   * \param parent The object to lock, or null for an empty pointer.
   */
   ...
   * Move constructor.
   *
   * \param rhs The locked pointer to move from.
   */
  LockedPtr(LockedPtr&& rhs) noexcept = default;
  /// Converting move constructor from a non-const locked pointer.
  ///
  /// \param rhs The locked pointer to move from.
  template <typename Type = SynchronizedType>
    requires std::is_const_v<Type>
  /* implicit */ LockedPtr(LockedPtr<Synchronized, LockPolicy>&& rhs) noexcept
      : lock_{std::move(rhs.lock_)} {}
  /// Converting move constructor from a same-level lock policy.
  ///
  /// \param other The locked pointer to move from.
  template <typename LockPolicyType>
    requires kSameLevelAs<LockPolicyType>
  explicit LockedPtr(
      LockedPtr<SynchronizedType, LockPolicyType>&& other) noexcept
      : lock_{std::move(other.lock_)} {}
  /// Converting move constructor from a non-const, same-level locked pointer.
  ///
  /// \param rhs The locked pointer to move from.
  template <typename Type = SynchronizedType, typename LockPolicyType>
    requires std::is_const_v<Type> && kSameLevelAs<LockPolicyType>
  ...
  /// Converting move assignment from a same-level lock policy.
  ///
  /// \param other The locked pointer to move from.
  /// \returns A reference to this locked pointer.
  template <typename LockPolicyType>
  ...
  /// Converting move assignment from a non-const, same-level locked pointer.
  ///
  /// \param other The locked pointer to move from.
  /// \returns A reference to this locked pointer.
  template <typename Type = SynchronizedType, typename LockPolicyType>
  ...
  /// Copy construction is disabled.
  ///
  /// \param rhs The locked pointer that would be copied from.
  LockedPtr(const LockedPtr& rhs) = delete;
  /// Copy assignment is disabled.
  ///
  /// \param rhs The locked pointer that would be assigned from.
  /// \returns A reference to this locked pointer.
   ...
   *
   * \returns A reference to the underlying lock object.
   */
  LockType& as_lock() noexcept { return lock_; }
  /// Access the underlying lock object, in const context.
  ///
  /// \returns A const reference to the underlying lock object.
   ...
   *
   * \returns True if this pointer is null, false if it holds a valid lock.
   */
   ...
   * Returns !isNull()
   *
   * \returns True if this pointer holds a valid lock.
   */
   ...
   *
   * \returns A pointer to the locked datum.
   */
   ...
   * This method should only be used if the LockedPtr is valid.
   *
   * \returns A reference to the locked datum.
   */
   ...
   * acquisition.
   *
   * \returns A non-const reference to the datum.
   */
   ...
   *
   * \returns A helper that re-locks this pointer when destroyed.
   */
   ...
   *
   * \returns A `LockedPtr` holding an exclusive lock.
   */
  template <
   ...
   * current lock is left in a null state.
   *
   * \returns A `LockedPtr` holding an upgrade lock.
   */
  template <
      typename SyncType = SynchronizedType,
      decltype(void(std::declval<typename SyncType::MutexType&>()
   ...
   * current lock is left in a null state.
   *
   * \returns A `LockedPtr` holding a shared lock.
   */
  template <
      typename SyncType = SynchronizedType,
      decltype(void(std::declval<typename SyncType::MutexType&>()
  ...
  ///
  /// \returns A pointer to the parent object, or null if this pointer is null.
 ...
 private:
  ...
  template <typename MT, typename LT = LockType, typename LP = LockPolicy>
    requires is_try<LP>
 ...
 * many times
 *
 * \param synchronized The object to lock.
 * \param args Extra arguments forwarded to the locking function.
 * \returns A locking helper for use with `lock()` or `synchronized()`.
 */
 ...
 *
 * \param func The callable invoked with the acquired `LockedPtr` instances.
 * \param lockers The locking helpers describing how to lock each object.
 ...
 *
 * \param one The first lockable to acquire.
 * \param two The second lockable to acquire.
 * \param lockables Any additional lockables to acquire.
 */
 ...
 *
 * \param l1 The first object to lock.
 * \param l2 The second object to lock.
 * \returns A tuple of locked pointers, one per object.
 */
 ...
 * \param l1 The first object to lock.
 * \param l2 The second object to lock.
 * \returns A pair of locked pointers, one per object.
 */
template <class Sync1, class Sync2>
 ...
 *
 * \param var The base name to disambiguate.
 */
 ...
 *
 * \param n1 Name bound to the first object's datum.
 * \param e1 The first `Synchronized` object.
 * \param n2 Name bound to the second object's datum.
 * \param e2 The second `Synchronized` object.
 */
