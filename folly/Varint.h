 ...
 * \param buf The output buffer to write the encoding into.
 * \returns The number of bytes written to \p buf.
 */
 ...
 *
 * \param val The value whose encoded size is measured.
 * \returns The number of bytes the varint encoding of \p val would use.
 */
 ...
 * Throws on error.
 *
 * \param data The range to read from; advanced past the decoded value.
...
enum class DecodeVarintError {
  TooManyBytes = 0, ///< The encoding used more bytes than a 64-bit value allows.
  TooFewBytes = 1, ///< The buffer ended before the encoding was complete.
};
 ...
 *
 * \param data The range to read from; advanced past the decoded value.
 * \returns The decoded value, or a DecodeVarintError on failure.
 */
template <class T>
...
///
/// \param val The ZigZag-encoded unsigned value.
/// \returns The original signed value.
