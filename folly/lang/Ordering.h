...
enum class ordering : std::int8_t {
  lt = -1, ///< The left operand is less than the right operand.
  eq = 0,  ///< The two operands are equal.
  gt = 1   ///< The left operand is greater than the right operand.
};

/// Convert a signed comparison value into the matching `ordering`.
///
/// @param c A value comparable against `T(0)`, such as the result of a
///     three-way comparison or `memcmp`.
/// @return `ordering::lt`, `ordering::eq`, or `ordering::gt` according to the
///     sign of `c`.
template <typename T>
...
/// Comparison predicate that is true when the underlying comparator `C`
/// reports the two operands as equal.
template <typename C>
...
/// Comparison predicate that is true when the underlying comparator `C`
/// reports the two operands as not equal.
template <typename C>
...
/// Comparison predicate that is true when the underlying comparator `C`
/// reports the left operand as less than the right operand.
template <typename C>
...
/// Comparison predicate that is true when the underlying comparator `C`
/// reports the left operand as less than or equal to the right operand.
template <typename C>
...
/// Comparison predicate that is true when the underlying comparator `C`
/// reports the left operand as greater than the right operand.
template <typename C>
...
/// Comparison predicate that is true when the underlying comparator `C`
/// reports the left operand as greater than or equal to the right operand.
template <typename C>
...
class partial_ordering : private detail::partial_ordering_<partial_ordering> {
 ...
 public:
  ...
  /// Construct the `unordered` value from the private tag type.
  ///
  /// @param tag The private tag selecting the `unordered` value.
  explicit constexpr partial_ordering(unordered_tag tag) noexcept
      : value_{undef} {}
  /// Construct from a total `ordering`, mapping it to the matching value.
  ///
  /// @param o The total ordering to convert.
  /* implicit */ constexpr partial_ordering(ordering o) noexcept
      : value_{int8_t(o)} {}

  /// Convert to a total `ordering`, throwing if the value is `unordered`.
  ///
  /// @return The equivalent `ordering` for `less`, `equivalent`, or `greater`.
  ...
  /// @param a The left operand.
  /// @param b The right operand.
  /// @return `true` if the two hold different values.
  friend bool operator!=(partial_ordering a, partial_ordering b) noexcept {
    return a.value_ != b.value_;
  }
