...
///
/// @param cond The condition the compiler is permitted to assume is true.
...
///
/// @param a A pointer into one separately allocated region.
/// @param b A pointer into another separately allocated region.
void compiler_may_unsafely_assume_separate_storage(
    const void* a, const void* b);

/// Function object that keeps the compiler from eliding a value's computation.
///
...
struct compiler_must_not_elide_fn {
  ...
  /// @param t The value to treat as used.
  template <typename T>
  FOLLY_ALWAYS_INLINE void operator()(T const& t) const noexcept;
};
/// Keeps the compiler from eliding a value's computation.
inline constexpr compiler_must_not_elide_fn compiler_must_not_elide{};

/// Function object that keeps the compiler from predicting a value.
///
...
struct compiler_must_not_predict_fn {
  ...
  /// @param t The value to treat as unpredictable.
  template <typename T>
  FOLLY_ALWAYS_INLINE void operator()(T& t) const noexcept;
};
/// Keeps the compiler from predicting a value.
...
struct unsafe_for_async_usage { // a convenience wrapper for the marker below:
  /// The marker member type alias tagging the class as unsafe for async usage.
  using folly_is_unsafe_for_async_usage = std::true_type;
};
