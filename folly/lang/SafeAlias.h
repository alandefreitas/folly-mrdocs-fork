...
struct manual_safe_val_t {
  ...
  /// Constructs the wrapped value from the given arguments.
  ///
  /// @param args Arguments forwarded to the wrapped value's constructor.
  template <typename... Args>
  explicit manual_safe_val_t(Args&&... args)
      : t_(static_cast<Args&&>(args)...) {}
  /// Constructs the wrapped value from the result of calling `fn`.
  ///
  /// @param inPlace Tag selecting in-place construction of type `T`.
  /// @param fn A callable whose result initializes the wrapped value.
  template <typename Fn>
  manual_safe_val_t(std::in_place_type_t<T> inPlace, Fn fn) : t_(fn()) {}

  /// Returns a reference to the wrapped value.
  ///
  /// @return A reference to the wrapped value.
  T& get() & noexcept { return t_; }
  /// Converts to a reference to the wrapped value.
  ///
  /// @return A reference to the wrapped value.
  ...
  /// Converts to a const reference to the wrapped value.
  ///
  /// @return A const reference to the wrapped value.
  operator const T&() const& noexcept { return t_; }
  /// Returns an rvalue reference to the wrapped value.
  ///
  /// @return An rvalue reference to the wrapped value.
  T&& get() && noexcept { return std::move(t_); }
  /// Converts to an rvalue reference to the wrapped value.
  ///
  /// @return An rvalue reference to the wrapped value.
...
///
/// @param t The referent to wrap.
/// @return A `manual_safe_ref_t` referring to `t`.
...
/// @return A `manual_safe_val_t` holding `t`.
...
///
/// @param fn A callable whose result is stored in the wrapper.
/// @return A `manual_safe_val_t` holding the result of `fn`.
...
class manual_safe_callable_t {
 ...
 public:
  ...
  /// Invokes the wrapped callable on an lvalue wrapper.
  ///
  ...
  /// Invokes the wrapped callable on a const lvalue wrapper.
  ///
  /// @param args Arguments forwarded to the wrapped callable.
  /// @return The result of invoking the wrapped callable.
  template <typename... Args>
  ...
  /// Invokes the wrapped callable on an rvalue wrapper.
  ///
  /// @param args Arguments forwarded to the wrapped callable.
  /// @return The result of invoking the wrapped callable.
  template <typename... Args>
...
///
/// @param fn The callable to wrap.
/// @return A `manual_safe_callable_t` holding `fn`.
template <
