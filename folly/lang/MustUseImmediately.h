...
/// Trait giving `T`'s must-use-immediately tag, or `std::false_type` if none.
template <typename T>
using must_use_immediately_t =
    typename detail::must_use_immediately_<void, T>::type;

/// `true` when `T` is a must-use-immediately type.
template <typename T>
inline constexpr bool must_use_immediately_v = must_use_immediately_t<T>::value;

#if defined(__cpp_concepts)
/// Satisfied when `T` is a must-use-immediately type.
template <typename T>
concept must_use_immediately = must_use_immediately_v<T>;
#endif

/// Passkey that gates the reconstruction ctor used by `curried_unsafe_mover_t`.
///
...
/// Mover returned from `unsafe_mover` on typical must-use-immediately types.
///
...
class curried_unsafe_mover_t {
 ...
 public:
  /// Store the constituent movers needed to later reconstitute `Outer`.
  ///
  ...
  ///
  /// @param outer Type-deduction pointer; should be `nullptr`.
  /// @param args The per-part movers that reconstruct `Outer`.
  explicit curried_unsafe_mover_t(Outer* outer, Args&&... args) noexcept
      : tup_{std::move(args)...} {
    // See must_use_immediately_unsafe_mover docblock
    static_assert((std::is_nothrow_move_constructible_v<Args> && ...));
  }
  /// Access the stored mover at index `Idx` as an rvalue reference.
  ///
  ...
  ///
  /// @return An rvalue reference to the stored mover at index `Idx`.
  template <size_t Idx>
  auto&& get() && noexcept {
    return std::move(detail::must_use_immediately_tuple::get<Idx>(tup_));
  }
  /// Access the stored mover at index `Idx` as an lvalue reference.
  ///
  /// @return An lvalue reference to the stored mover at index `Idx`.
  template <size_t Idx>
  auto& get() & noexcept {
    return detail::must_use_immediately_tuple::get<Idx>(tup_);
  }
  /// Reconstitute and return the original `Outer` value.
  ///
  /// @return A prvalue `Outer` rebuilt from the stored movers.
...
struct default_unsafe_mover_t {
 ...
 public:
  /// Take ownership of `t` by moving it into the mover.
  ///
  /// @param t The value to move out; must be nothrow-move-constructible.
  explicit default_unsafe_mover_t(T&& t) noexcept : t_(std::move(t)) {
    // See `must_use_immediately_unsafe_mover` docblock
    static_assert(std::is_nothrow_move_constructible_v<T>);
  }
  /// Reconstitute the stored value by moving it out.
  ///
  /// @return A prvalue `T` moved out of the mover.
...
/// Obtain a mover that destructively moves `t` out of a must-use-immediately
/// type, so it can be reconstituted later via the mover's `operator()`.
///
/// CRITICAL: Read the docs on the definition below!
///
/// @param t The value to move out; must be taken by-value by the caller.
/// @return A mover whose `operator() &&` yields a prvalue equivalent to `t`.
template <
    typename T,
...
struct must_use_immediately_crtp {
  ...
  ///
  /// @param other The instance to move from (this overload is deleted).
  must_use_immediately_crtp(must_use_immediately_crtp&& other) = delete;
  /// Deleted move assignment; the type is immovable.
  ///
  /// @param other The instance to move from (this overload is deleted).
  /// @return Reference to this instance.
  must_use_immediately_crtp& operator=(must_use_immediately_crtp&& other) =
      delete;
  /// Deleted copy constructor; the type is non-copyable.
  ///
  /// @param other The instance to copy from (this overload is deleted).
  must_use_immediately_crtp(const must_use_immediately_crtp& other) = delete;
  /// Deleted copy assignment; the type is non-copyable.
  ///
  /// @param other The instance to copy from (this overload is deleted).
  /// @return Reference to this instance.
  must_use_immediately_crtp& operator=(const must_use_immediately_crtp& other) =
      delete;
...
/// Build a `curried_unsafe_mover_t` from a type's bases and member pointers.
///
...
///
/// @param bases Tag listing the base classes to decompose.
/// @param members Tag listing the member pointers to decompose.
/// @param me The object to decompose; never const, never an lvalue ref.
/// @return A mover that reconstitutes `me` from its bases and members.
///
/// Invariants:
...
RetVal curried_unsafe_mover_from_bases_and_members(
    tag_t<Bases...> bases, vtag_t<PtrToMembers...> members, Me&& me) noexcept {
...
class wrap_must_use_immediately_t
 ...
 public:
  ...
  /// Alias-safety tag marking the wrapper as `unsafe` for lifetime analysis.
  ///
  ...
  /// Move `me` out of the wrapper, returning a mover that reconstitutes it.
  ///
  ...
  ///
  /// @param passkey Private access tag restricting who may call this.
  /// @param me The wrapper instance to move out.
  /// @return A mover reconstructing the wrapper from its `Inner` base.
  template <typename Me>
    requires std::is_base_of_v<Inner, Me>
  static my_curried_mover<Me> unsafe_mover(
      must_use_immediately_private_t passkey, Me&& me) noexcept {
    return curried_unsafe_mover_from_bases_and_members<
        wrap_must_use_immediately_t>(
        tag<Inner>, vtag</*no members*/>, static_cast<Me&&>(me));
  }
  /// Reconstruct the wrapper from a curried mover produced by `unsafe_mover`.
  ///
  /// Matches the constraint logic in our `unsafe_mover`.
  ///
  /// @param passkey Private access tag restricting who may call this.
  /// @param mover The curried mover holding the wrapped `Inner` value.
  ...
  explicit wrap_must_use_immediately_t(
      curried_unsafe_mover_private_t passkey,
