...
/// -fno-exceptions.
///
/// @param ex The exception object to throw.
...
///
/// @param ex The exception object to terminate with.
template <typename Ex>
...
///
/// @param args The arguments forwarded to the exception constructor.
template <typename Ex, typename... Args>
...
///
/// @param str The fmt format string used to build the exception message.
...
/// @param a The arguments to forward to the function.
/// @return The result of invoking the function with the arguments.
template <
    typename F,
    typename... A,
...
/// Invoke the provided function pointer or reference with the provided
/// arguments.
///
/// @param f The function to invoke.
/// @param a The arguments to forward to the function.
...
///
/// @param t The function to invoke and guard against a thrown exception.
/// @param c The handler invoked with the caught exception and trailing args.
/// @param a The extra arguments forwarded to the handler.
/// @return The result of t, or of c when an exception of type E is caught.
template <
    typename E,
    typename Try,
    typename Catch,
...
///
/// @param t The function to invoke and guard against a thrown exception.
/// @param c The handler invoked when any exception is caught.
/// @param a The extra arguments forwarded to the handler.
/// @return The result of t, or of c when an exception is caught.
template <
    typename Try,
    typename Catch,
    typename... CatchA,
...
///
/// @return The number of exceptions currently being handled on this thread.
...
///
/// @return An exception_ptr to the exception currently being handled, if any.
...
///
/// @return True if exception-ptr object inspection is supported.
...
///
/// @param ptr The exception_ptr whose stored exception is inspected.
/// @return The type_info of the stored exception, or null if unavailable.
...
/// Returns the address of the stored exception, optionally upcast to a type.
///
...
///
/// @param ptr The exception_ptr whose stored exception is inspected.
/// @param target The type to upcast to, or null for no upcasting.
/// @return The address of the stored exception, or null if not convertible.
...
///
/// @param ptr The exception_ptr whose stored exception is inspected.
/// @return The address of the stored exception, or null if empty.
inline void* exception_ptr_get_object( //
    std::exception_ptr const& ptr) noexcept {
...
///
/// @param ptr The exception_ptr whose stored exception is inspected.
/// @return A pointer to the stored exception as T, or null if not convertible.
template <typename T>
...
///
/// @param ptr The exception_ptr whose stored exception is inspected.
/// @return The number of exception_ptr instances sharing the stored exception.
...
///
/// @param ptr The exception_ptr whose stored exception is inspected.
/// @return True if this is the only reference to the stored exception.
...
///
/// @param ptr The exception_ptr whose stored exception is inspected.
/// @param tag The tag listing the candidate concrete types to match exactly.
/// @return A pointer to the stored exception as T, or null if no exact match.
template <typename T, typename... S>
T* exception_ptr_try_get_object_exact_fast(
    std::exception_ptr const& ptr, tag_t<S...> tag) noexcept {
...
///
/// @param ptr The exception_ptr whose stored exception is inspected.
/// @param hint The tag listing candidate concrete types to try as a fast path.
/// @return A pointer to the stored exception as T, or null if not convertible.
template <typename T, typename... S>
...
class get_exception_fn {
 public:
  /// Returns a pointer-like to the contained exception of type Ex, or null.
  /// @param src The exception container or exception_ptr to inspect.
  /// @return A pointer-like to the contained Ex, or null if none is present.
  ...
  /// Returns the contained exception via the const path for a mutable source.
  ///
  /// For a mutable ptr, use `folly::get_mutable_exception<Ex>(v)` instead.
  ///
  /// @param s The mutable exception container or exception_ptr to inspect.
  /// @return A pointer-like to the contained Ex, or null if none is present.
  template <typename Src>
  constexpr auto operator()(
  ...
  /// Deleted to prevent returning a pointer into an rvalue source.
  ///
  /// It is unsafe to use `get_exception()` to get a pointer into an rvalue.
  /// If you know what you're doing, add a `static_cast`.
  ///
  /// @param s The rvalue source (this overload is deleted).
  template <typename Src>
  void operator()(Src&& s) const noexcept = delete;
  /// Deleted to prevent returning a pointer into a const rvalue source.
  ///
  /// @param s The const rvalue source (this overload is deleted).
  template <typename Src>
  void operator()(const Src&& s) const noexcept = delete;
};
/// Function object for mutable access to a contained exception of type Ex.
template <typename Ex>
class get_mutable_exception_fn {
 public:
  /// Returns a pointer-like to the contained mutable exception, or null.
  /// @param src The mutable exception container or exception_ptr to inspect.
  /// @return A pointer-like to the contained Ex, or null if none is present.
  template <typename Src>
  ...
  /// Deleted for const sources; use `folly::get_exception<Ex>(v)` instead.
  ///
  /// @param src The const source (this overload is deleted).
  template <typename Src>
  void operator()(const Src& src) const noexcept = delete;
  ...
  ///
  /// @param src The rvalue source (this overload is deleted).
  template <typename Src>
  void operator()(Src&& src) const noexcept = delete;
  /// Deleted to prevent returning a pointer into a const rvalue source.
  ///
  /// @param src The const rvalue source (this overload is deleted).
  template <typename Src>
  void operator()(const Src&& src) const noexcept = delete;
};
/// Accesses a contained exception of type Ex for const access.
template <typename Ex = std::exception>
inline constexpr get_exception_fn<Ex> get_exception{};
/// Accesses a contained exception of type Ex for mutable access.
template <typename Ex = std::exception>
...
struct make_exception_ptr_with_fn {
 ...
 public:
  ...
  /// @param f The invocable whose return value becomes the managed exception.
  /// @return An exception_ptr managing the exception returned by f.
  ...
  /// @param tag The in_place_type tag selecting the exception type E.
  /// @param a The arguments forwarded to the constructor of E.
  /// @return An exception_ptr managing the constructed exception of type E.
  template <typename E, typename... A>
  FOLLY_ERASE std::exception_ptr operator()(
      std::in_place_type_t<E> tag, A&&... a) const noexcept {
    return operator()(make<E, make_arg_t<A&&>...>(static_cast<A&&>(a)...));
  }
  /// Builds an exception_ptr managing the moved or copied exception e.
  /// @param place The in_place tag selecting this overload.
  /// @param e The exception object moved or copied into the managed exception.
  /// @return An exception_ptr managing the given exception.
  template <typename E>
  FOLLY_ERASE std::exception_ptr operator()(
      std::in_place_t place, E&& e) const noexcept {
...
/// Constructs a std::exception_ptr, potentially more efficiently.
...
class exception_shared_string {
 ...
 public:
#if FOLLY_CPLUSPLUS >= 202002 && !defined(__NVCC__)
  /// Constructs from a string literal, stored without a refcount.
  /// @param p The string literal to store.
  constexpr explicit exception_shared_string(literal_c_str p) noexcept
      : tagged_what_{vtag<true>, p.ptr} {}
#endif

  /// Constructs from a string buffer of the given length.
  /// @param str A pointer to the string data to copy.
  /// @param len The length in bytes of the string data.
  exception_shared_string(char const* str, std::size_t len);

  /// Constructs from any string-like object exposing data() and size().
  /// @param str The string-like object whose contents are copied.
  template <
      typename String,
  ...
  /// Constructs a string of the given size, filled by the given formatter.
  /// @param size The number of bytes to allocate for the string.
  /// @param func The callable that writes the string contents into the buffer.
  ...
  /// Copy constructor, sharing the refcounted string with the source.
  /// @param other The string to copy from.
  exception_shared_string(exception_shared_string const& other) noexcept;
  /// Copy assignment, sharing the refcounted string with the source.
  /// @param other The string to copy from.
  /// @return A reference to this string.
  exception_shared_string& operator=(
      exception_shared_string const& other) noexcept;

  // It would be extra effort to implement move support in C++17, but there is
  // currently no demand for it.
#if FOLLY_CPLUSPLUS >= 202002 && !defined(__NVCC__)
  /// Move constructor, leaving the source in a safe-to-read empty state.
  /// @param that The string to move from.
  ...
  /// Move assignment, taking over the source's refcounted string.
  /// @param other The string to move from.
  /// @return A reference to this string.
  exception_shared_string& operator=(
      exception_shared_string&& other) noexcept;
#else
  ...
  ///
  /// @return A pointer to the stored null-terminated string.
  constexpr
#endif
