...
///
/// @param as Arguments forwarded to the constructor of the bound type.
/// @return A bound-args list that constructs the target in place.
template <typename, typename... As>
constexpr auto in_place(As&&... as);
/// Binds in-place construction via a maker callable plus the given arguments.
///
/// @param maker A callable that produces the bound value.
/// @param as Arguments forwarded to `maker`.
/// @return A bound-args list that constructs the target in place.
template <typename... As>
constexpr auto in_place_with(auto maker, As&&... as);

/// The primitive for representing lists of bound args.
///
/// Unary forms:
///  - `bind::args<V>`: if `V` is already `like_args`, wraps that binding,
///    unmodified.
///  - `bind::args<V&>` or `bind::args<V&&>`: binds a reference preserving the
///    value category of the input.
///
...
struct unsafe_move_args {
  /// Move-construct a bound-args list from another's moved-out contents.
  ///
  /// @param bargs The bound-args list to move from.
  /// @return A new bound-args list adopting `bargs`'s contents.
...
enum class category_t {
  /// The policy decides; standard storage uses `value`, argument-binding uses
  /// `ref`.
  unset = 0,
  ref, ///< Follows the reference category of the input, i.e. `InputT&&`.
  value, ///< Copies or moves, depending on the input ref type.
  /// Store: like `value` but fails on rvalue input; argument: pass
  /// `folly::copy()`.
  copy,
  /// Store: like `value` but fails on lvalue input; argument: pass the rvalue
  /// ref.
  move
};
/// Constness flag applied to a bound argument.
enum class constness_t {
  /// The policy decides; standard policies use `const` for refs, `mut` for
  /// values.
  unset = 0,
  constant, ///< Make the input `const` if it is not already.
  mut ///< Will NOT remove `const` from an input type.
};
/// The binding flags for a bound argument.
struct bind_info_t {
  /// How the argument is passed or stored (by reference, value, copy, move).
  category_t category;
  /// The constness applied to the argument.
  constness_t constness;

  /// Construct with both flags left `unset`.
  explicit bind_info_t() = default;
  /// Construct with explicit category and constness flags.
  ///
  /// @param ct The category flag.
  /// @param cn The constness flag.
...
///
/// CAUTION: Applicable ONLY to `BT` from `binding_t<BI, BT>`. Contrast to
/// `detail::is_in_place_maker`.
...
/// Concatenates several `like_args` lists, mapping each `bind_info_t` through
/// `BindInfoFn`.
///
/// This is the implementation base for the `bind::args` modifiers such as
/// `constant` and `const_ref`.
...
class merge_update_args : public like_args {
 ...
 protected:
  /// Protected destructor; the type is only used as a base class.
  ~merge_update_args() = default; // Only used as a base class.

 public:
  // Concatenate `Ts::binding_list_t...` after mapping their `bind_info_t`s
  // through `BindInfoFn`.
  /// The concatenated binding list with each `bind_info_t` mapped by
  /// `BindInfoFn`.
  using binding_list_t =
  ...
  /// Construct by concatenating the bound-arg lists in `ts`.
  ///
  /// @param ts The per-input bound-arg lists to concatenate.
  ...
  ///
  /// @return The stored tuple of bound values, moved out.
  // `lifetimebound` documented in `in_place_args_crtp_base`
  constexpr auto unsafe_tuple_to_bind() && noexcept [[clang::lifetimebound]] {
    return std::move(tup_);
  }

  /// Reconstruct from a moved-out tuple of bound values.
  ///
  /// @param tag Tag selecting the unsafe move-from-tuple constructor.
  /// @param t The tuple of bound values to adopt.
  constexpr merge_update_args(unsafe_move_args tag, tuple_to_bind_t t)
      : tup_(std::move(t)) {}
};

/// Deduces the stored binding type for a value passed to `args` and friends.
///
/// Cosmetic: makes the signatures of `bind::args<>` and similar templates show
/// rvalue reference bindings with `&&`.
template <typename T>
using deduce_args_t =
