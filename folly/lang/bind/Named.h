...
struct named_bind_info_t : Base {
  ...
  ///
  /// @return A const reference to the `Base` sub-object.
  constexpr const Base& as_base() const { return *this; }
  using Base::Base;
  /// Construct from a `Base` binding info, adding the identifier tag.
  ///
  /// @param b The base binding info to wrap.
...
class identifier {
 ...
 public:
  /// Name a bound-args modifier with this identifier, e.g. `"x"_id = ref{v}`.
  ///
  /// @param ba The bound-args modifier to name.
  /// @return A named bound-args list carrying this identifier.
  ...
  /// Name a plain value with this identifier, e.g. `"x"_id = var`.
  ///
  /// @param t The value to bind under this identifier.
  /// @return A named bound-args list carrying this identifier.
  template <typename T>
  ...
  /// The compile-time tag identifying this binding name.
...
///
/// Distinct from `id_type` for cleaner error messages.
template <typename UnderlyingSig>
struct self_id_type {
  /// The identifier type for the self-reference binding.
  using identifier_type = ext::identifier<ext::self_id_t{}>;
  /// The storage type carried by the underlying signature.
...
struct id_type {
  /// The identifier type for this named binding.
  using identifier_type = ext::identifier<Str>;
  /// The storage type carried by the underlying signature.
  using storage_type = UnderlyingSig::storage_type;
};

/// The identifier for the compile-time string `Str`; also spelled `"x"_id`.
...
/// User-defined literal building a `bind::id<Str>` from `"x"_id`.
///
/// @return The `identifier` for the literal string `Str`.
template <literal_string Str>
...
/// A special `identifier` tag that works just like `"x"_id`.
///
/// It is syntax sugar for "self-reference" scenarios, such as:
///   - Tagging the implicit scope/object parameter in `spawn_self_closure`.
