...
struct valid_align_value_fn {
  ...
  /// @return `true` if `align` is a power of two representable as
  ///     `std::uintptr_t`.
...
struct align_floor_fn {
  /// Returns the address rounded down to the given alignment.
  ///
  /// @param x The address to align.
  /// @param alignment The alignment value; must be valid.
  /// @return `x` rounded down to a multiple of `alignment`.
  ...
  /// Returns the pointer rounded down to the given alignment.
  ///
...
/// Callable object that rounds a pointer or address down to a given alignment.
...
struct align_ceil_fn {
  ...
  /// @param x The address to align.
  /// @param alignment The alignment value; must be valid.
  /// @return `x` rounded up to a multiple of `alignment`.
  constexpr std::uintptr_t operator()(
      std::uintptr_t x, std::size_t alignment) const {
    assert(valid_align_value(alignment));
...
/// Callable object that rounds a pointer or address up to a given alignment.
