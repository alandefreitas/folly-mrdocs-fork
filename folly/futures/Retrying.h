 ...
 *
 *  @param p The retry policy deciding whether another attempt is made.
 *  @param ff The future-factory invoked to produce each attempt.
 *  @returns A Future with the result of the first successful attempt.
 */
...
/// @param p The retry policy deciding whether another attempt is made.
/// @param ff The future-factory invoked to produce each attempt.
/// @returns A SemiFuture with the result of the first successful attempt.
template <class Policy, class FF>
...
///
/// @param max_tries The maximum number of attempts before giving up.
/// @returns A policy callable usable with retrying and retryingUnsafe.
...
/// Builds a retry policy with capped jittered exponential backoff that also
/// consults an inner policy to decide whether to retry.
///
/// @param max_tries The maximum number of attempts before giving up.
/// @param backoff_min The minimum backoff duration between attempts.
/// @param backoff_max The maximum backoff duration between attempts.
/// @param jitter_param The amount of randomized jitter applied to the backoff.
/// @param rng The random number generator used to compute the jitter.
/// @param p The inner policy consulted to decide whether to retry.
/// @returns A policy callable usable with retrying and retryingUnsafe.
template <class Policy, class URNG>
std::function<Future<bool>(size_t, const exception_wrapper&)>
retryingPolicyCappedJitteredExponentialBackoff(
...
/// Builds a retry policy that always retries with capped jittered exponential
/// backoff, up to a maximum number of tries.
///
/// @param max_tries The maximum number of attempts before giving up.
/// @param backoff_min The minimum backoff duration between attempts.
/// @param backoff_max The maximum backoff duration between attempts.
