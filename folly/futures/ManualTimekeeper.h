...
class ManualTimekeeper : public folly::Timekeeper {
 public:
  ...
  ///
  /// @param dur The delay, relative to now(), after which the future completes.
  /// @returns A SemiFuture that completes once the clock reaches now() + dur.
  ...
  ///
  /// @param dur The amount to advance the clock by.
  ...
  ///
  /// @returns The timekeeper's current clock value.
  std::chrono::steady_clock::time_point now() const;

  /// Returns the number of futures that are pending and have not yet been
  /// fulfilled
  ///
  /// @returns The number of pending, unfulfilled futures.
