...
class SharedPromise {
 public:
   ...
   *
   * @returns A SemiFuture tied to the shared core state.
   */
   ...
   *
   * @returns A Future tied to the shared core state.
   */
  Future<T> getFuture() const;

  /** Return the number of Futures associated with this SharedPromise

    @returns The count of futures extracted from this SharedPromise.
  */
  size_t size() const;

  /** Fulfill the SharedPromise with an exception_wrapper

    @param ew The exception used to fulfill the promise.
  */
  ...
  /** Fulfill the SharedPromise with an exception type E, which can be passed to
    ...
    @param e The exception used to fulfill the promise.
    */
  template <class E>
  ...
  /// handled.
  ///
  /// @param handler The function invoked when an interrupt is raised.
  void setInterruptHandler(
      std::function<void(exception_wrapper const&)> handler);
  ...
  /** Set the value (use perfect forwarding for both move and copy)
    ...
    @param value The value used to fulfill the promise.
  */
  template <class M>
  void setValue(M&& value);

  /// Fulfill this SharedPromise with a `Try`, completing all associated futures.
  ///
  /// @param t The result value or exception to fulfill the promise with.
  void setTry(Try<T>&& t);
  ...
  /** Fulfill this SharedPromise with the result of a function that takes no
    ...
    @param func The nullary function whose result fulfills the promise.
  */
  template <class F>
  void setWith(F&& func);
  ...
  ///
  /// @returns `true` if a result has been set, `false` otherwise.
