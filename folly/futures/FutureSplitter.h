...
class FOLLY_EXPORT FutureSplitterInvalid : public FutureException {
 public:
  /// Constructs an invalid-FutureSplitter exception.
...
class FutureSplitter {
 public:
   ...
   *
   * @param future The Future<T> to be split into multiple futures.
   */
   ...
   *
   * @returns A new Future completed when the original Future completes.
   */
  Future<T> getFuture() {
    if (promise_ == nullptr) {
   ...
   * This can be called an unlimited number of times per FutureSplitter.
   *
   * @returns A new SemiFuture completed when the original Future completes.
   */
 ...
 * improve readability.
 *
 * @param future The Future<T> to split.
 * @returns A FutureSplitter wrapping the given future.
 */
template <class T>
