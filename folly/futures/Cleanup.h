...
/// b.cleanup())).wait();
///
/// @param task The task whose result is produced once cleanup completes.
/// @param cleanup The cleanup task run after the task; it must not fail.
/// @returns A SemiFuture producing the task result after cleanup completes.
template <typename T>
...
class Cleanup {
 public:
  /// Constructs a Cleanup with an empty set of cleanup tasks.
  Cleanup() : safe_to_destruct_(false), cleanup_(folly::makeSemiFuture()) {}
  /// Destroys the Cleanup, requiring that cleanup() has already completed.
  ~Cleanup() {
  ...
  /// Runs the accumulated cleanup tasks in reverse of the order they were added.
  ///
 ...
 protected:
  ...
  ///
  /// @param c The cleanup task added under this scope.
  ...
  /// with a running SemiFuture returned from addCleanup().
  ///
  /// @param c The nested Cleanup whose cleanup task is added under this scope.
