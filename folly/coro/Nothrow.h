...
/// the only difference is performance when the awaitable fails with an error.
///
/// @param awaitable The awaitable whose exceptions are propagated cheaply.
/// @returns A `NothrowAwaitable` wrapping `awaitable`.
template <
    typename Awaitable,
    std::enable_if_t<
...
/// bypassing the expensive throw/catch stack unwinding mechanism.
///
/// This overload handles awaitables that must be used immediately.
///
/// @param awaitable The awaitable whose exceptions are propagated cheaply.
/// @returns A `NothrowAwaitable` wrapping `awaitable`.
template <
