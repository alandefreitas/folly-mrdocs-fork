...
/// Deduces the mutex type of a `SharedLock` from its constructor arguments.
...
///
/// @param lock The upgrade lock to transition to an exclusive lock.
/// @returns A `Task` yielding a `std::unique_lock` holding the mutex.
template <typename Mutex>
