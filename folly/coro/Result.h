...
class co_result final
 ...
 public:
  /// Constructs a co_result wrapping the given result container.
  ///
  /// @param result the Try or result to wrap
 ...
 public:
  /// Movable helper that reconstructs a co_result from a stored container.
  class unsafe_mover_t {
   public:
    /// Constructs the mover from the result container to move.
    ///
    /// @param result the result container to store
    explicit unsafe_mover_t(ContainerRef result) noexcept
        : result_(static_cast<ContainerRef>(result)) {}

    /// Reconstructs the co_result from the stored container.
    ///
    /// @returns the reconstructed co_result
  ...
  /// Creates a mover that can reconstruct this co_result.
  ///
  /// @param tag the private access tag (unused)
  /// @param me the co_result to move from
  /// @returns a mover holding the co_result's container
  static unsafe_mover_t unsafe_mover(
      folly::ext::must_use_immediately_private_t tag,
