...
class Mutex {
 ...
 public:
  ...
  /// Deleted copy constructor; the mutex is non-copyable.
  Mutex(const Mutex& other) = delete;
  /// Deleted move constructor; the mutex is non-movable.
  Mutex(Mutex&& other) = delete;
  /// Deleted copy assignment; the mutex is non-copyable.
  Mutex& operator=(const Mutex& other) = delete;
  /// Deleted move assignment; the mutex is non-movable.
  Mutex& operator=(Mutex&& other) = delete;

  /// Destroy the mutex, which must be unlocked with no waiters.
  ~Mutex();
  ...
  /// synchronously.
  ///
  /// \returns An awaitable that yields an RAII lock releasing on scope exit.
  ...
  ///
  /// \returns An awaitable that acquires the lock; the caller must unlock().
 ...
 private:
  ...
  class LockAwaiter {
   public:
    /// Construct an awaiter that locks the given mutex.
    ///
    /// \param mutex The mutex to acquire.
    explicit LockAwaiter(Mutex& mutex) noexcept : mutex_(mutex) {}

    /// Report whether the lock was acquired without suspending.
    ///
    /// \returns `true` if the mutex was locked synchronously.
    bool await_ready() noexcept { return mutex_.try_lock(); }

    /// Enqueue the awaiting coroutine to resume once the lock is acquired.
    ///
    /// \param awaitingCoroutine The coroutine to resume when the lock is held.
    /// \returns `true` if the coroutine suspends to wait for the lock.
    ...
    /// Complete the await once the lock has been acquired.
    void await_resume() noexcept {}

   protected:
    Mutex& mutex_; ///< The mutex this awaiter locks.
