...
class Baton {
 public:
  ...
  ///
  /// @param initiallySignalled Whether the Baton starts in the signalled state.
  ...
  ///
  /// @returns true if the Baton is in the signalled state.
  ...
  ///
  /// @returns A WaitOperation awaitable that completes when the Baton posts.
  ...
  /// Awaitable returned by operator co_await that waits for the Baton to post.
  class WaitOperation {
   public:
    /// Construct a wait operation bound to the given baton.
    ///
    /// @param baton The baton to wait on.
    explicit WaitOperation(const Baton& baton) noexcept : baton_(baton) {}

    /// Check whether the baton is already signalled.
    ///
    /// @returns true if the baton is signalled and no suspension is needed.
    bool await_ready() const noexcept { return baton_.ready(); }

    /// Suspend the awaiting coroutine until the baton is posted.
    ///
    /// @param awaitingCoroutine The coroutine to resume once the baton posts.
    ...
    /// Complete the await expression once the baton is signalled.
    void await_resume() noexcept {}

    /// Complete a `co_awaitTry` on the baton, which never fails.
    ///
    /// @returns An empty successful `folly::Try<void>`.
   ...
   protected:
    ...
    const Baton& baton_; ///< The baton being waited on.
    coroutine_handle<> awaitingCoroutine_; ///< The suspended awaiting coroutine.
    WaitOperation* next_; ///< Next waiter in the baton's wait list.
  };
