...
class SharedPromise {
 ...
 public:
  ...
  /// Move-constructs from another SharedPromise.
  ///
  /// @param other the SharedPromise to move from
  SharedPromise(SharedPromise&& other) noexcept;
  /// Move-assigns from another SharedPromise.
  ///
  /// @param other the SharedPromise to move from
  /// @returns a reference to this SharedPromise
  SharedPromise& operator=(SharedPromise&& other) noexcept;
  /// SharedPromise is not copy-constructible.
  ///
  /// @param other the SharedPromise that would be copied
  SharedPromise(const SharedPromise& other) = delete;
  /// SharedPromise is not copy-assignable.
  ///
  /// @param other the SharedPromise that would be copied
  /// @returns a reference to this SharedPromise
   ...
   *
   * @returns a future fulfilled when the promise is set
   */
   ...
   *
   * @returns the number of associated futures
   */
   ...
   *
   * @returns true if the promise has a value or exception set
   */
   ...
   *
   * @returns the result if fulfilled, otherwise an empty optional
   */
   ...
   */
  void setException(folly::exception_wrapper&& exception);
  ...
  void setValue(U&& input);
  /// Sets a void value in the promise.
  template <typename U = T>
   ...
   *
   * @param result the Try result to set
   */
  void setTry(TryType&& result);
