...
struct co_cleanup_fn {
  /// Invoke the tag-dispatched cleanup function for the given object.
  ///
  /// \param object The rvalue object to run async cleanup on.
  /// \returns The awaitable produced by the object's cleanup function.
  template <
      typename T,
      std::enable_if_t<
  ...
  /// Deleted overload that rejects lvalue arguments.
  ///
  /// \param object The lvalue that must not be passed for cleanup.
  template <typename T>
  void operator()(T& object) = delete;
};

/// Customization point object that invokes the async cleanup function for an
/// object.
///
