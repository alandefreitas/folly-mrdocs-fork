...
///
/// @param bargs the bound arguments (typically `bind::args{...}`)
/// @param make_inner_coro a callable that builds the inner coroutine
/// @returns the outer coroutine task for the closure
...
/// Creates an immediately-awaited async closure returning a `now_task`.
///
/// @param bargs the bound arguments (typically `bind::args{...}`)
/// @param make_inner_coro a callable that builds the inner coroutine
/// @returns a non-movable `now_task` for the closure
template <async_closure_config Cfg = async_closure_config{}>
