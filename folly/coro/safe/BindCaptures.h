...
/// Deduction guide for `capture` from its bound arguments.
template <typename... Ts>
capture(Ts&&...) -> capture<folly::bind::ext::deduce_args_t<Ts>...>;

/// Binds an async-closure capture whose pointer-like value is dereferenced
/// once to expose the underlying object.
template <typename... Ts>
...
/// Deduction guide for `capture_indirect` from its bound arguments.
template <typename... Ts>
capture_indirect(Ts&&...)
...
/// Sugar for `capture{mut_ref{...}}`, capturing a mutable reference.
template <typename... Ts>
...
/// Deduction guide for `capture_mut_ref` from its bound arguments.
template <typename... Ts>
capture_mut_ref(Ts&&...)
    -> capture_mut_ref<folly::bind::ext::deduce_args_t<Ts>...>;

/// Bind an in-place-constructed capture of type `T`.
///
/// Sugar for `capture{in_place<T>(...)}`.
///
/// @param as Arguments forwarded to construct the captured `T`.
/// @returns A capture binding that in-place-constructs a `T`.
template <typename T>
...
///
/// @param make_fn Factory invoked to construct the captured value.
/// @param as Arguments forwarded to the factory function.
/// @returns A capture binding that in-place-constructs via `make_fn`.
