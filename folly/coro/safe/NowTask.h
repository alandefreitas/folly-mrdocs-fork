...
/// Backwards-compatibility alias for `now_task`.
template <typename T = void>
...
/// The executor-bound, immovable counterpart of `now_task` awaited in place.
template <typename T>
class [[nodiscard]]
now_task_with_executor final : public detail::now_task_with_executor_base<T> {
 ...
 public:
  /// Escape hatch that unwraps this into the unsafe `TaskWithExecutor` API.
  /// @returns The underlying movable `TaskWithExecutor`.
  [[deprecated(
...
/// Converts a `safe_task` into an equivalent `now_task`.
/// @param t The safe task to convert.
/// @returns A `now_task` wrapping the given task.
template <safe_alias S, typename U>
auto to_now_task(safe_task<S, U> t);

/// An immovable `Task<T>` that must be `co_await`ed in the expression that created it.
template <typename T>
class FOLLY_CORO_TASK_ATTRS now_task final : public detail::now_task_base<T> {
 protected:
  ...
  friend auto to_now_task(Task<U> t);
  /// Converts a `safe_task` into a `now_task`; friended so it can construct one.
  /// @param t The safe task to convert.
  /// @returns A `now_task` wrapping the given task.
  template <safe_alias S, typename U> // can construct
  friend auto to_now_task(safe_task<S, U> t);
  /// Converts a `now_task` into a `now_task`; friended so it can construct and unwrap one.
  /// @param t The task to convert.
  /// @returns A `now_task` wrapping the given task.
  template <typename U> // can construct & `unwrapTask`
  friend auto to_now_task(now_task<U> t);

 public:
  /// Escape hatch that unwraps this task into the unsafe `Task` API.
  /// @returns The underlying movable `Task`.
  [[deprecated(
      "`as_unsafe()` is provided as an escape hatch for interoperating with "
      "older futures-based code, or other places not yet compatible with "
      "true structured concurrency patterns. Beware, the full `Task` API "
...
/// Converts a `now_task` into an equivalent `now_task`.
/// @param t The task to convert.
/// @returns A `now_task` wrapping the given task.
template <typename T>
...
/// @param t The value to return from the task.
/// @returns A `now_task` that completes with the given value.
template <class T>
...
/// @returns A `now_task` that completes with no value.
inline now_task<> make_now_task() {
  co_return;
}
/// Same as make_now_task(). See Unit
/// @param unit The unit value (unused).
/// @returns A `now_task` that completes with no value.
inline now_task<> make_now_task(Unit unit) {
  co_return;
}

/// Make a `now_task` that will trivially yield an exception.
/// @param ew The exception to yield from the task.
/// @returns A `now_task` that completes with the given exception.
template <class T>
