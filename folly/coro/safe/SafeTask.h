...
/// A `safe_task` meant to be the inner coro wrapped in an `async_closure`.
template <typename T = void>
...
/// A `safe_task` for non-static member coroutines on stateful classes.
template <typename T>
...
/// A `safe_task` that has been bound to an executor and is ready to run.
///
...
class [[nodiscard]] safe_task_with_executor final
 ...
 public:
  /// Reports the safe-alias level of this task to the SafeAlias machinery.
  template <safe_alias>
  using folly_private_safe_alias_t = safe_alias_constant<ArgSafety>;

  /// Unwraps this into a plain `TaskWithExecutor`, dropping the safety guarantees.
  ///
  /// @returns The underlying `TaskWithExecutor` with the full, unsafe API.
  [[deprecated(
...
class FOLLY_CORO_TASK_ATTRS safe_task final
 ...
 protected:
  ...
  /// Re-wraps this task as a `safe_task` with a different declared safety level.
  ///
  /// @returns A `safe_task` carrying the same coroutine at safety `NewSafety`.
 ...
 public:
  ...
  /// Unwraps this task into a plain `Task`, dropping the safety guarantees.
  ///
  /// @returns The underlying `Task` with the full, unsafe API.
  [[deprecated(
      "`as_unsafe()` is provided as an escape hatch for interoperating with "
      "older futures-based code, or other places not yet compatible with "
      "true structured concurrency patterns. Beware, the full `Task` API "
...
/// Coroutine traits selecting the promise type for a `safe_task` coroutine.
...
struct folly::coro::
  ...
  /// The promise type driving a `safe_task` coroutine.
  using promise_type =
