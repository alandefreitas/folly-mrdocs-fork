...
/// Extension points for customizing `folly::bind` storage and binding policies.
...
class bind_to_storage_policy<binding_t<BI, BindingType>> {
 ...
 public:
  /// The storage type used to hold the bound capture.
  using storage_type = typename standard::storage_type;
  /// The signature type produced for the bound capture.
  using signature_type = typename standard::signature_type;
};
} // namespace folly::bind::ext

namespace folly::coro {
/// Base class for objects whose lifetime is managed by an `async_closure`.
class AsyncObject;
/// An `AsyncObject` stored in a slot of an async scope.
class AsyncScopeSlotObject;
/// Proxy granting safe access to an async scope's context.
template <typename, size_t>
class safe_async_scope_context_proxy;
/// Owning pointer to an `AsyncObject` that does not live in a scope slot.
template <typename>
