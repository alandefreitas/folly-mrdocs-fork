...
enum class capture_proxy_kind {
  lval_ref, ///< Return an lvalue reference to the captured value.
  lval_ptr, ///< Return a pointer obtained from an lvalue.
  rval_ref, ///< Return an rvalue reference to the captured value.
  rval_ptr, ///< Return a pointer obtained from an rvalue.
};
...
class capture : public detail::capture_storage<capture<T>, capture, T> {
 public:
  /// Movable, and deep-const lvalue-reference copyable per the capture policy.
  ///
...
/// Capture wrapper marked `after_cleanup_` to bar refs from outliving cleanup.
template <typename T> // may be a value or reference
  requires(!detail::has_async_closure_co_cleanup<T>)
class after_cleanup_capture
    : public detail::
...
/// Heap-backed `after_cleanup_capture` for cleanup-free in-place arguments.
template <typename T>
...
/// Capture over a pointer, adding sugar so `*`/`->` reach through one level.
template <typename T>
class capture_indirect
    : public detail::
...
/// Pointer-holding `after_cleanup_capture` with reach-through dereference sugar.
template <typename T>
class after_cleanup_capture_indirect
...
/// Restricted view on a `co_cleanup_capture` ref that avoids the safety
/// downgrade by only accepting `maybe_value` args.
template <typename T>
