...
class on_stopped_and_error_t {
 ...
 public:
  /// Maps a non-value completion to its policy result, terminating when fatal.
  ///
  /// @param nv The non-value (stopped or error) completion.
  /// @returns The configured value for the completion, or terminates.
  template <
...
/// Policy that returns `Stopped` on cancellation and `Error` on error.
...
class FOLLY_CORO_TASK_ATTRS value_or_fatal final
 ...
 public:
  /// Marker tag identifying this type as a value-only awaitable.
  using folly_private_value_only_awaitable_t = std::true_type;
};
