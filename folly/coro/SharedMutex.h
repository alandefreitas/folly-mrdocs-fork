...
class SharedMutexFair : private folly::NonCopyableNonMovable {
 ...
 public:
  ...
  /// end of a scope.
  ///
  /// @returns true if the exclusive lock was acquired synchronously.
  ...
  ///
  /// @returns true if the shared lock was acquired synchronously.
  ...
  ///
  /// @returns true if the upgrade lock was acquired synchronously.
  ...
  ///
  /// @returns A SemiAwaitable that completes once the exclusive lock is acquired.
  ...
  /// acquired.
  ///
  /// @returns A SemiAwaitable yielding a scoped exclusive lock guard.
  ...
  ///
  /// @returns A SemiAwaitable that completes once the shared lock is acquired.
  ...
  /// acquired.
  ///
  /// @returns A SemiAwaitable yielding a scoped shared lock guard.
  ...
  ///
  /// @returns A SemiAwaitable that completes once the upgrade lock is acquired.
  ...
  /// acquired.
  ///
  /// @returns A SemiAwaitable yielding a scoped upgrade lock guard.
  ...
  /// .unlock() to release the lock.
  ///
  /// @returns A SemiAwaitable that completes once the lock becomes exclusive.
  ...
  ///
  /// @returns true if the upgrade lock was transitioned to an exclusive lock.
 ...
 private:
  ...
  /// Common base for the lock awaiters, holding the shared waiter state.
  class LockAwaiterBase {
   protected:
    friend class SharedMutexFair;

    /// Construct the awaiter base for a given mutex and lock type.
    ///
    /// @param mutex The mutex the awaiter is queued on.
    /// @param lockType The kind of lock this awaiter is requesting.
    ...
    /// Resume the coroutine waiting on this awaiter.
    void resume() noexcept { continuation_.resume(); }

    SharedMutexFair* mutex_; ///< The mutex this awaiter is queued on.
    LockAwaiterBase* nextAwaiter_; ///< Next awaiter in the waiters queue.
    coroutine_handle<> continuation_; ///< The suspended coroutine to resume.
    LockType lockType_; ///< The kind of lock this awaiter is requesting.
  };

  /// Awaiter that acquires an exclusive lock on the mutex.
  class LockAwaiter : public LockAwaiterBase {
   public:
    /// Construct an awaiter bound to the given mutex.
    ///
    /// @param mutex The mutex to acquire the exclusive lock on.
    explicit LockAwaiter(SharedMutexFair& mutex) noexcept
        : LockAwaiterBase(mutex, LockType::EXCLUSIVE) {}

    /// Check whether the exclusive lock was acquired synchronously.
    ///
    /// @returns true if the exclusive lock was acquired without suspending.
    bool await_ready() noexcept { return mutex_->try_lock(); }

    /// Suspend the awaiting coroutine until the exclusive lock is available.
    ///
    /// @param continuation The coroutine to resume once the lock is acquired.
    /// @returns true if the coroutine was suspended, false if it may continue.
    ...
    /// Complete the await expression once the exclusive lock is held.
    void await_resume() noexcept {}
  };

  /// Awaiter that acquires a shared lock on the mutex.
  class LockSharedAwaiter : public LockAwaiterBase {
   public:
    /// Construct an awaiter bound to the given mutex.
    ///
    /// @param mutex The mutex to acquire the shared lock on.
    explicit LockSharedAwaiter(SharedMutexFair& mutex) noexcept
        : LockAwaiterBase(mutex, LockType::SHARED) {}

    /// Check whether the shared lock was acquired synchronously.
    ///
    /// @returns true if the shared lock was acquired without suspending.
    bool await_ready() noexcept { return mutex_->try_lock_shared(); }

    /// Suspend the awaiting coroutine until the shared lock is available.
    ///
    /// @param continuation The coroutine to resume once the lock is acquired.
    /// @returns true if the coroutine was suspended, false if it may continue.
    FOLLY_CORO_AWAIT_SUSPEND_NONTRIVIAL_ATTRIBUTES bool await_suspend(
    ...
    /// Complete the await expression once the shared lock is held.
    void await_resume() noexcept {}
  };

  /// Awaiter that acquires an upgrade lock on the mutex.
  class LockUpgradeAwaiter : public LockAwaiterBase {
   public:
    /// Construct an awaiter bound to the given mutex.
    ///
    /// @param mutex The mutex to acquire the upgrade lock on.
    explicit LockUpgradeAwaiter(SharedMutexFair& mutex) noexcept
        : LockAwaiterBase(mutex, LockType::UPGRADE) {}

    /// Check whether the upgrade lock was acquired synchronously.
    ///
    /// @returns true if the upgrade lock was acquired without suspending.
    bool await_ready() noexcept { return mutex_->try_lock_upgrade(); }

    /// Suspend the awaiting coroutine until the upgrade lock is available.
    ///
    /// @param continuation The coroutine to resume once the lock is acquired.
    /// @returns true if the coroutine was suspended, false if it may continue.
    FOLLY_CORO_AWAIT_SUSPEND_NONTRIVIAL_ATTRIBUTES bool await_suspend(
    ...
    /// Complete the await expression once the upgrade lock is held.
    void await_resume() noexcept {}
  };

  /// Awaiter that transitions the held upgrade lock to an exclusive lock.
  class UnlockUpgradeAndLockAwaiter : public LockAwaiterBase {
   public:
    /// Construct an awaiter bound to the given mutex.
    ///
    /// @param mutex The mutex whose upgrade lock is being transitioned.
    explicit UnlockUpgradeAndLockAwaiter(SharedMutexFair& mutex) noexcept
        : LockAwaiterBase(mutex, LockType::EXCLUSIVE) {}

    /// Check whether the transition to exclusive completed synchronously.
    ///
    /// @returns true if the exclusive lock was acquired without suspending.
