...
/**
 * @brief Starts and later joins a dynamic, unbounded set of concurrent tasks.
 *
...
class AsyncScope {
 public:
  /// Constructs an empty scope with no started tasks.
  AsyncScope() noexcept;
   ...
   *
   * @returns The number of started tasks that have not yet completed.
   */
   ...
   * should run on.
   *
   * @param awaitable The task/awaitable to start on this scope.
   * @param returnAddress Customize entry point to the async stack (useful if
   *        this is called from async code already). If not set will default to
  ...
  ///
  /// @param awaitable The task/awaitable to start on this scope.
  /// @param returnAddress Custom entry point to the async stack.
  /// @param sourceLocation The call-site source location.
  template <typename Awaitable>
   ...
   *
   * @returns A Task that completes once all started tasks have completed.
   */
   ...
   * object destructs.
   *
   * @returns A SemiFuture that completes once all started tasks have completed.
   */
...
class CancellableAsyncScope {
 public:
  /// Constructs an empty scope with its own internal cancellation source.
  CancellableAsyncScope() noexcept
      : cancellationToken_(cancellationSource_.getToken()) {}
  /// Constructs an empty scope.
  ///
  /// @param throwOnJoin If true, rethrows the last unhandled exception on join.
  explicit CancellableAsyncScope(bool throwOnJoin) noexcept
      : cancellationToken_(cancellationSource_.getToken()),
        scope_(throwOnJoin) {}
  /// Constructs a scope whose internal token is merged with an external one.
  ///
  /// @param token An external cancellation token merged with the internal one.
   ...
   *
   * @param awaitable The task/awaitable to start on this scope.
   * @param token An optional cancellation token merged with the internal token.
   * @param returnAddress Custom entry point to the async stack.
   */
  template <typename Awaitable>
   ...
   * in order to have any effect.
   *
   * @param task The task to schedule on the current executor.
   * @param token An optional cancellation token merged with the internal token.
   * @returns A Task that completes once the given task has been scheduled.
   */
  template <class T>
   ...
   *
   * @returns True if cancellation was requested for this scope.
   */
