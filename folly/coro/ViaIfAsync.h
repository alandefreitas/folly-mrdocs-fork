...
class StackAwareViaIfAsyncAwaiter {
 ...
 public:
  /// Constructs the awaiter from an executor and an inner awaitable.
  ///
  /// @param executor the executor to resume on
  /// @param awaitable the inner awaitable
  ...
  /// Reports whether the inner awaiter is already ready.
  ///
  /// @returns true if the inner awaiter is ready
  decltype(auto) await_ready() noexcept(noexcept(awaiter_.await_ready())) {
    return awaiter_.await_ready();
  }

  /// Suspends the awaiting coroutine, rescheduling resumption on the executor.
  ///
  /// @param h the awaiting coroutine to resume
  /// @returns the value produced by the inner awaiter's await_suspend
  ...
  /// Retrieves the inner awaiter's result and tears down the via coroutine.
  ///
  /// @returns the value produced by the inner awaiter
  decltype(auto) await_resume() noexcept(noexcept(awaiter_.await_resume())) {
    viaCoroutine_.destroy();
    return awaiter_.await_resume();
  }

  /// Retrieves the inner awaiter's result as a Try and tears down the coroutine.
  ///
  /// @returns the inner result wrapped in a Try
  template <
...
#if FOLLY_HAS_RESULT
  /// Retrieves the inner result as a result and tears down the coroutine.
  ///
  /// @returns the inner result wrapped in a result
  template <
      typename Awaiter2 = Awaiter,
      typename Result =
...
class ViaIfAsyncAwaiter {
 ...
 public:
  ...
  /// Suspends the awaiting coroutine, rescheduling resumption on the executor.
  ///
  /// @param continuation the awaiting coroutine to resume
  /// @returns the value produced by the inner awaiter's await_suspend
  template <typename Promise>
  FOLLY_NOINLINE auto
...
class StackAwareViaIfAsyncAwaitable {
 public:
  ...
  ///
  /// @returns an awaiter that reschedules onto the executor
...
class ViaIfAsyncAwaitable {
 public:
  ...
  /// Adapts the awaitable into an async-stack-aware awaitable.
  ///
  /// @param cpo the customization-point tag (unused)
  /// @param self the awaitable to adapt
  /// @returns a stack-aware version of the awaitable
  friend StackAwareViaIfAsyncAwaitable<Awaitable> tag_invoke(
      cpo_t<co_withAsyncStack> cpo, ViaIfAsyncAwaitable&& self) {
...
/// Wraps an awaitable so co_awaiting it yields a Try instead of throwing.
///
/// @param awaitable the awaitable to wrap
/// @returns an awaitable that completes with a Try of the result
template <typename Awaitable>
  requires(!folly::ext::must_use_immediately_v<Awaitable>)
...
/// Wraps a must-use-immediately awaitable to yield a Try instead of throwing.
///
/// @param awaitable the awaitable to wrap
/// @returns an awaitable that completes with a Try of the result
template <typename Awaitable>
