...
/// Trait detecting whether `T` is an AutoCleanup wrapper.
template <typename T>
struct is_auto_cleanup : std::false_type {};

/// True when `T` is an AutoCleanup wrapper.
template <typename T>
...
/// @param coro The coroutine handle to attach cleanup to.
/// @param args The arguments checked for AutoCleanup wrappers to clean up.
template <typename Promise, typename... Args>
