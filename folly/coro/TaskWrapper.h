...
class TaskWrapperCrtp {
 public:
  /// The coroutine promise type for this wrapper.
  ...
  /// @param tw The wrapped task to attach the executor to.
  /// @returns The wrapped task-with-executor.
  ...
  /// Associates a cancellation token with the wrapped task.
  ///
  /// Pass `tw` by-value, since `&&` would break immediately-awaitable types.
  ///
  /// @param cancelToken The cancellation token to associate.
  /// @param tw The wrapped task to associate the token with.
  /// @returns The wrapped task carrying the cancellation token.
  ...
  /// Reschedules the wrapped task onto `executor` when awaited.
  ///
  /// Pass `tw` by-value, since `&&` would break immediately-awaitable types.
  ///
  /// @param executor The executor to resume the task on.
  /// @param tw The wrapped task to reschedule.
  /// @returns The wrapped awaitable bound to `executor`.
 ...
 protected:
  ...
  /// Constructs the wrapper around an inner task.
  ///
  /// @param t The inner task to wrap.
  ...
  /// Extracts the wrapped inner task.
  ///
  /// See "A note on object slicing" in `MustUseImmediately.h`
  ///
  /// @returns The unwrapped inner task.
 ...
 public:
  /// Produces a curried unsafe mover for relocating this wrapper.
  ///
  /// @param tag Unused private access tag.
  /// @param me The wrapper instance to move from.
  /// @returns A curried mover carrying the wrapped inner value.
  ...
  static my_curried_mover<Me> unsafe_mover(
      folly::ext::must_use_immediately_private_t tag, Me&& me) noexcept {
  ...
  /// Reconstructs the wrapper from a curried unsafe mover.
  ///
  /// @param tag Unused private access tag.
  /// @param mover The curried mover carrying the wrapped inner value.
  ...
  explicit TaskWrapperCrtp(
      folly::ext::curried_unsafe_mover_private_t tag,
...
class TaskWithExecutorWrapperCrtp {
 ...
 protected:
  /// Extracts the wrapped inner task-with-executor.
  ///
  /// See "A note on object slicing" in `MustUseImmediately.h`
  ///
  /// @returns The unwrapped inner task-with-executor.
  ...
  /// Constructs the wrapper around an inner task-with-executor.
  ///
  /// @param t The inner task-with-executor to wrap.
 ...
 public:
  ...
  //   co_await std::move(t);
  /// Awaits the wrapped task-with-executor (declaration only, see notes above).
  ///
  /// @returns The awaiter for the wrapped awaitable.
  ...
  /// Attaches the async stack to the wrapped awaitable via `co_withAsyncStack`.
  ///
  /// @param cpo Unused customization-point tag.
  /// @param twe The wrapper to attach the async stack to.
  /// @returns The wrapped awaitable with the async stack attached.
  friend auto
  tag_invoke(cpo_t<co_withAsyncStack> cpo, Derived&& twe) noexcept(
  ...
  /// The corresponding task type without an attached executor.
  using folly_private_task_without_executor_t = typename Cfg::WrapperTaskT;
  /// The safe-alias category of the wrapped inner awaitable.
  template <safe_alias Default>
