...
/// Function object type implementing `co_scope_exit`.
class co_scope_exit_fn {
 ...
 public:
  /// Creates a scope-exit coroutine running `action` with the captured state.
  /// @param action A Task-returning callable invoked on scope exit.
  /// @param args State captured and passed to the exit coroutine.
  /// @returns A task registering the cleanup action.
  template <typename Action, typename... Args>
