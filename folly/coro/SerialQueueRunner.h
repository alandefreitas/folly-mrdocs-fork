...
class SerialQueueRunner {
 ...
 public:
  /// Submits a work item to be run in sequence.
  ///
  /// @param task the work item to enqueue; it must not throw when awaited
  void add(Work task); // task must not throw when awaited!
  /// Signals that no more work items will be added.
  void done();

  /// Runs the enqueued work items in sequence.
  ///
  /// @returns a Task that completes when all work items have run
  Task<> run();
