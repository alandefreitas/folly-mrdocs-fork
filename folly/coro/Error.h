...
class co_error final {
 public:
  ...
  ///
  /// @param a Arguments forwarded to construct the `exception_wrapper`.
  template <
      typename... A,
      std::enable_if_t<
  ...
  ///
  /// @returns A const reference to the wrapped exception.
  const exception_wrapper& exception() const { return ex_; }

  /// Access the wrapped exception.
  ///
  /// @returns A mutable reference to the wrapped exception.
...
class co_stopped_may_throw_t final {
 public:
  ...
  ///
  /// @returns A `co_error` carrying an `OperationCancelled` exception.
...
/// Deprecated alias for `co_stopped_may_throw`.
