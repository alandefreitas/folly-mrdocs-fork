...
///
/// Throws `folly::FutureTimeout` if the timeout fires.
/// \refcode folly/docs/examples/folly/coro/DetachOnCancel.cpp
/// @param semiAwaitable The operation to await under the timeout.
/// @param timeoutDuration The duration after which the timer fires.
/// @param tk The timekeeper used to start the timer, or null for the default.
/// @returns A task completing with the operation's result, or an error on timeout.
template <
...
/// @param timeoutDuration The duration after which the timer fires.
/// @param tk The timekeeper used to start the timer, or null for the default.
/// @returns A task that always completes with the operation's result.
template <
    typename SemiAwaitable,
    typename Duration,
