...
class [[nodiscard]] AsyncGenerator {
 ...
 public:
  /// Constructs an empty generator with no associated coroutine.
  AsyncGenerator() noexcept : coro_() {}

  /// Move-constructs from another generator, leaving it empty.
  ///
  ...
  class [[nodiscard]] CleanupAwaitable {
   public:
    /// The awaiter that suspends the caller while cleanup runs.
    struct Awaiter {
      /// Reports whether cleanup can complete without suspending.
      ///
      /// @returns True if there is no cleanup coroutine to run.
      bool await_ready() noexcept { return !scopeExit_; }

      /// Suspends the awaiting coroutine and starts the cleanup coroutine.
      ///
      /// @param continuation The awaiting coroutine to resume after cleanup.
      /// @returns The handle to resume, or void when a new stack root is used.
      ...
      /// Completes the cleanup await expression.
      void await_resume() noexcept {}

      coroutine_handle<detail::ScopeExitTaskPromiseBase>
          scopeExit_; ///< Handle to the scope-exit coroutine that runs cleanup.
      folly::Executor::KeepAlive<> executor_; ///< Executor to resume cleanup on.
      folly::AsyncStackFrame asyncFrame_; ///< Async stack frame for the cleanup.
    };

    /// Produces the awaiter that drives the cleanup operation.
    ///
    /// @returns The awaiter for this cleanup awaitable.
   ...
   private:
    ...
    /// Customization point integrating this awaitable with the async stack.
    ///
    /// @param cpo The co_withAsyncStack customization-point tag.
    /// @param awaitable The awaitable to adapt.
    /// @returns The async-stack-aware awaitable.
    friend CleanupAwaitable tag_invoke(
        cpo_t<co_withAsyncStack> cpo, CleanupAwaitable awaitable) noexcept {
  ...
  class [[nodiscard]] CleanupSemiAwaitable {
   public:
    /// Binds the cleanup operation to an executor, producing an awaitable.
    ///
    /// @param executor The executor to resume on.
    /// @returns The executor-bound CleanupAwaitable.
    CleanupAwaitable viaIfAsync(Executor::KeepAlive<> executor) noexcept {
      return CleanupAwaitable{scopeExit_, std::move(executor)};
    }

    /// Reports the safe-alias level of this awaitable to the SafeAlias machinery.
    template <safe_alias>
    using folly_private_safe_alias_t = safe_alias_constant<safe_alias::unsafe>;
  ...
  ///
  /// @returns A semi-awaitable that performs cleanup when awaited.
  ...
  class NextResult {
   public:
    /// Constructs an empty result holding no value.
    NextResult() noexcept : hasValue_(false) {}

    /// Move-constructs from another result.
    ///
    ...
    /// Destroys the result, destructing the contained value if present.
    ~NextResult() {
      if (hasValue_) {
        value_.destruct();
      }
    ...
    /// Reports whether this result holds a value.
    ///
    ...
    /// Accesses a pointer to the contained value.
    ///
  ...
  class NextAwaitable {
   public:
    /// Reports whether the next value is already available without suspending.
    ///
    /// @returns True if the operation can complete without suspending.
    bool await_ready() noexcept { return !coro_; }

    /// Suspends the awaiting coroutine and resumes the generator.
    ///
    /// @param continuation The awaiting coroutine to resume once a value is ready.
    /// @returns The handle to resume, or void when a new stack root is used.
    template <typename Promise>
    FOLLY_NOINLINE auto await_suspend(
        coroutine_handle<Promise> continuation) noexcept {
    ...
    /// Completes the await expression and returns the next value.
    ///
    /// @returns The next NextResult, or an empty result at end of the stream.
    ...
    /// Completes the await expression, capturing any exception in a Try.
    ///
    /// @returns A Try holding the NextResult or the captured exception.
   ...
   private:
    ...
    friend NextAwaitable tag_invoke(
        cpo_t<co_withAsyncStack> cpo, NextAwaitable awaitable) noexcept {
  ...
  class NextSemiAwaitable {
   public:
    /// Binds the operation to an executor, producing an awaitable.
    ///
    /// @param executor The executor to resume on.
    /// @returns The executor-bound NextAwaitable.
    ...
    ///
    /// @param cancelToken The cancellation token to attach.
    /// @param awaitable The semi-awaitable to attach the token to.
    /// @returns The semi-awaitable carrying the cancellation token.
  ...
  ///
  /// @returns A semi-awaitable resolving to the next NextResult.
  ...
  ///
  /// @param invokeTag The co_invoke customization-point tag.
  /// @param genTag The tag identifying the generator type and signature.
  /// @param f The callable that produces the generator.
  /// @param a The arguments forwarded to the callable.
  /// @returns The AsyncGenerator produced by invoking the callable.
  template <typename F, typename... A, typename F_, typename... A_>
  friend AsyncGenerator tag_invoke(
      tag_t<co_invoke_fn> invokeTag,
      tag_t<AsyncGenerator, F, A...> genTag,
      F_ f,
      A_... a) {
    if constexpr (RequiresCleanup) {
...
/// Customization point that cleans up a CleanableAsyncGenerator via co_cleanup.
///
/// @param cpo The co_cleanup customization-point tag.
/// @param gen The generator to clean up.
/// @returns The cleanup semi-awaitable for the generator.
template <typename Reference, typename Value>
auto tag_invoke(
    cpo_t<co_cleanup> cpo, CleanableAsyncGenerator<Reference, Value>&& gen) {
