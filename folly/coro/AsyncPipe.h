...
class AsyncPipe {
 public:
  /// Destroys the pipe, closing it if it has not been closed already.
  ~AsyncPipe() {
  ...
  /// Move-constructs from another pipe, taking over its write end.
  ///
  /// @param pipe The pipe to move from.
  AsyncPipe(AsyncPipe&& pipe) noexcept {
    queue_ = std::move(pipe.queue_);
    onClosed_ = std::move(pipe.onClosed_);
  }

  /// Move-assigns from another pipe, closing this pipe first.
  ///
  /// @param pipe The pipe to move from.
  /// @returns A reference to this pipe.
  ...
  /// Creates a pipe, returning its read generator and write end.
  ///
  /// @param onClosed An optional callback invoked when the pipe is closed or
  /// the generator is destroyed.
  /// @returns A pair of the read-end generator and the write end.
  ...
  /// Writes a value to the pipe for the read end to consume.
  ///
  /// @param val The value to write into the pipe.
  /// @returns `true` if the value was enqueued, `false` if the read end was
  /// destroyed.
  ...
  /// Closes the pipe so that reading the given exception is thrown.
  ///
  /// @param ew The exception to deliver to the read end.
  ...
  /// Closes the pipe, completing the generator on the read end.
  void close() && {
    if (auto queue = queue_.lock()) {
  ...
  /// Returns whether the read end has been destroyed or the pipe closed.
  ///
  /// @returns `true` if the pipe is closed, otherwise `false`.
...
class BoundedAsyncPipe {
 public:
  ...
  ///
  /// @param tokens The maximum number of buffered writes before blocking.
  /// @param onClosed An optional callback invoked when the pipe is closed or
  /// the generator is destroyed.
  /// @returns A pair of the read-end generator and the bounded write end.
  ...
  ///
  /// @param u The value to write into the pipe.
  /// @returns A task producing `true` if the value was written, or `false` if
  /// the read end was destroyed.
  template <typename U = T>
  ...
  /// Closes the pipe so that reading the given exception is thrown.
  ///
  /// @param w The exception to deliver to the read end.
  ...
  ///
  /// @returns `true` if the underlying pipe is closed, otherwise `false`.
